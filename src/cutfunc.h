...
inline PS::F64 cutoff_dKdr(PS::F64 x, PS::F64 r_out_inv){
    ...
    return ( x < 0. || x>= 1. ) ? 0.
        : ( 140.* x*x*x * x_1*x_1*x_1
            * r_out_inv*EPGrav::g_1_inv );
}
inline PS::F64 cutoff_d2Kdr2(PS::F64 x, PS::F64 r_out_inv){
    const PS::F64 x_1   = x - 1.;
    return ( x < 0. || x>= 1. ) ? 0.
        : ( 420.* x*x * x_1*x_1 * (1.-2.*x)
            * r_out_inv*r_out_inv*EPGrav::g_1_inv*EPGrav::g_1_inv );
}
inline PS::F64 cutoff_d3Kdr3(PS::F64 x, PS::F64 r_out_inv){
    const PS::F64 x_1   = x - 1.;
    return ( x < 0. || x>= 1. ) ? 0.
        : ( 840.* x * x_1 * (1.+5.*x*x_1)
            * r_out_inv*r_out_inv*r_out_inv*EPGrav::g_1_inv*EPGrav::g_1_inv*EPGrav::g_1_inv );
}

inline PS::F64 cutoff_K(PS::F64 rij, PS::F64 r_out_inv){
    const PS::F64 x = ( EPGrav::gamma - rij*r_out_inv ) * EPGrav::g_1_inv;
    return cutoff_K(x);
}
