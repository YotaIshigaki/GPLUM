    ...
    public:
        ...
        */
        void executeV(const ReallocatableArray<T> & val_send,
            ...
            int cnt = 0;
            //val_recv.reserveAtLeast(val_send.size());
            val_recv.reserveAtLeast(val_send.size() - n_send_offset);
            //val_recv.clearSize();
...
*/
    ...
    static void setRankMultiDim(const S32 id, const S32 r) {
...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL            
            ReallocatableArray<MPI_Request> req_send(n_rank_send, n_rank_send, 1);
            ReallocatableArray<MPI_Request> req_recv(n_rank_recv, n_rank_recv, 1);
            ReallocatableArray<MPI_Status> status_send(n_rank_send, n_rank_send, 1);
            ReallocatableArray<MPI_Status> status_recv(n_rank_recv, n_rank_recv, 1);
            for(int i=0; i<n_rank_send; i++){
                S32 rank = rank_send[i];
                MPI_Isend(&val_send[n_send*i], n_send, GetDataType<T>(), rank, 0, MPI_COMM_WORLD, &req_send[i]);
            }
            for(int i=0; i<n_rank_recv; i++){
                S32 rank = rank_recv[i];
                MPI_Irecv(&val_recv[n_recv*i], n_recv, GetDataType<T>(), rank, 0, MPI_COMM_WORLD, &req_recv[i]);
            }
        ...
        static inline void isendrecvV(T * val_send, const int * rank_send, const int * n_send, const int * n_disp_send, const int n_rank_send, 
            ...
            for(int i=0; i<n_rank_send; i++){
                S32 rank = rank_send[i];
                MPI_Isend(&val_send[n_disp_send[i]], n_send[i], GetDataType<T>(), rank, 0, MPI_COMM_WORLD, &req_send[i]);
            }
            for(int i=0; i<n_rank_recv; i++){
                S32 rank = rank_recv[i];
                MPI_Irecv(&val_recv[n_disp_recv[i]], n_recv[i], GetDataType<T>(), rank, 0, MPI_COMM_WORLD, &req_recv[i]);
            }
            MPI_Waitall(n_rank_send, req_send.getPointer(), status_send.getPointer());
            MPI_Waitall(n_rank_recv, req_recv.getPointer(), status_recv.getPointer());
#else
...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
            ReallocatableArray<MPI_Request> req_send(n_rank_send_0+n_rank_send_1, n_rank_send_0+n_rank_send_1, 1);
            ReallocatableArray<MPI_Request> req_recv(n_rank_recv_0+n_rank_recv_1, n_rank_recv_0+n_rank_recv_1, 1);
            ReallocatableArray<MPI_Status> status_send(n_rank_send_0+n_rank_send_1, n_rank_send_0+n_rank_send_1, 1);
            ReallocatableArray<MPI_Status> status_recv(n_rank_recv_0+n_rank_recv_1, n_rank_recv_0+n_rank_recv_1, 1);
            for(int i=0; i<n_rank_send_0; i++){
                S32 rank = rank_send_0[i];
                MPI_Isend(&val_send_0[n_disp_send_0[i]], n_send_0[i], GetDataType<T0>(), rank, 0, MPI_COMM_WORLD, &req_send[i]);
            }
            for(int i=0; i<n_rank_send_1; i++){
                S32 rank = rank_send_1[i];
                MPI_Isend(&val_send_1[n_disp_send_1[i]], n_send_1[i], GetDataType<T1>(), rank, 0, MPI_COMM_WORLD, &req_send[i+n_rank_send_0]);
            }
            for(int i=0; i<n_rank_recv_0; i++){
                S32 rank = rank_recv_0[i];
                MPI_Irecv(&val_recv_0[n_disp_recv_0[i]], n_recv_0[i], GetDataType<T0>(), rank, 0, MPI_COMM_WORLD, &req_recv[i]);
            }
            for(int i=0; i<n_rank_recv_1; i++){
                S32 rank = rank_recv_1[i];
                MPI_Irecv(&val_recv_1[n_disp_recv_1[i]], n_recv_1[i], GetDataType<T1>(), rank, 0, MPI_COMM_WORLD, &req_recv[i+n_rank_recv_0]);
            }
...
#pragma omp critical
        ...
        F64 getTotalTime() const {
            ...
            */
            return collect_sample_particle + decompose_domain + exchange_particle
                ...
                //+ make_local_tree + make_global_tree
                + set_root_cell
                + calc_force + calc_moment_local_tree + calc_moment_global_tree + make_LET_1st + make_LET_2nd + exchange_LET_1st + exchange_LET_2nd
                + morton_sort_local_tree + link_cell_local_tree 
    ...
    inline F64 GetDistanceMin1DPeri(const F64 al,
        ...
        const F64 dx0 = al - bh;
        const F64 dx1 = bl - ah;
        const F64 dx_tmp_0 = (dx0*dx1>0.0) ? 0.0 : ((dx0>0.0) ? dx0 : dx1);
        const F64 dx2 = dx0 + len;
        const F64 dx3 = dx1 + len;
        const F64 dx_tmp_1 = (dx2*dx3>0.0) ? 0.0 : ((dx2>0.0) ? dx2 : dx3);
        const F64 dx4 = dx0 - len;
        const F64 dx5 = dx1 - len;
        const F64 dx_tmp_2 = (dx4*dx5>0.0) ? 0.0 : ((dx4>0.0) ? dx4 : dx5);
        const F64 dx = (dx_tmp_0<dx_tmp_1) ? ((dx_tmp_0<dx_tmp_2) ? dx_tmp_0 : dx_tmp_2) : ((dx_tmp_1<dx_tmp_2) ? dx_tmp_1 : dx_tmp_2);
        //if(Comm::getRank()==0){
        //    std::cerr<<"al= "<<al
        //             <<" bh= "<<bh
        //             <<" bl= "<<bl
        //             <<" ah= "<<ah
        //             <<" dx0= "<<dx0<<" dx1= "<<dx1
        //             <<" dx0_new= "<<dx0_new<<" dx1_new= "<<dx1_new
        //             <<std::endl;
        //}
        return dx;
    }
