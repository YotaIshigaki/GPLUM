            ...
            for(int iz=0;  iz<NZ; iz++){
                for(int iy=0;  iy<NY; iy++){
                    for(int ix=0;  ix<NX; ix++){
                        ...
                        const int my = iy - MMAX;
                        const int mz = iz - MMAX;
                        const F64vec mvec = F64vec(double(mx), double(my), double(mz));
            ...
            for(int mz=-NMIR; mz<=NMIR; mz++){
                for(int my=-NMIR; my<=NMIR; my++){
                    for(int mx=-NMIR; mx<=NMIR; mx++){
                        const F64vec off = F64vec(double(mx), double(my), double(mz));
                        double ptmp(0.0);
                        F64vec gtmp(0.0);
                        for(int j=0; j<N; j++){
                            F64vec dr = ptcl[j].pos - ptcl[i].pos;
