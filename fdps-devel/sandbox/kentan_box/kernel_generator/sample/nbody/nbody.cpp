...
void calcEnergy(const Tpsys & system,
    ...
    for(PS::S32 i = 0; i < nbody; i++){
        ...
        epot_loc += system[i].mass * (system[i].pot + system[i].mass / system[i].eps);
    }
    ekin_loc *= 0.5;
    epot_loc *= 0.5;
...
#ifdef TEST_GENERATED_KERNEL
    tree_grav.calcForceAllAndWriteBack(CalcForceLongEpEp(),
#else
    tree_grav.calcForceAllAndWriteBack(CalcGravityEP,
#endif
                                       CalcGravitySP,
                                       system_grav,
                                       dinfo);
