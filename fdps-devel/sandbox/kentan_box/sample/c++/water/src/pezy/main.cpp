...
void ScaleVelocity(Tpsys & system,const PS::F64 T){
  ...
  PS::S32 natom = PS::Comm::getSum(natom_local);
...
int main(int argc, char *argv[]){
  ...
  PS::F64 temperature = 80.0 / unit_temp;
  ...
  while((c=getopt(argc,argv,"o:s:n:w:h")) != -1){
    switch(c){
    case 'o':
	...
	std::cerr<<"n: n_group_limit (default: 256)"<<std::endl;
      }
      return 0;
    }
  }
  ...
  ips_tree.initialize(3.0*system_water.getNumberOfParticleLocal(),
		      0.2,n_leaf_limit,n_group_limit);
  ips_tree.calcForceAllAndWriteBackMultiWalk(DispatchKernel,
					     RetrieveKernel,
					     tag,system_water,
					     dinfo,n_walk_limit,true,PS::MAKE_LIST);
  for(int i=0;i<system_water.getNumberOfParticleLocal();i++){
