...
{
//    PS::S64 nptcl = 6291456;// atoi(argv[1]);
    PS::S64 nptcl = 262144;
...
//    FilePointer = fopen(filename, "w");
    ...
    for(PS::S64 iop = 0; iop < 2; iop++) {
        ...
        fprintf(stderr, "Generate particles ...\n");

        generateParticle(nptcl, fptcl);
        fptcl.adjustPositionIntoRootDomain(dinfo);

        fprintf(stderr, "Initial decomposition ...\n");        
        dinfo.decomposeDomainAll(fptcl);
        fptcl.exchangeParticle(dinfo);
        gather.calcForceAllAndWriteBack(calcForce<FP>(), fptcl, dinfo);
        scatter.calcForceAllAndWriteBack(calcForce<FP>(), fptcl, dinfo);
        symmetry.calcForceAllAndWriteBack(calcForce<FP>(), fptcl, dinfo);
        if(iop == 0) {
            longopen.calcForceAllAndWriteBack(calcForce<FP>(),
                                              calcForce<PS::SPJMonopole>(),
                                              fptcl, dinfo);
        } else {
            longperiodic.calcForceAllAndWriteBack(calcForce<FP>(),
                                                  calcForce<PS::SPJMonopoleCutoff>(),
                                                  fptcl, dinfo);
        }
