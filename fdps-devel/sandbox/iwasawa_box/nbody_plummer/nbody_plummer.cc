...
int main(int argc, char *argv[]){
...
#ifdef USE_GPU_CUDA
    const PS::F64 flops_per_proc = 3.1e12; // for K computer (8thread)
    ...
    while(time_sys < time_end){
        PS::TimeProfile tp_grav_tmp = tree_grav.getTimeProfile();
        weight += tp_grav_tmp.calc_force;
	    PS::Comm::barrier();
        ...
        if( n_loop < 4){
            ...
            if(PS::Comm::getRank() % 2 == 0) weight = 0.5;
            else weight = 1;
        ...
        else if( n_loop > 0 && (n_loop < 8 || n_loop % 16 == 0) ){
        ...
        Tloop = PS::GetWtime();
#ifdef USE_GPU_CUDA
        tree_grav.calcForceAllAndWriteBackMultiWalk(DispatchKernelWithSP, RetrieveKernel, system_grav, dinfo, 50);    
#else  
        ...
        PS::S32 rank_dinfo_max[30], rank_system_max[30], rank_dens_max[30], rank_hydr_max[30], rank_grav_max[30];
        ...
        PS::F64 wtime_calc_force_max = PS::Comm::getMaxValue(tp_grav.calc_force);
        PS::F64 wtime_calc_force_min = PS::Comm::getMinValue(tp_grav.calc_force);
        PS::F64 n_loc_max = PS::Comm::getMaxValue(system_grav.getNumberOfParticleLocal());
        PS::F64 n_loc_min = PS::Comm::getMinValue(system_grav.getNumberOfParticleLocal());
        PS::F64 weight_max = PS::Comm::getMaxValue(weight);
        PS::F64 weight_min = PS::Comm::getMinValue(weight);
        if(PS::Comm::getRank()==0){
            std::cout<<"wtime_calc_force_max= "<<wtime_calc_force_max
                     <<" wtime_calc_force_min= "<<wtime_calc_force_min
                     <<" n_loc_max= "<<n_loc_max
                     <<" n_loc_min= "<<n_loc_min
