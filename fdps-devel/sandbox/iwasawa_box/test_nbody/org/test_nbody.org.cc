...
struct CalcForceEpEp{
    void operator () (const EPIGrav * ep_i,
        ...
        if(n_ip > PhantomGRAPE::PG_NIMAX){
            std::cout<<"n_ip(ep-ep)="<<n_ip<<std::endl;
            std::cout<<"PhantomGRAPE::PG_NIMAX(ep-ep)="<<PhantomGRAPE::PG_NIMAX<<std::endl;
        }
        if(n_jp > PhantomGRAPE::PG_NJMAX){
            std::cout<<"n_jp(ep-ep)="<<n_jp<<std::endl;
            std::cout<<"PhantomGRAPE::PG_NJMAX(ep-ep)="<<PhantomGRAPE::PG_NJMAX<<std::endl;
        }
...
#pragma omp for
        for(int i=0; i<n_loc; i++){
            ...
            double r_tmp = 9999.9;
            while(r_tmp > 22.8 * 0.25){ // 22.8 is cutoff in Nbody units
