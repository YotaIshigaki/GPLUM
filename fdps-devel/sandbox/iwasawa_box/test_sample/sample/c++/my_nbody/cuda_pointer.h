...
//      }
    ...
    void htod(int first_loc, int count){
#ifdef SANITY_CHECK_CUDA_POINTER
        assert(size_t(count) < size);
#endif
        CUDA_SAFE_CALL(cudaMemcpy(dev_pointer+first_loc, host_pointer+first_loc, 
    ...
    void dtoh(int first_loc, int count){
#ifdef SANITY_CHECK_CUDA_POINTER
            assert(size_t(count) < size);
#endif
		CUDA_SAFE_CALL(cudaMemcpy(host_pointer+first_loc, dev_pointer+first_loc, 
