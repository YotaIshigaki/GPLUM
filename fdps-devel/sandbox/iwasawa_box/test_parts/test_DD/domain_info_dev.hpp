...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
        // NEW
        MPI_Comm comm_1d_[DIMENSION];
        MPI_Comm comm_sub_[DIMENSION];
        S32 rank_1d_[DIMENSION];
        S32 rank_sub_[DIMENSION];
        S32 n_proc_sub_[DIMENSION];
...
*/
        ...
        void collectSampleParticle(Tpsys & psys,
            ...
            if(psys.getFirstCallByDomainInfoCollectSampleParticle()) {
                //std::cout<<"target_number_of_sample_particle_= "<<target_number_of_sample_particle_<<std::endl;
...
// new version multi-dimensional gathering
	    ...
	    */
		...
		pos_domain_temp_buf[iy*n_domain_[2] + n_domain_[2] - 1].high_.z = pos_root_domain_.high_.z;
...
*/
		...
		*/
            ...
            // ****************************************************
	    if(first_call_by_decomposeDomain) {first_call_by_decomposeDomain = false;}
