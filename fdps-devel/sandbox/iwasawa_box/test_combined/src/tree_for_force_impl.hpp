...
#endif
	std::cout<<"0: getMemSizeUsed()="<<getMemSizeUsed()<<std::endl;
...
#pragma omp for schedule(dynamic, 4)
			...
			*/
			...
			MakeListUsingOuterBoundaryWithRootCellCheck
			    ...
			    if( pos_root_cell_.notOverlapped(ep_org[adr_tmp].getPos()-shift_image_domain_[ith][ii]) ) continue;  // add M.I. 2016/03/12
...
#pragma omp single
#endif	    
            {
		std::cout<<"1: getMemSizeUsed()="<<getMemSizeUsed()<<std::endl;		
                n_send_disp[0] = 0;
	...
	std::cout<<"2: getMemSizeUsed()="<<getMemSizeUsed()<<std::endl;		
...
#pragma omp for schedule(dynamic, 4)
			...
			*/
				    ...
				    if( pos_root_cell_.notOverlapped(pos_new) ) continue;  // added by M.I. 2016/03/12
...
#pragma omp parallel for
    ...
    calcCenterAndLengthOfRootCellOpenNoMargenImpl(const Tep2 ep[]){
        ...
        //const F64ort min_box  = GetMinBoxSingleThread(ep, n_loc_tot_);
...
#pragma omp parallel for schedule(dynamic, 4) 
                ...
                for(int iw=0; iw<n_walk; iw++){
                    ...
                    n_interaction_ep_ep_array[ith] += n_epj_array[iw]*n_epi_array[iw];
                    n_interaction_ep_sp_array[ith] += n_spj_array[iw]*n_epi_array[iw];
                    iw2ith[iw] = ith;
