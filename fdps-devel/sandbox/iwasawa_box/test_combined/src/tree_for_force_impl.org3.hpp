...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
	...
	const S32 n_thread = Comm::getNumberOfThread();
	std::cerr<<"n_thread="<<n_thread<<std::endl;
	const S32 n_walk_limit_per_thread = n_walk_limit / n_thread + ( (n_walk_limit%n_thread==0) ? 0 : 1 ) ;
	std::cerr<<"n_walk_limit_per_thread="<<n_walk_limit_per_thread<<std::endl;
	const S32 n_walk_limit_all_thread = n_walk_limit_per_thread * n_thread;
	std::cerr<<"n_walk_limit_all_thread="<<n_walk_limit_all_thread<<std::endl;
        const F64 time_offset = GetWtime();
        force_sorted_.resizeNoInitialize(n_loc_tot_);
        force_org_.resizeNoInitialize(n_loc_tot_);
	...
	static S32 *   n_epi_prev_array;
	static S32 *   n_epj_array;
	static S32 *   n_spj_array;
	static S32 *   n_epj_disp;
	static S32 *   n_spj_disp;
	static Tepi ** epi_array;
	static Tepj ** epj_array;
	static Tspj ** spj_array;
	static Tforce ** force_array;
	static Tforce ** force_prev_array;
	static bool first_call = true;
	if(first_call){
	    ng_disp          = new S32 [n_thread+1];
	    n_epi_array      = new S32[n_walk_limit_all_thread];
	    n_epi_prev_array = new S32[n_walk_limit_all_thread];
	    n_epj_array      = new S32[n_walk_limit_all_thread];
	    n_spj_array      = new S32[n_walk_limit_all_thread];
	    n_epj_disp       = new S32[n_walk_limit_all_thread+1];
	    n_spj_disp       = new S32[n_walk_limit_all_thread+1];
	    epi_array        = new Tepi*[n_walk_limit_all_thread];
	    epj_array        = new Tepj*[n_walk_limit_all_thread];
	    spj_array        = new Tspj*[n_walk_limit_all_thread];
	    force_array      = new Tforce*[n_walk_limit_all_thread];
	    force_prev_array = new Tforce*[n_walk_limit_all_thread];
	    first_call = false;
	}
	ng_disp[0] = 0;
	//for(S32 i=1; i<n_thread+1; i++){ng_disp[i] = (i*n_ipg) / n_thread;}
	for(S32 i=1; i<n_thread+1; i++){
	    ng_disp[i] = n_ipg / n_thread * i;
	    if( i <= n_ipg % n_thread){
		ng_disp[i] += i;
	    }
	    else{
		ng_disp[i] += n_ipg % n_thread;
	    }
	}
	for(S32 i=0; i<n_thread+1; i++){
	    std::cerr<<"ng_disp[i]="<<ng_disp[i]<<std::endl;
	}
        if(n_ipg > 0){
	    n_epj_disp[0] = 0;
	    n_spj_disp[0] = 0;
	    S32 n_walk_all_thread = 0;
	    S32 n_walk_prev_all_thread = 0;
	    S32 n_loop_limit = ng_disp[1] / n_walk_limit_per_thread + ((ng_disp[1]%n_walk_limit_per_thread == 0) ? 0 : 1);
	    std::cerr<<"n_loop_limit="<<n_loop_limit<<std::endl;
...
#pragma omp parallel 
		...
		spj_for_force_[ith].clearSize();
		S32 n_walk = 0;
		S32 ig_head = ng_disp[ith];
		S32 ig_tail = ng_disp[ith+1];
		S32 ig = ig_head;
		
		for(S32 n_loop=0; n_loop<n_loop_limit; n_loop++, ig += n_walk_limit_per_thread){
		    n_walk = (n_walk_limit_per_thread < ig_tail-ig) ? n_walk_limit_per_thread : ig_tail-ig;
		    S32 n_walk_offset = n_walk * ith;
		    n_walk_all_thread = n_walk * n_thread;
		    if( n_loop == n_loop_limit-1){
			if( ith >= n_ipg % n_thread){
			    n_walk_offset += (n_ipg % n_thread);
			    n_walk_all_thread += (n_ipg % n_thread);
			}
			else{
			    n_walk_all_thread -= n_thread-(n_ipg % n_thread);
			}
		    }
		    /*
		    for(int it=0; it<n_thread; it++){
#pragma omp barrier			
			if(ith == it){
			    std::cerr<<"ith="<<ith<<" ig="<<ig<<" n_walk="<<n_walk<<" n_walk_all_thread="<<n_walk_all_thread<<" n_walk_offset="<<n_walk_offset<<std::endl;
			}
		    }
		    */
		    S32 n_ep_cum_tmp = 0;
			...
			const S32 id_ipg = ig + iw;
			const S32 first_adr_ip = ipg_[id_ipg].adr_ptcl_;
			const S32 adr = n_walk_offset + iw;
			n_epi_array[adr] = ipg_[id_ipg].n_ptcl_;
			epi_array[adr]   = epi_sorted_.getPointer(first_adr_ip);
			force_array[adr] = force_sorted_.getPointer(first_adr_ip);
			makeInteractionList(id_ipg, false);
			n_epj_array[adr] = epj_for_force_[ith].size() - n_ep_cum_tmp;
			n_spj_array[adr] = spj_for_force_[ith].size() - n_sp_cum_tmp;
			//n_epj_disp[iw] = epj_for_force_[ith].size();
			//n_spj_disp[iw] = spj_for_force_[ith].size();
			n_ep_cum_tmp = epj_for_force_[ith].size();
			n_sp_cum_tmp = spj_for_force_[ith].size();
		    }
...
#pragma omp single
			...
			n_epj_disp[0] = 0;
			n_spj_disp[0] = 0;
			for(int iw=0; iw<n_walk_all_thread; iw++){
			    n_epj_disp[iw+1] =  n_epj_disp[iw]  + n_epj_array[iw];
			    n_spj_disp[iw+1] =  n_spj_disp[iw]  + n_spj_array[iw];
			}
			/*
			for(int iw=0; iw<n_walk_all_thread+1; iw++){
			    std::cerr<<"iw="<<iw<<std::endl;
			    std::cerr<<"n_epj_disp[iw]="<<n_epj_disp[iw]<<" n_spj_disp[iw]="<<n_spj_disp[iw]<<std::endl;
			}
			*/
		    }
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL			
...
#endif
			{
			    ret += pfunc_retrieve(n_walk_prev_all_thread, n_epi_prev_array, force_prev_array);
			    std::cerr<<"n_walk_prev_all_thread="<<n_walk_prev_all_thread<<std::endl;
			    for(int iw=0; iw<n_walk_prev_all_thread; iw++){
				std::cerr<<"n_epi_prev_array[iw]="<<n_epi_prev_array[iw]<<std::endl;
				for(int ip=0; ip<n_epi_prev_array[iw]; ip++){
				    std::cerr<<"force_prev_array[iw][ip].pot="<<force_prev_array[iw][ip].pot<<std::endl;
				    Epot += force_prev_array[iw][ip].pot;
				}
			    }
			}
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL			
#pragma omp barrier
		    ...
		    for(S32 iw=0; iw<n_walk; iw++){
			epj_array[n_walk_offset+iw] = epj_for_force_[ith].getPointer(n_epj_disp[n_walk_offset+iw] - n_epj_disp[n_walk_offset]);
			spj_array[n_walk_offset+iw] = spj_for_force_[ith].getPointer(n_spj_disp[n_walk_offset+iw] - n_spj_disp[n_walk_offset]);
		    }
...
#pragma omp single
		    ...
		    {
			/*
			for(int itmp=0; itmp<n_walk_all_thread; itmp++){
			    std::cerr<<"itmp="<<itmp<<std::endl;
			    std::cerr<<"epi_array[itmp]="<<epi_array[itmp]<<std::endl;
			    std::cerr<<"epi_array[itmp][0].id="<<epi_array[itmp][0].id<<std::endl;
			    std::cerr<<"n_epi_array[itmp]="<<n_epi_array[itmp]<<std::endl;
			    std::cerr<<"epj_array[itmp]="<<epj_array[itmp]<<std::endl;
			    std::cerr<<"n_epj_array[itmp]="<<n_epj_array[itmp]<<std::endl;
			    std::cerr<<"epj_array[itmp][0].id="<<epj_array[itmp][0].id<<std::endl;
			    std::cerr<<"spj_array[itmp]="<<spj_array[itmp]<<std::endl;
			    std::cerr<<"n_spj_array[itmp]="<<n_spj_array[itmp]<<std::endl;
			    std::cerr<<"spj_array[itmp][0].mass="<<spj_array[itmp][0].mass<<std::endl;
			}
			*/
			ret += pfunc_dispatch(n_walk_all_thread, epi_array, n_epi_array, epj_array, n_epj_array, spj_array, n_spj_array);
			/*
			std::cerr<<"n_loop="<<n_loop<<std::endl;
			for(S32 itmp=0; itmp<n_walk_all_thread; itmp++){
			    std::cerr<<"n_epi_array[itmp]="<<n_epi_array[itmp]<<std::endl;
			    for(S32 itmp2=0; itmp2<n_epi_array[itmp]; itmp2++){
				std::cerr<<"epi_array[itmp][itmp2].id="<<epi_array[itmp][itmp2].id<<std::endl;
			    }
			}
			*/
			std::cerr<<std::endl;
...
#pragma omp barrier
			...
			const S32 adr = n_walk_offset+iw;
			n_epi_prev_array[adr] = n_epi_array[adr];
			force_prev_array[adr] = force_array[adr];
		    }
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp barrier
#pragma omp single
#endif		    
		    std::cerr<<"n_walk_all_thread="<<n_walk_all_thread<<" n_walk_prev_all_thread="<<n_walk_prev_all_thread<<std::endl;
		    n_walk_prev_all_thread = n_walk_all_thread;
		    epj_for_force_[ith].clearSize();
		    spj_for_force_[ith].clearSize();
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp barrier
...
#pragma omp barrier
	...
	copyForceOriginalOrder();
	std::cerr<<"Epot="<<Epot*0.5*epj_sorted_[0].mass<<std::endl;	
	return ret;
    }
...
#ifdef UNDER_CONST    
    // in this version, each GPU is assigned to each thread.
    template<class TSM, class Tforce, class Tepi, class Tepj,
             class Tmomloc, class Tmomglb, class Tspj>
    template<class Tfunc_dispatch, class Tfunc_retrieve>
    S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcForceMultiWalkImpl2(TagForceLong,
			    Tfunc_dispatch pfunc_dispatch,
			    Tfunc_retrieve pfunc_retrieve,
			    const S32 n_walk_limit,
			    const bool clear){
	...
	static bool first_call = true;
	if(first_call){
	    n_epi_array      = new S32*[n_thread];
	    n_epi_prev_array = new S32*[n_thread];
	    n_epj_array      = new S32*[n_thread];
	    n_spj_array      = new S32*[n_thread];
	    n_epi_disp       = new S32*[n_thread];
	    n_epj_disp       = new S32*[n_thread];
	    n_spj_disp       = new S32*[n_thread];
	    epi_array        = new Tepi**[n_thread];
	    epj_array        = new Tepj**[n_thread];
	    spj_array        = new Tspj**[n_thread];
	    force_array      = new Tforce**[n_thread];
	    force_prev_array      = new Tforce**[n_thread];
	    for(S32 i=0; i<n_thread; i++){
		n_epi_array[i]      = new S32[n_walk_limit];
		n_epi_prev_array[i] = new S32[n_walk_limit];
		n_epj_array[i]      = new S32[n_walk_limit];
		n_spj_array[i]      = new S32[n_walk_limit];
		n_epi_disp[i]       = new S32[n_walk_limit+1];
		n_epj_disp[i]       = new S32[n_walk_limit+1];
		n_spj_disp[i]       = new S32[n_walk_limit+1];
		epi_array[i]        = new Tepi*[n_walk_limit];
		epj_array[i]        = new Tepj*[n_walk_limit];
		spj_array[i]        = new Tspj*[n_walk_limit];
		force_array[i]      = new Tforce*[n_walk_limit];
		force_prev_array[i]      = new Tforce*[n_walk_limit];
	    }
	    first_call = false;
	}
        for(S32 i=0; i<Comm::getNumberOfThread(); i++) n_cell_open_[i] = 0;
...
#pragma omp parallel
		...
		n_spj_disp[ith][0] = 0;
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp for schedule(dynamic, 4) reduction(+ : ret)
#endif
		for(S32 i=0; i<n_ipg; i++){
		    const S32 first_adr_ip = ipg_[i].adr_ptcl_;
		    epi_array[ith][n_walk] = epi_sorted_.getPointer(first_adr_ip);
		    force_array[ith][n_walk] = force_sorted_.getPointer(first_adr_ip);
		    makeInteractionList(i, false);
		    n_epi_array[ith][n_walk] = ipg_[i].n_ptcl_;
		    n_epj_disp[ith][n_walk+1] = epj_for_force_[ith].size();
		    n_spj_disp[ith][n_walk+1] = spj_for_force_[ith].size();
		    n_epj_array[ith][n_walk] = n_epj_disp[ith][n_walk+1] - n_epj_disp[ith][n_walk];
		    n_spj_array[ith][n_walk] = n_spj_disp[ith][n_walk+1] - n_spj_disp[ith][n_walk];
		    n_walk++;
		    if(n_walk == n_walk_limit && first_loop == false){
			ret += pfunc_retrieve(n_walk, n_epi_prev_array[ith], force_prev_array[ith]);
			for(int iw=0; iw<n_walk; iw++){
			    for(int ip=0; ip<n_epi_prev_array[ith][iw]; ip++){
				Epot += force_prev_array[ith][iw][ip].pot;
			    }
			}
		    }
		    if(n_walk == n_walk_limit){
			for(int iw=0; iw<n_walk; iw++){
			    epj_array[ith][iw] = epj_for_force_[ith].getPointer(n_epj_disp[ith][iw]);
			    spj_array[ith][iw] = spj_for_force_[ith].getPointer(n_spj_disp[ith][iw]);
			}
			ret += pfunc_dispatch(n_walk, epi_array[ith], n_epi_array[ith], epj_array[ith], n_epj_array[ith], spj_array[ith], n_spj_array[ith]);
			for(int iw=0; iw<n_walk; iw++){
			    n_epi_prev_array[ith][iw] = n_epi_array[ith][iw];
			    force_prev_array[ith][iw] = force_array[ith][iw];
			}
			epj_for_force_[ith].clearSize();
			spj_for_force_[ith].clearSize();
			n_walk = 0;
		...
		if(first_loop == true){
		    ...
		    ret += pfunc_dispatch(n_walk, epi_array[ith], n_epi_array[ith], epj_array[ith], n_epj_array[ith], spj_array[ith], n_spj_array[ith]);
		    ret += pfunc_retrieve(n_walk, n_epi_array[ith], force_array[ith]);
		    for(int iw=0; iw<n_walk; iw++){
			for(int ip=0; ip<n_epi_array[ith][iw]; ip++){
			    Epot += force_array[ith][iw][ip].pot;
			}
		    }
		}
		else if(n_walk != n_walk_limit){
		    int n_tmp = 0;
		    ret += pfunc_retrieve(n_walk_limit, n_epi_prev_array[ith], force_prev_array[ith]);
		    for(int iw=0; iw<n_walk_limit; iw++){
			n_tmp += n_epi_prev_array[ith][iw];
			for(int ip=0; ip<n_epi_prev_array[ith][iw]; ip++){
			    Epot += force_prev_array[ith][iw][ip].pot;
			}
		    }
    ...
    template<class Tfunc_dispatch, class Tfunc_retrieve>
    S32 TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcForceMultiWalkImpl2(TagForceShort,
			    Tfunc_dispatch pfunc_dispatch,
			    Tfunc_retrieve pfunc_retrieve,
			    const S32 n_walk_limit,
			    const bool clear){
...
#pragma omp for schedule(dynamic, 4) reduction(+ : ret)
		...
		for(S32 i=0; i<n_ipg; i++){
			...
			ret += pfunc_dispatch(n_walk, epi_array[ith], n_epi_array[ith], epj_array[ith], n_epj_array[ith]);
			for(int iw=0; iw<n_walk; iw++){
			    n_epi_prev_array[ith][iw] = n_epi_array[ith][iw];
			    force_prev_array[ith][iw] = force_array[ith][iw];
			}
