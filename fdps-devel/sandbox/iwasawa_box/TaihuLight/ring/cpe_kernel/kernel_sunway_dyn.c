...
static inline void CalcGrav_p_split2(
		...
		const EpjLM      *epj,     // double r[4], v[4]
...
void CalcGrav_p(const int n_epi, const EpiMM* epi, const EpjLM* epj, ForceMM4* force, const Force_coff* pcoff){
  int i;
...
//          int k;
//          double* cf=(double*)&f1;
//          for(k=0;k<4;k++) if(nan_check(&cf[k])==1) printf("id=%d, f[%d].r[%d] = nan\n",my_id,i,k);
//          cf=(double*)&f2;                     
//          for(k=0;k<4;k++) if(nan_check(&cf[k])==1) printf("id=%d, f[%d].r[%d] = nan\n",my_id,i+1,k);
//          cf=(double*)&f3;                    
//          for(k=0;k<4;k++) if(nan_check(&cf[k])==1) printf("id=%d, f[%d].r[%d] = nan\n",my_id,i+2,k);
...
void CalcGrav_ss(const int n_sat, EpiMM* sat, const int nj, const EpiMM* satj, ForceMM* forces, const Force_coff* ss_coff, const doublev4* dt, const doublev4* dtv){
  ...
  for(i=0; i<n_sat; i++){
    ...
    double dx_tmp = sat[i].r[0];
    double dy_tmp = sat[i].r[1];
    double dz_tmp = sat[i].r[2];
    double r_sq_tmp = dx_tmp*dx_tmp + dy_tmp*dy_tmp + dz_tmp*dz_tmp;
    double r_inv_tmp = 1.0 / sqrt(r_sq_tmp);
    double r_inv_sq_tmp = r_inv_tmp * r_inv_tmp;
    double pij_tmp = 1.0 * r_inv_tmp;
    double mri3_tmp = r_inv_sq_tmp*pij_tmp;
    forces[i].ax += mri3_tmp*dx_tmp;
    forces[i].ay += mri3_tmp*dy_tmp;
    forces[i].az += mri3_tmp*dz_tmp;
#endif
...
#if !defined(SPLIT) && !defined(FULL_ASM)
__attribute__((noinline))
void CalcGrav_sp(const int n_epi, const EpiMM* epi, const SpjLM* spj, ForceMM4* force) {
  int i;
...
__attribute__((noinline))
void ForceSumCheck(const int n_epi,
                   ForceMM4* force,
                   ForceMM* force_sum) {
    int i, j;
    for(i=0; i<n_epi; i+=4){
        int ii=i>>2;
        int n_rem = (n_epi-i) < 4 ? (n_epi-i) : 4;
        for(j=0; j<n_rem; j++){
            (*force_sum).ax += force[ii].ax[j];
            (*force_sum).ay += force[ii].ay[j];
            (*force_sum).az += force[ii].az[j];
        }
    }
    return;
}
...
static inline void Kick(const doublev4* dt_lm, const int ni, EpiMM* epi, ForceMM4* force) {
  ...
  for (i=0; i<ni; i+=4) {
    ...
    simd_load(vx, (double*)epi[i  ].v); //epi[i]   has x0, x1, x2, x3
    simd_load(vy, (double*)epi[i+1].v); //epi[i+1] has y0, y1, y2, y3
    simd_load(vz, (double*)epi[i+2].v);
    int ii=i>>2;
    simd_load(fx, (double*)force[ii].ax); //ax[0]=ax0, ax[1]=ax1
...
void ForceKernelSunWay1st(void* pin){
    int i, j, k, l;
#ifdef FORCE_SUM_CHECK
    ForceMM * force_tmp = (ForceMM*)(long)ldm_malloc(sizeof(ForceMM));
    (*force_tmp).ax = (*force_tmp).ay = (*force_tmp).az = (*force_tmp).pot = 0.0;
    FORCE_SUM_LOC.x = FORCE_SUM_LOC.y = FORCE_SUM_LOC.z = 0.0;
    sync_array_();
#endif
#ifdef PROFILE
  p_count    = 0;
  ...
  volatile Force_Kernel_Pars pars;
...
//  int row_cut=(n_walk_remain+7)/8;
  ...
  volatile EpiMM* epi_lm=(EpiMM*)(long)ldm_malloc((NI_LIMIT+epi_off)*EPI_SIZE);
//  for(i=0;i<epi_off;i++) {
...
//  }
...
#else  
  //! i particle 
  volatile EpiMM* epi_lm=(EpiMM*)(long)ldm_malloc(NI_LIMIT*EPI_SIZE);
#endif
  ...
  volatile ForceMM4* force_lm=(ForceMM4*)(long)ldm_malloc((2+NI_LIMIT/4)*FORCE_SIZE);
  force_lm +=2;
  ...
  volatile EpjLM* epj_lm[2];
  epj_lm[0]=(EpjLM*)(long)ldm_malloc(4*EPJ_SIZE);
  epj_lm[1]=(EpjLM*)(long)ldm_malloc(4*EPJ_SIZE);
  ...
  volatile SpjLM* spj_lm[2];
  spj_lm[0]=(SpjLM*)(long)ldm_malloc(4*SPJ_SIZE);
  spj_lm[1]=(SpjLM*)(long)ldm_malloc(4*SPJ_SIZE);
  ...
  volatile int* jlst=(int*)(long)ldm_malloc(NLST_LIMIT*sizeof(int));
...
//satellite number
  ...
  */
  ...
  if(MY_RANK_MPI==59){
      printf("A) my_id=%d, n_walk_lm=%d \n", my_id, n_walk_lm);
  }
  sync_array_();
  */
  
  //! for j particle
  volatile dma_desc epj_input;
  volatile unsigned int epj_reply;
  dma_descriptor_init(&epj_input, &epj_reply);
  ...
  //! for sp particle
  volatile dma_desc spj_input;
  volatile unsigned int spj_reply;
  dma_descriptor_init(&spj_input, &spj_reply);
  ...
  volatile dma_desc force_sat_input;
  volatile unsigned int fin_reply;
  dma_descriptor_init(&force_sat_input, &fin_reply);
  ...
  volatile dma_desc force_sat_output;
  volatile unsigned int fout_reply;
  dma_descriptor_init(&force_sat_output, &fout_reply);
...
//#endif
    ...
    for (j=0; j<n_epi_block; j++) {
      ...
      if(MY_RANK_MPI==59 && my_id < 20){
          printf("1) my_id=%d, n_walk_lm=%d \n", my_id, n_walk_lm);
      }
      */
      ...
      for (k=0; k<n_epj_block; k++) {
        ...
        if (last_loop_flag) {
          ...
          //nj_local--; // remove planet // commented out by M.I.
...
//          int ik;
//          for (ik=0;ik<4;ik++) 
//            if(((EpjLM*)(EP_POINTER[1]))[jlst[l-4+ik]].r[0]!=epj_lm[sw][ik].r[0]) 
//              printf("epmpe[%d].x=%e, epj[%d][%d].x=%e\n",l-4,((EpjLM*)(EP_POINTER[1]))[jlst[l-4]].r[0],sw,0,epj_lm[sw][0].r[0]);
//            printf("epmpe[%d].x=%e, epj[%d][%d].x=%e\n",l-3,((EpjLM*)(EP_POINTER[1]))[jlst[l-3]].r[0],sw,1,epj_lm[sw][1].r[0]);
//            printf("epmpe[%d].x=%e, epj[%d][%d].x=%e\n",l-2,((EpjLM*)(EP_POINTER[1]))[jlst[l-2]].r[0],sw,2,epj_lm[sw][2].r[0]);
//            printf("epmpe[%d].x=%e, epj[%d][%d].x=%e\n",l-1,((EpjLM*)(EP_POINTER[1]))[jlst[l-1]].r[0],sw,3,epj_lm[sw][3].r[0]);
            //          }
          //#endif
...
#endif
        } // end of "for (l=4; l<nj_local4; l+=4)"
        sw = 1-sw;
#ifdef USE_DMA
...
//#endif
      ...
      /*
#ifdef FORCE_SUM_CHECK
      ForceSumCheck(ni_local, force_lm, force_tmp);
#endif
      */
...
//#endif
  ...
  } // end of "for(i=0; i<n_walk_lm; i++)" loop

  /*
  for(i=0;i<my_id*1000000; i++) NOP();
  if(MY_RANK_MPI==59){
      printf("3) my_id=%d, n_walk_lm=%d \n", my_id, n_walk_lm);
  }  
  */
  
#ifdef FORCE_SUM_CHECK
    doublev4 force_sum_tmp;
    //simd_load(force_sum_tmp, (double*)&MY_FORCE_SUM);
    simd_load(force_sum_tmp, (double*)force_tmp);
    sync_array_();
    reduce(&force_sum_tmp);
    sync_array_();
    //simd_store(force_sum_tmp, (double*)&MY_FORCE_SUM);
    simd_store(force_sum_tmp, (double*)force_tmp);
    sync_array_();
    if(my_id==63){
        FORCE_SUM_LOC.x = (*force_tmp).ax;
        FORCE_SUM_LOC.y = (*force_tmp).ay;
        FORCE_SUM_LOC.z = (*force_tmp).az;
    }
    ldm_free(force_tmp, sizeof(ForceMM));
#endif
  ...
  if(n_walk_lm==0) {
#ifdef SPLIT
    volatile EpiMM* sat_lm_all = (EpiMM*) work;
    volatile ForceMM* force_sat_all = (ForceMM*) ((long)sat_lm_all+sat_lm_size);
#else
    volatile EpiMM* sat_lm_all = (EpiMM*)(long)ldm_malloc(sat_lm_size);
    volatile ForceMM* force_sat_all = (ForceMM*)(long)ldm_malloc(n_sat4*FORCEMM_SIZE);
#endif
    int kk;
//#ifdef M_CHECK
...
void ForceKernelSunWay2nd(void* pin) {
...
#ifdef FORCE_SUM_CHECK
  doublev4 force_sum_tmp;
  ForceMM *  force_sat_tmp = (ForceMM*)(long)ldm_malloc(sizeof(ForceMM));
#endif
  ...
  volatile EpiMM* sat_lm=(EpiMM*)(long)ldm_malloc(n_sat_t4*EPI_SIZE);
  ForceMM* force_sat=(ForceMM*)(long)ldm_malloc(n_sat*FORCEMM_SIZE);
  volatile EpiMM* sati=(EpiMM*)(long)ldm_malloc(n_sat*EPI_SIZE);
  ...
  sat_lm[pars.n_sat].v[3]=-1.0; // by M.I.
  //while(sat_reply<1);
...
#ifdef FORCE_SUM_CHECK
  (*force_sat_tmp).ax = (*force_sat_tmp).ay = (*force_sat_tmp).az = (*force_sat_tmp).pot = 0.0;
  for(i=0; i<n_sat; i++){
      (*force_sat_tmp).ax += force_sat[i].ax;
      (*force_sat_tmp).ay += force_sat[i].ay;
      (*force_sat_tmp).az += force_sat[i].az;
  }
  sync_array_();
  for(i=0;i<my_id*1000000;i++) NOP(); // delay each CPE with different cycles to avoid DMA competition.
  if(MY_RANK_MPI == 0){
      printf("A) my_id=%d, force_sat_tmp=%e, %e, %e %p, force_sum_tmp=%p \n",
             my_id, (*force_sat_tmp).ax, (*force_sat_tmp).ay, (*force_sat_tmp).az, force_sat_tmp, &force_sum_tmp);
  }
  sync_array_();
  simd_load(force_sum_tmp, (double*)force_sat_tmp);
  sync_array_();
  reduce(&force_sum_tmp);
  sync_array_();
  simd_store(force_sum_tmp, (double*)force_sat_tmp);

  sync_array_();
  for(i=0;i<my_id*1000000;i++) NOP();
  if(MY_RANK_MPI == 0){
      printf("B) my_id=%d, force_sat_tmp=%e, %e, %e %p, force_sum_tmp=%p \n",
             my_id, (*force_sat_tmp).ax, (*force_sat_tmp).ay, (*force_sat_tmp).az, force_sat_tmp, &force_sum_tmp);
  }
  if(MY_RANK_MPI == 0 && my_id==63){
      FORCE_SUM_LOC.x += (*force_sat_tmp).ax;
      FORCE_SUM_LOC.y += (*force_sat_tmp).ay;
      FORCE_SUM_LOC.z += (*force_sat_tmp).az;
  }
  ldm_free(force_sat_tmp, sizeof(ForceMM));
#endif
