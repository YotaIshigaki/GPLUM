...
namespace ParticleSimulator{
    ...
    bool IsOpen(TagSearchLongScatter,
               ...
               const F64ort & pos_target_box,
               const F64 r_crit_sq){
        const F64vec pos = tc_first[adr_tc].mom_.getPos();
        bool ret = ((pos_target_box.getDistanceMinSQ(pos) <= r_crit_sq) 
                    || pos_target_box.contained(tc_first[adr_tc].mom_.getVertexOut()));
        return ret;
    }    
    ...
    void CopyInfoClose(TagForceLong,
                       ...
                       ReallocatableArray<Tsp> & sp_list,
                       ReallocatableArray<S32> & id_sp_list){
      //        std::cerr<<"&id_sp_list(in)= "<<&id_sp_list<<std::endl;
        //        std::cerr<<"id_sp_list.getPointer()(in)= "<<id_sp_list.getPointer()<<std::endl;        
        S32 cnt_adr_ptcl = adr_ptcl;
        id_ep_list.reserveEmptyAreaAtLeast(n_ptcl);
...
//            std::cerr<<"&id_sp_list= "<<&id_sp_list<<std::endl;
//            std::cerr<<"id_sp_list.getPointer()= "<<id_sp_list.getPointer()<<std::endl;
    ...
    template<class TSM, class TMLM, class TCM, class Ttc, class Ttp,
             class Tep, class Tsp>
    void MakeListUsingTreeRecursiveTop(const ReallocatableArray<Ttc> & tc_first,
        ...
        if( !IsOpen(typename TSM::search_type(), tc_first, adr_tc, pos_target_box, r_crit_sq) ){
            S32 n_ptcl = tc_first[adr_tc].n_ptcl_;
            if(n_ptcl > 0){
                CopyInfoDistant(typename TSM::force_type(), typename TCM::list_content_type(), tc_first, adr_tc, adr_tc+adr_tree_sp_first, sp_list, id_sp_list);
            }
            return;
        }
        MakeListUsingTreeRecursive<TSM, TMLM, TCM, Ttc, Ttp, Tep, Tsp>
            (tc_first, adr_tc, tp_first, ep_first, ep_list, id_ep_list, sp_first, sp_list,
             id_sp_list, pos_target_box, r_crit_sq, n_leaf_limit, adr_tree_sp_first);        
    }
