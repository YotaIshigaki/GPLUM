...
namespace ParticleSimulator{
    ...
    public:
        S32 n_interaction_ep_ep_;
        S32 n_interaction_ep_sp_;
        S32 n_epi_ave_;
        S32 n_epj_ave_;
        S32 n_spj_ave_;
    };

    class IdRecorderForInteraction{
    public:
        S32 id_ipg_; // id of corresponding ipg.
        S32 ith_; // thread id of id_epj_recorder and id_spj_recorder
        S32 n_epj_; // number of epj in id_epj_recorder.
        S64 adr_epj_; // first address of correspondig epj in id_epj_recorder.
        S32 n_spj_;
        S64 adr_spj_;
        void set(const S32 _id_ipg,
                 const S32 _ith,
                 const S32 _n_epj,
                 const S64 _adr_epj,
                 const S32 _n_spj,
    ...
    public:
...
#endif
#ifdef USE_96BIT_KEY
        TreeParticle() : key_hi_(SetMSB( U64(0) )), key_lo_(SetMSB( U32(0) )), adr_ptcl_(SetMSB( U32(0) )) {}
#else
        TreeParticle() : key_hi_(SetMSB( U64(0) )), adr_ptcl_(SetMSB( U32(0) )) {}
#endif
        template<class Tep>
        void setFromEP(const Tep & ep, const U32 adr){
            KeyT key;
#ifdef PHI_R_TREE
            static const F64 MY_PI_2 = 2.0*4.0*atan(1.0);
            const F64 x = ep.getPos().x;
            const F64 y = ep.getPos().y;
            const F64 z = ep.getPos().z;
            const F64 r = sqrt(x*x + y*y);
            F64 phi = atan2(y, x);
            if(phi < 0.0) phi += MY_PI_2;
            key = MortonKey::getKey( F64vec(phi, r, z) );
#else
            key = MortonKey::getKey( ep.getPos() );
#endif
            key_hi_ = key.hi_;
#ifdef USE_96BIT_KEY
            key_lo_ = key.lo_;
        ...
        void setFromSP(const Tsp & sp, const U32 adr){
            KeyT key;
#ifdef PHI_R_TREE
            key = MortonKey::getKey( sp.getPosForTree() );
#else
            key = MortonKey::getKey( sp.getPos() );
#endif
            key_hi_ = key.hi_;
#ifdef USE_96BIT_KEY
            key_lo_ = key.lo_;
...
#ifdef USE_96BIT_KEY
            fout<<std::oct<<"key_hi_= "<<key_hi_
                <<"key_lo_= "<<key_lo_
                <<std::dec<<std::endl;
#else
            fout<<std::oct<<"key_hi_="<<key_hi_<<std::dec<<std::endl;
#endif
...
// modified by M.I.
    ...
    public:
        ...
        U64 ncost_;
        S32 cpe_id_;
        CostInfo() : gid_(S32(-1)), ncost_(U64(0)), cpe_id_(S32(-1)) {};
        ~CostInfo() {};
        const CostInfo & operator = (const CostInfo & rhs){
            gid_ = rhs.gid_;
            ncost_ = rhs.ncost_;
            cpe_id_ = rhs.cpe_id_;
            return (*this);
        }
        // To sort the decreasing order
        bool operator() (const CostInfo & left,
                         const CostInfo & right) const {
            return right.ncost_ < left.ncost_;
        }
    };
    ...
    public:
        ...
        MomentMonopoleInAndOut(const F32 m, const F32vec & p){ 
            mass = m;
            pos = p;
        }
