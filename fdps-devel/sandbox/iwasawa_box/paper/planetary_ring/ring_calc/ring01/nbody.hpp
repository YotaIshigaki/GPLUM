...
void CalcForceLoopMerge(Ttree  & tree,
...
#ifdef REPRODUCE_CHECK
    static PS::ReallocatableArray<Satellite> SATELLITE_prev;
    static PS::ReallocatableArray<SatelliteForce> SATELLITE_FORCE_prev;
    static PS::ReallocatableArray<SatelliteForce> SATELLITE_FORCE_LOC_prev;
    static PS::ParticleSystem<FPGrav> system_prev;
    static bool first = true;
    if(first){
        system_prev.initialize();
        first = false;
    }
    ...
    system_prev.setNumberOfParticleLocal(n_loc);
    const PS::S32 n_proc = PS::Comm::getNumberOfProc();
    PS::F64vec ang[2];
    PS::F64vec ang_sat[2];
    PS::S32 err_loc = 0;
    ...
    if(initial){
...
#ifdef REPRODUCE_CHECK
        ang[0] = ang[1] = ang_sat[0] = ang_sat[1] = 0.0;
        for(PS::S32 loop=0; loop<2; loop++){
            PS::Comm::barrier();
            if(PS::Comm::getRank()==0) std::cerr<<"REPRODUCE_CHECK"<<std::endl;
            ...
            for(PS::S32 i=0; i<n_loc; i++){
                system_prev[i] = system[i];
            }
            PS::Comm::barrier();
            if(PS::Comm::getRank()==0) std::cerr<<"Before Force Calc "<<ang[loop]<<std::endl;
            tree.calcForceAllAndWriteBackReuseListMultiWalkIndexLoopMerge
                (func_dis, func_ret, 0,
                 system, dinfo, n_walk_limit,
                 false,  reuse_flag);
            PS::Comm::barrier();
            if(PS::Comm::getRank()==0) std::cerr<<"After Force Calc "<<ang[loop]<<std::endl;
            
            for(PS::S32 i=0; i<n_loc; i++){
                ang[loop] += system[i].mass*(system[i].pos^system[i].vel);
            }
            for(PS::S32 i=0; i<N_SATELLITE; i++){
                ang_sat[loop] += SATELLITE[i].mass*(SATELLITE[i].pos^SATELLITE[i].vel);
            }
            PS::Comm::barrier();
            if(PS::Comm::getRank()==0){
                std::cerr<<"ang[loop]= "<<ang[loop]
                         <<" ang_sat[loop]= "<<ang_sat[loop]
                         <<std::endl;
            }
        ...
        err_loc = 0;
        //if( (ang[0] != ang[1]) || (ang_sat[0] != ang_sat[1]) ) err_loc = 1;
        if( (ang[0] != ang[1]) ) err_loc = 1;
        err_glb = PS::Comm::getSum(err_loc);
        if(err_glb == 0){
            if(PS::Comm::getRank()==0) std::cerr<<"PASS"<<std::endl;
        }
        else{
            if(PS::Comm::getRank()==0) std::cerr<<"FAIL"<<std::endl;
...
#else
        std::cerr<<"before) calcForceAllAndWriteBackReuseListMultiWalkIndexLoopMerge"<<std::endl;
        tree.calcForceAllAndWriteBackReuseListMultiWalkIndexLoopMerge
            (func_dis, func_ret, 0,
             system, dinfo, n_walk_limit,
             false,  reuse_flag);
        std::cerr<<"after) calcForceAllAndWriteBackReuseListMultiWalkIndexLoopMerge"<<std::endl;
#endif
...
template<class Tptcl>
void Rotate(Tptcl * sys, const PS::S32 n, const PS::F64 theta){    
    if(n <= 0) return;
...
template<class Tptcl>
void RotateAndShiftPtcl(Tptcl * ptcl,
                        const PS::S32 n,
...
#else
    Rotate(ptcl, n, theta_rot);
    for(PS::S32 i=0; i<N_SATELLITE; i++){
        if(fabs(z_coord_offset_loc) < fabs(SATELLITE[i].pos.z)){
            z_coord_offset_loc = SATELLITE[i].pos.z;
        }
    }
    for(PS::S32 i=0; i<n; i++){
        if(fabs(z_coord_offset_loc) < fabs(ptcl[i].pos.z)){
            z_coord_offset_loc = ptcl[i].pos.z;
        }
    }
#endif
...
void ConvertCar2Cyl(Tsys & sys){
    const auto n = sys.getNumberOfParticleLocal();
    for(auto i=0; i<n; i++){
        const auto r  = sqrt( (sys[i].pos.x*sys[i].pos.x)+(sys[i].pos.y*sys[i].pos.y) );
        auto th = atan2(sys[i].pos.y, sys[i].pos.x);
        if(th < 0.0){
            th += 2.0*M_PI;
        }
        sys[i].pos.x = th;
...
template<typename Tsys>
void ConvertCyl2Car(Tsys & sys){
    const auto n = sys.getNumberOfParticleLocal();
    for(auto i=0; i<n; i++){
        auto th = sys[i].pos.x;
        const auto r  = sys[i].pos.y;
        const auto x_new = r * cos(th);
        const auto y_new = r * sin(th);
        sys[i].pos.x = x_new;
        sys[i].pos.y = y_new;
    }
}
