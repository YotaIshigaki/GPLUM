...
static PS::F64 EPS_FOR_PP = 0.1/SIZE_OF_MESH;
...
int main(int argc, char **argv)
{
    ...
    const PS::S32 n_leaf_limit = 16;
    //const PS::S32 n_leaf_limit = 1;
    const PS::S32 n_grp_limit = 256;
    //const PS::S32 n_grp_limit = 1;
    PS::PM::ParticleMesh pm;
    PS::ParticleSystem<FPtreepm> ptcl;
    ptcl.initialize();
    ptcl.setAverageTargetNumberOfSampleParticlePerProcess(n_smp);
    PS::DomainInfo domain_info;
    domain_info.initialize(coef_ema);
    run_param this_run;
    ...
    treepm_tree.initialize(3*ptcl.getNumberOfParticleGlobal(), this_run.theta, n_leaf_limit, n_grp_limit);
#ifdef NEW_IMPL
    treepm_tree.setExchagneLETMode(PS::EXCHANGE_LET_P2P_FAST);
#endif
    ...
    if(PS::Comm::getRank()==0){
        for(auto i=0; i<10; i++){
            std::cerr<<"i= "<<i<<" ptcl[i].acc= "<<ptcl[i].acc<<std::endl;
        }
    }
    //PS::Comm::barrier();
    //exit(1);
    ...
    while(this_run.znow > this_run.zend) {
       ...
       //treepm_tree.clearTimeProfile();
       //ptcl.clearTimeProfile();
       treepm_tree.clearCounterAll();
       ptcl.clearCounterAll();
       domain_info.clearTimeProfile();
       wtime_tot = wtime_pm = 0.0;

       PS::Comm::barrier();
       
       PS::F64 wtime_tot_offset = PS::GetWtime();
       treepm_tree.calcForceAllAndWriteBack
           (calc_pp_force<EPJtreepm>(),
            calc_pp_force<PS::SPJMonopoleCutoff>(),
            ptcl,
       ...
       PS::F64 wtime_pm_offset = PS::GetWtime();
       pm.calcForceAllAndWriteBack(ptcl, domain_info);
       wtime_pm += (PS::GetWtime() - wtime_pm_offset);
...
#ifdef NEW_IMPL
       //PS::F64 wgh = 1.0;
       const auto wgh = treepm_tree.getTimeProfile().calc_force;
       domain_info.collectSampleParticle(ptcl, true, wgh);
       //domain_info.decomposeDomainMultiStep2(false);
       domain_info.decomposeDomainMultiStep2(true);
#else
       domain_info.decomposeDomainAll(ptcl);
#endif
       ...
       ptcl.adjustPositionIntoRootDomain(domain_info);
#ifdef NEW_IMPL
       ptcl.exchangeParticle2(domain_info);
#else
       ptcl.exchangeParticle(domain_info);
#endif
       PS::Comm::barrier();
       ...
       PS::TimeProfile tp_tree  = treepm_tree.getTimeProfile();
       PS::TimeProfile tp_ptcl  = ptcl.getTimeProfile();
       PS::TimeProfile tp_dinfo = domain_info.getTimeProfile();
       PS::TimeProfile tp_total = tp_tree + tp_ptcl + tp_dinfo;
       const auto n_rank_allgather_global = treepm_tree.getNumberOfExLetAllgatherGlobal();
       const auto n_ptcl_exch_glb = ptcl.getNumberOfParticleSendGlobal();
       const auto n_proc_exch_glb = ptcl.getNumberOfProcExchangeGlobal();
       const auto n_ep_let_glb = treepm_tree.getNumberOfExLetEpGlobal();
       const auto n_sp_let_glb = treepm_tree.getNumberOfExLetSpGlobal();
       if(PS::Comm::getRank()==0){
           std::cerr<<"*************"<<std::endl;
           std::cerr<<"n_rank_allgather_global= "<<n_rank_allgather_global
                    <<" n_rank_allgather_global/n_proc= "<<(PS::F64)n_rank_allgather_global/PS::Comm::getNumberOfProc()
                    <<std::endl;
           std::cerr<<" n_ptcl_exch_glb= "<<n_ptcl_exch_glb
                    <<" n_ptcl_exch_glb/n_proc= "<<(PS::F64)n_ptcl_exch_glb/PS::Comm::getNumberOfProc()
                    <<std::endl;
           std::cerr<<"n_proc_exch_glb= "<<n_proc_exch_glb
                    <<" n_proc_exch_glb/n_proc= "<<n_proc_exch_glb/PS::Comm::getNumberOfProc()
                    <<std::endl;
           std::cerr<<"n_ep_let_glb= "<<n_ep_let_glb
                    <<" n_ep_let_glb/n_proc= "<<(PS::F64)n_ep_let_glb/PS::Comm::getNumberOfProc()
                    <<" n_sp_let_glb= "<<n_sp_let_glb
                    <<" n_sp_let_glb/n_proc= "<<(PS::F64)n_sp_let_glb/PS::Comm::getNumberOfProc()
                    <<std::endl;
           
           std::cerr<<"wtime_tot= "<<wtime_tot<<std::endl;
           std::cerr<<"wtime_pm= "<<wtime_pm
                    <<" tp_total.getTotalTime()= "<<tp_total.getTotalTime()
                    <<" (sum= "<<wtime_pm + tp_total.getTotalTime()<<" )"
                    <<std::endl;

           tp_total.dump(std::cerr, 1);
       }
