...
namespace ParticleSimulator{
    ...
    public:
        ...
        static S32 getCellID(const S32 lev, const Tkey mkey){
        ...
        static F64 getCorrespondingFullLength(const S32 lev){
             F64 len = getInstance().half_len_*2.0;
             for(S32 i=0; i<lev; i++){
                 len *= 0.5;
             }
             return len;
    ...
    public:
        ...
        template<typename Tkey>
        static S32 getCellID(const S32 lev, const Tkey & mkey){
            const auto s = mkey >> ( (kLevMax - lev) * 3 );
            return (S32)(s.hi_ & 0x7);
        }
        ...
        template<typename Tkey>
        static F64ort getPosTreeCell(const S32 lev, const Tkey & key){
            F64vec cen = getInstance().center_;
            F64 hlen = getInstance().half_len_;
            for(S32 i=0; i<lev; i++){
