...
namespace ParticleSimulator{
    ...
    public:
        /*
        S32 next_adr_; // address of next tree cell
        S32 more_adr_; // address of next tree cell
        S32 n_cell_; // number of children cells
        F32 full_len_sq_; // full length of a side of cell
        F32vec pos_; // geometric center of cell
        S32 id_cell_; // (0 to 7)for debug

        S32 n_ptcl_;
        U32 adr_ptcl_;
        S32 level_; // for debug
        */
        S32 next_adr_; // address of next tree cell
        S32 more_adr_; // address of next tree cell
        S32 n_ptcl_;
        ...
        TreeCell(){
            pos_ = 0.0;
            full_len_sq_ = 0.0;
            next_adr_ = more_adr_ = -1;
            n_cell_ = 0;
            id_cell_ = -1;
            n_ptcl_ = adr_ptcl_ = level_ = 0;
            mom_.init();
        }
        void clear(){
            pos_ = 0.0;            
        ...
        void checkTree(const Tep * const first_ep_ptr,
            ...
            //std::cerr<<"check tree"<<std::endl;
            const TreeCell * tc_cur = first_tc_ptr + this->more_adr_;
            S32 n_cum = 0;
            while(1){
                //std::cerr<<"n_cum= "<<n_cum<<" tc_cur->level_= "<<tc_cur->level_<<std::endl;
                //fout<<"n_cum= "<<n_cum<<" tc_cur->level_= "<<tc_cur->level_<<std::endl;
                if(tc_cur->more_adr_ != -1){
                    //fout<<"go deeper"<<" tc_cur->more_adr_="<<tc_cur->more_adr_<<std::endl;
                    tc_cur = first_tc_ptr + tc_cur->more_adr_;
                }
                else if(tc_cur->next_adr_ != -1) {
                    //fout<<"go next"<<" tc_cur->next_adr_="<<tc_cur->next_adr_<<std::endl;
                    //std::cerr<<"tc_cur->n_ptcl_= "<<tc_cur->n_ptcl_<<std::endl;
                    n_cum += tc_cur->n_ptcl_;
                    tc_cur = first_tc_ptr + tc_cur->next_adr_;
                }
                else{
                    //fout<<"last cell"<<std::endl;
                    n_cum += tc_cur->n_ptcl_;
                    break;
                }
                //std::cerr<<std::endl;
            }
            if(n_cum != first_tc_ptr->n_ptcl_){
                //fout<<"n_cum= "<<n_cum<<" first_tc_ptr->n_ptcl_= "<<first_tc_ptr->n_ptcl_<<std::endl;                
                err = 1;
            }
#if 0
            while(tc_cur->next_adr_ != -1){
                if(tc_cur->more_adr_ != -1){
                    std::cerr<<"go deeper"<<std::endl;
                    //fout<<"go deeper"<<std::endl;
                    tc_cur = first_tc_ptr + tc_cur->more_adr_;
                }
                else if(tc_cur->next_adr_ != -1) {
                    std::cerr<<"go next"<<std::endl;
                    //fout<<"go next"<<std::endl;
                    tc_cur = first_tc_ptr + tc_cur->next_adr_;
                }
                /*
                const Tep * ptcl = first_ep_ptr + tc_cur->adr_ptcl_;
                for(S32 ip=0; ip<tc_cur->n_ptcl_; ip++, ptcl++){
                ...
                */
                /*
                //hozon
                U32 more_bit = tc_cur->more_adr_ != -1;
                U32 next_bit = tc_cur->next_adr_ != -1; 
                if(!more_bit && !next_bit) break;
                // if it has more pointer, go deeper, otherwise go next.
                S32 adr = (tc_cur->more_adr_*more_bit + tc_cur->next_adr_*next_bit*(more_bit^0x1);
                tc_cur = first_tc_ptr + adr;
                */
            }
#endif
        }
        ...
        // for DEBUG
        void dump(std::ostream & fout=std::cout) const {
            fout<<"level_= "<<level_<<std::endl;
            fout<<"n_ptcl_= "<<n_ptcl_<<std::endl;
            fout<<"next_adr_= "<<next_adr_<<std::endl;
            fout<<"more_adr_= "<<more_adr_<<std::endl;
            fout<<"adr_ptcl_= "<<adr_ptcl_<<std::endl;
            mom_.dump(fout);
        }
        /*
