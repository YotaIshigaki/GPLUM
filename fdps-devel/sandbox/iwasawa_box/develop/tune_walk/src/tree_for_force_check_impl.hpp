...
namespace ParticleSimulator{
    ...
    checkMakeLocalTree(const F64 tolerance, std::ostream & fout){
        /*
        for(S32 i=0; i<tc_loc_.back().level_+1; i++){
            std::cerr<<"level= "<<i<<std::endl;
            for(S32 j=adr_tc_level_partition_[i]; j<adr_tc_level_partition_[i+1]; j++){
                std::cerr<<"j= "<<j<<std::endl;
                tc_loc_[j].dump(std::cerr);
            }
            std::cerr<<std::endl;
        }
        */
        ...
        tc_loc_.getPointer()->checkTree(epj_sorted_.getPointer(),
                                        tc_loc_.getPointer(),
    ...
    checkMakeGlobalTreeImpl(TagForceLong,
        ...
        */
        tc_glb_.getPointer()->checkTree(epj_sorted_.getPointer(),
                                        tc_glb_.getPointer(),
                                        center, length_*0.5, n_leaf_limit_,
                                        tolerance, err, fout);        
    }
