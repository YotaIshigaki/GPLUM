...
namespace ParticleSimulator{
    ...
    inline void LinkCell(ReallocatableArray<Ttc> & tc_array,
                         ...
                         S32 & lev_max,
                         //const F64vec center, // center pos of root cell
                         const F64 full_length, // full length of root cell
                         const S32 n_tot,
                         const S32 n_leaf_limit){

        S32 n_th = Comm::getNumberOfThread();
        static S32 * n_cell;
        static S32 * n_cell_offset;
        static ReallocatableArray<Ttc> * tc_buf;
        static bool first = true;
        if(first){
            n_cell = new S32[n_th];
            n_cell_offset = new S32[n_th+1];
            tc_buf = new ReallocatableArray<Ttc>[n_th];
        }
        first = false;
        // set root cell
        adr_tc_level_partition[0] = 0;
        adr_tc_level_partition[1] = 1;
        S32 lev_cur = 0;
        F64 full_len_sq_cur = full_length*full_length;
        //F64vec center_cur = center;
        tc_array.resizeNoInitialize(1);
        tc_array[0].level_ = lev_cur;
        tc_array[0].n_ptcl_ = n_tot;
        tc_array[0].adr_ptcl_ = 0;
        tc_array[0].next_adr_ = -1;
        tc_array[0].n_cell_ = 0;
        tc_array[0].more_adr_ = -1;
        tc_array[0].id_cell_ = 0;
        tc_array[0].full_len_sq_ = full_len_sq_cur;
        //tc_array[0].pos_ = center;
        S32 id_cell_left = 0;
        S32 id_cell_right = 0;

        while(1){
            //std::cerr<<"lev_cur= "<<lev_cur<<" full_len_sq_cur= "<<full_len_sq_cur<<std::endl;            
            for(S32 i=0; i<n_th; i++){
                tc_buf[i].clearSize();
                n_cell[i] = 0;
            }
            ...
            // do not uses chedule(dynamic)
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp parallel for schedule(static)
#endif
            for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                tc_array[i].more_adr_ = -1;
                if(tc_array[i].isLeaf(n_leaf_limit)) continue;
                const S32 ith = Comm::getThreadNum();
                const S32 adr_ptcl_left  = tc_array[i].adr_ptcl_;
                const S32 adr_ptcl_right = adr_ptcl_left + tc_array[i].n_ptcl_ - 1;
                S32 n_cell_tmp = 0;
                for(S32 j=0; j<N_CHILDREN; j++){
                    S32 part_id = GetPartitionID(tp, adr_ptcl_left, adr_ptcl_right, j, lev_cur+1);
                    if(part_id != -1){
                        tc_buf[ith].increaseSize();
                        tc_buf[ith].back().id_cell_ = j;
                        tc_buf[ith].back().adr_ptcl_ = part_id;
                        tc_buf[ith].back().next_adr_ = -2; // -2 is not youngest child
                        n_cell_tmp++;
                    }
                }
                for(S32 j=1; j<n_cell_tmp; j++){
                    tc_buf[ith].back(-j).n_ptcl_ = tc_buf[ith].back(-j+1).adr_ptcl_ - tc_buf[ith].back(-j).adr_ptcl_;
                }
                tc_buf[ith].back().n_ptcl_   = tc_array[i].n_ptcl_ - (tc_buf[ith].back().adr_ptcl_ - tc_buf[ith].back(-n_cell_tmp+1).adr_ptcl_);
                tc_buf[ith].back().next_adr_ = tc_array[i].next_adr_;
                tc_array[i].n_cell_ = n_cell_tmp;
                n_cell[ith] += n_cell_tmp;
            }
            n_cell_offset[0] = 0;
            for(S32 i=0; i<n_th; i++){
                n_cell_offset[i+1] = n_cell_offset[i] + n_cell[i];
            }
            tc_array.increaseSize( n_cell_offset[n_th] );
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL            
#pragma omp parallel
#endif            
            {
...
#pragma omp for schedule(static)
                ...
                for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                    if(tc_array[i].isLeaf(n_leaf_limit)){
                        ...
                        tc_array[i].more_adr_ = n_cell_offset[ith]+id_cell_right+1+n_cnt;
                        n_cnt += tc_array[i].n_cell_;
                    }
                }
                const S32 i_tail = n_cell_offset[ith+1];
                const S32 i_head = n_cell_offset[ith];
                for(S32 i=i_head; i<i_tail; i++){
                    tc_array[id_cell_right+1+i].adr_ptcl_ = tc_buf[ith][i-i_head].adr_ptcl_;
                    tc_array[id_cell_right+1+i].n_ptcl_   = tc_buf[ith][i-i_head].n_ptcl_;
                    tc_array[id_cell_right+1+i].next_adr_ = (tc_buf[ith][i-i_head].next_adr_ == -2) ? (id_cell_right+i+2) : tc_buf[ith][i-i_head].next_adr_;
                    tc_array[id_cell_right+1+i].level_    = lev_cur+1;
                    tc_array[id_cell_right+1+i].more_adr_ = -1;
                    tc_array[id_cell_right+1+i].id_cell_  = tc_buf[ith][i-i_head].id_cell_;
                    tc_array[id_cell_right+1+i].full_len_sq_  = full_len_sq_cur*0.25;
                }
            }
            S32 n_cell_new = n_cell_offset[n_th];
            ...
            lev_cur++;
            full_len_sq_cur *= 0.25;

            adr_tc_level_partition[lev_cur+1] = id_cell_right + 1;
            if(lev_cur == TREE_LEVEL_LIMIT) break;
        }
        lev_max = lev_cur;
    }
...
#if 0
    // old version (for recursive tree)
    template<class Ttc>
...
#pragma omp parallel for
            ...
            for(S32 j=head; j<next; j++){
                ...
                if( n_tmp == 0) continue;
                    ...
                    S32 n_cell = tc_tmp->n_cell_;
                    for(S32 k=0; k<n_cell; k++){
                        Ttc * tc_tmp_tmp = tc + ((tc_tmp->more_adr_)+k);
...
#pragma omp parallel for
...
#if 1
    template<class Tipg, class Ttc>
    inline void MakeIPGroupLong(ReallocatableArray<Tipg> & ipg_first,
                                const ReallocatableArray<Ttc> & tc_first,
                                const S32 n_grp_limit){
        S32 adr_cell = tc_first[0].more_adr_;
        while(adr_cell != -1){
            const Ttc * tc_cur = tc_first.getPointer(adr_cell);
            if( tc_cur->isLeaf(n_grp_limit) ){
                ipg_first.increaseSize();
                ipg_first.back().copyFromTC(*tc_cur);
                adr_cell = tc_cur->next_adr_;
            }
            else{
                adr_cell = tc_cur->more_adr_;
            }
        }
    }    
#else
    // for recurcive version
  ...
  // NOTE: This FUnction mimic sirial code 
    ...
    inline void SearchSendParticleLong(const ReallocatableArray<Ttc> & tc_first,
                                       ...
                                       const F64ort pos_target, //pos of domain
                                       const F64 inv_theta_sq,
                                       const S32 n_leaf_limit){
        S32 adr_cell = tc_first[0].more_adr_;
        while(adr_cell != -1){
            Ttc * tc_cur = tc_first.getPointer(adr_cell);
            F64vec pos_cell = tc_cur->mom_.getPos();
            F64 r_crit_sq =  tc_cur->full_len_sq_ * inv_theta_sq;
            if(tc_cur->isLeaf(n_leaf_limit)){
                const S32 n_ptcl = tc_cur->n_ptcl_;
                id_ep_send.reserveEmptyAreaAtLeast(n_ptcl);
                S32 adr_ptcl = tc_cur->adr_ptcl_;
                for(S32 ip=0; ip<n_ptcl; ip++){
                    id_ep_send.pushBackNoCheck(adr_ptcl++);
                }
                adr_cell = tc_cur->next_adr_;
            }
            else if(pos_target.getDistanceMinSQ(pos_cell) <= r_crit_sq){
                adr_cell = tc_cur->more_adr_;
            }
            else{
                id_sp_send.push_back(adr_cell);
                adr_cell = tc_cur->next_adr_;
            }
        }
    }    
    ...
    void MakeInteractionListLongEPSP(const ReallocatableArray<Ttc> & tc_first,
	    ...
	    N_CALL_TRAVERSE++;
            ...
            F64 r_crit_sq = tc_cur->full_len_sq_ * inv_theta_sq;
            F64vec pos_cell = tc_cur->mom_.getPos();
            if(pos_target_box.getDistanceMinSQ(pos_cell) > r_crit_sq){
                //sp_list.increaseSize();
                //sp_list.back().copyFromMoment(tc_cur->mom_);
                Tsp sp_tmp;
                sp_tmp.copyFromMoment(tc_cur->mom_);
                sp_list.pushBackNoCheck(sp_tmp);
                adr_cell = tc_cur->next_adr_;
            }

            else if(tc_cur->isLeaf(n_leaf_limit)){
                S32 adr_ptcl = tc_cur->adr_ptcl_;
                S32 n_ptcl = tc_cur->n_ptcl_;
                //ep_list.reserveEmptyAreaAtLeast( n_ptcl );
                //sp_list.reserveEmptyAreaAtLeast( n_ptcl );              
                for(S32 ip=0; ip<n_ptcl; ip++, adr_ptcl++){
                    if( !GetMSB(tp_first[adr_ptcl].adr_ptcl_) ){
                        ep_list.pushBackNoCheck( ep_first[adr_ptcl] );
                    }
                    else{
                        sp_list.pushBackNoCheck( sp_first[adr_ptcl] );
                    }
                }
                adr_cell = tc_cur->next_adr_;                
            }
