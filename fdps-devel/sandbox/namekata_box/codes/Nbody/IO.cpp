...
void IO_Controller::write_data(Nbody_Objects& Nbody_objs) {

   if (time >= output_time) {
      //* Write misc data
      write_misc_data();
      //* Write Nbody data
      write_Nbody_data(Nbody_objs);
...
void IO_Controller::read_data(Nbody_Objects& Nbody_objs, PS::S32 file_num) {
    
   //* Read misc data
   read_misc_data(file_num);
   //* Read Nbody data
   read_Nbody_data(Nbody_objs,file_num);
   //std::ofstream file;
   //file.open("check.txt",std::ios::trunc);
   //   for (PS::S32 i=0; i<Nbody_objs.system.getNumberOfParticleLocal(); i++) {
   //      file << Nbody_objs.system[i].x.x << " "
   //           << Nbody_objs.system[i].x.y << " "
   //           << Nbody_objs.system[i].x.z << std::endl;
   //   }
   //file.close();
   //std::exit(0);
   //* Initialize domain info
   Nbody_objs.dinfo.initialize(); 
   //Nbody_objs.dinfo.setBoundaryCondition(PS::BOUNDARY_CONDITION_PERIODIC_XYZ);
   //Nbody_objs.dinfo.setPosRootDomain(PS::F64vec(-0.5e0*Lx,-0.5e0*Ly,-0.5e0*Lz),                                
   //                                  PS::F64vec(+0.5e0*Lx,+0.5e0*Ly,+0.5e0*Lz)); 
   Nbody_objs.dinfo.decomposeDomainAll(Nbody_objs.system);
   //* Exchange particles
   Nbody_objs.system.exchangeParticle(Nbody_objs.dinfo);
...
void IO_Controller::write_Nbody_data(Nbody_Objects& Nbody_objs) {
   //* Local variables
   std::string filename;
   std::ostringstream string_stream;
   PS::S32 numPtclGlbl  = Nbody_objs.system.getNumberOfParticleGlobal();
   PS::S32 numPtclLocal = Nbody_objs.system.getNumberOfParticleLocal();
   std::vector<Nbody_IO> IO_buf;
   Nbody_IO tmp;
   ...
   for(PS::S32 i=0; i<numPtclLocal; i++){
      tmp.copyFromFP(Nbody_objs.system[i]);
   ...
   MPI::Datatype etype = MPI::BYTE.Create_contiguous(sizeof(Nbody_IO));
   etype.Commit();
   // (2) filtype
   ...
   if (myrank > 0) {
      for (int irank=0; irank<myrank; irank++) 
         disp += ptcl_nums[irank] * sizeof(Nbody_IO); // in bytes
   }
   // (4) write
...
void IO_Controller::read_Nbody_data(Nbody_Objects& Nbody_objs, PS::S32 file_num) {
   //* Local variables
   std::string filename;
   std::ostringstream string_stream;
   PS::S32 numPtclGlbl;
   ...
   input_file.read(reinterpret_cast<char*>(&numPtclLocal), 
                   sizeof(numPtclLocal));
   std::vector<Nbody_IO> IO_buf(numPtclLocal); // specify # of elements
   input_file.read(reinterpret_cast<char*>(&IO_buf[0]),
                   numPtclLocal * sizeof(Nbody_IO)); 
   input_file.close();
#else
   //* MPI-parallel
   int myrank = MPI::COMM_WORLD.Get_rank();
   ...
   // (4) Read
   std::vector<Nbody_IO> IO_buf(numPtclLocal);
   MPI::Status status;
   ...
   Nbody_objs.system.setNumberOfParticleLocal(numPtclLocal);
   for(PS::S32 i=0; i<numPtclLocal; i++){
      Nbody_IO tmp = IO_buf[i];
      tmp.copyToFP(Nbody_objs.system[i]);
   }
