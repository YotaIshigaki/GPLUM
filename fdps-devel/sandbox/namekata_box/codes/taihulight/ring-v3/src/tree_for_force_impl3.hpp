...
#pragma omp single
    ...
    if (Comm::getRank() == 0) {
        ...
        filenum << std::setfill('0') << std::setw(5) << Comm::getRank();
#ifdef USE_NITADORI_ALLGATHER_IN_EX_LET
        filename = "spj_super_domain-new" + filenum.str() + ".txt";
#else
        filename = "spj_super_domain-old" + filenum.str() + ".txt";
#endif
        ofs.open(filename.c_str(), std::ios::trunc);
        for (S32 i=0; i<n_proc_1d; i++) {
            //ofs << i << " " 
            //    << std::setprecision(15)
            //    << spj_super_domain[i].mass << " "
            //    << spj_super_domain[i].pos_phi 
            //    << std::endl;
            ofs << i << " " 
                << spj_super_domain[i].pos_phi 
                << std::endl;
    ...
    const F64 etime = GetWtime() - wtime_offset_in;
    if (Comm::getRank() == 0) std::cout << "etime(allgather,exlet) = " << etime << std::endl;
    athread_halt();
    Finalize();
    std::exit(0);
#endif
