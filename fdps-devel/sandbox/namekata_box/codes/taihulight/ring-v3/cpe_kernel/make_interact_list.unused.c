...
void MakeListUsingTreeWithStack(void * adr_tc_first,
    ...
    tcLM *tc_cur   = (tcLM *) ldm_malloc( bsize_tc );
        ...
        if( !(tc_cur->n_ptcl_ <= n_leaf_limit || tc_cur->level_ == TREE_LEVEL_LIMIT) ){ // not leaf
            ...
            if (open_bit == OPEN_BIT_IV) {
                ...
                for(i=0; i<N_CHILDREN; i++){
                    // new
                    /*
                    //if(tc_child[i].n_ptcl_ <= n_leaf_limit) continue;
                    if(tc_child[i].n_ptcl_ <= n_leaf_limit){
                        open_bit |= (1 << i);
                        continue;
                    }
                    */
                    ...
                    dx = (dx < (len_peri_x - dx)) ? dx : (len_peri_x - dx);
                    dx = (hlen1.x < dx) ? (dx-hlen1.x) : 0.0;
                    dy = (hlen1.y < fabs(cen0.y - cen1.y)) ? fabs(cen0.y - cen1.y)-hlen1.y : 0.0;
                    dz = (hlen1.z < fabs(cen0.z - cen1.z)) ? fabs(cen0.z - cen1.z)-hlen1.z : 0.0;
    #else
                    F64vec_ cen1;
...
void MakeListUsingTree(void *args) {
    ...
    while (reply_get !=1 ) {}
    int myrank = (int)largs[0];
    int n_ipg  = (int)largs[1];
    void *adr_ipg_            = (void *)largs[2];
    void *adr_tc_glb_         = (void *)largs[3];
    void *adr_tp_glb_         = (void *)largs[4];
    void *adr_id_epj          = (void *)largs[5];
    void *adr_id_spj          = (void *)largs[6];
    void *adr_n_disp_epj      = (void *)largs[7];
    void *adr_n_disp_spj      = (void *)largs[8];
    void *adr_n_epj           = (void *)largs[9];
    void *adr_n_spj           = (void *)largs[10];
    void *adr_r_crit_sq       = (void *)largs[11];
    int n_leaf_limit_  = (int)largs[12];
    int sp_tree_offset = (int)largs[13];
    #if PHI_R_TREE
    void *adr_len_peri_x      = (void *)largs[14];
    #endif
    int ca_id_epj = (int)largs[15];
    int ca_id_spj = (int)largs[16];
    void *adr_n_epj_shortfall = (void *)largs[17];
    void *adr_n_spj_shortfall = (void *)largs[18];
    #ifdef REMOVE_VERTEX
    void *adr_cen_tree = (void *)((unsigned long*)args)[19];
    ...
    int *id_epj_cpe_local = (int *) ldm_malloc( bsize_id_epj_array );
    int *id_spj_cpe_local = (int *) ldm_malloc( bsize_id_spj_array );
    int *n_disp_epj_cpe_local = (int *) ldm_malloc( bsize_n_disp_epj );
    int *n_disp_spj_cpe_local = (int *) ldm_malloc( bsize_n_disp_spj );
    int *n_epj_cpe_local = (int *) ldm_malloc( bsize_n_epj );
    int *n_spj_cpe_local = (int *) ldm_malloc( bsize_n_spj );
    int *n_epj_tbl = (int *) ldm_malloc( bsize_n_epj * NUMBER_OF_CPE);
    int *n_spj_tbl = (int *) ldm_malloc( bsize_n_spj * NUMBER_OF_CPE);
    ipgLM *ipg_ = (ipgLM *) ldm_malloc( bsize_ipg );
    double *r_crit_sq = (double *) ldm_malloc( sizeof(double) );
    double *len_peri_x = (double *) ldm_malloc( sizeof(double) );
    #ifdef REMOVE_VERTEX
    F64vec_ *cen_tree      = (double *) ldm_malloc( sizeof(double)*3 );
    double  *half_len_tree = (double *) ldm_malloc( sizeof(double) );
    #endif
