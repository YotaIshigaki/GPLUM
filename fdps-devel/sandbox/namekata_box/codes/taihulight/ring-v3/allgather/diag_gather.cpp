...
struct DiagGather{
	...
	void gather_cm_fast_test(double vals[], double work[]){
		int root = rank_phi % size_rad;
		double sum;
...
int main(int ac, char **av){
   ...
   filenum << std::setfill('0') << std::setw(5) << my_rank;
	...
	for(int i=0; i<dg.size_glb; i++){
		if(i == dg.rank_glb){
         ...
         for (std::vector<double>::iterator it = vals1.begin(); it != vals1.end(); ++it ) {
				printf("%e ", *it);
			}
			puts("");
			fflush(stdout);
...
#if 0
   //* Output vals[] after gather_cm_fast
   filename = "./vals-before-" + filenum.str() + ".txt";
   ofs.open(filename.c_str(), std::ios::trunc);
   for (int i=0; i<vals2.size(); i++)
       ofs << i << " " << vals2[i] << std::endl;
   ofs.close();
#endif
   
	dg.reorder_result(vals2.data(), vals3.data());
#if 0
   //* Output vals[] after reorder_result
   filename = "./vals-after-" + filenum.str() + ".txt";
   ofs.open(filename.c_str(), std::ios::trunc);
   for (int i=0; i<vals3.size(); i++)
       ofs << i << " " << vals3[i] << std::endl;
   ofs.close();
#endif
	...
	for(int i=0; i<dg.size_glb; i++){
		if(i == dg.rank_glb){
         ...
         for (std::vector<double>::iterator it = vals3.begin(); it != vals3.end(); ++it) {
				printf("%e ", *it);
			}
			puts("");
			fflush(stdout);
   ...
   //* [Test]
   dg.gather_cm_fast_test(vals2.data(), work.data());
   dg.reorder_result(vals2.data(), vals3.data());
#if 1
   //* Output vals[] after gather_cm_fast_test
   filename = "./test-" + filenum.str() + ".txt";
   ofs.open(filename.c_str(), std::ios::trunc);
   //for (int i=0; i<vals2.size(); i++)
   //    ofs << i << " " << vals2[i] << std::endl;
   for (int i=0; i<vals3.size(); i++)
