...
dict_tot = {"wtime_calc_shift_z": 0.0,
            ...
            "wtime_rotate": 0.0}
dict_tot_cum = {"wtime_calc_shift_z": 0.0,
                "exchange_particle": 0.0,
                "calc_force_all": 0.0,
                "wtime_copy_epi_to_psys": 0.0,
                "wtime_rotate": 0.0}
dict_tot_cum_1st = {"wtime_calc_shift_z": 0.0,
                    "exchange_particle": 0.0,
                    "calc_force_all": 0.0,
                    "wtime_copy_epi_to_psys": 0.0,
                    "wtime_rotate": 0.0}
...
dict_copy = {"set_particle_local_tree": 0.0,
             ...
             "wtime_copy_epi_to_psys": 0.0,
             "wtime_rotate": 0.0}
dict_copy_cum = {"set_particle_local_tree": 0.0,
                 "add_moment_as_sp_local_tree": 0.0,
                 "set_particle_global_tree": 0.0,
                 "add_moment_as_sp_global_tree": 0.0,
                 "wtime_copy_epi_to_psys": 0.0,
                 "wtime_rotate": 0.0}
dict_copy_cum_1st = {"set_particle_local_tree": 0.0,
                     "add_moment_as_sp_local_tree": 0.0,
                     "set_particle_global_tree": 0.0,
                     "add_moment_as_sp_global_tree": 0.0,
                     "wtime_copy_epi_to_psys": 0.0,
...
dict_tree_cum = {"morton_sort_local_tree": 0.0,
                 "link_cell_local_tree": 0.0,
                 "calc_moment_local_tree": 0.0,
                 "morton_sort_global_tree": 0.0,
                 "link_cell_global_tree": 0.0,
                 "calc_moment_global_tree": 0.0}
dict_tree_cum_1st = {"morton_sort_local_tree": 0.0,
                     "link_cell_local_tree": 0.0,
                     "calc_moment_local_tree": 0.0,
                     "morton_sort_global_tree": 0.0,
                     "link_cell_global_tree": 0.0,
...
dict_let_cum = {"wtime_ex_let_sd_2_allgather_bd": 0.0,
                "wtime_ex_let_sd_2_ex_n_d_d": 0.0,
                "wtime_ex_let_sd_2_ex_n_d_sd_1d_ring": 0.0,
                "wtime_ex_let_sd_2_ex_n_d_sd_sub": 0.0,
                "wtime_ex_let_sd_2_allgather_sd": 0.0,
                ...
                "wtime_ex_let_sd_2_ep_sp_in_sd": 0.0,
                "wtime_ex_let_sd_2_make_list": 0.0}
dict_let_cum_1st = {"wtime_ex_let_sd_2_allgather_bd": 0.0,
                    "wtime_ex_let_sd_2_ex_n_d_d": 0.0,
                    "wtime_ex_let_sd_2_ex_n_d_sd_1d_ring": 0.0,
                    "wtime_ex_let_sd_2_ex_n_d_sd_sub": 0.0,
                    "wtime_ex_let_sd_2_allgather_sd": 0.0,
...
dict_list_cum = {"make_all_interaction_list_id": 0.0}
dict_list_cum_1st = {"make_all_interaction_list_id": 0.0}
...
dict_force_cum = {"calc_force": 0.0}
dict_force_cum_1st = {"calc_force": 0.0}
...
dict_comm_cum = {"exchange_particle": 0.0, "exchange_LET_tot": 0.0}
dict_comm_cum_1st = {"exchange_particle": 0.0, "exchange_LET_tot": 0.0}
...
for str_line in file:
    ...
    for i in range(len(str_list)):
        for key in dict_tot:
            if(str_list[i] == key):
                if(dict_tot[key] == 0.0):
                    dict_tot_cum_1st[key] += float(str_list[i+1])
        ...
        for key in dict_copy:
            if(str_list[i] == key):
                if(dict_copy[key] == 0.0):
                    dict_copy_cum_1st[key] += float(str_list[i+1])
        ...
        for key in dict_tree:
            if(str_list[i] == key):
                if(dict_tree[key] == 0.0):
                    dict_tree_cum_1st[key] += float(str_list[i+1])
        ...
        for key in dict_list:
            if(str_list[i] == key):
                if(dict_list[key] == 0.0):
                    dict_list_cum_1st[key] += float(str_list[i+1])
        ...
        for key in dict_force:
            if(str_list[i] == key):
                if(dict_force[key] == 0.0):
                    dict_force_cum_1st[key] += float(str_list[i+1])
        ...
        for key in dict_comm:
            if(str_list[i] == key):
                if(dict_comm[key] == 0.0):
                    dict_comm_cum_1st[key] += float(str_list[i+1])
        ...
        for key in dict_let:
            if(str_list[i] == key):
                if(dict_let[key] == 0.0):
                    dict_let_cum_1st[key] += float(str_list[i+1])
        ...
        if("n_loop"==str_list[i] and n_loop == 0):
            ...
            wtime_const_local_tree_cum += wtime_const_local_tree
            wtime_calc_moment_local_tree_cum += wtime_calc_moment_local_tree
            print("wtime_const_local_tree_cum", wtime_const_local_tree_cum/n_merge)
            print("wtime_calc_moment_local_tree_cum", wtime_calc_moment_local_tree_cum/n_merge) 
            """
            

            """
            for key in dict_tot:
                dict_tot_cum[key] += dict_tot[key]
                print("    tot(1st):", key, dict_tot_cum_1st[key]/n_loop)
                print("    tot:", key, dict_tot_cum[key]/n_merge_cum)
                dict_tot[key] = 0.0
            for key in dict_copy:
                dict_copy_cum[key] += dict_copy[key]
                print("    copy(1st):", key, dict_copy_cum_1st[key]/n_loop)
                print("    copy:", key, dict_copy_cum[key]/n_merge_cum)
                dict_copy[key] = 0.0
                
            for key in dict_tree:
                dict_tree_cum[key] += dict_tree[key]
                print("    tree(1st):", key, dict_tree_cum_1st[key]/n_loop)
                print("    tree:", key, dict_tree_cum[key]/n_merge_cum)
                dict_tree[key] = 0.0
                
            for key in dict_list:
                dict_list_cum[key] += dict_list[key]
                print("    list(1st):", key, dict_list_cum_1st[key]/n_loop)
                print("    list:", key, dict_list_cum[key]/n_merge_cum)
                dict_list[key] = 0.0
                
            for key in dict_force:
                dict_force_cum[key] += dict_force[key]
                print("    force(1st):", key, dict_force_cum_1st[key]/n_loop)
                print("    force:", key, dict_force_cum[key]/n_merge_cum)
                dict_force[key] = 0.0
                
            for key in dict_comm:
                dict_comm_cum[key] += dict_comm[key]
                print("    comm(1st):", key, dict_comm_cum_1st[key]/n_loop)
                print("    comm:", key, dict_comm_cum[key]/n_merge_cum)
                dict_comm[key] = 0.0

            for key in dict_let:
                if(key != "wtime_ex_let_sd_2_make_list"):
                    wtime_let_comm += dict_let[key]
                    wtime_let_comm_1st += dict_let_cum_1st[key]
                else:
                    wtime_let_const += dict_let[key]
                    wtime_let_const_1st += dict_let_cum_1st[key]
                dict_let[key] = 0.0
                dict_let_cum_1st[key] = 0.0
                
            print("    let_comm(1st)", wtime_let_comm_1st/n_loop)
            print("    let_comm", wtime_let_comm/n_merge_cum)
            print("    let_const(1st)", wtime_let_const_1st/n_loop)
            print("    let_const", wtime_let_const/n_merge_cum)
