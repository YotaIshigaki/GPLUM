...
__thread_local doublev4 d1v4=1.0;
__thread_local doublev4 d0v4=0.0;
__thread_local doublev4 dhv4=0.5;
__thread_local doublev4 d8v4=8.0;
//__thread_local doublev4 dlowone;
__thread_local doublev4 dhuge=HUGE;
__thread_local const uint256 glb_tiny = { REP4(1L << 55) };


#ifdef PROFILE
__thread_local unsigned long p_count, sp_count, t_count, tr_count, kd_count, init_count, mem_count, jlst_count, dmaj_count, dmas_count, geti_count, dmaw_count, st_count, pl_count, ss_count,gets_count, puti_count, puts_count, rfst_count, t2_count;
...
#ifdef PROFILE
    p_count = sp_count = t_count = kd_count = tr_count = 0;
    init_count = mem_count = jlst_count = dmaj_count = dmas_count = geti_count = dmaw_count = st_count = pl_count = ss_count = gets_count = puti_count = puts_count = rfst_count = 0;
#ifdef SUNWAY_FORCE_KERNEL_CHECK
    ch_count=0;
#endif
    t_count = rtc_();
...
#ifdef DEBUG
    if(my_id==ID_CHECK)  printf("allocating work buffer\n");
#endif
...
#ifdef DEBUG
    if(my_id==ID_CHECK) printf("Initializing satellite pointer\n");
#endif
    //satellite number
    // EpiMM* sat_lm;
    // ForceMM* force_sat;
...
//        cpe_broadcast(pars.n_sat*8,sat_lm[0].r);
        if(first_loop_flag) {
          ...
          EpiMM* sat_lm = (EpiMM*) work;
          ForceMM* force_sat = (ForceMM*) ((long)sat_lm+sat_lm_size);
          memset(force_sat,0,n_sat4*FORCEMM_SIZE);
          first_loop_flag=0;
//        
          volatile unsigned int fsat_reply=0;
          athread_put(PE_MODE, force_sat, &pars.force_sat[my_id*n_sat4], n_sat4*FORCEMM_SIZE, (void*)&fsat_reply, 0,0);
          while(fsat_reply<1);
//          sat_reply = 0;
...
//        else cpe_broadcast(pars.n_sat*4,(double*)force_sat);
...
#ifdef DEBUG
    if(my_id==ID_CHECK) printf("Staring i_walk=%d\n",ki);
#endif
...
#endif    
    //! i particle 
    EpiMM* epi_lm=(EpiMM*)(long)ldm_malloc(alloc_size*EPI_SIZE);
#ifdef M_CHECK
    if(my_id==ID_CHECK) printf("EPI_ALLOC = %d, position =%ld\n",alloc_size*EPI_SIZE,(long)epi_lm);
#endif
    //! each force structure packs four particle togethers
    ForceMM4* force_lm=(ForceMM4*)(long)ldm_malloc((2+alloc_size/4)*FORCE_SIZE);
    force_lm +=2;
#ifdef M_CHECK              
    if(my_id==ID_CHECK) printf("FORCE_ALLOC = %d, position=%ld\n",(1+alloc_size/4)*FORCE_SIZE,(long)force_lm);
#endif

    //! left bytes (limit - epi - force - epj/spj)
    int LDM_REMAIN = LDM_B_LIMIT - alloc_size*(EPI_SIZE+FORCEMM_SIZE)-8*(EPI_SIZE+EPJ_SIZE+SPJ_SIZE+FORCEMM_SIZE) - work_size;

#ifdef DEBUG
    if(my_id==ID_CHECK) printf("LDM remaining size=%d\n",LDM_REMAIN);
#endif

    int NJ_LIMIT= (LDM_REMAIN/sizeof(int))>>2<<2;      //! j particle index list size limit
    int n_epj_block = (n_epj)/NJ_LIMIT;     //! index list block number, one more for planet
    int n_epj_remain = (n_epj)%NJ_LIMIT;    //! last index list size, one more fore planet
    if (n_epj_remain) n_epj_block++;
    else n_epj_remain = NJ_LIMIT;

    //! sp particle index list size limit (same as j particle)
    int NS_LIMIT= NJ_LIMIT;
    int n_spj_block = n_spj/NS_LIMIT;    //! index list block number
    int n_spj_remain = n_spj%NS_LIMIT;
    if (n_spj_remain) n_spj_block++;
    else n_spj_remain = NS_LIMIT;
    ...
    for (j=0; j<n_epi_block; j++) {
      ...
      if(ni_off>0) memset(&epi_lm[ni_local],0,ni_off*EPI_SIZE); 
      //      for (k=ni_local; k<ni_local4; k++) epi_lm[k].r[3]=0.0;
#ifdef PROFILE
      tjend = rtc_();
...
#ifdef DEBUG
      if(my_id==ID_CHECK) printf("J address list allocation, size=%d\n",nj_alloc_size);
#endif
      int* jlst=(int*)(long)ldm_malloc(nj_alloc_size*sizeof(int));
#ifdef M_CHECK          
      if(my_id==ID_CHECK) printf("JLST_ALLOC = %d, position=%ld\n",nj_alloc_size*sizeof(int),(long)jlst);
#endif                  
      int sw=0;
...
//#endif
        ...
        if(ns_edge<4) memset(&spj_lm[sw][ns_edge],0,(4-ns_edge)*SPJ_SIZE);
        //for (l=ns_edge; l<4; l++) spj_lm[sw][l].r[3]=0;
#ifdef PROFILE
        tkstart=rtc_();
#endif
#ifdef SPLIT
      ...
      ldm_free(jlst,nj_alloc_size*sizeof(int));
#ifdef M_CHECK          
      if(my_id==ID_CHECK) printf("JLST_FREE = %d\n",nj_alloc_size*sizeof(int));
#endif
      ...
      else memset(&force_sat[sw][0],0,4*FORCEMM_SIZE);
      sw=!sw;
      ...
      if(nsat_edge<4) memset(&sat_lm[sw][nsat_edge],0,(4-nsat_edge)*EPI_SIZE);
#ifdef PROFILE
...
//#endif
    ...
    ldm_free(epi_lm,alloc_size*EPI_SIZE);
    force_lm-=2;
    ldm_free(force_lm,(2+alloc_size/4)*FORCE_SIZE);
...
#endif
  ForceMM* force_sat = (ForceMM*) ((long)work+sat_lm_size);
  volatile unsigned int fsat_reply=0;
  athread_get(PE_MODE, &pars.force_sat[my_id*n_sat4], force_sat, pars.n_sat*FORCEMM_SIZE, (void*)&fsat_reply, 0,0,0);
  while(fsat_reply<1);
#ifdef PROFILE
  ...
  tstart = rtc_();
#endif
  ReduceForceSat(pars.n_sat, force_sat);
#ifdef PROFILE
  tend = rtc_();
...
#endif
    fsat_reply=0;
    athread_put(PE_MODE, force_sat, pars.force_sat, pars.n_sat*FORCEMM_SIZE, (void*)&fsat_reply, 0,0);
    while(fsat_reply!=1);
#ifdef PROFILE
    tend = rtc_();
...
void ForceKernelSunWay2nd(void* pin) {
  ...
  if (n_sat_toff>0) memset(&sat_lm[pars.n_sat],0,n_sat_toff*EPI_SIZE);
  ...
  if(my_id==ID_CHECK) {
    ...
    printf("I-J    \t%e\t%lu\n", (double)   p_count/1.5e9,   p_count); // I-J force
    printf("I-SP   \t%e\t%lu\n", (double)  sp_count/1.5e9,  sp_count); // I-SP force
    printf("I-SAT  \t%e\t%lu\n", (double)  st_count/1.5e9,  st_count); // I-SAT force
    printf("I-PT   \t%e\t%lu\n", (double)  pl_count/1.5e9,  pl_count); // I-Planet force
    printf("SAT-SAT\t%e\t%lu\n", (double)  ss_count/1.5e9,  ss_count); // I-SAT force
    printf("I-K/D  \t%e\t%lu\n", (double)  kd_count/1.5e9,  kd_count); // I kick/drift
    printf("I-Trans\t%e\t%lu\n", (double)  tr_count/1.5e9,  tr_count); // I transformation
    printf("I-init \t%e\t%lu\n", (double)init_count/1.5e9,init_count); // before I block loop
    printf("IF-init\t%e\t%lu\n", (double) mem_count/1.5e9, mem_count); // I force initial
    printf("GET-i  \t%e\t%lu\n", (double)geti_count/1.5e9,geti_count); // DMA i
    printf("GET-lst\t%e\t%lu\n", (double)jlst_count/1.5e9,jlst_count); // DMA Jlst
    printf("GET-j  \t%e\t%lu\n", (double)dmaj_count/1.5e9,dmaj_count); // DMA J
    printf("GET-jw \t%e\t%lu\n", (double)dmaw_count/1.5e9,dmaw_count); // DMA J wait
    printf("GET-sp \t%e\t%lu\n", (double)dmas_count/1.5e9,dmas_count); // DMA sp
    printf("GET-sat\t%e\t%lu\n", (double)gets_count/1.5e9,gets_count); // DMA satellite
    printf("PUT-i  \t%e\t%lu\n", (double)puti_count/1.5e9,puti_count); // DMA put i
    printf("PUT-fst\t%e\t%lu\n", (double)puts_count/1.5e9,puts_count); // DMA put sat force
    printf("RD-fst \t%e\t%lu\n", (double)rfst_count/1.5e9,rfst_count); // reduce sat force
    
    printf("Tot \t%e\t%lu\n", (double)   t_count/1.5e9,   t_count);
    printf("Tot \t%e\t%lu\n", (double)  t2_count/1.5e9,  t2_count);
#ifdef SUNWAY_FORCE_KERNEL_CHECK
    printf("CHF \t%e\t%lu\n", (double)  ch_count/1.5e9,  ch_count);
#endif
  }
#endif
}
