...
#else
	volatile KeyIndex buf[2048]; // 32 KiB in stack
...
#  endif
	...
	volatile KeyIndex work[NSPLIT][NCHUNK_SORT] __attribute__((aligned(256))); // 32 KiB in stack
	volatile KeyIndex buf[1024] __attribute__((aligned(256))); // 16 KiB in stack

	int ilast = -1;

	volatile dma_desc desc_put;
...
void final_kernel(){
	...
	if(n < 3000){
            ...
            volatile KeyIndex *buf = ldm_malloc(buf_size);
...
void split_kernel(){
	...
	volatile unsigned long parts[NSPLIT-1];

	volatile int off[NSPLIT] = {0, };
	int cnt[NSPLIT] = {0, };
	...
	volatile KeyIndex buf[1024]; // 16 KiB in stack
	int n = end - beg;
