...
#if 0
   // for debug 
        PS::F64 cm_mass_ref = MASS_DUST_TOTAL; // is constant if 
        PS::F64vec cm_pos_ref = CM_POS_DUST_TOTAL;
        for(PS::S32 i=0; i<n_walk; i++){
            ...
            cm_pos /= cm_mass;
            if(PS::Comm::getRank() == 0 && i== 0){
                std::cerr<<"rank= "<<PS::Comm::getRank()<<" i= "<<i
                         <<" cm_mass= "<<cm_mass
            ...
            if( fabs(cm_mass_ref-cm_mass)/cm_mass_ref > 1e-7
                || fabs(cm_pos.x-cm_pos_ref.x)/cm_pos_ref.x > 1e-7
                || fabs(cm_pos.y-cm_pos_ref.y)/cm_pos_ref.y > 1e-7){
                std::cerr<<"rank= "<<PS::Comm::getRank()<<" i= "<<i
                         <<" cm_mass= "<<cm_mass
                         <<" cm_mass_ref= "<<cm_mass_ref
                         <<" cm_pos= "<<cm_pos
        ...
        if(PS::Comm::getRank()==0){
            std::cerr<<"OK1 @DispatchKernelWithSPKickDrift"<<std::endl;
        }
        //exit(1);
#endif
    //******** debug [start] *********
...
PS::S32 DispatchKernelWithSPKickDrift(
                                      ...
                                      const Tepj      epj_org[],
                                      const Tspj      spj_org[]){
    EpjBase * epj = new EpjBase[10000000];
    SpjBase * spj = new SpjBase[10000000];
    for(PS::S32 i=0; i<n_walk; i++){
        for(PS::S32 j=n_disp_epj[i]; j<n_disp_epj[i]+n_epj[i]; j++){
            PS::S32 adr = adr_epj[j];
            epj[adr].pos  = epj_org[adr].pos;
            epj[adr].mass = epj_org[adr].mass;
            epj[adr].vel  = epj_org[adr].vel;
            epj[adr].id   = epj_org[adr].id;
        }
    }
    for(PS::S32 i=0; i<n_walk; i++){
        for(PS::S32 j=n_disp_spj[i]; j<n_disp_spj[i]+n_spj[i]; j++){
            PS::S32 adr = adr_spj[j];
            spj[adr].pos  = spj_org[adr].pos;
            spj[adr].mass = spj_org[adr].mass;
        }
    }    
...
PS::S32 DispatchKernelWithSPKickDrift(
                                      ...
                                      const Tspj      spj[]){
    #if 0
        PS::F64 cm_mass_ref = MASS_DUST_TOTAL;
        PS::F64vec cm_pos_ref = CM_POS_DUST_TOTAL;
        for(PS::S32 i=0; i<n_walk; i++){
            PS::F64 cm_mass = 0.0;
            PS::F64vec cm_pos = 0.0;            
