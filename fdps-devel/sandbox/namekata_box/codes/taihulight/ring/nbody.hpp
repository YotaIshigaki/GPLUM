...
/*
Void DumpProfile(std::ofstream & fout,
                 const ProfileForOutput & data){
    const PS::S32 n_proc = PS::Comm::getNumberOfProc();
    fout<<"rank= "<<PS::Comm::getRank()<<std::endl;
    fout<<"time_sys= "<<time_sys<<std::endl;
    fout<<"n_glb= "<<data.n_glb<<std::endl;
    fout<<"n_loc= "<<data.n_loc<<std::endl;
    fout<<"n_proc= "<<n_proc<<std::endl;
    fout<<"<n_epi>(local)= "<<data.n_epi_ave_loc<<std::endl;
    fout<<"<n_epj>(local)= "<<data.n_epj_ave_loc<<std::endl;
    fout<<"<n_spj>(local)= "<<data.n_spj_ave_loc<<std::endl;
    fout<<"<n_epi>(global)= "<<data.n_epi_ave_glb<<std::endl;
    fout<<"<n_epj>(global)= "<<data.n_epj_ave_glb<<std::endl;
    fout<<"<n_spj>(global)= "<<data.n_spj_ave_glb<<std::endl;

    fout<<"n_epi_max= "<<data.n_epi_max<<std::endl;
    fout<<"rank_epi_max= "<<data.rank_epi_max<<std::endl;
    fout<<"n_epi_min= "<<data.n_epi_min<<std::endl;
    fout<<"rank_epi_min= "<<data.rank_epi_min<<std::endl;

    fout<<"n_epj_max= "<<data.n_epj_max<<std::endl;
    fout<<"rank_epj_max= "<<data.rank_epj_max<<std::endl;
    fout<<"n_epj_min= "<<data.n_epj_min<<std::endl;
    fout<<"rank_epj_min= "<<data.rank_epj_min<<std::endl;
    
    fout<<"n_spj_max= "<<data.n_spj_max<<std::endl;
    fout<<"rank_spj_max= "<<data.rank_spj_max<<std::endl;
    fout<<"n_spj_min= "<<data.n_spj_min<<std::endl;
    fout<<"rank_spj_min= "<<data.rank_spj_min<<std::endl;
            
    fout<<"n_ipg= "<<TREE_POINTER->getNumberOfIPG()<<std::endl;
    fout<<"n_ep_send_tot_glb= "<<n_ep_send_tot_glb
        <<" n_ep_send_tot_glb/n_proc= "<<n_ep_send_tot_glb/n_proc
        <<std::endl;
    fout<<"n_sp_send_tot_glb= "<<n_sp_send_tot_glb
        <<" n_sp_send_tot_glb/n_proc= "<<n_sp_send_tot_glb/n_proc
        <<std::endl;
    fout<<"n_sp_send_tot_isendrecv_glb= "<<n_sp_send_tot_isendrecv_glb
        <<" n_sp_send_tot_isendrecv_glb/n_proc= "<<n_sp_send_tot_isendrecv_glb/n_proc
        <<std::endl;
    fout<<"band_width(ep, per proc)= "
        <<(double)(n_ep_send_tot_glb*sizeof(Epj)) / PS::wtime_alltoallv_ep / 1e9 / n_proc
        <<" [GB/s]"
        <<std::endl;
    fout<<"band_width(sp, per proc)= "
        <<(double)(n_sp_send_tot_glb*sizeof(SPJMonopoleScatterSW)) / PS::wtime_alltoallv_sp / 1e9 / n_proc
        <<" [GB/s]"
        <<std::endl;
    fout<<"band_width(sp(isend+irecv), per proc)= "
        <<(double)(n_sp_send_tot_isendrecv_glb*sizeof(SPJMonopoleScatterSW)) / PS::wtime_alltoallv_sp_isendrecv / 1e9 / n_proc
        <<" [GB/s]"
        <<std::endl;            
    DumpProfileParticleSystem(prof_system, fout);
    DumpProfileTree(prof_tree, fout);
}
*/
...
void CalcForceLoopMerge(Ttree  & tree,
    ...
    for(PS::S32 i=0; i<n_loop_merge; i++){
        ...
        if(PS::Comm::getRank() == 0){
            ...
            fout_debug<<"n_tree_cell_loc= "<<n_tree_cell_loc_loc
                      <<" n_tree_cell_glb= "<<n_tree_cell_glb_loc
                      <<std::endl;
            fout_debug<<"n_tree_cell_loc(ave)= "<<(PS::F64)n_tree_cell_loc_glb/n_proc
                      <<" n_tree_cell_glb(ave)= "<<(PS::F64)n_tree_cell_glb_glb/n_proc
                      <<std::endl;
            fout_debug<<"n_op_tot= "<<n_op_tot<<std::endl;
            /*
            ...
            */
            ...
            fout_debug<<"n_loc_max= "<<n_loc_max<<" rank_n_loc_max= "<<rank_n_loc_max<<std::endl;
            fout_debug<<"n_loc_min= "<<n_loc_min<<" rank_n_loc_min= "<<rank_n_loc_min<<std::endl;
            
            fout_debug<<"n_epi_max= "<<n_epi_max<<" rank_epi_max= "<<rank_epi_max<<std::endl;
            fout_debug<<"n_epi_min= "<<n_epi_min<<" rank_epi_min= "<<rank_epi_min<<std::endl;
            fout_debug<<"n_epj_max= "<<n_epj_max<<" rank_epj_max= "<<rank_epj_max<<std::endl;
            fout_debug<<"n_epj_min= "<<n_epj_min<<" rank_epj_min= "<<rank_epj_min<<std::endl;
            fout_debug<<"n_spj_max= "<<n_spj_max<<" rank_spj_max= "<<rank_spj_max<<std::endl;
            fout_debug<<"n_spj_min= "<<n_spj_min<<" rank_spj_min= "<<rank_spj_min<<std::endl;
            
            fout_debug<<"n_ipg= "<<TREE_POINTER->getNumberOfIPG()<<std::endl;
            fout_debug<<"n_fp_send_tot_glb= "<<n_fp_send_tot_glb
            ...
            fout_debug<<"band_width(sp(isend+irecv), per proc)= "
                      ...
                      <<std::endl;            
            */
            fout_debug<<"speed= "<<n_op_tot / (prof_tree.calc_force_all+prof_system.exchange_particle) * 1e-12 <<" [Tflops]"<<std::endl;
            std::cerr<<"speed= "<<n_op_tot / (prof_tree.calc_force_all+prof_system.exchange_particle) * 1e-12 <<" [Tflops]"<<std::endl;
            DumpProfileDomain(prof_domain, fout_debug);
        ...
        //TREE_POINTER->n_interaction_ep_ep_local_ = TREE_POINTER->n_interaction_ep_sp_local_ = 0;
