...
#pragma omp single
    ...
    initialize(const U64 n_glb_tot,
        ...
        //epi_org_.reserve( np_ave+(np_ave+3)/3 + 1024 );
        ...
        */
        if( (FORCE_TYPE)(typename TSM::force_type()).force_type == FORCE_TYPE_LONG){
            if(theta_ > 0.0){
                //const S64 n_tmp = epi_org_.capacity() + (S64)((F64)2000 * pow((0.5 / theta_), DIMENSION));
...
#ifdef USE_STD_SORT
        std::sort(tp_loc_.getPointer(), tp_loc_.getPointer()+n_loc_tot_, LessOPKEY());
#else
...
#ifdef USE_STD_SORT
        std::sort(tp_glb_.getPointer(), tp_glb_.getPointer()+n_glb_tot_, LessOPKEY());        
        /*
...
#pragma omp parallel for
	...
	const S32 n_ipg = ipg_.size();
        //Comm::barrier(); if(Comm::getRank()==0) std::cerr<<"n_ipg= "<<n_ipg<<std::endl;
        ...
        __real_athread_spawn((void *)slave_GetMinIpgBox, args);
        athread_join();
        /*
        if(Comm::getRank()==0){
	...
	for(S32 i=0; i<n_ipg; i++){
	    ...
	    const S32 adr = ipg_[i].adr_ptcl_;
            ...
            for(S32 j=adr; j<adr+n; j++){
                if ( (std::isfinite(epj_sorted_loc_[j].pos.x) != true) ||
                     (std::isfinite(epj_sorted_loc_[j].pos.y) != true) ||
                     (std::isfinite(epj_sorted_loc_[j].pos.z) != true) ){
                    num_nan_loc++;
                    std::cerr<<"j= "<<j
                             <<"epj_sorted_loc_[j].pos= "<<epj_sorted_loc_[j].pos
                             <<std::endl;
                }
            }
            std::cerr<<"num_nan_loc= "<<num_nan_loc<<std::endl;
            if(Comm::getRank()==59){
                std::cerr<<"i= "<<i
                         <<" n= "<<n
                         <<" adr= "<<adr
