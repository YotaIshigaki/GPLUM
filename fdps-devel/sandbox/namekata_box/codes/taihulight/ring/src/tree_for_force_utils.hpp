...
extern "C"{
    #include <athread.h>
    #include <cpe_func.h>
    void SLAVE_FUN(LinkCell)(void *);
    void SLAVE_FUN(CalcMoment)(void *);
    void SLAVE_FUN(CalcMomentLongGlobalTree)(void *);
    void SLAVE_FUN(CalcMomentLean)(void *);
    void SLAVE_FUN(CalcMomentLongGlobalTreeLean)(void *);
}
#endif
...
namespace ParticleSimulator{
    ...
    inline S32vec CalcIDOfImageDomain(const F64ort & pos_root_domain,
        ...
        if( pos_root_domain.contained(pos_target) ){
            //return S32vec(0.0); // is this correct? 2017_0301
            return S32vec(0);
        }
...
// GetPartitionID returnes -1, if there is no appropriate tree cell.
...
#ifdef SUNWAY
    template<class Ttc>
    inline void LinkCellSunWay(ReallocatableArray<Ttc> & tc_array,
                               S32 adr_tc_level_partition[],
                               const TreeParticle tp[],
                               S32 & lev_max,
                               const S32 n_tot,
        ...
        if (Comm::getRank() == 2) {
            std::ofstream ofs;
            std::string filename = "tp.txt";
            std::cout << "outputing ... " << filename << std::endl;
            ofs.open(filename.c_str(),std::ios::trunc);
            for (S32 i=0; i<n_tot; i++) {
               if (i > 3090000) {
               ofs << std::setw(8)  << std::right << i << " "
                   << std::setw(24) << std::right << tp[i].key_ << std::endl;
               }
            }
            ofs.close();
        ...
        std::exit(0);
#endif
        //* Initialize tree cells at level=0
        tc_array.resizeNoInitialize(N_CHILDREN*2); 
        tc_array[0].n_ptcl_ = n_tot;
        tc_array[0].adr_tc_ = N_CHILDREN;
        tc_array[0].adr_ptcl_ = 0;
        ...
        //* Link tree cells on CPEs
#if 0
        if (Comm::getRank() == 2) {// for debug
        std::cout << "n_tot = " << n_tot << std::endl;
#endif
        if (tc_array[0].n_ptcl_ > n_leaf_limit) {
            S32 lev_max_ini = lev_max;
            S32 ca_tc_array;
            S32 OOM_flag,offset;
            for(;;) {
                //** Reset
                lev_max = lev_max_ini;
                ca_tc_array = tc_array.capacity();
                OOM_flag = 0;
                offset = 0;
                ...
                args[1] = (unsigned long) tc_array.getPointer();
                args[2] = (unsigned long) adr_tc_level_partition;
                args[3] = (unsigned long) tp;
                args[4] = (unsigned long) &lev_max;
                args[5] = (unsigned long) n_tot;
                args[6] = (unsigned long) n_leaf_limit;
                args[7] = (unsigned long) ca_tc_array;
                args[8] = (unsigned long) &OOM_flag;
                args[9] = (unsigned long) &offset;

                //** Call a CPE function
                __real_athread_spawn((void*)slave_LinkCell, args);
                athread_join();

                //** Update tc_array.size() and extend tc_array if needed
                if (offset > 0) {
                    //std::cout << "offset = " << offset << " (RANK:" << Comm::getRank() << ")" << std::endl;
                    tc_array.resizeNoInitialize(offset);
                }
        ...
        Comm::barrier();
        if (Comm::getRank() == 0) std::cerr<<"End point of LinkCell"<<std::endl;
        Finalize();
        std::exit(0);
#endif
    }
    ...
    inline void CalcMoment(S32 adr_tc_level_partition[], 
                           ...
                           const S32 lev_max,
                           const S32 n_leaf_limit){
        #ifdef DEBUG_PRINT_CALC_MOMENT_LOCAL
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 01 @CalcMoment"<<std::endl;
            for(S32 i=0; i<=lev_max+1; i++){
                std::cerr<<"i= "<<" adr_tc_level_partition[i]= "<<adr_tc_level_partition[i]<<std::endl;
            }
        }
        #endif
        //#if defined(SUNWAY) && !defined(REDUCE_MEMORY_DEV)
#ifdef SUNWAY
        //#if 0
        //***** debug(start) *****
        //S32 flag_loc,flag,j_tgt=-1;
        //flag_loc = 0;
        ...
        //    }
        //}
        //MPI::COMM_WORLD.Allreduce(&flag_loc,&flag,1,MPI::INT,MPI::SUM);
        //if (flag > 0) {
        //    //* Output
        //    if (Comm::getRank() == 0) {
        //        std::cout << std::endl;
        //        std::cout << "#### particle clusters in leafs ####" << std::endl;
        //    }
        //    for (S32 irank=0; irank<Comm::getNumberOfProc(); irank++) {
        //        if (irank = Comm::getRank()) {
        //            if (flag_loc > 0) {
        //                std::cout << "myrank = " << Comm::getRank() << "  " 
        //                          << "flag_loc = " << flag_loc << std::endl;
        //                std::ofstream fp;
        //                std::string fname;
        //                std::stringstream fnum;
        //                fnum << std::setfill('0') << std::setw(5) << Comm::getRank();
        //                fname = "./tree/data" + fnum.str() + ".dat";
        //                fp.open(fname.c_str(),std::ios::trunc); 
        //                Ttc * tc_tmp = tc + j_tgt;
        //                const int n_tmp = tc_tmp->n_ptcl_;
        //                const S32 adr = tc_tmp->adr_ptcl_;
        //                for(S32 k=adr; k<adr+n_tmp; k++){
        //                    //std::cout << epj[k].pos.x << "  "
        //                    //          << epj[k].pos.y << "  "
        //                    //          << epj[k].pos.z << "  "
        //                    //          << std::endl;
        //                    fp << epj[k].pos.x << "  "
        ...
        F64 wtime_stop_lev_max = 0.0;
        F64 wtime_stop_lev_max_next = 0.0;
        #endif
        double rsrch = Tepj::r_search;
        for (S32 i=lev_max; i>=0; --i){
            //* Compute the arguments of slave_CalcMoment()
            unsigned long args[6];
            args[0] = (unsigned long) epj;
            args[1] = (unsigned long) tc;
            args[2] = (unsigned long)(n_leaf_limit); 
            args[3] = (unsigned long)(adr_tc_level_partition[i]);   // `head`
            args[4] = (unsigned long)(adr_tc_level_partition[i+1]); // `next`
            args[5] = (unsigned long)(&rsrch); // the address of the search radius
            //* Compute moment on CPEs
            __real_athread_spawn((void*)slave_CalcMoment, args);
            athread_join();
        #ifdef DEBUG_PRINT_CALC_MOMENT_LOCAL
            if(i==lev_max) wtime_stop_lev_max = GetWtime();
            if(i==lev_max-1) wtime_stop_lev_max_next = GetWtime();
        #endif
        }
        #ifdef DEBUG_PRINT_CALC_MOMENT_LOCAL
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 02 @CalcMoment"<<std::endl;
            std::cerr<<"tc[0].mass= "<<tc[0].mom_.mass<<std::endl;
            std::cerr<<"tc[0].pos.x= "<<tc[0].mom_.pos.x<<"   "<<tc[0].mom_.pos.y<<"   "<<tc[0].mom_.pos.z<<std::endl;
            for(S32 i=8; i<16; i++){
                std::cerr<<"i= "<<i<<std::endl;
                std::cerr<<"tc[i].mass= "<<tc[i].mom_.mass<<std::endl;
                std::cerr<<"tc[i].n_ptcl_= "<<tc[i].n_ptcl_<<std::endl;
                std::cerr<<"tc[i].level_= "<<tc[i].level_<<std::endl;
                std::cerr<<"tc[i].pos= "<<tc[i].mom_.pos.x<<"   "<<tc[i].mom_.pos.y<<"   "<<tc[i].mom_.pos.z<<std::endl;
            }
        }
        /*
        F64 wtime_stop = GetWtime();
        if(Comm::getRank() == 0) {
            std::cerr<<"calc moment local (total)= "<<wtime_stop - wtime_offset
                     <<" calc moment local (lev_max)= "<<wtime_stop_lev_max - wtime_offset
                     <<" calc moment local (lev_max-1)= "<<wtime_stop_lev_max_next - wtime_offset
                     <<std::endl;
            for(S32 i=0; i<=lev_max+1; i++){
                std::cerr<<"i= "<<" adr_tc_level_partition[i]= "<<adr_tc_level_partition[i]<<std::endl;
            }
...
#pragma omp parallel for
    ...
    template<class Ttc, class Tepj>
    inline void CalcMomentLean(S32 adr_tc_level_partition[], 
                               Ttc tc[],
                               Tepj epj[],
                               const S32 lev_max,
                               const S32 n_leaf_limit){
        #ifdef DEBUG_PRINT_CALC_MOMENT_LOCAL_LEAN
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 01 @CalcMomentLean"<<std::endl;
            for(S32 i=0; i<=lev_max+1; i++){
                std::cerr<<"i= "<<" adr_tc_level_partition[i]= "<<adr_tc_level_partition[i]<<std::endl;
            }
        }
        ...
        for (S32 i=lev_max; i>=0; --i){
            ...
            args[5] = (unsigned long)(&rsrch); // the address of the search radius
            //* Compute moment on CPEs
            __real_athread_spawn((void*)slave_CalcMomentLean, args);
            athread_join();
        }
        #ifdef DEBUG_PRINT_CALC_MOMENT_LOCAL_LEAN
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 02 @CalcMomentLean"<<std::endl;
            std::cerr<<"tc[0].mass= "<<tc[0].mass<<std::endl;
            std::cerr<<"tc[0].pos.x= "<<tc[0].pos.x<<"   "<<tc[0].pos.y<<"   "<<tc[0].pos.z<<std::endl;
            //std::cerr<<"tc[0].pos_phi= "<<tc[0].pos_phi<<" tc[0].pos_r= "<<tc[0].pos_r<<std::endl;
            for(S32 i=8; i<16; i++){
                std::cerr<<"tc[i].mass= "<<tc[i].mass<<std::endl;
                std::cerr<<"tc[i].n_ptcl_= "<<tc[i].n_ptcl_<<std::endl;
                std::cerr<<"tc[i].level_= "<<tc[i].level_<<std::endl;
                std::cerr<<"tc[i].pos= "<<tc[i].pos.x<<"   "<<tc[i].pos.y<<"   "<<tc[i].pos.z<<std::endl;
                //std::cerr<<"tc[i].pos_phi= "<<tc[i].pos_phi<<" tc[i].pos_r= "<<tc[i].pos_r<<std::endl;
            }
        }
        #endif
    }
    ...
    inline void CalcMomentLongGlobalTree(S32 adr_tc_level_partition[], 
                                         ...
                                         const S32 lev_max,
                                         const S32 n_leaf_limit){
        #ifdef DEBUG_PRINT_CALC_MOMENT_GLOBAL
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 01 @CalcMomentGlobal"<<std::endl;
            for(S32 i=0; i<=lev_max+1; i++){
                std::cerr<<"i= "<<" adr_tc_level_partition[i]= "<<adr_tc_level_partition[i]<<std::endl;
            }
        }
...
//#ifdef SUNWAY
        ...
        for (S32 i=lev_max; i>=0; --i){
            ...
            args[0] = (unsigned long) tc;
            args[1] = (unsigned long) tp;
            args[2] = (unsigned long) epj;
            args[3] = (unsigned long) spj;            
            args[4] = (unsigned long)(n_leaf_limit);
            args[5] = (unsigned long)(adr_tc_level_partition[i]);   // `head`
            args[6] = (unsigned long)(adr_tc_level_partition[i+1]); // `next`
            args[7] = (unsigned long)(&rsrch); // the address of the search radius
            //* Compute moment on CPEs
            __real_athread_spawn((void*)slave_CalcMomentLongGlobalTree, args);
            athread_join();
        }
#else
    //#ifdef REDUCE_MEMORY
...
#pragma omp parallel for
            ...
            for(S32 j=head; j<next; j++){
                ...
                if( n_tmp == 0) continue;
                    ...
                    const S32 adr = tc_tmp->adr_ptcl_;
                    for(S32 k=adr; k<adr+n_tmp; k++){
                        const U32 adr_ptcl = tp[k].adr_ptcl_;
                        if( GetMSB(tp[k].adr_ptcl_) == 0 ){
                            tc_tmp->mom_.accumulateAtLeaf(epj[adr_ptcl]);
                        }
                        else{
                            tc_tmp->mom_.accumulate(spj[ClearMSB(adr_ptcl)].convertToMoment());
                        }
                    }
                    tc_tmp->mom_.set();
                    /*
...
#pragma omp parallel for
        ...
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 02 @CalcMomentGLobal"<<std::endl;
            std::cerr<<"tc[0].mass= "<<tc[0].mom_.mass<<std::endl;
            std::cerr<<"tc[0].pos.x= "<<tc[0].mom_.pos.x<<"   "<<tc[0].mom_.pos.y<<"   "<<tc[0].mom_.pos.z<<std::endl;
            for(S32 i=8; i<16; i++){
                std::cerr<<"i= "<<i<<std::endl;
    ...
    template<class Ttc, class Tepj, class Tspj>
    inline void CalcMomentLongGlobalTreeLean(S32 adr_tc_level_partition[], 
                                             Ttc tc[],
                                             TreeParticle tp[],
                                             Tepj epj[],
                                             Tspj spj[],
                                             const S32 lev_max,
                                             const S32 n_leaf_limit){
        #ifdef DEBUG_PRINT_CALC_MOMENT_GLOBAL_LEAN
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 01 @CalcMomentGlobalLean"<<std::endl;
            for(S32 i=0; i<=lev_max+1; i++){
                std::cerr<<"i= "<<" adr_tc_level_partition[i]= "<<adr_tc_level_partition[i]<<std::endl;
            }
        }
        ...
        for (S32 i=lev_max; i>=0; --i){
            ...
            args[7] = (unsigned long)(&rsrch); // the address of the search radius
            //* Compute moment on CPEs
            __real_athread_spawn((void*)slave_CalcMomentLongGlobalTreeLean, args);
            athread_join();
        }
        #ifdef DEBUG_PRINT_CALC_MOMENT_GLOBAL_LEAN
        Comm::barrier();
        if(Comm::getRank() == 0) {
            std::cerr<<"OK 02 @CalcMomentGlobalLean"<<std::endl;
            std::cerr<<"tc[0].mass= "<<tc[0].mass<<std::endl;
            std::cerr<<"tc[0].pos.x= "<<tc[0].pos.x<<"   "<<tc[0].pos.y<<"   "<<tc[0].pos.z<<std::endl;
            for(S32 i=8; i<16; i++){
                std::cerr<<"tc[i].mass= "<<tc[i].mass<<std::endl;
  ...
  // NOTE: This FUnction mimic sirial code 
    ...
    inline void MakeIPGroupUseGLBTreeLong(ReallocatableArray<Tipg> & ipg_first,
        ...
        /*
        if(Comm::getRank()==59){
            std::cerr<<"A0) adr_tc_loc= "<<adr_tc_loc
                     <<" adr_tc_glb= "<<adr_tc_glb
                     <<std::endl;
        }
        */
        const Ttcloc * tc_loc_tmp = tc_loc_first.getPointer(adr_tc_loc);
        ...
        /*
        if(Comm::getRank()==59){
            std::cerr<<"A1) n_loc_tmp= "<<n_loc_tmp
                     <<" n_glb_tmp= "<<n_glb_tmp
                     <<std::endl;
        }
        */
        if(n_loc_tmp == 0 || n_glb_tmp == 0) return;
            ...
            /*
            if(Comm::getRank()==59){
                std::cerr<<"B0) ipg_first.size()= "<<ipg_first.size()<<std::endl;
            }
            */
            ipg_first.increaseSize();
            ipg_first.back().copyFromTC(*tc_loc_tmp);
            /*
            if(Comm::getRank()==59){
                std::cerr<<"B1) ipg_first.size()= "<<ipg_first.size()<<std::endl;
            } 
            */           
	    ...
	    //ipg_first.back().vertex_ = GetMinBoxSingleThread(epi_first.getPointer(adr_tmp), n_loc_tmp);
            ...
            /*
            if(Comm::getRank()==59){
                std::cerr<<"D0) adr_tc_loc_tmp= "<<adr_tc_loc_tmp
                         <<" adr_tc_glb_tmp= "<<adr_tc_glb_tmp
                         <<std::endl;
            }
            */
            for(S32 i=0; i<N_CHILDREN; i++){
                /*
                if(Comm::getRank()==59){
                    std::cerr<<"C0) i= "<<i<<std::endl;
                }
                */
                MakeIPGroupUseGLBTreeLong
		  (ipg_first, tc_loc_first, tc_glb_first, epi_first, 
		   adr_tc_loc_tmp+i, adr_tc_glb_tmp+i, n_grp_limit, n_leaf_limit);
                //if(Comm::getRank()==59) std::cerr<<"C1) i= "<<i<<std::endl;
            }
            /*
            if(Comm::getRank()==59){
                std::cerr<<"D1) adr_tc_loc_tmp= "<<adr_tc_loc_tmp
                         <<" adr_tc_glb_tmp= "<<adr_tc_glb_tmp
                         <<std::endl;
            } 
            */           
        ...
        /*
        if(Comm::getRank()==59){
            std::cerr<<"A2) adr_tc_loc= "<<adr_tc_loc
                     <<" adr_tc_glb= "<<adr_tc_glb
                     <<std::endl;
        }
        */
