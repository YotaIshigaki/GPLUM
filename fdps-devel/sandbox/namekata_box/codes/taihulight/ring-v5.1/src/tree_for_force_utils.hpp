...
// GetPartitionID returnes -1, if there is no appropriate tree cell.
    ...
    inline void LinkCellSunWay(Ttcarray & tc_array,
                               S32 adr_tc_level_partition[],
                               const TreeParticle tp[],
                               S32 & lev_max,
                               const S32 n_tot,
        ...
        tc_array.reserve(tc_array.getLeftSizeOfMemoryPool()-1024);
        tc_array.setSize(N_CHILDREN*2);
#else
        tc_array.resizeNoInitialize(N_CHILDREN*2);
#endif
        tc_array[0].n_ptcl_ = n_tot;
    ...
    inline void CalcMoment(S32 adr_tc_level_partition[], 
        ...
        // [Check]
        if (Comm::getRank() == 1) {
            std::cout << "outputing epj @CalcMoment()" << std::endl;
            std::ofstream ofs;
            std::stringstream filenum;
            std::string filename;
            filenum << std::setfill('0') << std::setw(5) << Comm::getRank();
            filename = "./epj" + filenum.str() + ".txt";
            ...
            ofs.close();
            std::cout << "output of epj is completed!" << std::endl;
        }
        Comm::barrier();
        if (Comm::getRank() == 0) std::cout << "OK bokujo!" << std::endl;
        athread_halt();
        Finalize();
        std::exit(0);
#endif                                                                         
...
#endif // DEBUG_PRINT_CALC_MOMENT_LOCAL
...
#endif // DEBUG_PRINT_CALC_MOMENT_GLOBAL
#if defined(SUNWAY)
        double rsrch = Tepj::r_search;
        for (S32 i=lev_max; i>=0; --i){
            //* Compute the arguments of slave_CalcMoment()
...
#pragma omp parallel for
  ...
  // NOTE: This FUnction mimic sirial code 
    ...
    inline void MakeIPGroupUseGLBTreeLong(ReallocatableArray<Tipg> & ipg_first,
					  const ReallocatableArray<Ttcloc> & tc_loc_first,
					  const TempArray<Ttcglb> & tc_glb_first,
					  const ReallocatableArray<Tepi> & epi_first,
					  const S32 adr_tc_loc,
					  const S32 adr_tc_glb,
					  const S32 n_grp_limit,
    ...
    inline void MakeIPGroupUseGLBTreeLong(ReallocatableArray<Tipg> & ipg_first,
        ...
        if(n_loc_tmp == 0 || n_glb_tmp == 0) return;
#ifdef USE_MEMORY_POOL
        else if( (tc_glb_tmp->n_ptcl_<=n_grp_limit  || tc_glb_tmp->level_ == TREE_LEVEL_LIMIT) ||
                 (tc_loc_tmp->n_ptcl_<=n_leaf_limit || tc_loc_tmp->level_ == TREE_LEVEL_LIMIT) ){
#else
        else if( tc_glb_tmp->isLeaf(n_grp_limit) || tc_loc_tmp->isLeaf(n_leaf_limit)){
#endif
            ipg_first.increaseSize();
