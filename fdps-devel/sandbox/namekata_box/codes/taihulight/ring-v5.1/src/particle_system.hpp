...
#ifdef DEBUG_PRINT_EX_PTCL8_V3
                Comm::barrier();
                if(Comm::getRank()==0)std::cerr<<"OK3-a @exchangeParticle8v3"<<std::endl;
#endif
                // Calculate __n_send[]
                for (S32 i=0; i<n_loc; i++) {
                   const S32 srank = dest_rank[i];
                   assert((0 <= srank) && (srank < n_proc));
                   //if ((srank < 0) || (n_proc <= srank)) {
                   //    std::cerr << "(my_rank,i,srank) = " << my_rank << " " << i << " " << srank << std::endl;
                   //    assert((0 <= srank) && (srank < n_proc));
                   //}
                   n_send[srank]++;
                }
...
#ifdef DEBUG_PRINT_EX_PTCL8_V3
                Comm::barrier();
                if(Comm::getRank()==0)std::cerr<<"OK3-b @exchangeParticle8v3"<<std::endl;
#endif
                // Release memory 
                dest_rank.free();
