...
void CopyTCToETC(void *args){
    ...
    volatile tcLM  *tc  = (tcLM *)  ldm_malloc( bsize_tc_array );
    volatile etcLM *etc = (etcLM *) ldm_malloc( bsize_etc_array );
...
void CopyEPISortedToEPJSortedLoc(void *args){
    ...
    volatile epiLM  *epi     = (epiLM *) ldm_malloc( bsize_epi_array );
    volatile epjLM  *epj     = (epjLM *) ldm_malloc( bsize_epj_array );
    volatile epjLM  *epj_org = (epjLM *) ldm_malloc( bsize_epj_array );
    //* Compute moments on each CPE
    // (loop counters)
    int i,j,k; 
    // (local vars. for DMA comm.)
...
void CopyIndirect(void * arg){
    ...
    volatile double data_lm[ELEMENT_SIZE_LIMIT];
    volatile int adr_lm[LOCAL_MEMORY_SIZE_LIMIT/4];    
...
void CopyIndirectInverse2(void * arg){
    ...
    int my_id = athread_get_id(-1);
    int data_size_limit_in_byte = LOCAL_MEMORY_SIZE_LIMIT;
    volatile double data_lm[LOCAL_MEMORY_SIZE_LIMIT/8];
    int n_element = (int)(((unsigned long*)arg)[0]);
    double * data_src = (double*)((unsigned long*)arg)[1];
    double * data_dst = (double*)((unsigned long*)arg)[2];
    int element_size_in_byte = (int)(((unsigned long*)arg)[3]);
...
void CopyTCMomToSPJ(void * args){
    ...
    volatile tcLM *tc = (tcLM *) ldm_malloc( bsize_tc * CHUNK_SIZE );
    volatile spjLM *spj = (spjLM *) ldm_malloc( bsize_spj * CHUNK_SIZE );
    //* Perform copy
    // (loop counters)
    int i,j;
    // (local vars. for DMA)
...
void CopyETCMomToSPJ(void * args){
    ...
    size_t bsize_spj = sizeof(spjLM);
    volatile etcLM *tc = (etcLM *) ldm_malloc( bsize_tc * CHUNK_SIZE );
    volatile spjLM *spj = (spjLM *) ldm_malloc( bsize_spj * CHUNK_SIZE );
    //* Perform copy
    // (loop counters)
    int i,j;
...
void CopyEpjOrgToEpiSortedEpjSorted(void *args){
    ...
    volatile epjLM * epj_org = (epjLM *) ldm_malloc( bsize_epj_array );
    volatile epiLM * epi_sor = (epiLM *) ldm_malloc( bsize_epi_array );
    volatile epjLM * epj_sor = (epjLM *) ldm_malloc( bsize_epj_array );
    volatile tpLM  * tp_loc  = (tpLM *) ldm_malloc( bsize_tp_array );
...
#else
            //-(epi_sorted_)
            epi_sor[j].pos.x   = epj_org[j].pos.x;
            epi_sor[j].pos.y   = epj_org[j].pos.y;
            epi_sor[j].pos.z   = epj_org[j].pos.z;
            epi_sor[j].mass    = epj_org[j].mass;
            epi_sor[j].vel.x   = epj_org[j].vel.x;
            epi_sor[j].vel.y   = epj_org[j].vel.y;
            epi_sor[j].vel.z   = epj_org[j].vel.z;
            epi_sor[j].id      = epj_org[j].id;
            //-(epj_sorted_)
            epj_sor[j].pos.x   = epj_org[j].pos.x;
            epj_sor[j].pos.y   = epj_org[j].pos.y;
            epj_sor[j].pos.z   = epj_org[j].pos.z;
            epj_sor[j].mass    = epj_org[j].mass;
            epj_sor[j].vel.x   = epj_org[j].vel.x;
            epj_sor[j].vel.y   = epj_org[j].vel.y;
            epj_sor[j].vel.z   = epj_org[j].vel.z;
            epj_sor[j].id      = epj_org[j].id;
#endif
        }
