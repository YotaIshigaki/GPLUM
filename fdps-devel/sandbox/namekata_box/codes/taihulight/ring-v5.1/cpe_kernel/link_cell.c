...
__thread_local dma_desc dma_get, dma_put; // SHOULD NOT be modified by volatile if using -O2
...
//static int getCellID(int lev, KeyT_ mkey){
static int getCellID(int lev, const tpLM * tp){
    U64_ s;
#ifdef USE_96BIT_KEY
    int kLevMaxUp = 21;
    int kLevMaxLo = 10;
    int kLevMax   = 31;
    if(lev <= kLevMaxUp){
        s = tp->key_hi_ >> ( (kLevMaxUp - lev) * 3 );
    }
    else{
        s = tp->key_lo_ >> ( (kLevMaxLo - (lev-kLevMaxUp)) * 3 );
    }
#else
    int kLevMax = 21;
    s = tp->key_hi_ >> ( (kLevMax - lev) * 3 );
#endif
    return (s & 0x7);
}
...
static int GetPartitionID(void *adr_tp,
    ...
    while (reply_get != 2) {}
    if( getCellID(lev, &tp_r) < ref) return -1;
    if( getCellID(lev, &tp_l) > ref) return -1;
    ...
    while(l < r){
        ...
        while (reply_get != 1) {}
        //** Compute val_cen
        int val_cen = getCellID(lev, &tp_c);
        if (ref > val_cen){
            l = cen+1;
        } else {
            r = cen;
        ...
        while (reply_get != 1) {}
        //** Check 
        if( getCellID(lev, &tp_l) == ref ) return l;
    }
    return -1;
}
...
static int GetPartitionID_wo_DMA(tpLM tp[],
    ...
    //* Get tp[left] & tp[right]
    if( getCellID(lev, &tp[r]) < ref) return -1;
    if( getCellID(lev, &tp[l]) > ref) return -1;
    //* Find with binary search
    while(l < r){
        //** Compute cen
        int cen = (l+r) / 2;
        //** Compute val_cen
        int val_cen = getCellID(lev, &tp[cen]);
        if (ref > val_cen){
            l = cen+1;
        } else {
            r = cen;
        ...
        //** Check 
        if( getCellID(lev, &tp[l]) == ref ) return (l+left);
    }
    return -1;
} 
...
void LinkCell(void *args) {
    ...
    volatile tcLM *tc_parent = (tcLM *)ldm_malloc( bsize_tc_parent );
    volatile tcLM *tc_child  = (tcLM *)ldm_malloc( bsize_tc_child );
    volatile tpLM *tp        = (tpLM *)ldm_malloc( bsize_tp_array );
    volatile int *n_cell_new_tbl = (int *)ldm_malloc( sizeof(int) * NUMBER_OF_CPE);
    volatile int *adr_tc_level_partition = (int *)ldm_malloc( sizeof(int) * (TREE_LEVEL_LIMIT+2) );
#ifdef CHECK_LINK_CELL
    if ((myrank == RANK_CHECK) && (my_id == ID_CHECK)) {
        printf("bsize_tc             = %d\n",bsize_tc);
        printf("bsize_tp             = %d\n",bsize_tp);
    ...
    int ista,iend;
    // (other vars.)
    int n_cell_new,offset;
    volatile int lev_max;
    int use_tp_buffer;
    ...
    while(1){
...
#if defined(CHECK_LINK_CELL)
                if (myrank == RANK_CHECK && my_id == ID_CHECK) {
                    printf("List of n_ptcl_ (n_cnt = %d):\n",n_cnt);
                    for (j=0; j<n_cnt; j++)
                        printf("j = %d,  adr[j] = %d, n_ptcl_ = %d\n",j,adr[j],tc_child[adr[j]].n_ptcl_);
                    printf("Content of tc_child[]:\n");
                    for (j=0; j<N_CHILDREN; j++)
                        printf("j = %d, tc_child[j].n_ptcl_ = %d\n",j,tc_child[j].n_ptcl_);
                }
#endif
...
#if defined(CHECK_LINK_CELL)
                if (myrank == RANK_CHECK && my_id == ID_CHECK) {
                    printf("tc_parent[ii].adr_tc_ = %d\n",tc_parent[ii].adr_tc_);
                    printf("DMA put of tc_child completed.\n");
                }
#endif
                // Update n_cell_new
        ...
        sync_array_();
#ifdef CHECK_LINK_CELL
        if (myrank == RANK_CHECK && my_id == ID_CHECK) printf("chunk-loop ends\n");
        sync_array_();
#endif
#if defined(TIME_LINK_CELL)
       ...
       sync_array_();
       if (myrank == RANK_CHECK && my_id == ID_CHECK) {
           printf("n_tc_tgt (next)  = %d\n",n_tc_tgt);
           printf("n_loc (next)     = %d\n",n_loc);
           printf("my_offset (next) = %d\n",my_offset);
       }
       sync_array_();
#endif
       ...
       sync_array_();
       if (myrank == RANK_CHECK && my_id == ID_CHECK) {
           printf("prefix sum calculation started.\n");
       }
       sync_array_();
#endif
#if defined(TIME_LINK_CELL)
       ...
       sync_array_();
       if (myrank == RANK_CHECK && my_id == ID_CHECK) {
           printf("prefix sum calculation ended.\n");
       }
       sync_array_();
#endif
#if 0
       ...
       sync_array_();
       if (myrank == RANK_CHECK && my_id == ID_CHECK) {
           printf("tc_array[].adr_tc_ is set.\n");
       }
       sync_array_();
#endif
        ...
        sync_array_();
        if (myrank == RANK_CHECK && my_id == ID_CHECK) {
            printf("checking memory of MPE buffers.\n");
        }
        sync_array_();
#endif
        ...
        sync_array_();
        if (myrank == RANK_CHECK && my_id == ID_CHECK) {
            printf("OOM_flag is updated (lev_max = %d):\n",lev_max);
            printf("offset      = %d\n",offset);
            printf("ca_tc_array = %d\n",ca_tc_array);
        }
        sync_array_();
