...
#ifdef PHI_R_TREE
        nx = (U64_)( (epj[i].pos_phi - cx) * normalized_factor_);
        ny = (U64_)( (epj[i].pos_r - cy) * normalized_factor_);
#else        
...
#ifdef KEY_3D
__attribute__((noinline))
void conv_mtn(int N, int ioffset,
              double pos_phi, double pos_r, double pos_z,
              tpLM tp[], double *length_, F64vec_ *center_){
    int i;
    int kLevMax = 21;
    double half_len_ = 0.5 * (*length_);
    double normalized_factor_ = (1.0/(half_len_*2.0))*(1<<kLevMax);
...
__attribute__((noinline))
//void conv_mtn(int N, int ioffset, epjLM epj[], tpLM tp[], double *length_, F64vec_ *center_){
void conv_mtn(int N, int ioffset, double pos_phi, double pos_r, double pos_z,
              tpLM tp[], double *length_, F64vec_ *center_){
    int i;
    int kLevMax = 21;
    double half_len_ = 0.5 * (*length_);
...
void conv_mtn(int N, int ioffset, epjLM epj[], tpLM tp[],
    ...
    for (i=0; i<N; i++){
        ...
        tp[i].key_.up_ = ( retx<<2 | rety<<1 | retz ); // the compile option must be -O0 or -O1.
        ...
        tp[i].key_.lo_ = ( retx_lo<<2 | rety_lo<<1 | retz_lo ); // the compile option must be -O0 or -O1.
#else
        separateBit(nx, retx);
        separateBit(ny, rety);
        separateBit(nz, retz);
