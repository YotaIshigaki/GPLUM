...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    PS::TREE_Z_COORD_OFFSET = CalcZCoordShift(&system_grav[0], system_grav.getNumberOfParticleLocal(), r_phy*0.1);
        ...
        if(PS::Comm::getRank() == 0){
            fout_wtime<<"dinfo.pos_domain[my_rank].low_= "<<dinfo.getPosDomain(my_rank).low_
                      <<" dinfo.pos_domain[my_rank].high_= "<<dinfo.getPosDomain(my_rank).high_
                      <<std::endl;
            fout_wtime<<"n_proc_ep_send_= "<<tree_grav.comm_table_.n_proc_ep_send_
                      <<" n_proc_ep_recv_= "<<tree_grav.comm_table_.n_proc_ep_recv_
                      <<std::endl;
            fout_wtime<<"n_proc_sp_isend_= "<<tree_grav.comm_table_.n_proc_sp_isend_
                      <<" n_proc_sp_irecv_= "<<tree_grav.comm_table_.n_proc_sp_irecv_
                      <<std::endl;
            fout_wtime<<"n_proc_ep_send_tot= "<<n_proc_ep_send_tot
                      <<" n_proc_ep_send_tot/n_proc= "<<((PS::F64)n_proc_ep_send_tot)/n_proc
                      <<std::endl;
            fout_wtime<<"n_proc_sp_isend_tot= "<<n_proc_sp_isend_tot
                      <<" n_proc_sp_isend_tot/n_proc= "<<((PS::F64)n_proc_sp_isend_tot)/n_proc
                      <<std::endl;
            fout_wtime<<"n_proc_sp_irecv_tot= "<<n_proc_sp_irecv_tot
