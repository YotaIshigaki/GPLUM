...
void ChangePtclToSatellite(Tpsys & sys,
    ...
    if(PS::Comm::getRank() == 1){
        std::cerr<<"n_sat= "<<n_sat<<std::endl;
        for(PS::S32 i=0; i<n_sat; i++){
            std::cerr<<"SATELLITE[i].pos= "<<SATELLITE[i].pos<<std::endl;
        }
    }
    */
}
...
/*
template<class Tsat>
void SetSatelliteKeplerDisk(Tsat & sat,
                            const PS::S64 n_sat,
                            const PS::F64 ax_in,
                            const PS::F64 ax_out,
                            const PS::F64 ecc_rms, // normalized
...
void SetParticleKeplerDiskCyl2(Tpsys & psys,
    ...
    const PS::S64 id_y_head = box.low_.y / dr;
    const PS::S64 id_y_tail = box.high_.y / dr;
    //const PS::F64 offset_theta = dtheta*0.25;
    ...
    for(PS::S64 i=id_x_head-3; i<=id_x_tail+3; i++){
        ...
        /*
        if(PS::Comm::getRank()==50 && pos_x > 0.3141592 && pos_x < 0.3141593){
    ...
    for(PS::S64 i=id_x_head-3; i<=id_x_tail+3; i++){
        ...
        */
        ...
        for(PS::S64 j=id_y_head-3; j<=id_y_tail+3; j++){
            PS::F64 pos_y = j*dr+0.5*dr;
            if(pos_y < box.low_.y || pos_y >= box.high_.y) continue;
            //pos.push_back(PS::F64vec(pos_x, pos_y, (-0.5-mt.genrand_res53())*dz));
            //pos.push_back(PS::F64vec(pos_x, pos_y, -0.5*dz));
    ...
    /*
    for(PS::S64 i=0; i<n_loc; i++){
        PS::F64 pos_x = psys[i].pos.x;
        PS::F64 pos_y = psys[i].pos.y;
        PS::F64 pos_z = psys[i].pos.z;
        PS::F64 pos_phi = atan2(pos_y, pos_x);
        if(pos_phi < 0.0) pos_phi += 8.0*atan(1.0);
        PS::F64 pos_r = sqrt(pos_x*pos_x + pos_y*pos_y);
        PS::F64vec pos_tmp(pos_phi, pos_r, pos_z);

        if(!(PS::IsInBox1d(pos_tmp, box, 0)) && (PS::IsInBox1d(pos[i], box, 0))){
            if(PS::Comm::getRank()==50){
                std::cerr<<"B) my_rank= "<<PS::Comm::getRank()
                         <<" i= "<<i
                         <<" pos_phi= "<<pos_phi
                         <<" pos[i].x= "<<pos[i].x
                         <<" box.low_.x= "<<box.low_.x
                         <<" box.high_.x= "<<box.high_.x
                         <<std::endl;
            }
        }
    }
