...
namespace ParticleSimulator{
    ...
    void SetPtclSortedGlb(const S64 n_glb_tot,
        ...
        //if (Comm::getRank() == 0) {
            ...
            filename = "old" + filenum.str() + ".txt";
            //filename = "new" + filenum.str() + ".txt";
            ...
            for (S32 i=0; i<n_glb_tot; i++) {
                ...
                const U32 adr = tp[i].adr_ptcl_;
                if (GetMSB(adr) == 0) {
                    ofs << epj_sorted[adr].pos << " "
                        << epj_sorted[adr].mass << " "
                        << epj_sorted[adr].vel << " "
                        << epj_sorted[adr].id << " ";
                } else {
                    const U32 adr_new = ClearMSB(adr);
                    ofs << spj_sorted[adr_new].pos << " "
                        << spj_sorted[adr_new].mass << " "
                        << spj_sorted[adr_new].pos_phi << " "
                        << spj_sorted[adr_new].pos_r << " ";
                }
                ofs << std::endl;
            }
#else
            for (S32 i=0; i<adr_epj_org2glb.size(); i++) {
                ofs << i << " " << adr_epj_org2glb[i] << std::endl;
            }
            for (S32 i=0; i<adr_epj_buf2glb.size(); i++) {
                ofs << i << " " << adr_epj_buf2glb[i] << std::endl;
            }
            for (S32 i=0; i<adr_spj_buf2glb.size(); i++) {
                ofs << i << " " << adr_spj_buf2glb[i] << std::endl;
            }
#endif
            ofs.close();
        //}
#endif
        //Comm::barrier();
        //const F64 wt_end = GetWtime();
        //if (Comm::getRank() == 0) std::cout << "etime = " << wt_end - wt_start << std::endl;
        //athread_halt();
        //Finalize();
        //std::exit(0);
    }
    ...
    void SetAdrGlb1st(const S32 n,
        ...
        if (Comm::getRank() == 0) {
            ...
            //filename = "new" + filenum.str() + ".txt";
            ofs.open(filename.c_str(),std::ios::trunc);
            for (S32 i=0; i<adr_epj_loc2glb.size(); i++) {
                ofs << i << " " << adr_epj_loc2glb[i] << std::endl;
            }
            ofs.close();
        }
#endif
    ...
    void SetAdrGlb2nd(const S32 n_epj_sorted,
        ...
        //if (Comm::getRank() == 0) {
            ...
            filename = "new" + filenum.str() + ".txt";
            ofs.open(filename.c_str(),std::ios::trunc);
            for (S32 i=0; i<adr_epj_glb2loc.size(); i++) {
                ofs << i << " " << adr_epj_glb2loc[i] << std::endl;
            }
            ofs.close();
        //}
#endif
...
#pragma omp single
    ...
    exchangeLocalEssentialTreeSuperDomain2(const DomainInfo & dinfo,
        ...
        Comm::barrier();
        if(my_rank == 50){
            std::cerr<<"my super domain"<<std::endl;
