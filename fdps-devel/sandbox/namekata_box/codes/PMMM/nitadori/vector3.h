...
struct vector3{
	REAL x, y, z;
	INLINE vector3(const vector3 &src) : x(src.x), y(src.y), z(src.z) {}
	INLINE vector3(){
		x = y = z = REAL(0);
	}
	...
	INLINE const vector3<REAL> abseach() const {
...
#endif
	int print(FILE *fp = stdout, const char *fmt = "(%e, %e, %e)") const {
		return fprintf(fp, fmt, x, y, z);
	}
