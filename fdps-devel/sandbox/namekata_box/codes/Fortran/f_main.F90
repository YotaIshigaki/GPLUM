...
subroutine f_main()
   ...
   double precision :: t0,t1,et_per_proc=0.0d0
  ...
  !call setup_IC(fdps_ctrl,psys_num,ntot)
   call setup_sample_IC(fdps_ctrl,psys_num,ntot)
  ...
  !fname = 'acc_initial.dat'
   !* (1-1) binary
  !open(unit=9,file=trim(fname),action='write',status='replace', &
  !     form='unformatted',access='stream')
  !   do i=1,ntot
  !      write(9)ptcl(i)%acc%x,ptcl(i)%acc%y,ptcl(i)%acc%z
  !   end do
  !close(unit=9)
   !* (1-2) ascii
  !open(unit=9,file=trim(fname),action='write',status='replace', &
  !     form='formatted')
  !   do i=1,ntot
  !      write(9,'(1es25.16e3)')ptcl(i)%acc%x
  !      write(9,'(1es25.16e3)')ptcl(i)%acc%y
  !      write(9,'(1es25.16e3)')ptcl(i)%acc%z
  !   end do
  !close(unit=9)
  ...
  !call fdps_ctrl%get_psys_fptr(psys_num,ptcl)
  !fname = 'acc_profile.dat' 
  !open(unit=9,file=trim(fname),action='write',status='replace')
  !   do i=1,nloc
  !      r   = sqrt(ptcl(i)%pos*ptcl(i)%pos) 
  !      acc = sqrt(ptcl(i)%acc*ptcl(i)%acc) 
  !     !write(9,'(i6,1x,1es25.16e3)')i,ptcl(i)%mass
  !     !write(9,'(3es25.16e3)')ptcl(i)%pos(1:3)
  !      write(9,'(2es25.16e3)')r,acc
  !   end do
  !close(unit=9)
  !nullify( ptcl )
  !call fdps_ctrl%PS_finalize()
     ...
     !end if
      ...
      if (fdps_ctrl%get_rank() == 0) then
         if ( (time_sys >= time_diag) .or. &
            ...
            100 format("time: ",1es25.16e3,", energy error: ",1es25.16e3)
      ...
      call fdps_ctrl%calc_force_all_and_write_back(tree_num,    &
                                                   ...
                                                   dinfo_num)
      t1 = MPI_Wtime()
      et_per_proc = et_per_proc + t1-t0
     !write(*,150)fdps_ctrl%get_rank(), &
     !            fdps_ctrl%get_nptcl_loc(psys_num)
     !150 format("myrank = ",i6," nptcl = ",i6)
...
subroutine setup_IC(fdps_ctrl,psys_num,nptcl_glb)
   ...
   integer :: nptcl_rem,nptcl_loc
   integer :: id_offset
   integer, dimension(:), allocatable :: list_of_nptcl_loc
   double precision :: r2,cm_mass
   type(fdps_f64vec) :: cm_pos,cm_vel
   type(full_particle), dimension(:), pointer :: ptcl
   ...
   !* Compute # of local particles and inform it to FDPS
   allocate( list_of_nptcl_loc(0:nprocs-1) )
   list_of_nptcl_loc = nptcl_glb/nprocs
   nptcl_rem = mod(nptcl_glb,nprocs)
   if (nptcl_rem /= 0) then
      do irank=0,nprocs-1
         if ((irank+1) <= nptcl_rem) then
            list_of_nptcl_loc(irank) = list_of_nptcl_loc(irank) + 1
         end if
      end do
   end if
   nptcl_loc = list_of_nptcl_loc(myrank)
   write(*,100)nptcl_loc,myrank
   100 format("# of local ptcls = ",i8,' (',i4,')')
   call fdps_ctrl%set_nptcl_loc(psys_num,nptcl_loc)

   !* Create an uniform sphere of particles
   !** compute the offset of particle ID
   id_offset = 0
   do irank=0,nprocs-1
      if (irank < myrank) then
         id_offset = id_offset + list_of_nptcl_loc(irank-1)
      end if
   end do
...
!-----------------------------------------------------------------------
!//////////////////        S U B R O U T I N E        //////////////////
!////////////////// < S E T U P _ S A M P L E _ I C > //////////////////
!-----------------------------------------------------------------------
subroutine setup_sample_IC(fdps_ctrl,psys_num,nptcl_glb)
   use fdps_vector
   use fdps_module
   use user_defined_types
   implicit none
...
end subroutine setup_sample_IC
