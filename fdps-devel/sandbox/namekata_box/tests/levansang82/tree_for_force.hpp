...
namespace ParticleSimulator{
    ...
    private:
        ...
        class EPJWithR{
            Tepj epj_;
            F64 r_search_;  //******// changed: F64vec r_search_; 
        public:
            Tepj getEPJ() const { return epj_; }
            F64vec getPos() const { return epj_.getPos(); }
            F64 getCharge() const { return epj_.getCharge(); }
            F64 getRSearch() const { return r_search_; }   //******// changed: F64vec getRSearch() const { return r_search_; }             
            void copyFromEPJ(const Tepj & epj){ epj_ = epj; }
            void setRSearch(const F64 r_search){ r_search_ = r_search; } //******// changed: void setRSearch(const F64vec r_search){ r_search_ = r_search; }
