...
PS::F64 W(const PS::F64vec dr, const PS::F64 h){   //******// PS::F64 W(const PS::F64vec dr, const PS::F64 hx, const PS::F64 hy, const PS::F64 hz){             (a different kernel)
...
PS::F64vec gradW(const PS::F64vec dr, const PS::F64 h){      //******// PS::F64vec  gradW(const PS::F64vec dr, const PS::F64 hx, const PS::F64 hy, const PS::F64 hz){  
   const PS::F64 H = kernelSupportRadius * h;
   const PS::F64 s = sqrt(dr * dr) / H;
   const PS::F64 s1 = (1.0 - s < 0) ? 0 : 1.0 - s;
   const PS::F64 s2 = (0.5 - s < 0) ? 0 : 0.5 - s;
...
class Dens{
   public:
   PS::F64 dens;
   PS::F64 smth;    //******// changed:  PS::F64vec smth;
   void clear(){
      dens = 0;
                    //******// added:: smth.x = 0.0;
                                    // smth.y = 0.0;
...
struct FP{
   ...
   PS::F64 getRSearch() const{                //******/ changed: PS::F64vec getRSearch() const{
   ...
   void operator () (const EP* const ep_i, const PS::S32 Nip,
      ...
      for(PS::S32 i = 0 ; i < Nip ; ++i){
         ...
         for(PS::S32 j = 0 ; j < Njp ; ++j){
            ...
            dens[i].dens += ep_j[j].mass * W(dr, ep_i[i].smth);   //******// changed:  dens[i].dens += ep_j[j].mass * W(dr, ep_i[i].smth.x, ep_i[i].smth.y, ep_i[i].smth.z)   (also change W above)
              //******// added (if dependent on sum_j):  dens[i].smth.x = a function; 
                                                      // dens[i].smth.y = a function;  
                                                      // dens[i].smth.z = a function;  
         }
               //******// added (if only dependent on i):  dens[i].smth.x = a function; 
                                                        // dens[i].smth.y = a function;  
                                                        // dens[i].smth.z = a function;       
     }
   }
   ...
   void operator () (const EP* const ep_i, const PS::S32 Nip,
      ...
      for(PS::S32 i = 0; i < Nip ; ++ i){
         ...
         for(PS::S32 j = 0; j < Njp ; ++j){
            ...
            const PS::F64vec gradW_ij = 0.5 * (gradW(dr, ep_i[i].smth) + gradW(dr, ep_j[j].smth)); //******// const PS::F64vec gradW_ij = 0.5 * ( gradW(dr, ep_i[i].smth.x, ep_i[i].smth.y, ep_i[i].smth.z) + gradW(dr, ep_j[j].smth.x, ep_j[j].smth.y, ep_j[j].smth.z));
                                                                                                                                                        //  (also change gradW above) 
         ...
         hydro[i].dt = C_CFL * 2.0 * ep_i[i].smth / v_sig_max;      //******// changed (maybe):  hydro[i].dt = C_CFL * 2.0 * 0.33333333 * (ep_i[i].smth.x + ep_i[i].smth.y + ep_i[i].smth.z) / v_sig_max;
      }
   }
};
   ...
   for(PS::F64 x = 0 ; x < box->x * 0.5 ; x += dx){
      for(PS::F64 y = 0 ; y < box->y ; y += dx){
         for(PS::F64 z = 0 ; z < box->z ; z += dx){
            ...
            ith.eng  = 2.5;
            ith.id   = i++;
            ith.smth = 0.012;                //******// changed:  ith.smth.x = 0.03;
                                                               // ith.smth.y = 0.02;
                                                               // ith.smth.z = 0.01;
            ptcl.push_back(ith);
         }
