...
namespace TEST_DEFS {
    ...
    const std::string harmonic_log_file{"test_bin/force_dihedral_harmonic_log.dat"};
    const std::string harmonic_ref_file{"unit_test/ref/force_dihedral_harmonic_ref.dat"};

    const std::string OPLS_log_file{"test_bin/force_dihedral_OPLS_3_log.dat"};
    const std::string OPLS_ref_file{"unit_test/ref/force_dihedral_OPLS_3_ref.dat"};
};
...
void test_move(Tpsys &atom){
    ...
    for(PS::S64 i=0; i<atom.getNumberOfParticleLocal(); ++i){
        ...
        if(atom[i].getAtomID() == 3){
            //--- move for dihedral torsion test
            auto pos_local = pos_tmp - Normalize::realPos( PS::F64vec{0.5} );
                 pos_local = VEC_EXT::rot_x(pos_local, TEST_DEFS::range/PS::F64(TEST_DEFS::n_loop));
...
TEST_F(TestForceDihedral, harmonic){
    if(PS::Comm::getRank() == 0){
        ...
        MODEL::loading_param_bond(ENUM::what(MolName::AA_propan_1_ol),
                                  param_line,
                                  MODEL::coef_table.bond);

        param_line = {"CT", "CT", "CT", "none"};
        MODEL::loading_param_angle(ENUM::what(MolName::AA_propan_1_ol),
                                   param_line,
                                   MODEL::coef_table.angle);

        param_line = {"dihedral", "CT", "CT", "CT", "CT", "cos", "0.0", "3.0", "3"};
        MODEL::loading_param_torsion(ENUM::what(MolName::AA_propan_1_ol),
                                     param_line,
                                     MODEL::coef_table.torsion);

        MODEL::coef_table.mask_scaling[MolName::AA_propan_1_ol] = MD_DEFS::MaskList{};
        ...
        for(PS::S32 i=0; i<TEST_DEFS::n_atom; ++i){
            ...
            atom[i].setMolType(  MolName::AA_propan_1_ol );
        ...
        atom[0].setPos( PS::F64vec{0.5} + Normalize::normPos( PS::F64vec{ 0.5, 1.0, 0.0} ) );
        atom[1].setPos( PS::F64vec{0.5} + Normalize::normPos( PS::F64vec{ 0.5, 0.0, 0.0} ) );
        atom[2].setPos( PS::F64vec{0.5} + Normalize::normPos( PS::F64vec{-0.5, 0.0, 0.0} ) );
        atom[3].setPos( PS::F64vec{0.5} + Normalize::normPos( PS::F64vec{-0.5, 1.0, 0.0} ) );
    }
...
TEST_F(TestForceDihedral, OPLS3){
    if(PS::Comm::getRank() == 0){
        //--- model parameters
        MODEL::coef_table.bond.clear();
        MODEL::coef_table.angle.clear();
        ...
        param_line = {"dihedral", "CT", "CT", "CT", "CT", "OPLS_3", "1.711", "-0.5", "0.663"};
        MODEL::loading_param_torsion(ENUM::what(MolName::AA_propan_1_ol),
                                     param_line,
                                     MODEL::coef_table.torsion);
