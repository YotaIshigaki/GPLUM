...
!********************************
   !-(force parameters)
! The calculation result depends on the force parameter "theta"
!   theta = 0.5 => calculation stops
!   theta = 0.0 => calculation runs
...
!********************************
   ...
   open(unit=9,file=trim(fname),action='write',status='replace')
      do i=1,ntot
         write(9,'(3es25.16e3)')ptcl(i)%acc%x,ptcl(i)%acc%y,ptcl(i)%acc%z
      end do
   close(unit=9)
   nullify(ptcl)
   ...
   do 
     ...
     !end if
      ...
      !***** Debug(1) *****
      fname = 'posvel_before_kick.dat'
      call fdps_ctrl%get_psys_fptr(psys_num,ptcl)
      open(unit=9,file=trim(fname),action='write',status='replace')
         do i=1,ntot
            write(9,'(6es25.16e3)')ptcl(i)%pos%x,ptcl(i)%pos%y,ptcl(i)%pos%z, &
...
!
!*********************************************************
  ! Assume a Neptune mass & radius body in cgs units
  ! Note that 
  ! the calculation runs if we set small mass (m_tot=1.0) body.
   double precision, parameter :: m_tot=1.0d29
   double precision, parameter :: rmax=2.5d9,r2max=rmax*rmax
!*********************************************************
   !* Local variables
   integer :: i,j,k,ierr
...
!*******************************************
! Assume a Distant body
! Simple parallel displacement for x-axis
      do i=1,nptcl_glb
          ptcl(i)%pos%x = ptcl(i)%pos%x + 5.d10
     !   !ptcl(i)%pos%x = ptcl(i)%pos%x + real(5.0d10,kind=c_double)
      end do
!*******************************************
