...
class Nbody_Objects {
   public:
      ...
      void calc_gravity(const PS::S32 numPtcl1D, const PS::S32 nstep) {
         //* Local variables
         const PS::S32 numPtcl1D_debug = 32;
         //const PS::S32 nstep_debug = 1;
         //const PS::S32 nstep_debug = 29;
         const PS::S32 nstep_debug = 40;
         //const PS::S32 nstep_debug = 47;
         //const PS::S32 nstep_debug = 61;
         PS::S32 numPtclLocal = system.getNumberOfParticleLocal();
         ...
         if (numPtcl1D == numPtcl1D_debug && nstep == nstep_debug) {
             PS::Comm::barrier();
             if (PS::Comm::getRank() == 1) std::cout << "PM part start." << std::endl;
         }
         ...
         pp_tree.flag_debug_=0;
         if (numPtcl1D == numPtcl1D_debug && nstep == nstep_debug) {
             pp_tree.flag_debug_=1;
             PS::Comm::barrier();
             if (PS::Comm::getRank() == 1) std::cout << "PP part start." << std::endl;
         }
         //=================
         //* PP part 
         //=================
         ...
         if (numPtcl1D == numPtcl1D_debug && nstep == nstep_debug) {
             PS::Comm::barrier();
             if (PS::Comm::getRank() == 1) std::cout << "PP part (getForce) start." << std::endl;
         }
         for (PS::S32 i=0; i<numPtclLocal; i++) {
         ...
         if (numPtcl1D == numPtcl1D_debug && nstep == nstep_debug) {
             PS::Comm::barrier();
             if (PS::Comm::getRank() == 1) std::cout << "PP part ended." << std::endl;
         }
...
int main(int argc, char* argv[]) {
   ...
   //* Initialize FDPS 
   const PS::S64 mpool_size = 100000000; // default
   //const PS::S64 mpool_size = 105000000; // test
   //const PS::S64 mpool_size = 134217728; // 2^{27}
   //const PS::S64 mpool_size = 268435456; // 2^{28}
   PS::Initialize(argc,argv,mpool_size);
   ...
   for (PS::S32 numPtcl1D=4; numPtcl1D <= 32; numPtcl1D += 2) {
      ...
      for (PS::S32 nstep=1; nstep <= numTrials; nstep++) {
         ...
         Nbody_objs.calc_gravity(numPtcl1D, nstep);
