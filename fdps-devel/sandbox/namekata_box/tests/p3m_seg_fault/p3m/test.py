...
class Automatic_Tester:
    ...
    def create_tests(self):
        ...
        # (1) w/ MPI
        CXX = ["time mpicxx"]
        #CXXFLAGS = ["-std=c++11 -O3 -DPARTICLE_SIMULATOR_MPI_PARALLEL", \
        #            "-std=c++11 -O3 -DPARTICLE_SIMULATOR_MPI_PARALLEL -DPARTICLE_SIMULATOR_THREAD_PARALLEL -fopenmp"]
        CXXFLAGS = ["-std=c++11 -Wall -Wextra -O0 -g3 -ggdb3 -DPARTICLE_SIMULATOR_MPI_PARALLEL", \
                    "-std=c++11 -Wall -Wextra -O0 -g3 -ggdb3 -DPARTICLE_SIMULATOR_MPI_PARALLEL -DPARTICLE_SIMULATOR_THREAD_PARALLEL -fopenmp"]
    ...
    def test(self):
        ...
        for mkvars in self.__mkvars:
            ...
            # For debug
            mat = re.search("fopenmp", mkvars["CXXFLAGS"])
            if (mat is None):
                results.append(0)
                continue
            else:
