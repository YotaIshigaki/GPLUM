...
#pragma omp critical
#endif
                {
                    ...
                    // Copy old data 
                    memcpy((void *)data_, (void *)data_old, sizeof(T)*size_);
...
#ifdef __GNUC__
        void __attribute__((always_inline)) ReallocInnerWithDebug(const int new_cap, const bool debug_flag){
#else
        void ReallocInnerWithDebug(const int new_cap, const bool debug_flag){
#endif
            //capacity_ = new_cap;
            if(alloc_mode_ == 0){
                capacity_ = getNewCapacity(new_cap);
...
#pragma omp critical
#endif
                {
                    ...
                    int id_old = id_mpool_;
                    void * ret = NULL;
                    //std::cout<<"&id_mpool_="<<&id_mpool_<<std::endl;
                    MemoryPool::allocWithDebug(sizeof(T)*capacity_, id_mpool_, &data_, ret, debug_flag);
                    data_ = (T*)ret;
                    if(id_old != id_mpool_){
                        MemoryPool::freeMem(id_old);
                    }
...
#pragma omp critical
        ...
        int alloc_mode() const {return alloc_mode_; }
...
#ifdef __GNUC__
        void __attribute__((always_inline)) reserveEmptyAreaAtLeastWithDebug (const int n_add, const bool debug_flag){
#else
        void reserveEmptyAreaAtLeastWithDebug (const int n_add, const bool debug_flag){
#endif
            const int n = n_add + size_;
            if( n >= capacity_){
#if SANITY_CHECK_REALLOCATABLE_ARRAY > 1
                ...
                ReallocInnerWithDebug(new_cap, debug_flag);
            }
        }
