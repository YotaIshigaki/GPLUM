...
subroutine f_main()
   ...
   !* Test
   integer(c_int) :: list_mode
   integer(c_long_long) :: id
   type(full_particle), pointer :: epj
   ...
   do 
     ...
     !end if
      ...
      if (mod(num_loop,4) == 0) then
         call fdps_ctrl%decompose_domain_all(dinfo_num,psys_num)
         call fdps_ctrl%exchange_particle(psys_num,dinfo_num)
         list_mode = fdps_make_list_for_reuse
      else
         list_mode = fdps_reuse_list
      end if
...
subroutine setup_IC(fdps_ctrl,psys_num,nptcl_glb)
...
#ifdef TEST_GET_EPJ_FROM_ID
         !* Check for API get_epj_from_id
         if (ptcl(i)%id == 512) then
            write(*,*)'id = ',ptcl(i)%id
            write(*,*)'x  = ',ptcl(i)%pos%x
            write(*,*)'y  = ',ptcl(i)%pos%y
            write(*,*)'z  = ',ptcl(i)%pos%z
            write(*,*)'-----'
         end if
#endif
      end do
   ...
   !* Check for API sort_particle
   pfunc_comp = c_funloc(comp)
   call fdps_ctrl%sort_particle(psys_num,pfunc_comp)
   call fdps_ctrl%get_psys_fptr(psys_num,ptcl)
   fname = 'initial_sorted.dat'
   open(unit=9,file=trim(fname),action='write',status='replace')
      do i=1,nptcl_glb
         write(9,*)ptcl(i)%id,ptcl(i)%pos%x
      end do
   close(unit=9)
   call fdps_ctrl%ps_finalize()
   stop 0
