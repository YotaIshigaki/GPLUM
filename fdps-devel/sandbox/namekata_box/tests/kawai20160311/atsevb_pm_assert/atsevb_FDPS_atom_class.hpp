...
class FP :
  ...
  public:
    ...
    //}
    void copyFromForceParticleMesh(const PS::F32vec & force_pm) {
        PS::F64vec f;
        f.x = force_pm.x;
        f.y = force_pm.y;
        f.z = force_pm.z;
        //* Error check
        if (std::isnan(force_pm.x) || 
            std::isnan(force_pm.y) || 
            std::isnan(force_pm.z)) {
            std::cout << "----- NaN detected @ copyFromFrocePM() -----" << std::endl;
            std::cout << " fPM.x = " << force_pm.x << std::endl;
            std::cout << " fPM.y = " << force_pm.y << std::endl;
            std::cout << " fPM.z = " << force_pm.z << std::endl;
        }
        this->addFieldCoulomb( Normalize::realPMForce(f) );
    }
