...
#pragma omp parallel for
        for(int k=0; k<NZ; k++) for(int j=0; j<NY; j++) for(int i=0; i<NX; i++)
        {
            ...
            for(nz=-NMAX; nz<=NMAX; nz++) for(ny=-NMAX; ny<=NMAX; ny++) for(nx=-NMAX; nx<=NMAX; nx++)
            {
                ...
                etime_cf += get_time_of_day() - time_offset;
                Slm<2*p, real_t> slm;
                time_offset = get_time_of_day();
                slm.eval_opt(-dx, dy, dz); // eval S_l^{-m}
                etime_slm += get_time_of_day() - time_offset;
        ...
        for (int k = 0; k < NZ; k++)
            for (int j = 0; j < NY; j++)
                for (int i = 0; i < NX; i++)
                    for (int lm = 0; lm < LEN2; lm++)
        {
            const int idx = lm + LEN2 * (i + NX * (j + NY * k));
            output_file << idx << "    " << gf_r[k][j][i][lm] << std::endl;
        }
        output_file.close();
        std::exit(0);
#endif
        std::cout << "etime (slm) = " << etime_slm << std::endl;
        std::cout << "etime (cf)  = " << etime_cf << std::endl;
    }
        ...
        for (int k = 0; k < NZ; k++)
            for (int j = 0; j < NY; j++)
                for (int i = 0; i < 1+NX/2; i++)
                    for (int lm = 0; lm < LEN2; lm++)
        {
            const int idx = lm + LEN2 * (i + (1+NX/2) * (j + NY * k));
            output_file << idx << "    " << gf_k[k][j][i][lm] << std::endl;
        }
        output_file.close();
        std::exit(0);
#endif
        ...
        Cell_FMM<p> cell[NZ][NY][NX])
{
    ...
    for(int lm=0; lm<LEN; lm++){
        ...
        if (lm == 0) {
            std::ofstream output_file;
            output_file.open("rbuf.txt",std::ios::trunc);
            for(k=0; k<NZ; k++) for(j=0; j<NY; j++) for(i=0; i<NX; i++)
            {
                const int idx = i + NX * (j + NY * k);
                output_file << idx << "    " << rbuf[k][j][i] << std::endl;
            }
            output_file.close();
        }
#endif
                ...
                for (int lm=0; lm<LEN; lm++)
    {
        if (mm_k[k][j][i][lm].real() != 0 ||
            ...
            const int idx = lm + LEN * (i + (1+NX/2) * (j + NY * k));
            output_file << idx << "    "
                        << mm_k[k][j][i][lm]
                        << std::endl;
        }
...
int main(){
...
#endif
    gf.gen_gf_k();
    time_end = get_time_of_day();
    std::cout << "setGreenFunction: " << time_end - time_start << std::endl;
    ...
    time_end = get_time_of_day();
    std::cout << "P2M: " << time_end - time_start << std::endl;

    time_start = get_time_of_day();
    M2L_convolution_PBC<PFMM, NC, NC, NC> (gf, cell);
    time_end = get_time_of_day();
    std::cout << "M2L_convolution_PBC: " << time_end - time_start << std::endl;
    ...
    std::cout << "dipole = " << dipole.x << "   " << dipole.y << "   " << dipole.z << std::endl;
#endif
    for(int i=0; i<NC3; i++){
    ...
    time_end = get_time_of_day();
    std::cout << "L2P: " << time_end - time_start << std::endl;
...
#if defined(COMPARE_WITH_EWALD_METHOD)
    puts("Ewald sum");
    // Direct Ewald
#ifdef EWALD_FILE
    {
...
#endif // COMPARE_WITH_EWALD_METHOD
