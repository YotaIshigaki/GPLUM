...
#include"multientrance_array.hpp"
...
namespace ParticleSimulator{
    ...
    enum SEARCH_MODE{
        ...
        LONG_SYMMETRY,
        LONG_PARTICLE_MESH_MULTIPOLE, // new for PM^3
        SHORT_GATHER,
        SHORT_SCATTER,
        SHORT_SYMMETRY,
    };
    ...
    struct TagSearchLongParticleMeshMultipole{};  
    ...
    */
    ...
    struct SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE {
        typedef TagForceLong force_type;
        typedef TagKeyMeshBased key_type;
        typedef TagSearchLongParticleMeshMultipole search_type;
        typedef TagSearchBoundaryConditionOpenPeriodic search_boundary_type;
        typedef TagIpgLongNormal ipg_type;
        typedef TagNeighborSearchNo neighbor_search_type;
        enum{
            search_type_id = LONG_PARTICLE_MESH_MULTIPOLE,
        };
    };
...
*/
    ...
    public:
        ...
        template<class T> static inline void allGatherV(const T * val_send, // in
            ...
            const int n_proc = Comm::getNumberOfProc();
            int *n_recv = new int[n_proc];
            int *n_recv_disp = new int[n_proc];
            int n_send_tmp = n_send;
            MPI_Allgather(&n_send_tmp, 1, GetDataType<int>(),
                          n_recv, 1, GetDataType<int>(),
                          MPI_COMM_WORLD);
            n_recv_disp[0] = 0;
            for (int i=1; i<n_proc; ++i)
                n_recv_disp[i] = n_recv_disp[i-1] + n_recv[i-1];
            T * val_send_tmp = const_cast<T*>(val_send);
            MPI_Allgatherv(val_send_tmp, n_send, GetDataType<T>(),
...
#pragma omp critical
    ...
    //////////////////////////////////////////////////////////////////////////
    // SFINAE for checking if a target class has a particular method or not
    //////////////////////////////////////////////////////////////////////////
#ifdef PARTICLE_SIMULATOR_USE_SFINAE_OLD_IMPL
    // [Note]
    //     The following implementations are required to compile in old systems.
    //     They will be deleted if C++ compilers in existing super computer
    //     systems fully support C++11.
    //
    // (1) getId
    template<typename Tptcl>
    struct HasgetIdMethod
    {
       template<typename U, int(U::*)() > struct SFINAE0 {};
       template<typename U, int(U::*)() const > struct SFINAE1 {};
       template<typename U, long long int(U::*)() > struct SFINAE2 {};
       template<typename U, long long int(U::*)() const > struct SFINAE3 {};
       template<typename U> static char Test(SFINAE0<U, &U::getId> *);
       template<typename U> static char Test(SFINAE1<U, &U::getId> *);
       template<typename U> static char Test(SFINAE2<U, &U::getId> *);
       template<typename U> static char Test(SFINAE3<U, &U::getId> *);
       template<typename U> static int Test(...);
       static const bool value = sizeof(Test<Tptcl>(0)) == sizeof(char);
    };
    template<class Tptcl>
    S64 GetMyId(Tptcl ptcl, std::true_type)
    {
       return ptcl.getId();
    }
    template<class Tptcl>
    S64 GetMyId(Tptcl ptcl, std::false_type)
    {
       return -1;
    }
    template <class Tptcl>
    S64 GetMyId(Tptcl ptcl) {
    ...
    {
#ifdef PARTICLE_SIMULATOR_TWO_DIMENSION
       template<typename U, Vector2<float>(U::*)() > struct SFINAE0 {};
       template<typename U, Vector2<float>(U::*)() const > struct SFINAE1 {};
       template<typename U, Vector2<double>(U::*)() > struct SFINAE2 {};
       template<typename U, Vector2<double>(U::*)() const > struct SFINAE3 {};
#else
       template<typename U, Vector3<float>(U::*)() > struct SFINAE0 {};
       template<typename U, Vector3<float>(U::*)() const > struct SFINAE1 {};
       template<typename U, Vector3<double>(U::*)() > struct SFINAE2 {};
       template<typename U, Vector3<double>(U::*)() const > struct SFINAE3 {};
#endif
    ...
    {
#ifdef PARTICLE_SIMULATOR_TWO_DIMENSION
       template<typename U, MatrixSym2<float>(U::*)() > struct SFINAE0 {};
       template<typename U, MatrixSym2<float>(U::*)() const > struct SFINAE1 {};
       template<typename U, MatrixSym2<double>(U::*)() > struct SFINAE2 {};
       template<typename U, MatrixSym2<double>(U::*)() const > struct SFINAE3 {};
#else
       template<typename U, MatrixSym3<float>(U::*)() > struct SFINAE0 {};
       template<typename U, MatrixSym3<float>(U::*)() const > struct SFINAE1 {};
       template<typename U, MatrixSym3<double>(U::*)() > struct SFINAE2 {};
       template<typename U, MatrixSym3<double>(U::*)() const > struct SFINAE3 {};
#endif
    ...
    struct HasgetMultipoleOrderMethod
    {
       ...
       // Note that we used int(*)() instead of int(U::*)(),
       // because we assume getMultipoleOrder is a static member function,
       // which is actually defined in the global name space.
       template<typename U> static char Test(SFINAE<U, &U::getMultipoleOrder> *);
       template<typename U> static int Test(...);
       static const bool value = sizeof(Test<Tspj>(0)) == sizeof(char);
    };
    template<class Tspj>
    S32 GetMyMultipoleOrder(std::true_type)
    {
       return Tspj::getMultipoleOrder();
    }
    template<class Tspj>
    S32 GetMyMultipoleOrder(std::false_type)
    {
       return -1;
    }
    template <class Tspj>
    S32 GetMyMultipoleOrder() { // wrapper function
...
#else // PARTICLE_SIMULATOR_USE_SFINAE_OLD_IMPL
    // (1) getId
    template<typename Tptcl>
    ...
    template <class Tptcl>
    S64 GetMyId(Tptcl & ptcl) { // wrapper function
       return GetMyId(ptcl, std::integral_constant<bool, HasgetIdMethod<Tptcl>::value>());
    }
    // (2) getRSearch
    template<typename Tptcl>
    ...
    template <class Tptcl>
    F64 GetMyRSearch(Tptcl & ptcl) { // wrapper function
       return GetMyRSearch(ptcl, std::integral_constant<bool, HasgetRSearchMethod<Tptcl>::value>());
    }
    // (3) getDipole
    template<typename Tspj>
    class HasgetDipoleMethod
    {
#ifdef PARTICLE_SIMULATOR_TWO_DIMENSION
        template<class U,
            class = typename std::enable_if<
                (std::is_same<decltype(&U::getDipole), Vector2<float>(U::*)()>::value ||
                 std::is_same<decltype(&U::getDipole), Vector2<float>(U::*)() const>::value ||
                 std::is_same<decltype(&U::getDipole), Vector2<double>(U::*)()>::value ||
                 std::is_same<decltype(&U::getDipole), Vector2<double>(U::*)() const>::value
                ) &&
                std::is_member_pointer<decltype(&U::getDipole)>::value
                    >::type
        >
        static std::true_type check(int);
#else
        template<class U,
            class = typename std::enable_if<
                (std::is_same<decltype(&U::getDipole), Vector3<float>(U::*)()>::value ||
                 std::is_same<decltype(&U::getDipole), Vector3<float>(U::*)() const>::value ||
                 std::is_same<decltype(&U::getDipole), Vector3<double>(U::*)()>::value ||
                 std::is_same<decltype(&U::getDipole), Vector3<double>(U::*)() const>::value
                ) &&
                std::is_member_pointer<decltype(&U::getDipole)>::value
                    >::type
        >
    ...
    // (4) getQuadrupole
    template<typename Tspj>
    class HasgetQuadrupoleMethod
    {
#ifdef PARTICLE_SIMULATOR_TWO_DIMENSION
        template<class U,
            class = typename std::enable_if<
                (std::is_same<decltype(&U::getQuadrupole), MatrixSym2<float>(U::*)()>::value ||
                 std::is_same<decltype(&U::getQuadrupole), MatrixSym2<float>(U::*)() const>::value ||
                 std::is_same<decltype(&U::getQuadrupole), MatrixSym2<double>(U::*)()>::value ||
                 std::is_same<decltype(&U::getQuadrupole), MatrixSym2<double>(U::*)() const>::value
                ) &&
                std::is_member_pointer<decltype(&U::getQuadrupole)>::value
                    >::type
        >
        static std::true_type check(int);
#else
        template<class U,
            class = typename std::enable_if<
                (std::is_same<decltype(&U::getQuadrupole), MatrixSym3<float>(U::*)()>::value ||
                 std::is_same<decltype(&U::getQuadrupole), MatrixSym3<float>(U::*)() const>::value ||
                 std::is_same<decltype(&U::getQuadrupole), MatrixSym3<double>(U::*)()>::value ||
                 std::is_same<decltype(&U::getQuadrupole), MatrixSym3<double>(U::*)() const>::value
                ) &&
                std::is_member_pointer<decltype(&U::getQuadrupole)>::value
                    >::type
        >
    ...
    struct HasgetMultipoleOrderMethod
    {
        template<class U,
            class = typename std::enable_if<
                !std::is_member_pointer<decltype(&U::getMultipoleOrder)>::value
                    >::type
        >
        static std::true_type check(int);
        template <class U>
    ...
    template<class Tspj>
    constexpr S32 GetMyMultipoleOrder(std::true_type)
    {
       return Tspj::getMultipoleOrder();
    }
    template<class Tspj>
    constexpr S32 GetMyMultipoleOrder(std::false_type)
    {
       return -1;
    }
    template <class Tspj>
    constexpr S32 GetMyMultipoleOrder() { // wrapper function
       return GetMyMultipoleOrder<Tspj>(std::integral_constant<bool, HasgetMultipoleOrderMethod<Tspj>::value>());
    }               
    // (5) getMultipole
    template<typename Tspj, typename Tbuf>
    class HasgetMultipoleMethod
    {
        template<class U, class V,
            class = typename std::enable_if<
                ...
                std::is_same<decltype(&U::template getMultipole<V>), void(U::*)(int, V *) const>::value
                    >::type
        >
        static std::true_type check(int);
        template <class U, class V>
    ...
    template<class Tspj, class Tbuf>
    void GetMyMultipole(Tspj & spj, S32 buflen, Tbuf buf[], std::true_type)
    {
        spj.getMultipole(buflen, buf);
    }
    template<class Tspj, class Tbuf>
    void GetMyMultipole(Tspj & spj, S32 buflen, Tbuf buf[], std::false_type)
    {
        for (int i=0; i<buflen; i++) buf[i]=Tbuf(0);
    }
    template <class Tspj, class Tbuf>
    void GetMyMultipole(Tspj & spj, S32 buflen, Tbuf buf[]) { // wrapper function
        GetMyMultipole(spj, buflen, buf, std::integral_constant<bool, HasgetMultipoleMethod<Tspj, Tbuf>::value>());
    }
#endif // PARTICLE_SIMULATOR_PMM_EXPERIMENTAL_FEATURE
#endif // PARTICLE_SIMULATOR_USE_SFINAE_OLD_IMPL
    ...
    public:
        ...
        F64 calc_force_only; // pfunc_ep_ep & pfunc_ep_sp (added by D.N. for perf. meas. of PMMM/TreePM)
        ...
        void dump(std::ostream & fout=std::cout) const {
            ...
            fout<<"calc_force_only="<<calc_force_only<<std::endl;
        ...
        TimeProfile operator + (const TimeProfile & rhs) const{
            ...
            ret.calc_force_only = this->calc_force_only + rhs.calc_force_only;
