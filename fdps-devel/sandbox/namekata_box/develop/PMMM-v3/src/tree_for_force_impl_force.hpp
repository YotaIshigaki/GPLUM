...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
            ...
            for(S32 i=0; i<n_ipg; i++){
                ...
                const F64 time_start = GetWtime();
#endif
                calcForceOnly( pfunc_ep_ep, pfunc_ep_sp, i, clear);
#if !defined(PARTICLE_SIMULATOR_THREAD_PARALLEL)
                time_profile_.calc_force_only += GetWtime() - time_start;
#endif
                ...
                //if (i == 1) {
                    ...
                    ss << "intlist_" 
                       << std::setfill('0') << std::setw(3) << Comm::getRank() << "_"
                       << std::setfill('0') << std::setw(5) << i << ".txt";
                    filename = ss.str();
                    output_file.open(filename.c_str(), std::ios::trunc);
                    output_file.setf(std::ios_base::scientific,
                                     std::ios_base::floatfield);
#if 1
                    const S32 offset = ipg_[i].adr_ptcl_;
                    const S32 n_epi  = ipg_[i].n_ptcl_;
                    for (S32 i=0; i<n_epi; i++) {
                        const S32 id_i = epi_sorted_[offset+i].id;
                        const S32 n_epj = epj_for_force_[ith].size();
                        for (S32 j=0; j<n_epj; j++) {
                            const S32 id_j = epj_for_force_[ith][j].id; 
                            if (id_i != id_j)
                                output_file << id_i << ", " << id_j << std::endl;
                        }
                    }
#else
                    output_file << std::setprecision(15) << std::showpos;
                    const F64ort vertex_out_ = PMM::GetMinBoxRoundedUpToParticleMeshCell((ipg_[i]).vertex_in_,
                                                                                         pos_unit_cell_.low_,
                                                                                         width_pm_cell_, icut_);
                    output_file << "# vertex_out_ " << std::endl;
                    output_file << vertex_out_.low_  << std::endl;
                    output_file << vertex_out_.high_ << std::endl;
                    output_file << std::endl << std::endl; // delimiter
                    output_file << "# epi_sorted_" << std::endl;
                    const S32 offset = ipg_[i].adr_ptcl_;
                    const S32 n_epi  = ipg_[i].n_ptcl_;
                    for (S32 i=0; i<n_epi; i++){
                        output_file << epi_sorted_[offset + i].getPos() << std::endl;
                    }
                    output_file << std::endl << std::endl; // delimiter
                    output_file << "# epj_for_force_" << std::endl;
                    const S32 n_epj = epj_for_force_[ith].size();
                    for (S32 j=0; j<n_epj; j++) {
                        output_file << epj_for_force_[ith][j].getPos() << std::endl;
                    }
                    output_file << std::endl << std::endl; // delimiter
                    output_file << "# spj_for_force_" << std::endl;
                    const S32 n_spj = spj_for_force_[ith].size();
                    for (S32 j=0; j<n_spj; j++) {
                        output_file << spj_for_force_[ith][j].getPos() << std::endl;
                    }
#endif
                    output_file.close();
                    //Finalize(); std::exit(0);
    ...
    makeInteractionListImpl(TagForceLong,
        ...
        if (theta_ > 0.0){
            ...
            S32 adr_tc = 0;
            TargetBox<TSM> target_box;
            target_box.set(ipg_[adr_ipg], icut_);
