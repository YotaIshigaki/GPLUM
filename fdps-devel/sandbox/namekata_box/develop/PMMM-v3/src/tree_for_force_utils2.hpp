namespace ParticleSimulator{
    ...
    template<class Ttc>
    inline F64ort GetOuterBoundaryOfMyTree(TagSearchLongParticleMeshMultipole,
                                           const ReallocatableArray<Ttc> & tc_first){
        return tc_first[0].mom_.getVertexIn();
    }
    template<class Ttc>
    ...
    inline void ExchangeOuterBoundary(TagSearchLongParticleMeshMultipole,
                                      F64ort * my_outer,
                                      ReallocatableArray<F64ort> & outer){
        Comm::allGather(my_outer, 1, outer.getPointer());
    }
    ...
    inline void FindScatterParticle(const ReallocatableArray<Ttc> & tc_first,
        ...
        outer_boundary_of_my_tree = GetOuterBoundaryOfMyTree(typename TSM::search_type(), tc_first);
        S32 adr_tc = 0;
...
#pragma omp for schedule(dynamic, 4)
            ...
            for(S32 i=0; i<n_proc; i++){
                ...
                if (typeid(TSM) != typeid(SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE)) {
                    ...
                    CalcNumberAndShiftOfImageDomain
                        (shift_per_image_tmp[ith],  dinfo.getPosRootDomain().getFullLength(),
                         outer_boundary_of_my_tree, outer_boundary_of_tree[i], pa, icut, false);
                }
                ...
                for(S32 j=n_image_tmp_prev; j<n_image_tmp; j++){
                    ...
                    F64ort pos_target_domain;
                    if (typeid(TSM) != typeid(SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE)) {
                        pos_target_domain = dinfo.getPosDomain(i).shift(shift_per_image_tmp[ith][j]);
                    } else {
                        pos_target_domain = outer_boundary_of_tree[i].shift(shift_per_image_tmp[ith][j]);
                    }
                    TargetBox<TSM> target_box;
                    target_box.set(pos_target_domain,
                                   outer_boundary_of_tree[i].shift(shift_per_image_tmp[ith][j]),
                                   icut);
                    MakeListUsingTreeRecursiveTop
                        <TSM, Ttc, TreeParticle, Tep, Tsp,
                         Twalkmode, TagChopLeafFalse, TagCopyInfoCloseNoSp, TagChopNonleafFalse>
                        (tc_first, adr_tc, tp_first,
                         ep_first, adr_ep_send_tmp[ith],
                         sp_first, adr_sp_send_tmp[ith],
                         target_box,
                         r_crit_sq, n_leaf_limit, lev_leaf_limit, 
                         adr_tree_sp_first, F64vec(0.0), lev_pm_cell);
...
#pragma omp for schedule(dynamic, 4)
            ...
            for(S32 i=0; i<n_proc; i++){
                ...
                for(S32 j=n_image_tmp_prev; j<n_image_tmp; j++){
                    ...
                    MakeListUsingTreeRecursiveTop
                        <SEARCH_MODE_SCATTER, Ttc, TreeParticle, Tep, Tsp,
                         Twalkmode, TagChopLeafFalse, TagCopyInfoCloseNoSp, TagChopNonleafFalse>
                        (tc_first, adr_tc, tp_first,
                         ep_first, adr_ep_send_tmp[ith],
...
#pragma omp parallel for schedule(dynamic, 4)
        ...
        for(S32 i=0; i<n_image_tot; i++){
            ...
            //        if (ep[adr].id == 20)
            //            std::cout << "ptcl ID=20 is sent from rank 0!" << std::endl;
            //    }
            //}
                    ...
                    */
                    ...
                    if(Comm::getRank() == 0){
                        ...
                        tc_tmp[ith].getPointer()->checkTree(epj_sorted_tmp[ith].getPointer(),
                                                            ...
                                                            n_leaf_limit_A, lev_leaf_limit_,
                                                            1e-4, err);
...
#pragma omp parallel for
    ...
    // for long force (for normal key)
    template<class Ttp, class Tepj, class Tspj>
    inline void SetLocalEssentialTreeToGlobalTreeWithKeyNormalImpl
...
#pragma omp for
                ...
                for(S32 i=n_loc; i<n_loc_ep; i++){
                    tp_glb[i].template setFromEP<Tepj, TagKeyNormal>(epj_org[i], i);
                }
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp for
#endif
                for(S32 i=n_loc_ep; i<n_loc_ep_sp; i++){
                    ...
                    tp_glb[i].template setFromSP<Tspj, TagKeyNormal>(spj_org[i_src], i_src);
                }
            }
        }
    }

    // for long force (for mesh-based key)
    template<class Ttp, class Tepj, class Tspj>
    inline void SetLocalEssentialTreeToGlobalTreeWithKeyMeshBasedImpl
    (const ReallocatableArray<Tepj> & epj_org,
     const ReallocatableArray<Tspj> & spj_org,
     const S32 n_loc,
     ReallocatableArray<Ttp> & tp_glb,
...
#endif
                for(S32 i=n_loc; i<n_loc_ep; i++){
                    tp_glb[i].template setFromEP<Tepj, TagKeyMeshBased>(epj_org[i], i);
                }
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp for
#endif
                for(S32 i=n_loc_ep; i<n_loc_ep_sp; i++){
                    const S32 i_src = i-n_loc_ep;
                    tp_glb[i].template setFromSP<Tspj, TagKeyMeshBased>(spj_org[i_src], i_src);
                }
            }
        }
    }

    // for short force (for normal key)
...
#pragma omp for
    ...
    // for short force (for mesh-based key)
    template<class Ttp, class Tepj>
    inline void SetLocalEssentialTreeToGlobalTreeWithKeyMeshBasedImpl
    (
     const ReallocatableArray<Tepj> & epj_org,
...
#pragma omp for
    ...
    inline void SetOuterBoxGlobalTreeForLongCutoffRecursive(Ttc tc[],
        ...
        for(S32 i=0; i<N_CHILDREN; i++){
            ...
            else if(tc[adr_tc+i].isLeaf(n_leaf_limit,lev_leaf_limit)) continue;
            else{
                SetOuterBoxGlobalTreeForLongCutoffRecursive(tc, n_leaf_limit, lev_leaf_limit, 
                                                            r_cut, child_adr_tc,
    ...
    inline void SetOuterBoxGlobalTreeForLongCutoffTop(TagSearchLongCutoff,
        ...
        if( tc[0].n_ptcl_ < 0 || tc[0].isLeaf(n_leaf_limit,lev_leaf_limit) ) return;
        S32 adr_tc = N_CHILDREN;
        SetOuterBoxGlobalTreeForLongCutoffRecursive(tc, n_leaf_limit, lev_leaf_limit,
                                                    r_cut, adr_tc, tc_hlen, tc_cen);
    }
    template<class Ttc, class Tepj>
    ...
    template<class Ttc, class Tepj>
    inline void SetOuterBoxGlobalTreeForLongCutoffTop(TagSearchLongParticleMeshMultipole,
                                                      Ttc tc[],
                                                      Tepj epj[],
                                                      const S32 n_leaf_limit,
                                                      const S32 lev_leaf_limit,
