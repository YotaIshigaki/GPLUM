...
namespace ParticleSimulator{
    ...
    (ReallocatableArray<F64vec> & shift_image_domain,
     ...
     const bool clear = true){
        ...
        pos_my_domain_idx.low_  = MortonKey<TagKeyMeshBased>::getPMCellID(pos_my_domain.low_);
        pos_my_domain_idx.high_ = MortonKey<TagKeyMeshBased>::getPMCellID(pos_my_domain.high_);
        pos_target_domain_idx.low_  = MortonKey<TagKeyMeshBased>::getPMCellID(pos_target_domain.low_) - S32vec(icut);
        pos_target_domain_idx.high_ = MortonKey<TagKeyMeshBased>::getPMCellID(pos_target_domain.high_) + S32vec(icut);
        ...
        for(S32 lev=1;; lev++){
            ...
            for(S32 ix=-lev_x; ix<=lev_x; ix++){
                for(S32 iy=-lev_y; iy<=lev_y; iy++){
                    ...
                    pos_image_tmp_idx.low_  = MortonKey<TagKeyMeshBased>::getPMCellID(pos_image_tmp.low_) - S32vec(icut);
                    pos_image_tmp_idx.high_ = MortonKey<TagKeyMeshBased>::getPMCellID(pos_image_tmp.high_) + S32vec(icut);
...
#else
    // binary search
    template<class Ttp, class Tkey>
    inline S32 GetPartitionID(const Ttp tp[],
                              const S32 left,
                              const S32 right,
                              const S32 ref,
                              const S32 lev){
        if( MortonKey<Tkey>::getCellID(lev, tp[right].getKey()) < ref) return -1;
        if( MortonKey<Tkey>::getCellID(lev, tp[left].getKey()) > ref) return -1;
        S32 l = left;
        S32 r = right;
        if (l == r)
            if( MortonKey<Tkey>::getCellID(lev, tp[l].getKey()) == ref ) return l;
        while(l < r){
            S32 cen = (l+r) / 2;
            S32 val_cen = MortonKey<Tkey>::getCellID(lev, tp[cen].getKey());
            if(ref > val_cen){
                l = cen+1;
            }
            else{
    ...
    inline void LinkCell(ReallocatableArray<Ttc> & tc_array,
        ...
        if((tc_array[0].n_ptcl_ <= n_leaf_limit) && (lev_max >= lev_leaf_limit)) return;
        S32 id_cell_left = 0;
...
#pragma omp parallel for reduction(+: n_cell_new)
            ...
            for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                ...
                if (n_ptcl_tmp == 0) continue;
                if ((n_ptcl_tmp <= n_leaf_limit) && (lev_max >= lev_leaf_limit)) continue;
                const S32 adr_ptcl_tmp = tc_array[i].adr_ptcl_;
                S32 adr[N_CHILDREN];
                S32 n_cnt = 0;
                for(S32 j=0; j<N_CHILDREN; j++){
                    const S32 adr_tc_tmp = tc_array[i].adr_tc_ + j;
                    tc_array[adr_tc_tmp].adr_ptcl_ = GetPartitionID<TreeParticle, Tkey>(tp, adr_ptcl_tmp, adr_ptcl_tmp+n_ptcl_tmp-1, j, lev_max+1);
            ...
            for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                const S32 id_tmp = i;
                if(!tc_array[id_tmp].isLeaf(n_leaf_limit,lev_leaf_limit)){
...
#pragma omp parallel for
            ...
            for(S32 j=head; j<next; j++){
                ...
                tc_tmp->mom_.init();
                if( n_tmp == 0) continue;
                else if(tc_tmp->isLeaf(n_leaf_limit,lev_leaf_limit)){
...
#pragma omp parallel for
    ...
    inline void MakeIPGroupLongGLBTreeCellAsIPGBox(ReallocatableArray<Tipg> & ipg_first,
        ...
        if(n_loc_tmp == 0 || n_glb_tmp == 0) return;
        else if( tc_glb_tmp->isLeaf(n_grp_limit,lev_grp_limit) || 
                 tc_loc_tmp->isLeaf(n_leaf_limit,lev_leaf_limit)){
            ipg_first.increaseSize();
            ...
            for(S32 i=0; i<N_CHILDREN; i++){
                MakeIPGroupLongGLBTreeCellAsIPGBox(ipg_first,
                                                   ...
                                                   n_grp_limit, lev_grp_limit,
                                                   n_leaf_limit, lev_leaf_limit);
            }
        }
    }
    //#endif
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    ...
                    SearchSendParticleLong<Ttc, Tep>
                        (tc_first,   tc_first[adr_tc_child].adr_tc_, ep_first,
                         ...
                         r_crit_sq, n_leaf_limit, lev_leaf_limit);
#else
                    SearchSendParticleLong<Ttc, Tep>
                        (tc_first,   tc_first[adr_tc_child].adr_tc_, ep_first,
                         id_ep_send, id_sp_send, pos_target_box,
                         r_crit_sq*0.25, n_leaf_limit, lev_leaf_limit);
#endif
                }
                else{
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    ...
                    SearchSendParticleLongCutoff<Ttc, Tep, Tsp>
                        (tc_first, tc_first[adr_tc_child].adr_tc_,
                         ...
                         pos_target_domain, r_crit_sq*0.25, r_cut_sq,
                         n_leaf_limit, lev_leaf_limit, shift);
                }
                else{
                    S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    ...
                    SearchSendParticleLongCutoffWithRootCellCheck<Ttc, Tep, Tsp>
                        (tc_first, tc_first[adr_tc_child].adr_tc_,
                         ep_first, ep_send, sp_send, child_cell_box,
                         pos_target_domain, r_crit_sq*0.25, r_cut_sq,
                         n_leaf_limit, lev_leaf_limit, pos_root_cell, shift);
                }
                else{
                    S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                    //ep_send.reserve( ep_send.size()+n_child );
...
//#ifdef __HPC_ACE__
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    ...
                    MakeInteractionListLongEPSP<Ttc, Ttp, Tep, Tsp>
                        (tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, 
                         ...
                         pos_target_box, r_crit_sq, n_leaf_limit, lev_leaf_limit);
#else
                    MakeInteractionListLongEPSP<Ttc, Ttp, Tep, Tsp>
                        (tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, 
                         ep_list,   sp_first, sp_list, 
                         pos_target_box, r_crit_sq*0.25, n_leaf_limit, lev_leaf_limit);
#endif
                }
                else{
            ...
            if(n_child == 0) continue;
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    MakeInteractionListLongSymmetryEPSP<Ttc, Ttp, Tep, Tsp>
                        (tc_first, tc_first[adr_tc_child].adr_tc_, tp_first, ep_first, 
                         ep_list,   sp_first, sp_list, 
                         pos_target_box_in, pos_target_box_out, r_crit_sq*0.25,
                         n_leaf_limit, lev_leaf_limit);
                }
                else{
                    S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    ...
                    MakeInteractionListLongCutoffEPSP<Ttc, Ttp, Tep, Tsp>
                        (tc_first, tc_first[adr_tc_child].adr_tc_, tp_first,
                         ...
                         pos_target_box, r_crit_sq*0.25, r_cut_sq,
                         n_leaf_limit, lev_leaf_limit);
                }
                else{
                    S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
    ...
    template<class Ttc, class Ttp, class Tep>
    void MakeInteractionListLongCutoffByCellEPForZeroTheta(const ReallocatableArray<Ttc> & tc_first,
                                                           const S32 adr_tc,
                                                           const ReallocatableArray<Ttp> & tp_first,
                                                           const ReallocatableArray<Tep> & ep_first,
                                                           ReallocatableArray<Tep> & ep_list,
        ...
        U32 open_bits = 0;
        for(S32 i=0; i<N_CHILDREN; i++){
            const F64ort vertex_in_ = tc_first[adr_tc+i].mom_.getVertexIn();
            // Check if pos_target_box intersects with tc_*[].mom_.vertex_in_
            open_bits |= (pos_target_box.contained(vertex_in_) << i);
            // Check if tc_*[].mom_.vertex_in_ is contained by pos_target_box
            open_bits |= (vertex_in_.isContainedBy(pos_target_box) << (i + N_CHILDREN));
        }
        for(S32 i=0; i<N_CHILDREN; i++){
            const S32 adr_tc_child = adr_tc + i;
            Ttc * tc_child = tc_first.getPointer(adr_tc_child);
            ...
            if(n_child == 0) continue;
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    if (!(open_bits>>(i+N_CHILDREN) & 0x1)) {
                        MakeInteractionListLongCutoffByCellEPForZeroTheta<Ttc, Ttp, Tep>
                            (tc_first, tc_first[adr_tc_child].adr_tc_, tp_first,
                             ep_first, ep_list, 
                             pos_target_box, n_leaf_limit, lev_leaf_limit);
                    } else {
                        // In this case, we can add all ep to ep_list because
                        // this tree cell is contained by pos_target_box.
                        S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                        ep_list.reserveEmptyAreaAtLeast( n_child );
                        for (S32 ip=0; ip<n_child; ip++, adr_ptcl_tmp++){
                            if( GetMSB(tp_first[adr_ptcl_tmp].adr_ptcl_) == 0){
                    ...
                    for (S32 ip=0; ip<n_child; ip++, adr_ptcl_tmp++){
                        if( GetMSB(tp_first[adr_ptcl_tmp].adr_ptcl_) == 0){
                            if (pos_target_box.contains(ep_first[adr_ptcl_tmp].getPos())) {
                                ep_list.pushBackNoCheck(ep_first[adr_ptcl_tmp]);
                            }
                        }
                    }
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    MakeListUsingOuterBoundary<Ttc, Tep2, Tep3>
                        (tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_list,
                         pos_target_box, n_leaf_limit, lev_leaf_limit, shift);
                }
                else{
                    S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                    ep_list.reserveEmptyAreaAtLeast( n_child );
            ...
            if( (open_bits>>i) & 0x1){
                ...
                if( !(tc_child->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
                    MakeListUsingOuterBoundaryWithRootCellCheck<Ttc, Tep2, Tep3>
                        (tc_first, tc_first[adr_tc_child].adr_tc_, ep_first, ep_list,
                         pos_target_box, n_leaf_limit, lev_leaf_limit, pos_root_cell, shift);
                }
                else{
                    S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                    ep_list.reserveEmptyAreaAtLeast( n_child );
    ...
    inline void MakeListUsingOuterBoundaryAndInnerBoundary(const Ttc * tc_first,
        ...
        for(S32 i=0; i<N_CHILDREN; i++){
            ...
            if( (open_bits>>i) & 0x1){
                ...
                if(n_child == 0) continue;
                if( !(tc_child[i].isLeaf(n_leaf_limit,lev_leaf_limit)) ){
    ...
    inline U32 IsOverlappedUsingOuterBoundaryOfTree(const Ttc * tc_first,
        ...
        if( !tc_first->isLeaf(n_leaf_limit,lev_leaf_limit) ){
            IsOverlappedUsingOuterBoundaryOfTreeImpl(tc_first, adr_tc, pos_box,
    ...
    inline void MakeLETListByDoubleWalkImpl(const Ttca * tc_first_A,
        ...
        for(S32 i=0; i<N_CHILDREN; i++){
            if( tc_first_B[adr_tc_B+i].mom_.getVertexOut().contained(pos_domain) 
                || IsOverlappedUsingOuterBoundaryOfTree(tc_first_A, tc_first_B[adr_tc_B+i].mom_.getVertexIn(),
...
/*
            ...
            if( (~(open_bits>>i)) & 0x1 ){
                open_bits |= IsOverlappedUsingOuterBoundaryOfTree(tc_first_A, tc_first_B[adr_tc_B+i].mom_.getVertexIn(),
                                                                  n_leaf_limit_A, lev_leaf_limit_A) << i;
                //open_bits |= IsOverlappedUsingOuterBoundaryOfTree(tc_first_A, tc_first_B[adr_tc_B+i].mom_.getVertexOut(),
                                                                    n_leaf_limit_A, lev_leaf_limit_A) << i;
...
*/
        ...
        for(S32 i=0; i<N_CHILDREN; i++){
            if( (open_bits>>i) & 0x1){
                ...
                const S32 n_child = tc_child->n_ptcl_;
                if(n_child == 0) continue;
                else if( !(tc_child->isLeaf(n_leaf_limit_B,lev_leaf_limit_B)) ){
    ...
    inline void MakeLETListByDoubleWalk(const Ttca * tc_first_A,
        ...
        if( !tc_first_B->isLeaf(n_leaf_limit_B, lev_leaf_limit_B) ){
            MakeLETListByDoubleWalkImpl(tc_first_A, tc_first_B, adr_tc_B,
                                        ep_first_B, pos_domain, n_leaf_limit_A, lev_leaf_limit_A,
                                        n_leaf_limit_B, lev_leaf_limit_B, id_ptcl_send);
        }
        else{
            if( tc_first_B->mom_.getVertexOut().contained(pos_domain) || 
                IsOverlappedUsingOuterBoundaryOfTree(tc_first_A, tc_first_B->mom_.getVertexIn(),
                                                     n_leaf_limit_A, lev_leaf_limit_A)){
    ...
    inline void SearchNeighborListOneParticleImpl(const Tvec & pos_target,
                                                  ...
                                                  const S32 n_leaf_limit,
                                                  const S32 lev_leaf_limit){
        if( tc_first[adr_tc].isLeaf(n_leaf_limit,lev_leaf_limit) ){
                ...
                if( n_child == 0 ) continue;
                    ...
                    SearchNeighborListOneParticleSymmetry
                        (pos_target, r_search_i_sq, tc_first, tp_first, adr_tc_child+i,
                         ep_first, ep_list, n_leaf_limit, lev_leaf_limit);
                }
            }
        }
    ...
    inline CountT MakeListUsingOuterBoundaryAndInnerBoundaryIteration
        ...
        while( !adr_tc_stack.empty() ){
            ...
            if( tc_first[adr_tc_curr].isLeaf(n_leaf_limit,lev_leaf_limit) ){
                // leaf
    ...
    inline CountT MakeInteractionListLongEPSPIteration
        ...
        else if( tc_first[adr_tc].isLeaf(n_leaf_limit,lev_leaf_limit) ){
            S32 adr_ptcl_tmp = tc_first[adr_tc].adr_ptcl_;
        ...
        while( !adr_tc_stack.empty() ){
            ...
            for(S32 i=0; i<N_CHILDREN; i++){
                ...
                if(n_child == 0) continue;
                    ...
                    if( !(tc_curr->isLeaf(n_leaf_limit,lev_leaf_limit)) ){
...
#include"tree_for_force_utils3.hpp"
