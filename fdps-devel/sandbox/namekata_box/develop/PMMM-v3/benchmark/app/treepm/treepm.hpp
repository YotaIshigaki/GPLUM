...
inline PS::F64 pfactor_S2(const PS::F64 rad, const PS::F64 eps_pm) 
{
    const PS::F64 xi = 2.0*rad/eps_pm;
    if (xi <= 1.0) {
        return 1.0 - xi*(208.0 
                        +(xi*xi)*(-112.0 
                                 +(xi*xi)*(56.0 
    ...
    void operator () (EPItreepm *iptcl,
        ...
        for (PS::S32 i=0; i < ni; i++) {
            ...
            for (PS::S32 j=0; j < nj; j++) {
                PS::F64 mj = jptcl[j].getCharge();
                PS::F64vec dr = iptcl[i].pos - jptcl[j].pos;
                PS::F64 rsq = dr*dr;
                PS::F64 rad = sqrt(rsq+eps2);
                PS::F64 pfact = pfactor_S2(rad, 3.0/SIZE_OF_MESH);
                PS::F64 gfact = gfactor_S2(rad, 3.0/SIZE_OF_MESH);
                PS::F64 rinv  = 1.0/rad;
                PS::F64 mrinv = mj*rinv;
                PS::F64 mrinv3 = mj*CUBE(rinv);
                ppforce[i].acc -= dr*gfact*mrinv3;
                ppforce[i].pot -= pfact*mrinv;
            }
            // Self-integration term
            ppforce[i].pot += iptcl[i].mass * (208.0/(70.0*3.0/SIZE_OF_MESH));
        }
    }
};
#endif
