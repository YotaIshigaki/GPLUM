...
#pragma omp parallel for
    ...
    makeIPGroupImpl(TagForceShort){
        //std::cout << "tc_loc_[0].level_ = " << tc_loc_[0].level_ << std::endl;
...
#pragma omp parallel for
    ...
    */
    ...
    template<class TSM, class Tforce, class Tepi, class Tepj,
             class Tmomloc, class Tmomglb, class Tspj>
    std::vector<ClusterInfo> TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    getClusterList(const F64 dt_crit){
        const std::vector<F64ort> vertex_list_loc = getClusterListImpl(typename TSM::search_type(), dt_crit);
        // interprocess merging
        const S32 n_proc  = Comm::getNumberOfProc();
        const S32 my_rank = Comm::getRank();
        S32 * counts = new S32[n_proc];
        counts[my_rank] = vertex_list_loc.size();
        MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                      counts, 1, MPI_INT, MPI_COMM_WORLD);
        S32 * displs = new S32[n_proc];
        S32 count_tot = counts[0];
        displs[0] = 0;
        for (S32 i = 1; i < n_proc; i++) {
            displs[i] = displs[i-1] + counts[i-1];
            count_tot += counts[i];
        }
        std::vector<F64ort> vertex_list_glb;
        vertex_list_glb.resize(count_tot);
        const S32 displ = displs[my_rank];
        for (S32 i = 0; i < vertex_list_loc.size(); i++) {
            vertex_list_glb[displ + i] = vertex_list_loc[i];
        }
        MPI_Datatype type = GetDataType<F64ort>();
        MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                       &vertex_list_glb[0], counts, displs, type,
                       MPI_COMM_WORLD);
        std::vector<F64ort> vertex_list_tmp;
        vertex_list_tmp.resize(count_tot);
        for (S32 i = 0; i < vertex_list_glb.size(); i++)
            vertex_list_tmp[i] = vertex_list_glb[i];
#if 0
        MergeOrthotopeList(vertex_list_tmp, vertex_list_glb);
#else
        std::vector<std::vector<S32>> rank_list_tmp, rank_list_glb;
        rank_list_tmp.resize(count_tot); // The use of reserve results in an segmentaion fault.
        rank_list_glb.resize(count_tot);
        for (S32 i = 0; i < n_proc; i++) {
            for (S32 k = 0; k < counts[i]; k++) {
                const S32 adr = displs[i] + k;
                assert(0 <= adr && adr < count_tot);
                rank_list_tmp[adr].push_back(i); 
                rank_list_glb[adr].push_back(i); 
            }
        }
        MergeOrthotopeList(vertex_list_tmp, rank_list_tmp,
                           vertex_list_glb, rank_list_glb);
        const S32 n_cluster = vertex_list_glb.size();
        std::vector<ClusterInfo> cluster_list;
        cluster_list.resize(n_cluster);
        for (S32 i = 0; i < vertex_list_glb.size(); i++) {
            ...
            ClusterParticle cp;
            cp.pos = vertex_list_glb[i].getCenter();
            TreeParticle tp;
            tp.setFromEP(cp, 0);
            // set
            cluster_list[i].id = tp.getKey();
            cluster_list[i].vertex = vertex_list_glb[i];
            cluster_list[i].ranks = rank_list_glb[i];
        }
        // check
        //if (Comm::getRank() == 0) {
        //    for (S32 i = 0; i < list_glb.size(); i++) {
        //        std::cout << "########" << std::endl;
        //        std::cout << "i = " << i << " box = " << list_glb[i] << std::endl;
        //        std::cout << "--------" << std::endl;
        //        for (S32 k = 0; k < rank_list_glb[i].size(); k++) {
        //            std::cout << "k = " << k << " rnk = " << rank_list_glb[i][k] << std::endl; 
        //        }
        //    }
        //}
        //Finalize();
    ...
    template<class TSM, class Tforce, class Tepi, class Tepj,
             class Tmomloc, class Tmomglb, class Tspj>
    std::vector<F64ort> TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    getClusterListImpl(TagSearchShortGather, const F64 dt_crit){
        return GetClusterList(adr_tc_level_partition_glb_,
                              tc_glb_.size(),
                              tc_glb_.getPointer(),
                              epj_sorted_.getPointer(), 
                              lev_max_glb_, n_leaf_limit_,
                              dt_crit);
    ...
    template<class TSM, class Tforce, class Tepi, class Tepj,
             class Tmomloc, class Tmomglb, class Tspj>
    std::vector<F64ort> TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    getClusterListImpl(TagSearchShortScatter, const F64 dt_crit){
        return GetClusterList(adr_tc_level_partition_glb_,
                              tc_glb_.size(),
                              tc_glb_.getPointer(),
                              epj_sorted_.getPointer(), 
    ...
    template<class TSM, class Tforce, class Tepi, class Tepj,
             class Tmomloc, class Tmomglb, class Tspj>
    std::vector<F64ort> TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    getClusterListImpl(TagSearchShortSymmetry, const F64 dt_crit){
        return GetClusterList(adr_tc_level_partition_glb_,
                              tc_glb_.size(),
                              tc_glb_.getPointer(),
                              epj_sorted_.getPointer(), 
