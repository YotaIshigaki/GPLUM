...
void CoolingHeating(PS::ParticleSystem<FP_gas> & psys,
    ...
    for (PS::S32 i = 0; i < n_loc; i++) {
        ...
        // Apply the bottom temperature
        const double Tmin = 10.0 / run_param::unit::temp; // 10[K]
        const double Umin = Tmin / (run_param::ism::mu * (run_param::ism::gamma - 1.0));
        if (Unew < Umin) Unew = Umin;

#ifdef DEBUG_PRINT_CALC_COOLING_HEATING
        if (PS::Comm::getRank() == 5) {
            std::cout << "================================================" << std::endl;
            std::cout << " i = " << i << " id = " << psys[i].id << std::endl;
            std::cout << "[Bef.]" << std::endl;
            std::cout << "   nH      = " << nH << std::endl;
            std::cout << "   T       = " << T << std::endl;
            std::cout << "   log10nH = " << log10nH << std::endl;
            std::cout << "   log10T  = " << log10T << std::endl;
            std::cout << "   dt_norm = " << dt_normalized << std::endl;
            std::cout << "   dt      = " << dt << " [s] = " << dt/phys_const::Myr << " [Myr]" << std::endl;
            std::cout << "   U       = " << Unow << std::endl;
            std::cout << "   pres    = " << psys[i].pres << std::endl;
            std::cout << "   dens    = " << psys[i].dens << std::endl;
            std::cout << "   eng     = " << psys[i].eng << std::endl;
        }
#endif
        // Update thermodynamic variables such as pressure
        psys[i].setPressureFromSpecificInternalEnergy(Unew);

#ifdef DEBUG_PRINT_CALC_COOLING_HEATING
        if (PS::Comm::getRank() == 5) {
            std::cout << "-------------------------" << std::endl;
            std::cout << "[Aft.]" << std::endl;
            std::cout << "   U       = " << Unew << std::endl;
            std::cout << "   dUdt    = " << dUdt << " [erg/g/s]" << std::endl;
            std::cout << "   T       = " << psys[i].getTemperature() * run_param::unit::temp << std::endl;
            std::cout << "   pres    = " << psys[i].pres << std::endl;
            std::cout << "   eng     = " << psys[i].eng << std::endl;
        }
#endif
