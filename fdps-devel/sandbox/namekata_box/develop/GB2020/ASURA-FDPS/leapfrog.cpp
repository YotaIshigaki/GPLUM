...
#if !defined(ASURA_FDPS_USE_ISOTHERMAL_EOS)
        psys[i].eng_half = psys[i].eng + 0.5 * dt * psys[i].eng_dot;
#else
        psys[i].eng_half = psys[i].eng;
#endif
...
void FinalKick(PS::ParticleSystem<FP_gas>& psys, const PS::F64 dt){
    for (PS::S32 i = 0; i < psys.getNumberOfParticleLocal(); i++){
        ...
        // Velocity dumping
#if defined(ASURA_FDPS_ENABLE_VELOCITY_DUMPING)
        psys[i].vel *= std::exp(- psys[i].snds * dt / psys[i].h);
#endif
