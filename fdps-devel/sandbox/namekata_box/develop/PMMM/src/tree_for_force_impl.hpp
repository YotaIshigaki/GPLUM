...
namespace ParticleSimulator{
    ...
    PMM::ParticleMeshMultipoleParameters TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    getParticleMeshMultipoleParameters(const DomainInfo & dinfo) const {
        if (typeid(TSM) != typeid(SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE)) {
            PARTICLE_SIMULATOR_PRINT_ERROR("This API is usable only for SEARCH_MODE_LONG_PARTICLE_MESU_MULTIPOLE.");
            Abort(-1);
        }
        PMM::ParticleMeshMultipoleParameters param;
        param.p             = pfmm_;
        param.icut          = icut_;
        param.bc            = dinfo.getBoundaryCondition();
        param.pos_unit_cell = pos_ref_keycalc_;
        param.n_cell        = n_cell_;
        return param;
    }
...
#pragma omp critical
    ...
    initialize(const U64 n_glb_tot,
        ...
        if (param.sanity_check(0)) err=true;
        if (err) {
            std::cerr<<"SEARCH_MODE: " << typeid(TSM).name() << std::endl;
            std::cerr<<"Force: " << typeid(Tforce).name() << std::endl;
            std::cerr<<"EPI: " << typeid(Tepi).name() << std::endl;
...
#pragma omp parallel for
    ...
    setRootCell(const DomainInfo & dinfo){
        ...
        if(dinfo.getBoundaryCondition() == BOUNDARY_CONDITION_OPEN){
            ...
            //pos_ref_keycalc_ = F64ort(F64vec(0.0), F64vec(1.0)); // just for test
        }
        else{
...
#pragma omp parallel for
    ...
    makeInteractionListLongForZeroTheta(TagWithCutoffByCell, const S32 adr_ipg){
        const S32 ith = Comm::getThreadNum();
        const F64ort pos_target_box = GetVertexOutOfTargetBox((ipg_[adr_ipg]).vertex_in_,
                                                              pos_ref_keycalc_.low_,
...
#pragma omp critical
    ...
    correctPosRootCellAndConfPMCellImpl(){
        // Compute cell widths
        width_pm_cell_.x  = pos_ref_keycalc_.getFullLength().x / n_cell_.x;
        width_pm_cell_.y  = pos_ref_keycalc_.getFullLength().y / n_cell_.y;
        width_pm_cell_.z  = pos_ref_keycalc_.getFullLength().z / n_cell_.z;
        ...
        if(!flag_reuse){
            FindScatterParticle<TSM, TreeCell<Tmomloc>, TreeParticle,
                ...
                (tc_loc_, tp_glb_,
                 ...
                 r_crit_sq,
                 pos_ref_keycalc_.low_, width_pm_cell_, icut_);            
