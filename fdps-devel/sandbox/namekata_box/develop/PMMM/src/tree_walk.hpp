...
namespace ParticleSimulator{
    ...
    struct TargetBox<SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE>{
        ...
        void set(const Tipg & ipg){
            vertex_in_ = ipg.vertex_in_;
            // We must set vertex_out_ somewhere else.
        }
        unsigned int contains(const F64vec & pos) const {
            return vertex_out_.contains(pos); 
        }
    ...
    template<>
    inline void SetTargetBoxExLet<SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE>
    (TargetBox<SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE> & box,
     const F64ort & in,
     const F64ort & out){
        box.vertex_in_ = in;
    }
    ...
    ////////////
    // set vertex_out_ of target box so that it fits particle mesh cells
    inline F64ort GetVertexOutOfTargetBox(const F64ort vertex_in_,
                                          const F64vec pos,
                                          const F64vec width,
                                          const S32 icut) {
        assert(vertex_in_.isValid());

        F64ort vertex_out_;
        F64vec idx_f;
        S32vec idx;

        // Calculate vertex_out.low_
        if (vertex_in_.low_.x != -LARGE_FLOAT) {
            idx_f.x = (vertex_in_.low_.x - pos.x)/width.x;
            assert(fabs(idx_f.x) < (F64) std::numeric_limits<S32>::max());
            if (idx_f.x >= 0.0) idx.x = (S32) idx_f.x;
            else idx.x = ((S32) idx_f.x) - 1;
            vertex_out_.low_.x = pos.x + width.x * (idx.x - icut);
        } else {
            // vertex_in_.low_.x is treated as -\infinity.
            vertex_out_.low_.x = vertex_in_.low_.x;
        }
        if (vertex_in_.low_.y != -LARGE_FLOAT) {
            idx_f.y = (vertex_in_.low_.y - pos.y)/width.y;
            assert(fabs(idx_f.y) < (F64) std::numeric_limits<S32>::max());
            if (idx_f.y >= 0.0) idx.y = (S32) idx_f.y;
            else idx.y = ((S32) idx_f.y) - 1;
            vertex_out_.low_.y = pos.y + width.y * (idx.y - icut);
        } else {
            vertex_out_.low_.y = vertex_in_.low_.y;
        }
        if (vertex_in_.low_.z != -LARGE_FLOAT) {
            idx_f.z = (vertex_in_.low_.z - pos.z)/width.z;
            assert(fabs(idx_f.z) < (F64) std::numeric_limits<S32>::max());
            if (idx_f.z >= 0.0) idx.z = (S32) idx_f.z;
            else idx.z = ((S32) idx_f.z) - 1;
            vertex_out_.low_.z = pos.z + width.z * (idx.z - icut);
        } else {
            vertex_out_.low_.z = vertex_in_.low_.z;
        }

        // Calculate vertex_out_.high_
        if (vertex_in_.high_.x != LARGE_FLOAT) {
            idx_f.x = (vertex_in_.high_.x - pos.x)/width.x;
            assert(fabs(idx_f.x) < (F64) std::numeric_limits<S32>::max());
            if (idx_f.x >= 0.0) idx.x = (S32) idx_f.x;
            else idx.x = ((S32) idx_f.x) - 1;
            vertex_out_.high_.x = pos.x + width.x * (idx.x + icut + 1);
        } else {
            vertex_out_.high_.x = vertex_in_.high_.x;
        }
        if (vertex_in_.high_.y != LARGE_FLOAT) {
            idx_f.y = (vertex_in_.high_.y - pos.y)/width.y;
            assert(fabs(idx_f.y) < (F64) std::numeric_limits<S32>::max());
            if (idx_f.y >= 0.0) idx.y = (S32) idx_f.y;
            else idx.y = ((S32) idx_f.y) - 1;
            vertex_out_.high_.y = pos.y + width.y * (idx.y + icut + 1);
        } else {
            vertex_out_.high_.y = vertex_in_.high_.y;
        }
        if (vertex_in_.high_.z != LARGE_FLOAT) {
            idx_f.z = (vertex_in_.high_.z - pos.z)/width.z;
            assert(fabs(idx_f.z) < (F64) std::numeric_limits<S32>::max());
            if (idx_f.z >= 0.0) idx.z = (S32) idx_f.z;
            else idx.z = ((S32) idx_f.z) - 1;
            vertex_out_.high_.z = pos.z + width.z * (idx.z + icut + 1);
        } else {
            vertex_out_.high_.z = vertex_in_.high_.z;
        }

        return vertex_out_;
    }
    template<class TSM>
    inline void SetVertexOutOfTargetBox(TargetBox<TSM> & box,
                                        const F64vec pos,
                                        const F64vec width,
                                        const S32 icut){
        assert(0);
    ...
    template<>
    inline void SetVertexOutOfTargetBox<SEARCH_MODE_LONG_SYMMETRY>
    (TargetBox<SEARCH_MODE_LONG_SYMMETRY> & box,
     const F64vec pos,
     const F64vec width,
     const S32 icut){
    ...
    template<>
    inline void SetVertexOutOfTargetBox<SEARCH_MODE_LONG_SCATTER>
    (TargetBox<SEARCH_MODE_LONG_SCATTER> & box,
     const F64vec pos,
     const F64vec width,
     const S32 icut){
    ...
    template<>
    inline void SetVertexOutOfTargetBox<SEARCH_MODE_LONG>
    (TargetBox<SEARCH_MODE_LONG> & box,
     const F64vec pos,
     const F64vec width,
     const S32 icut){
    ...
    template<>
    inline void SetVertexOutOfTargetBox<SEARCH_MODE_LONG_CUTOFF>
    (TargetBox<SEARCH_MODE_LONG_CUTOFF> & box,
     const F64vec pos,
     const F64vec width,
     const S32 icut){
    ...
    template<>
    inline void SetVertexOutOfTargetBox<SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE>
    (TargetBox<SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE> & box,
     const F64vec pos,
     const F64vec width,
     const S32 icut){
        box.vertex_out_ = GetVertexOutOfTargetBox(box.vertex_in_,
                                                  pos, width, icut);
    }
    // set vertex_out_ of target box so that it fits particle mesh cells
    ////////////
                ...
                if( !((open_bit>>(i+N_CHILDREN)) & 0x1) ) continue;
                    ...
                    MakeListUsingTreeRecursive<TSM, Ttc, Ttp, Tep, Tsp,
                        ...
                        (tc_first, adr_tc_child+i, tp_first, ep_first, adr_ep_list, sp_first, adr_sp_list,
                         target_box, r_crit_sq*0.25, n_leaf_limit, lev_leaf_limit, adr_tree_sp_first, len_peri);
                }
                else{ // far
