...
void testfunc() {
    PS::Comm::barrier();
    if (PS::Comm::getRank() == 0) std::cout << "test start! @main" << std::endl;

    PS::PMM::MultidimensionalArray<double, 3> buf;
    PS::S32 sizes[3];
    sizes[0] = sizes[1] = sizes[2] = 16;
    buf.initialize(sizes);
    buf.freeMem();
}
...
int main(int argc, char *argv[]) {
    ...
    const PS::S32 n_tot = psys.getNumberOfParticleGlobal();
    const PS::F32 theta = 0.0;
    const PS::U32 n_leaf_limit = 8;
    const PS::U32 n_group_limit = 64;
    PS::PMM::ParticleMeshMultipoleParameters param;
    param.p = 5;
    param.icut = 2;
    param.n_cell = PS::S32vec(8, 8, 8);
    PS::TreeForForceLong<FP_nbody, FP_nbody, FP_nbody>::MonopoleGeometricCenterParticleMeshMultipole tree;
    tree.initialize(n_tot, param, theta, n_leaf_limit, n_group_limit);
    ...
    PS::PMM::ParticleMeshMultipole pm;
    param = tree.getParticleMeshMultipoleParameters(dinfo);
    pm.initialize(param);
