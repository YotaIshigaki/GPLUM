...
namespace math{
	...
	template <typename type> inline type abs(const type arg){
	...
	///////////
	template <typename type> inline type power(const type base, const int exp){
		return pow(base, exp);
	}
	template <typename type> inline type root(const type base, const int idx){
		if(idx == 1){
			return base;
		}else if(idx == 2){
			return sqrt(base);
