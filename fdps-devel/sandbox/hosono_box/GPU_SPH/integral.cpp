...
PS::F32 getTimeStepGlobal(const PS::ParticleSystem<RealPtcl>& sph_system){
	PS::F32 dt = 1.0e+30;//set VERY LARGE VALUE
...
void InitialKick(PS::ParticleSystem<RealPtcl>& sph_system, const system_t sysinfo){
	...
	for(PS::S32 i = 0 ; i < sph_system.getNumberOfParticleLocal() ; ++ i){
		sph_system[i].vel_half = sph_system[i].vel + 0.5f * sysinfo.dt * (sph_system[i].acc /* + sph_system[i].grav */);
		sph_system[i].eng_half = sph_system[i].eng + 0.5f * sysinfo.dt * sph_system[i].eng_dot;
	}
}
...
void Predict(PS::ParticleSystem<RealPtcl>& sph_system, const system_t sysinfo){
	#pragma omp parallel for
	for(PS::S32 i = 0 ; i < sph_system.getNumberOfParticleLocal() ; ++ i){
		sph_system[i].vel += sysinfo.dt * (sph_system[i].acc /* + sph_system[i].grav */);
...
void FinalKick(PS::ParticleSystem<RealPtcl>& sph_system, const system_t sysinfo){
	#pragma omp parallel for
	for(PS::S32 i = 0 ; i < sph_system.getNumberOfParticleLocal() ; ++ i){
		sph_system[i].vel = sph_system[i].vel_half + 0.5f * sysinfo.dt * (sph_system[i].acc /* + sph_system[i].grav */);
		sph_system[i].eng = sph_system[i].eng_half + 0.5f * sysinfo.dt * sph_system[i].eng_dot;
	}
}
