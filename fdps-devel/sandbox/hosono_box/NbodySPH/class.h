...
class RealPtcl{
	...
	void writeAscii(FILE* fp) const{
		//fprintf(fp, "%ld\t%ld\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\t%.16lf\n",  this->id,  this->mat,  this->mass,  this->pos.x,  this->pos.y,  this->pos.z,  this->vel.x,  this->vel.y,  this->vel.z,  this->acc.x,  this->acc.y,  this->acc.z, mass*grav.x, mass*grav.y, mass*grav.z);
