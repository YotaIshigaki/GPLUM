                                          ...
                                          PS::ParticleSystem<T> & system)
{
    ...
    PS::F64 *fcache = (PS::F64 *)malloc(sizeof(PS::F64)*n_loc*3);
    assert(fcache);
    fread(fcache, sizeof(PS::F64), n_loc*3, fp);
    ...
    PS::S64 *dcache = (PS::S64 *)malloc(sizeof(PS::S64)*n_loc);
    assert(dcache);
    fread(dcache, sizeof(PS::S64), n_loc, fp);
    for(PS::S32 i = 0; i < n_loc; i++) {
        system[i].id = (PS::S32)dcache[i];
    }
    free(dcache);
