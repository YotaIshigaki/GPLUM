...
subroutine f_main()
   ...
   implicit none
   !* Local parameters
#ifndef READ_NPTCL_FROM_FILE
   integer, parameter :: ntot=2**10
  !integer, parameter :: ntot=2**11
  !integer, parameter :: ntot=2**12
  !integer, parameter :: ntot=2**13
  !integer, parameter :: ntot=2**14
  !integer, parameter :: ntot=2**15
  !integer, parameter :: ntot=2**16
  !integer, parameter :: ntot=2**17
  !integer, parameter :: ntot=2**18
#else
   integer :: ntot
#endif
   !-(force parameters)
...
#ifdef READ_NPTCL_FROM_FILE
   !* Read number of particles
   if (fdps_ctrl%get_rank() == 0) then
      fname = 'nptcl.input'
      open(unit=9,file=trim(fname),action='read',status='old')
         read(9,*)ntot
      close(unit=9)
   end if
   call fdps_ctrl%broadcast(ntot,1,0)
#endif
...
subroutine setup_IC(fdps_ctrl,psys_num,nptcl_glb)
   ...
   if (myrank == 0) then
      ...
      cm_mass = 0.0d0
      do i=1,nptcl_glb
         cm_pos%x = cm_pos%x + ptcl(i)%mass * ptcl(i)%pos%x
         cm_pos%y = cm_pos%y + ptcl(i)%mass * ptcl(i)%pos%y
         cm_pos%z = cm_pos%z + ptcl(i)%mass * ptcl(i)%pos%z
         cm_vel%x = cm_vel%x + ptcl(i)%mass * ptcl(i)%vel%x
         cm_vel%y = cm_vel%y + ptcl(i)%mass * ptcl(i)%vel%y
         cm_vel%z = cm_vel%z + ptcl(i)%mass * ptcl(i)%vel%z
         cm_mass = cm_mass  + ptcl(i)%mass
      end do
      cm_pos%x = cm_pos%x/cm_mass
      cm_pos%y = cm_pos%y/cm_mass
      cm_pos%z = cm_pos%z/cm_mass
      cm_vel%x = cm_vel%x/cm_mass
      cm_vel%y = cm_vel%y/cm_mass
      cm_vel%z = cm_vel%z/cm_mass
      do i=1,nptcl_glb
         ptcl(i)%pos%x = ptcl(i)%pos%x - cm_pos%x
         ptcl(i)%pos%y = ptcl(i)%pos%y - cm_pos%y
         ptcl(i)%pos%z = ptcl(i)%pos%z - cm_pos%z
         ptcl(i)%vel%x = ptcl(i)%vel%x - cm_vel%x
         ptcl(i)%vel%y = ptcl(i)%vel%y - cm_vel%y
         ptcl(i)%vel%z = ptcl(i)%vel%z - cm_vel%z
      end do
