...
subroutine f_main()
...
#endif
#ifdef CHECK_CONSERVATIVE_QUANTITIES
   ...
   if (fdps_ctrl%get_rank() == 0) then
      ...
      wtime_per_step = wtime/dble(nstep)
      write(*,*)'nstep = ',nstep
      write(*,*)'wtime = ',wtime,' [s]'
...
subroutine initial_kick(fdps_ctrl,psys_num,dt)
   ...
   call fdps_ctrl%get_psys_fptr(psys_num,ptcl)
   do i=1,nptcl_loc
      ptcl(i)%vel_half%x = ptcl(i)%vel%x + 0.5d0 * dt * ptcl(i)%acc%x
      ptcl(i)%vel_half%y = ptcl(i)%vel%y + 0.5d0 * dt * ptcl(i)%acc%y
      ptcl(i)%vel_half%z = ptcl(i)%vel%z + 0.5d0 * dt * ptcl(i)%acc%z
...
subroutine full_drift(fdps_ctrl,psys_num,dt)
   ...
   call fdps_ctrl%get_psys_fptr(psys_num,ptcl)
   do i=1,nptcl_loc
      ptcl(i)%pos%x = ptcl(i)%pos%x + dt * ptcl(i)%vel_half%x
      ptcl(i)%pos%y = ptcl(i)%pos%y + dt * ptcl(i)%vel_half%y
      ptcl(i)%pos%z = ptcl(i)%pos%z + dt * ptcl(i)%vel_half%z
   end do
   nullify(ptcl)
...
subroutine predict(fdps_ctrl,psys_num,dt)
   ...
   call fdps_ctrl%get_psys_fptr(psys_num,ptcl)
   do i=1,nptcl_loc
      ptcl(i)%vel%x = ptcl(i)%vel%x + dt * ptcl(i)%acc%x
      ptcl(i)%vel%y = ptcl(i)%vel%y + dt * ptcl(i)%acc%y
      ptcl(i)%vel%z = ptcl(i)%vel%z + dt * ptcl(i)%acc%z
...
subroutine final_kick(fdps_ctrl,psys_num,dt)
   ...
   call fdps_ctrl%get_psys_fptr(psys_num,ptcl)
   do i=1,nptcl_loc
      ptcl(i)%vel%x = ptcl(i)%vel_half%x + 0.5d0 * dt * ptcl(i)%acc%x
      ptcl(i)%vel%y = ptcl(i)%vel_half%y + 0.5d0 * dt * ptcl(i)%acc%y
      ptcl(i)%vel%z = ptcl(i)%vel_half%z + 0.5d0 * dt * ptcl(i)%acc%z
...
subroutine check_cnsrvd_vars(fdps_ctrl,psys_num)
   ...
   do i=1,nptcl_loc
      mom_loc%x = mom_loc%x + ptcl(i)%vel%x * ptcl(i)%mass
      mom_loc%y = mom_loc%y + ptcl(i)%vel%y * ptcl(i)%mass
      mom_loc%z = mom_loc%z + ptcl(i)%vel%z * ptcl(i)%mass
      eng_loc   = eng_loc + ptcl(i)%mass &
                          *(ptcl(i)%eng  &
                           +0.5d0*(ptcl(i)%vel%x*ptcl(i)%vel%x &
