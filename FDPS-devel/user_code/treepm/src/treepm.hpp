...
public:
  ...
  int readParticleBinary(FILE *fp) {
	...
	if( is_little_endian){
	    ...
	    pos.x = reverseEndian(x) / 64.0;
	    pos.y = reverseEndian(y) / 64.0;
	    pos.z = reverseEndian(z) / 64.0;
	    vel.x = reverseEndian(vx) / 3200.0;
	    vel.y = reverseEndian(vy) / 3200.0;
	    vel.z = reverseEndian(vz) / 3200.0;
