...
int main(int argc, char *argv[]){
    ...
    while(time_sys < time_end){
        ...
        /*
        PS::S32 nloc = system_grav.getNumberOfParticleLocal();
        for(PS::S32 i = 0; i < nloc; i++) {
            printf("res %+e %+e %+e %+e\n", system_grav[i].acc[0], system_grav[i].acc[1], system_grav[i].acc[2], system_grav[i].pot);
        }
        PS::Finalize();
        exit(0);
        */
        ...
        //if( n_loop != 0 && (n_loop < 12 || (n_loop % 4 == 0 ))){
...
*/
        ...
        if(PS::Comm::getRank() == 0){
            fout_eng<<time_sys<<"   "<<Etot0<<"   "<<(Etot1-Etot0)/Etot0<<std::endl;            
        }
        Kick(system_grav, dt*0.5);
        n_loop++;
    }
