...
int main(int argc, char *argv[]) {
    ...
    PS::S64 n_loop = 0, n_loop_p = 0;
    PS::S32 id_snap = 0;

    PS::F64 wtime0 = getWallclockTime();
    PS::F64 wtime1;
    ...
    while(time_sys < time_end){
        ...
        if(PS::Comm::getRank() == 0){
            if( (time_sys >= time_diag) || ( (time_sys + dt) - time_diag ) > (time_diag - time_sys) ){
                wtime1 = getWallclockTime();
                fout_eng << time_sys << "   " << (Etot1 - Etot0) / Etot0 << std::endl;
                fprintf(stderr, "time: %10.7f energy: %+e energy error: %+e wtime: %+.3e %+.3e\n",
                        time_sys, Etot1, (Etot1 - Etot0) / Etot0,
                        wtime1 - wtime0,
                        (wtime1 - wtime0) / (n_loop - n_loop_p));
                fprintf(stdout, "time: %10.7f energy: %+e energy error: %+e wtime: %+.3e %+.3e\n",
                        time_sys, Etot1, (Etot1 - Etot0) / Etot0,
                        wtime1 - wtime0,
                        (wtime1 - wtime0) / (n_loop - n_loop_p));
                time_diag += dt_diag;
                wtime0 = getWallclockTime();
                n_loop_p = n_loop;
            }            
        }
