...
struct v2r8{
	...
	v2r8 rcpa() const {
		return v2r8(__builtin_fj_rcpa_v2r8(val));
	}
	__attribute__ ((always_inline))
	v2r8 rcpa_x8() const {
		const v2r8 x(val);
		const v2r8 y0 = v2r8(__builtin_fj_rcpa_v2r8(x.val));
		const v2r8 one(1.0);
		const v2r8 h  = one - x*y0;
		const v2r8 h2 = h*h;
#if 0
		const v2r8 p  = (one+h)*(one+h*h)*(one+h2*h2);
		return p*y0;
#else
		v2r8 y1 = y0 + (h + h*h)*y0;
		return y0 + (h + h2*h2)*y1; // 1-mul, 5-fma
#endif
	}
	__attribute__ ((always_inline))
	v2r8 rcpa_x3() const {
		...
		const v2r8 y0 = v2r8(__builtin_fj_rcpa_v2r8(x.val));
#if 0
		const v2r8 p1 = v2r8(1.5) - x*y0;   // 0.5 + h
		const v2r8 p2 = p1*p1 - v2r8(0.25); // h + h^2
		return y0 + y0*p2; // 3-FMA, 3rd-order convergence
#else
		const v2r8 h = v2r8(1.0) - x*y0;
		const v2r8 p = h + h*h;
		return y0 + p*y0;
#endif
	}
	...
	static v2r8 max(const v2r8 lhs, const v2r8 rhs){
		return v2r8(__builtin_fj_max_v2r8(lhs.val, rhs.val));
	}
	static v2r8 min(const v2r8 lhs, const v2r8 rhs){
		return v2r8(__builtin_fj_min_v2r8(lhs.val, rhs.val));
	}
};
...
struct v2r8_bcl{
	...
	operator v2r8() const {
		return v2r8(__builtin_fj_unpacklo_v2r8(val, val));
	}
};
...
struct v2r8_bch{
	...
	operator v2r8() const {
		return v2r8(__builtin_fj_unpackhi_v2r8(val, val));
	}
};
