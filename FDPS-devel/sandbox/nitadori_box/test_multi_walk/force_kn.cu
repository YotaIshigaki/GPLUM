		...
		const float   eps2)
{
	...
	for(int j=j_head; j<j_tail; j+=N_THREAD_GPU){
		if(N_THREAD_GPU > 32)  __syncthreads();
...
#else
			// *(v4sf *)&dev_epj[nj_tot] = __builtin_ia32_cvtpd2ps256(*(v4df *)&epj[iw][j]);
			v4sf tmp = __builtin_ia32_cvtpd2ps256(*(v4df *)&epj[iw][j]);
			__builtin_ia32_movntps((float *)&dev_epj[nj_tot], tmp);
#endif
            nj_tot++;
		}
...
#else
			v4sf tmp = __builtin_ia32_cvtpd2ps256(*(v4df *)&spj[iw][j]);
			// *(v4sf *)&dev_epj[nj_tot] = __builtin_ia32_shufps(tmp, tmp, 57);
			tmp = __builtin_ia32_shufps(tmp, tmp, 57);
			__builtin_ia32_movntps((float *)&dev_epj[nj_tot], tmp);
#endif
            nj_tot++;
		}
