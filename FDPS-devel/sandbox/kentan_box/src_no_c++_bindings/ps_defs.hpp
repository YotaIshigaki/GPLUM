...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
	    T ret;
	    MPI_Allreduce(&val, &ret, 1, GetDataType<T>(), MPI_MIN, MPI_COMM_WORLD);
	    return ret;
#else
	    return val;
#endif
...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
	    T ret;
	    MPI_Allreduce(&val, &ret, 1, GetDataType<T>(), MPI_MAX, MPI_COMM_WORLD);
	    return ret;
#else
	    return val;
#endif
...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
	    T ret;
	    MPI_Allreduce(&val, &ret, 1, GetDataType<T>(), MPI_SUM, MPI_COMM_WORLD);
	    return ret;
#else
	    return val;
#endif
...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
            bool global;
            MPI_Allreduce(&local, &global, 1, MPI_C_BOOL, MPI_LAND, MPI_COMM_WORLD);
            return global;
#else
            return local;
#endif
	  ...
	  // NOP
        ...
        template<class T> static inline void allGatherV(const T * val_send,
            ...
            MPI_Allgatherv(val_send, n_send, GetDataType<T>(),
			   val_recv, n_recv, n_recv_disp, GetDataType<T>(), MPI_COMM_WORLD);
#else
            for(int i=0; i<n_send; i++) val_recv[i] = val_send[i];
...
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
            MPI_Alltoall(val_send, n, GetDataType<T>(),
			 val_recv, n, GetDataType<T>(), MPI_COMM_WORLD);
#else
            for(int i=0; i<n; i++) val_recv[i] = val_send[i];
#endif
