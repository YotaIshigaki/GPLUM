...
void setParticlesColdUniformSphere(Tpsys & psys,
    ...
    //n_loc = n_glb; 
    n_loc = n_glb / PS::Comm::getNumberOfProc();
    ...
    for(PS::S32 i = 0; i < n_loc; i++){
        ...
        psys[i].vel  = vel[i];
        //psys[i].id = i;
	psys[i].id   = i + n_loc*PS::Comm::getRank();
    }
    delete [] mass;
    delete [] pos;
    delete [] vel;
...
int main(int argc, char *argv[]) {
    ...
    system_grav.setAverageTargetNumberOfSampleParticlePerProcess(200);
    PS::S32 n_loc    = 0;
    PS::F32 time_sys = 0.0;
    ...
    */
	...
	PS::Comm::barrier();
        system_grav.exchangeParticle(dinfo);
	PS::Comm::barrier();
	const auto tp = system_grav.getTimeProfile();
	if(PS::Comm::getRank()==0){
	    std::cout<<"n_loc= "<<n_loc
		     <<" tp.exchange_particle__find_particle= "
		     <<tp.exchange_particle__find_particle
		     <<" tp.exchange_particle__find_particle_2= "
		     <<tp.exchange_particle__find_particle_2
		     <<" tp.exchange_particle__find_particle_3= "
		     <<tp.exchange_particle__find_particle_3
		     <<std::endl;
	}
#ifdef MULTI_WALK
