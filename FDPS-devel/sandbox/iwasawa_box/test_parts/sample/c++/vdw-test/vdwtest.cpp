...
int main(int argc, char *argv[]){
    ...
    dinfo.setPosRootDomain(PS::F64vec(-100.0),
			   PS::F64vec(100.0));
#endif
    ...
    PS::F64 Tloop = 0.0;
    PS::F64 wtime_step = 0.0;
    PS::F64 wtime_step_beg = 0.0;
    PS::S32 snp_id = 0;
    PS::S32 n_loop_output = 0;
    ...
    while(time_sys < time_end){
        ...
        if( time_sys == dt || fmod(time_sys, 1.0/32.0) == 0.0){
            system_grav.adjustPositionIntoRootDomain(dinfo);
            dinfo.decomposeDomainAll(system_grav);
            system_grav.exchangeParticle(dinfo);
            tree_grav.calcForceAllAndWriteBack
                (CalcForceFpFp,  system_grav, dinfo, true, PS::MAKE_LIST_FOR_REUSE);
        }
        else{
            tree_grav.calcForceAllAndWriteBack
                (CalcForceFpFp,  system_grav, dinfo, true, PS::REUSE_LIST);
        }
#else
        system_grav.adjustPositionIntoRootDomain(dinfo);
	...
	tree_grav.calcForceAllAndWriteBack
        ...
        Kick(system_grav, dt*0.5);
        wtime_step += PS::GetWtime() - wtime_step_beg;
        n_loop_output++;
	...
	//        tree_grav.dump_calc_cost(PS::Comm::getMaxValue(Tloop), fout_tcal);
        ...
        /*
        PS::F64 vel_max_loc = 0.0;
        PS::F64 vel_disp_loc = 0.0;
        PS::S32 n_loc = system_grav.getNumberOfParticleLocal();
        for(PS::S32 i=0; i<n_loc; i++){
            vel_max_loc = std::max(sqrt(system_grav[i].vel*system_grav[i].vel), fabs(vel_max_loc));
            vel_disp_loc += system_grav[i].vel*system_grav[i].vel;
        }
        PS::F64 vel_max_glb = PS::Comm::getMaxValue(vel_max_loc);
        PS::F64 vel_disp_glb = sqrt(PS::Comm::getSum(vel_disp_loc)/n_tot);
        */
        if(PS::Comm::getRank() == 0){
            /*
            std::cerr<<"vel_max_glb= "<<vel_max_glb
                     <<" vel_disp_glb= "<<vel_disp_glb
                     <<std::endl;
            */
            fout_eng<<time_sys<<"   "<<(Etot1-Etot0)/Etot0<<std::endl;
	    if(time_sys >= time_diag) {
                fprintf(stderr, "time: %10.7f energy error: %+e %e\n",
                        time_sys, (Etot1 - Etot0) / Etot0, wtime_step/n_loop_output);
                fprintf(stdout, "time: %10.7f energy error: %+e %e\n",
                        time_sys, (Etot1 - Etot0) / Etot0, wtime_step/n_loop_output);
                n_loop_output = 0;
                wtime_step = 0.0;
		time_diag += dt_diag;
