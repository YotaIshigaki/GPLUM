...
int main(int argc, char *argv[]) {
    ...
    */
    system.sortParticle([](const FP & left, const FP & right){return(left.id < right.id);});
    /*
    if(PS::Comm::getRank()==0){
        std::cerr<<"Sort ID"<<std::endl;
        for(PS::S32 i=0; i<n_loc; i++){
            std::cerr<<"system[i].id= "<<system[i].id<<std::endl;
        }
    }
    */
    ...
    /*
    if(PS::Comm::getRank()==0){
        std::cerr<<"Sort ID (inv)"<<std::endl;
        for(PS::S32 i=0; i<n_loc; i++){
            std::cerr<<"system[i].id= "<<system[i].id<<std::endl;
        }
    }
    */
    system.sortParticle([](const FP & left, const FP & right){return(left.pos.x < right.pos.x);});
    /*
    if(PS::Comm::getRank()==0){
        std::cerr<<"Sort POS.X"<<std::endl;
        for(PS::S32 i=0; i<n_loc; i++){
            std::cerr<<"system[i].pos.x= "<<system[i].pos.x<<std::endl;
        }
    }
    */
    system.sortParticleStable([](const FP & left, const FP & right){return(left.id < right.id);});
    for(PS::S32 i=0; i<n_loc-1; i++){
        assert(system[i].id<=system[i+1].id);
        if(system[i].id==system[i+1].id){
            assert(system[i].pos.x<=system[i+1].pos.x);
        }
    }
    PS::Comm::barrier();
    ...
    if(PS::Comm::getRank()==0){
        std::cerr<<"Sort POS.X"<<std::endl;
        for(PS::S32 i=0; i<n_loc; i++){
            std::cerr<<" id= "<<system[i].id
                     <<" system[i].pos.x= "<<system[i].pos.x
                     <<std::endl;
        }
    }
    */
