...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    tree_grav.calcForce(CalcGravity<FPGrav>, CalcGravity<PS::SPJMonopole>, true);
    for(PS::S32 i=0; i<n_loc; i++) system_grav[i].copyFromForce( tree_grav.getForce(i) );

#endif

    system_grav.dumpMemSizeUsed(std::cout);
    tree_grav.dumpMemSizeUsed(std::cout);
    std::cout<<std::endl;
