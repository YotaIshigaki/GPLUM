...
#pragma omp parallel for
    ...
    exchangeLocalEssentialTreeImpl(TagSearchShortSymmetry, const DomainInfo & dinfo){
        ...
        static S32 * id_proc_dest;
        static ReallocatableArray<S32> * id_ptcl_send;
        static ReallocatableArray<bool> * flag_send; // NEW
        static ReallocatableArray<F64vec> * shift_image_box;
        static ReallocatableArray<S32> * ip_disp;
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
        static MPI::Request * req_send;
        static MPI::Request * req_recv;
        const S32 my_rank = Comm::getRank();
        ...
        if(first){
            ...
            epj_send_buf = new ReallocatableArray<Tepj>[n_thread];
            id_ptcl_send = new ReallocatableArray<S32>[n_thread];
            flag_send = new ReallocatableArray<bool>[n_thread];
            shift_image_box = new ReallocatableArray<F64vec>[n_thread];
            ip_disp = new ReallocatableArray<S32>[n_thread];
            for(S32 i=0; i<n_thread; i++){
                id_ptcl_send[i].reserve(n_surface_for_comm_);
                epj_send_buf[i].reserve(n_surface_for_comm_);
                flag_send[i].reserve(n_loc_tot_ + (n_loc_tot_ / 4) + 100);
                shift_image_box[i].reserve(5*5*5);
                ip_disp[i].reserve(3*3*3);
            }
#ifdef PARTICLE_SIMULATOR_MPI_PARALLEL
            req_send = new MPI::Request[n_proc];
            req_recv = new MPI::Request[n_proc];
...
#pragma omp parallel
        {
            const S32 ith = Comm::getThreadNum();
            flag_send[ith].reserveAtLeast( n_loc_tot_ ); // NEW
            S32 n_proc_src_2nd = 0;
            bool pa[DIMENSION];
            dinfo.getPeriodicAxis(pa);
            epj_send_buf[ith].clearSize();
...
#pragma omp for
            for(S32 ib=0; ib<n_proc_src_1st; ib++){
                ...
                S32vec id_image_new;
                S32vec id_image_old = -9999;
                shift_image_box[ith].clearSize();
                ip_disp[ith].clearSize();
                if(dinfo.getBoundaryCondition() == BOUNDARY_CONDITION_OPEN){
                    shift_image_box[ith].pushBackNoCheck( F64vec(0.0) );
                    ip_disp[ith].pushBackNoCheck(i_head);
                    ip_disp[ith].pushBackNoCheck(i_tail);
                }
                else{
                    for(S32 ip=i_head; ip<i_tail; ip++){
                        const F64vec pos_target = epj_recv_1st_buf[ip].getPos();
                        id_image_new = CalcIDOfImageDomain(pos_root_domain, pos_target, pa);
                        if(id_image_old != id_image_new){
                            ip_disp[ith].push_back(ip);
#ifdef PARTICLE_SIMULATOR_TWO_DIMENSION
                            shift_image_box[ith].push_back(F64vec(id_image_new.x*size_root_domain.x, id_image_new.y*size_root_domain.y));
#else
                            shift_image_box[ith].push_back(F64vec(id_image_new.x*size_root_domain.x, id_image_new.y*size_root_domain.y, id_image_new.z*size_root_domain.z));
#endif
                            id_image_old = id_image_new;
                        }
                    }
                    ip_disp[ith].push_back(i_tail);
                }

                const S32 n_image = shift_image_box[ith].size();
                for(S32 ii=0; ii<n_image; ii++){
                    const F64vec shift = shift_image_box[ith][ii];
                    const S32 adr_tc_tmp = tc_loc_[0].adr_tc_;
                    const F64ort pos_domain = dinfo.getPosDomain(id_proc_tmp).shift(shift);
		    //#if 1
#ifdef FAST_COMM_FOR_2ND_EXCHANGE
                    for(S32 jp=0; jp<n_loc_tot_; jp++){
                        flag_send[ith][jp] = false;
                    }
                    for(S32 ip=ip_disp[ith][ii]; ip<ip_disp[ith][ii+1]; ip++){
                        const F64vec pos_target = epj_recv_1st_buf[ip].getPos();
                        const F64 len_target = epj_recv_1st_buf[ip].getRSearch();
                        const F64ort particle_box(pos_target, len_target);
                        id_ptcl_send[ith].clearSize();
                        if( !tc_loc_[0].isLeaf(n_leaf_limit_) ){
                            MakeListUsingInnerBoundaryForSymmetryExclusiveSetFlag
                                (tc_loc_.getPointer(),     adr_tc_tmp,
                                 epj_sorted_.getPointer(), flag_send[ith],
                            ...
                            for(S32 ip=0; ip<n_tmp; ip++, adr_ptcl_tmp++){
                                ...
                                if(dis_sq0 <= len_sq && dis_sq1 > len_sq){
                                    flag_send[ith][adr_ptcl_tmp] = true;
                                }
                            }
                        }
                    }
                    id_ptcl_send[ith].clearSize();
                    for(S32 jp = 0; jp < n_loc_tot_; jp++){
                        if(flag_send[ith][jp] == true){
                            id_ptcl_send[ith].push_back(jp);
                        }
                    }
#elif 1
                    // use std::set
...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp) 
    ...
    calcForce(Tfunc_ep_ep pfunc_ep_ep,
        ...
        //S64 n_list_ep_ep_max_par_loop = 0; // for DEBUG
        //S64 n_list_ep_sp_max_par_loop = 0; // for DEBUG
        //S32 id_ipg_ep_ep = 0; // for DEBUG
        //S32 id_ipg_ep_sp = 0; // for DEBUG
        if(n_ipg > 0){
	  //#pragma omp parallel for reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp) 
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp) 
            for(S32 i=0; i<n_ipg; i++){
	      //std::cout<<"Comm::getThreadNum()="<<Comm::getThreadNum()<<std::endl;
                makeInteractionList(i);
...
#ifdef DEBUGTMP
                if(epj_for_force_[Comm::getThreadNum()].size() > 10000 || spj_for_force_[Comm::getThreadNum()].size() > 10000){
                    std::cout<<"i="<<i<<std::endl;
                    std::cout<<"epj_for_force_[Comm::getThreadNum()].size()="<<epj_for_force_[Comm::getThreadNum()].size()
                             <<" spj_for_force_[Comm::getThreadNum()].size()="<<spj_for_force_[Comm::getThreadNum()].size()<<std::endl;
                }
/*

                if( n_list_ep_ep_max_par_loop < epj_for_force_[Comm::getThreadNum()].size() ){
                    n_list_ep_ep_max_par_loop = epj_for_force_[Comm::getThreadNum()].size();
                    id_ipg_ep_ep = i;
                }
                if( n_list_ep_sp_max_par_loop < spj_for_force_[Comm::getThreadNum()].size() ){
                    n_list_ep_sp_max_par_loop = spj_for_force_[Comm::getThreadNum()].size();
                    id_ipg_ep_sp = i;
                }
*/
#endif
