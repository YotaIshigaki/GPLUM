...
// for debug
        ...
        void calcForceAllAndWriteBackWithCheck(Tfunc_ep_ep pfunc_ep_ep,
					       ...
					       const bool clear_force,
					       std::ofstream & fout){
	  //calcForceAll(pfunc_ep_ep, pfunc_ep_sp, psys, dinfo, clear_force, fout);
        ...
        void dump_calc_cost(const double & tcal, std::ostream & fout){
            ...
            double n_op_per_interaction_ep_ep = 28.0; // use the 3rd orde convergence with potential
            //double n_op_per_interaction_ep_sp = 28.0; // use the 3rd orde convergence with potential
            double n_op_per_interaction_ep_sp = 57.0;
            S64 n_interaction_ep_ep_tot = Comm::getSum(n_interaction_ep_ep_);
            S64 n_interaction_ep_sp_tot = Comm::getSum(n_interaction_ep_sp_);
            fout<<"n_loc_tot_= "<<n_loc_tot_<<" n_glb_tot_= "<<n_glb_tot_
                <<" ni_ave_= "<<ni_ave_<<" nj_ave_= "<<nj_ave_
                <<" n_interaction_ep_ep_tot= "<<n_interaction_ep_ep_tot
                <<" n_interaction_ep_sp_tot= "<<n_interaction_ep_sp_tot
                <<" speed: "<<((double)(n_interaction_ep_ep_tot)*n_op_per_interaction_ep_ep + (double)(n_interaction_ep_sp_tot)*n_op_per_interaction_ep_sp)/tcal*1e-12<<" [Tflops] efficiency= "
                <<((double)(n_interaction_ep_ep_tot)*n_op_per_interaction_ep_ep + (double)(n_interaction_ep_sp_tot)*n_op_per_interaction_ep_sp)/tcal/(speed_per_node*Comm::getNumberOfProc())
                <<" wtime_exlet_comm_= "<<wtime_exlet_comm_<<" wtime_exlet_a2a_= "<<wtime_exlet_a2a_<<" wtime_exlet_a2av_= "<<wtime_exlet_a2av_
                <<" Tcomm_scatterEP_tmp_= "<<Tcomm_scatterEP_tmp_<<std::endl;
/*
