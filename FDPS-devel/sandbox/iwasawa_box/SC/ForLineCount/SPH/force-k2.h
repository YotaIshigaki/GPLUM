...
struct CalcHydroForce{
	...
	for(int i=0; i<Nip; i+=2){
		...
		const v2r8 AV = ((v_sig * w_ij) * (idens + jdens).rcpa_x8() * (iBal + jBal)); // 1rcpa 1sub 5add 10mul (25add 31mul 2rsqrt 2min 50op)
		const v2r8 gradWs = jmass * (kernel::gradW(si, rinv)*iHinv4 + kernel::gradW(sj, rinv)*kernel::pow4(v2r8(jHinv)));  
