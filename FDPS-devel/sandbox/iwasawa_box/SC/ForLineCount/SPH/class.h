...
class RealPtcl{
 public:
    ...
    void setPos(const PS::F64vec& pos){this->pos = pos;}
    void writeAscii(FILE* fp) const{
...
namespace EPI{
	...
	PS::F64vec getPos() const{return this->pos;}
	PS::F64 getRSearch() const{return kernel_t::supportRadius() * this->smth;}
    };
    class Grav{
    public:
	PS::F64vec pos;
...
namespace EPJ{
	...
	PS::S64    id;
	PS::F64vec getPos() const{return this->pos;}
	PS::F64 getCharge(void) const{return this->mass;}
	void copyFromFP(const RealPtcl& rp){
	    this->mass = rp.mass;
	    this->pos  = rp.pos;
	    this->id   = rp.id;
