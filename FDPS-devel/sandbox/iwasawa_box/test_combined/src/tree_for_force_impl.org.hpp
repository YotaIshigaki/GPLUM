...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
	...
	//const S32 ng = n_ipg / n_thread + ( (n_ipg % n_thread == 0) ? 0 : 1);
	static S32 * ng_offset;
...
#pragma omp parallel
		...
		for(S32 ig=ig_head; ig<ig_tail; ig++){
		    S32 n_walk = (n_walk_limit < gend-ig) ? n_walk_limit : gend-ig;
