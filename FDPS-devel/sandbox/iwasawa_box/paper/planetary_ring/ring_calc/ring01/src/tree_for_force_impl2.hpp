...
#pragma omp parallel
#endif
        {
            ...
            args[0] = (unsigned long)(offset);
    #ifdef REMOVE_TP_LOC
            args[1] = (unsigned long)(epj_sorted_loc_.getPointer());
            args[2] = (unsigned long)(epj_org_.getPointer());
            args[3] = (unsigned long)(sizeof(epj_sorted_loc_[0]));
    #else
            args[1] = (unsigned long)(tp_loc_.getPointer());
            args[2] = (unsigned long)(tp_glb_.getPointer());
            args[3] = (unsigned long)(sizeof(tp_loc_[0]));
    #endif
            __real_athread_spawn((void*)slave_CopyDirect, args);
            athread_join();
...
#pragma omp for
        ...
        if(Comm::getRank() == 0){
            ...
            for(S32 i=0; i<tp_glb_.size(); i++){
                std::cerr<<"i= "<<i
                         <<" tp_glb_[i].key_hi_= "<<tp_glb_[i].key_hi_
#ifdef USE_96BIT_KEY
