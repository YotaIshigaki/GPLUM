...
public:
    void operator()(EPItreepm *iptcl,
        ...
        /*
        PS::F64 xi[IPTCL_MAX][3];
        PS::F64 ai[IPTCL_MAX][3];
        PS::F64 pi[IPTCL_MAX];
        PS::F64 xj[JPTCL_MAX][3];
        PS::F64 mj[JPTCL_MAX];
        const int j_loop_max = ((nj-1) / JPTCL_MAX) + 1;
        assert(ni <= IPTCL_MAX);
        for (int i=0; i<ni; i++){
            xi[i][0] = iptcl[i].pos.x;
            xi[i][1] = iptcl[i].pos.y;
            xi[i][2] = iptcl[i].pos.z;
            for(int k=0; k<3; k++){
                ai[i][k] = 0.0;
            }
            pi[i] = 0.0;
        ...
        for (int j_loop=0; j_loop<j_loop_max; j_loop++){
            ...
            int j_cnt = 0;
            for (int j=j_head; j<j_tail; j++){
                mj[j_cnt] = jptcl[j].mass;
                xj[j_cnt][0] = jptcl[j].pos.x;
                xj[j_cnt][1] = jptcl[j].pos.y;
                xj[j_cnt][2] = jptcl[j].pos.z;
                j_cnt++;
            }
            assert(j_cnt == nj_tmp);
            g5_set_xmj(0, nj_tmp, xj, mj);
            //g5_set_eps_to_all(eps);
            g5_set_n(nj);
            g5_calculate_force_on_x(xi, ai, pi, ni);
        }
        if(1){
            std::cerr<<"iptcl[0].pos= "<<iptcl[0].pos
                     <<" ppforce[0].acc= "<<ppforce[0].acc
                     <<" ai[0]= "<<ai[0][0]<<"  "<<ai[0][1]<<"  "<<ai[0][2]
                     <<std::endl;
            //for(int j=0; j<nj; j++){
            //    std::cerr<<"jptcl[j].mass= "<<jptcl[j].mass
            //             <<" pos= "<<jptcl[j].pos<<std::endl;
            //}
            DEBUG_PRINT++;
        }
        for (PS::S32 i = 0; i < ni; i++) {
            for(int k=0; k<3; k++){
                ppforce[i].acc[k] += ai[i][k];
            }
        }
        */
    }
