...
//#define VERSION_OF_PREPROC2 (1)
#define VERSION_OF_PREPROC2 (2)
...
void CopyEPJLocToEPJGlb(void *args){ 
    ...
    volatile epjLM  *epj = (epjLM *) ldm_malloc( bsize_epj_array );
    volatile int    *adr = (int *)   ldm_malloc( bsize_adr_array );
...
void Preproc1_CopyEPJLocToEPJGlb(void *args) {
    ...
    volatile size_t bsize_adr_array = sizeof(int) * CHUNK_SIZE;
    volatile int    *adr = (int *)   ldm_malloc( bsize_adr_array );
    //* Compute moments on each CPE
    // (loop counters)
    int i,j,k; 
...
void Preproc2_CopyEPJLocToEPJGlb(void *args) {
    ...
    volatile int *adr                    = (int *) ldm_malloc( bsize_adr_array );
    volatile int *adr_epj_loc_group_head = (int *) ldm_malloc( bsize_buf_array );
    volatile int *adr_epj_glb_group_head = (int *) ldm_malloc( bsize_buf_array );
    volatile int *group_size_epj_loc     = (int *) ldm_malloc( bsize_buf_array );
    //* Compute moments on each CPE
    // (loop counters)
    int i,j,k; 
    // (local vars. for DMA comm.)
    ...
    volatile unsigned long adr_dma_put[3];
    //** DMA setting 
    reply_get = 0;
    dma_set_op(&dma_get, DMA_GET);
    dma_set_mode(&dma_get, PE_MODE);
...
void CopyEPJLocToEPJGlb(void *args){ 
    ...
    dma_descriptor_init(&dma_put, &reply_put);
    // (number of group)
    volatile int n_group;
    // (addresses of group info.)
    volatile unsigned long adr_group_info[3];
    //* Get n_group
    reply_get = 0;
    dma_set_op(&dma_get, DMA_GET);
    dma_set_mode(&dma_get, PE_MODE);
...
void Preproc2_CopyEPJLocToEPJGlb(void *args) {
    ...
    volatile int *adr_epj_glb_group_head = (int *) ldm_malloc( bsize_buf_array );
    volatile int *adr_epj_loc_group_head = (int *) ldm_malloc( bsize_buf_array );
    volatile int *group_size_epj_glb     = (int *) ldm_malloc( bsize_buf_array );
...
void Preproc2_CopyEPJLocToEPJGlb(void *args) {
    ...
    assert(CHUNK_SIZE < BUF_SIZE); // we assume BUF_SIZE > CHUNK_SIZE
    size_t bsize_adr_array = sizeof(int) * CHUNK_SIZE;
    size_t bsize_buf_array = sizeof(int) * BUF_SIZE;
    volatile int *adr                    = (int *) ldm_malloc( bsize_adr_array );
    volatile int *adr_epj_glb_group_head = (int *) ldm_malloc( bsize_buf_array );
    ...
    for (i=0; i<n_loc; i+=CHUNK_SIZE) {
        ...
        if ((group_id > 1) && (group_id + CHUNK_SIZE > BUF_SIZE)) {
            // In this case, it can be happened that the number of group information
            // is larger than BUF_SIZE. So, we put the group information calculated
            // before now to the main memory.
            //printf("flush in Preproc2_CopyEPJLocToEPJGlb()!\n");
            ...
            dma_set_mode(&dma_put, PE_MODE);
            dma_set_reply(&dma_put, &reply_put);
            dma_set_size(&dma_put, sizeof(int) * group_id); // we put 0 to (group_id-1) only.
            // (1) Put to adr_epj_glb_group_head_[my_id]
            dma(dma_put, (long)((int *)adr_dma_put[0] + n_group_already_written),
                (long*)(adr_epj_glb_group_head));
            // (2) Put to adr_epj_loc_group_head_[my_id]
            dma(dma_put, (long)((int *)adr_dma_put[1] + n_group_already_written), 
                (long*)(adr_epj_loc_group_head));
            // (3) Put to group_size_epj_glb_[my_id]
            dma(dma_put, (long)((int *)adr_dma_put[2] + n_group_already_written), 
                (long*)(group_size_epj_glb));
            //** Wait all
            dma_wait(&reply_put, 3);
            while (reply_put != 3) {}

            //*** Update n_group_already_written
            n_group_already_written += group_id;

            //*** Move (group_id) th information to 0th 
            adr_epj_glb_group_head[0] = adr_epj_glb_group_head[group_id];
            adr_epj_loc_group_head[0] = adr_epj_loc_group_head[group_id];
            group_size_epj_glb[0]     = group_size_epj_glb[group_id];
            group_id                  = 0;
...
#else The value of the macro `VERSION_OF_PREPROC2` is incorrect.
#error 
#endif //VERSION_OF_PREPROC2
