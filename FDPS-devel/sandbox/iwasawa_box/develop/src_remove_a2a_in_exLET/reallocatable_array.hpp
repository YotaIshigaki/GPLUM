...
namespace  ParticleSimulator{
    ...
    enum class MemoryAllocMode : int {Default=0, Pool=1};
    ...
    private:
        ...
        void constructorFunction(const int cap){
            ...
            else if(alloc_mode_ == MemoryAllocMode::Pool) {
...
#pragma omp critical
        ...
        void dumpImpl() const {
            ...
            std::cerr<<"alloc_mode_= "<<static_cast<int>(alloc_mode_)<<std::endl;
            //std::cerr<<"ALLOCMODE= "<<ALLOCMODE<<" alloc_mode_= "<<alloc_mode_<<std::endl;
        }
        int n_expand_;
...
#pragma omp critical
    ...
    public:
#if SANITY_CHECK_REALLOCATABLE_ARRAY > 0
        ReallocatableArray() : data_(nullptr), size_(0), capacity_(0), capacity_org_(0), id_mpool_(-1), alloc_mode_(MemoryAllocMode::Default), n_expand_(0) {}
        ReallocatableArray(MemoryAllocMode alloc_mode) : data_(nullptr), size_(0), capacity_(0), capacity_org_(0), id_mpool_(-1), alloc_mode_(alloc_mode), n_expand_(0) {}
        ReallocatableArray(int cap, int size, MemoryAllocMode alloc_mode) : data_(nullptr), size_(size), capacity_org_(0), id_mpool_(-1), alloc_mode_(alloc_mode), n_expand_(0) {
            constructorFunction(cap);
        }
#else
        ReallocatableArray() : data_(nullptr), size_(0), capacity_(0), capacity_org_(0), id_mpool_(-1), alloc_mode_(MemoryAllocMode::Default) {}
        ReallocatableArray(MemoryAllocMode alloc_mode) : data_(nullptr), size_(0), capacity_(0), capacity_org_(0), id_mpool_(-1), alloc_mode_(alloc_mode) {}
...
//#ifdef SANITY_CHECK_REALLOCATABLE_ARRAY
        ...
        void freeMem(const int free_alloc_mode = -1){
            if( alloc_mode_ == MemoryAllocMode::Default && (free_alloc_mode == -1  || free_alloc_mode == 0) ){
                if(capacity_ > 0){
                    capacity_org_ = capacity_;
                    size_ = capacity_ = 0;
                    //std::cerr<<"A) data_= "<<data_<<std::endl;
                    delete [] data_;
                    data_ = nullptr;
                    //capacity_ = 10;
                    //data_ = new T[capacity_];
            ...
            else if( alloc_mode_ == MemoryAllocMode::Pool && (free_alloc_mode == -1  || free_alloc_mode == 1) ){
