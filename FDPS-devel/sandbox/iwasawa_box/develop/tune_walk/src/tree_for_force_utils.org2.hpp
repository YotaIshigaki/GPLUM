...
#pragma omp parallel for schedule(static)
            ...
            for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                ...
                if(ith == 1){
                    std::cerr<<" i= "<<i
                             <<" adr_ptcl_left= "  <<adr_ptcl_left
                             <<" adr_ptcl_right= "<<adr_ptcl_right
                             <<std::endl;
                }
                */
                /*
                for(S32 j=adr_ptcl_left; j<adr_ptcl_right+1; j++){
                    std::cerr<<std::dec;
                    std::cerr<<"j= "<<j<<std::oct<<" tp[j].key_= "<<tp[j].key_
                             <<" MortonKey::getCellID(lev_cur+1, tp[j].key_)= "<<MortonKey::getCellID(lev_cur+1, tp[j].key_)
                             <<std::endl;
                    std::cerr<<std::dec;                    
                }
                */
                ...
                //tc_array[i].more_adr_ = n_cell[ith];
...
#pragma omp for schedule(static)
                ...
                for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                    ...
                    std::cerr<<"n_cell_offset[ith]+id_cell_right+1+n_cnt="<<n_cell_offset[ith]+id_cell_right+1+n_cnt<<std::endl;
                    */
                    std::cerr<<"n_cell_offset[ith]= "<<n_cell_offset[ith]<<std::endl;
                ...
                /*
                if(ith == 1){
                    std::cerr<<" i_head= "<<i_head
                             <<" i_tail= "<<i_tail
                             <<std::endl;
                } 
                */               
                for(S32 i=i_head; i<i_tail; i++){
                    //if(ith == 1) std::cerr<<" id_cell_right+1+i= "<<id_cell_right+1+i<<std::endl;
                    /*
                    tc_array[id_cell_right+1+i].adr_ptcl_ = tc_buf[ith][i].adr_ptcl_;
                    tc_array[id_cell_right+1+i].n_ptcl_   = tc_buf[ith][i].n_ptcl_;
                    tc_array[id_cell_right+1+i].next_adr_ = (tc_buf[ith][i].next_adr_ == -2) ? (id_cell_right+i+2) : tc_array[id_cell_right+1+i].next_adr_;
                    tc_array[id_cell_right+1+i].level_    = lev_cur+1;
                    tc_array[id_cell_right+1+i].more_adr_ = -1;
                    tc_array[id_cell_right+1+i].id_cell_  = tc_buf[ith][i].id_cell_;
                    */
                    ...
                    tc_array[id_cell_right+1+i].next_adr_ = (tc_buf[ith][i-i_head].next_adr_ == -2) ? (id_cell_right+i+2) : tc_array[id_cell_right+1+i].next_adr_;
                    tc_array[id_cell_right+1+i].level_    = lev_cur+1;
                    tc_array[id_cell_right+1+i].more_adr_ = -1;
            ...
            /*
            std::cerr<<"id_cell_left= "<<id_cell_left<<" id_cell_right= "<<id_cell_right<<std::endl;
            std::cerr<<"n_cell_new= "<<n_cell_new<<std::endl;
            for(S32 i=0; i<n_th; i++){
                std::cerr<<"n_cell[i]= "<<n_cell[i]
                         <<" n_cell_offset[i]= "<<n_cell_offset[i]
                         <<std::endl;
            }
            */
            /*
            std::cerr<<"A: id_cell_left= "<<id_cell_left<<" id_cell_right= "<<id_cell_right<<std::endl;                
            for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                std::cerr<<"i= "<<i<<std::endl;
                tc_array[i].dump(std::cerr);
            }
            */
            ...
            id_cell_right += n_cell_new;
            /*
            std::cerr<<"B: id_cell_left= "<<id_cell_left<<" id_cell_right= "<<id_cell_right<<std::endl;
            for(S32 i=id_cell_left; i<id_cell_right+1; i++){
                std::cerr<<"i= "<<i<<std::endl;
                tc_array[i].dump(std::cerr);
            }
...
#pragma omp parallel for
  ...
  // NOTE: This FUnction mimic sirial code 
    ...
    void MakeInteractionListLongEPSP(const ReallocatableArray<Ttc> & tc_first,
                                     ...
                                     const S32 n_leaf_limit){
        U32 open_bits = 0;
        const S32 n_child = tc_first[adr_tc].n_child_;
        for(S32 i=0; i<n_child; i++){
