...
void DumpTimeProfile2(const PS::TimeProfile & tp, const PS::TimeProfile & tp_max, const PS::S32 rank_max[], std::ostream & fout){
    ...
    //fout<<"walk_tree_for_force= "<<tp.walk_tree_for_force<<", max= "<<tp_max.walk_tree_for_force<<", rank= "<<rank_max[id++]<<std::endl;
...
void getTimeProfileMax(const PS::TimeProfile & tp, const PS::S32 rank, PS::TimeProfile & tp_max, PS::S32 rank_max[]){
    ...
    //PS::Comm::getMaxValue(tp.walk_tree_for_force, rank, tp_max.walk_tree_for_force, rank_max[id++]);
}
...
void ReadSnapShot(PS::ParticleSystem<FPGrav> & sys, 
    ...
    if( PS::Comm::getRank() < n_snp_init ){
        ...
        fin.open(sinput);
	if(!fin){
	    std::cerr<<"initial file can not open: "<<sinput<<std::endl;
	    PS::Abort();
	}
        fin>>n_tot_glb_snp;
        fin>>n_disk_glb_snp;
    ...
    std::cout<<"n_tot_glb_snp="<<n_tot_glb_snp<<std::endl;
    ...
    std::cout<<"n_tot_loc="<<n_tot_loc<<std::endl;
    ...
    std::cout<<"n_tot_glb="<<n_tot_glb<<std::endl;
...
int main(int argc, char *argv[]){
...
#ifdef USE_GPU_CUDA
    //const PS::F64 flops_per_proc = 1.31e12; // K20X
    //const PS::F64 flops_per_proc = 3.95e12; // K20X
    const PS::F64 flops_per_proc = 3.09e12; // GTX680
    //const PS::F64 n_op_ep_ep_grav = 34.0;
    //const PS::F64 n_op_ep_sp_grav = 34.0;
...
#elif defined(USE_K2)
    dinfo.setNumberOfDomainMultiDimension(nx, ny, nz/2);
#elif defined(USE_K8)
...
#ifdef USE_GPU_CUDA
    //PS::S32 n_walk_limit = 100;
    tree_grav.calcForceAllAndWriteBackMultiWalk(DispatchKernelWithSP, RetrieveKernel, 0, 
    ...
    while(time_sys < time_end){
        ...
        //if( n_loop % 8 == 0) timer.initialize(fout_tcal);
...
*/
        ...
        fout_tcal<<"system_grav.getNumberOfParticleGlobal()= "<<system_grav.getNumberOfParticleGlobal()
                 <<" system_grav.getNumberOfParticleLocal()= "<<system_grav.getNumberOfParticleLocal()<<std::endl;
        fout_tcal<<"n_let_ep_send_1st_local= "<<tree_grav.getNumberOfLETEPSend1stLocal()
                 <<" n_let_ep_recv_1st_local= "<<tree_grav.getNumberOfLETEPRecv1stLocal()
                 <<" n_let_sp_send_1st_local= "<<tree_grav.getNumberOfLETSPSend1stLocal()
                 <<" n_let_sp_recv_1st_local= "<<tree_grav.getNumberOfLETSPRecv1stLocal()<<std::endl;
        fout_tcal<<"n_let_ep_send_1st_global= "<<tree_grav.getNumberOfLETEPSend1stGlobal()
                 <<" n_let_ep_recv_1st_global= "<<tree_grav.getNumberOfLETEPRecv1stGlobal()
                 <<" n_let_sp_send_1st_global= "<<tree_grav.getNumberOfLETSPSend1stGlobal()
                 <<" n_let_sp_recv_1st_global= "<<tree_grav.getNumberOfLETSPRecv1stGlobal()<<std::endl;
        fout_tcal<<"n_ptcl_send_local= "<<system_grav.getNumberOfParticleSendLocal()
                 <<" n_ptcl_recv_local= "<<system_grav.getNumberOfParticleRecvLocal()
                 <<" n_ptcl_send_golbal= "<<system_grav.getNumberOfParticleSendGlobal()
                 <<" n_ptcl_recv_golbal= "<<system_grav.getNumberOfParticleRecvGlobal()<<std::endl;
        fout_tcal<<"n_cell_open_local= "<<tree_grav.getNumberOfCellOpenLocal()
                 <<" n_cell_open_global= "<<tree_grav.getNumberOfCellOpenGlobal()<<std::endl;
...
/*
        PS::F64 wtime_mk_lt_local = tp.make_local_tree + tp.calc_moment_local_tree;
        PS::F64 wtime_mk_gt_local = tp.make_global_tree + tp.calc_moment_global_tree;
        PS::F64 wtime_mk_lt_local_max, wtime_mk_gt_local_max;
        PS::S32 rank_mk_lt_local_max, rank_mk_gt_local_max;
        PS::Comm::getMaxValue(wtime_mk_lt_local, rank, wtime_mk_lt_local_max, rank_mk_lt_max);
        PS::Comm::getMaxValue(wtime_mk_gt_local, rank, wtime_mk_gt_local_max, rank_mk_lt_max);
        fout_tcal<<"wtime_mk_lt_local= "<<wtime_mk_lt_local<<", max= "<<wtime_mk_lt_local_max<<", rank= "<<rank_mk_lt_max<<std::endl;
        fout_tcal<<"wtime_mk_lt_local= "<<wtime_mk_lt_local<<", max= "<<wtime_mk_lt_local_max<<", rank= "<<rank_mk_lt_max<<std::endl;
*/
        fout_tcal<<std::endl;
        fout_tcal<<std::endl;
        ...
        std::cout<<"speed= "<<(PS::F64)(n_op_tot)/(wtime_tot)*1e-12<<"[Tflops]"<<std::endl;
        std::cout<<"efficiency= "<<(PS::F64)(n_op_tot)/(wtime_tot)/(flops_per_proc*PS::Comm::getNumberOfProc())<<std::endl;
        std::cout<<"wtime_tot= "<<wtime_tot<<std::endl;
        std::cout<<"n_op_tot= "<<n_op_tot<<std::endl;

        timer.dump(std::cout);

        DumpTimeProfile0(tp_dinfo, tp_dinfo_max, rank_dinfo_max, std::cout);
        DumpTimeProfile1(tp_system, tp_system_max, rank_system_max, std::cout);
        std::cout<<"n_int_ep_ep_grav= "<<n_int_ep_ep_grav<<" n_int_ep_sp_grav= "<<n_int_ep_sp_grav<<std::endl;
        std::cout<<"ni_ave= "<<(PS::F64)(system_grav.getNumberOfParticleGlobal())/tree_grav.getNumberOfWalkGlobal()
                 <<" nj_ave(EP-EP)= "<<(PS::F64)(n_int_ep_ep_grav)/system_grav.getNumberOfParticleGlobal()
                 <<" nj_ave(EP-SP)= "<<(PS::F64)(n_int_ep_sp_grav)/system_grav.getNumberOfParticleGlobal()<<std::endl;
        DumpTimeProfile2(tp_grav, tp_grav_max, rank_grav_max, std::cout);
        ...
        if(PS::Comm::getRank() == 0){
            fout_eng<<time_sys<<"   "<<(Etot1-Etot0)/Etot0<<std::endl;
            std::cout<<time_sys<<"   "<<(Etot1-Etot0)/Etot0<<std::endl;
        }
