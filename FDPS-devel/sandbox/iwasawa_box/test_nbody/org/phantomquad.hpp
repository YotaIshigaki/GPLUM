...
/*
        if(ni > NIMAX){
            ...
            std::cout<<"NIMAX="<<NIMAX<<std::endl;
        }
        if(nj > NJMAX){
            std::cout<<"nj="<<nj<<std::endl;
            std::cout<<"NJMAX="<<NJMAX<<std::endl;
        }
*/
		assert(ni <= NIMAX);
		assert(nj <= NJMAX);
...
private:
			...
			v2r8 &pot)
	{
		...
		pot -= madd05 * ri1;
#if 0
		ax += madd25*dx -  ri5*qr_x;
		ay += madd25*dy -  ri5*qr_y;
		az += madd25*dz -  ri5*qr_z;
#elif 0
		ax = (ax - ri5*qr_x) + madd25*dx;
		ay = (ay - ri5*qr_y) + madd25*dy;
		az = (az - ri5*qr_z) + madd25*dz;
#else
	   	ax = v2r8::nmsub(ri5, qr_x, ax); ax = v2r8::madd(madd25, dx, ax);
	   	ay = v2r8::nmsub(ri5, qr_y, ay); ay = v2r8::madd(madd25, dy, ay);
	   	az = v2r8::nmsub(ri5, qr_z, az); az = v2r8::madd(madd25, dz, az);
#endif
	}
	__attribute__ ((noinline))
