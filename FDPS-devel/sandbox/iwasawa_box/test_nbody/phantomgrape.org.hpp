...
public:
    ...
    Vec4(const T &_z, const T &_y, const T &_x, const T &_w){
        assert(sizeof(Vec4) == sizeof(T)*4);
        z = _z;
        y = _y;
        x = _x;
    ...
    void set(const T _z, const T _y, const T _x, const T _w){
        z = _z;
        y = _y;
        x = _x;
        w = _w;
    ...
    friend std::ostream& operator <<(std::ostream& _c, const Vec4& v4){
        _c<<std::setprecision(15)<<v4.x<<"   "<<v4.y<<"    "<<v4.z<<"   "<<v4.w;
        return _c;
    }
};
...
////////
/*
inline void vseparation_squared(const v4r4 lc,
                                const v4r4 mx[],
                                double *r2){
    const V2R8 ZERO = V2R8_setzero();

    const V2R8 ix = V2R8_set((double)lc.x, (double)lc.x);
    const V2R8 iy = V2R8_set((double)lc.y, (double)lc.y);
    const V2R8 iz = V2R8_set((double)lc.z, (double)lc.z);
    const V2R8 len = V2R8_set((double)lc.w, (double)lc.w);

    for(int in=0; in<4; in++){
        const int i0 = in*2;
        const int i1 = in*2+1;
        const V2R8 jx = V2R8_set((double)mx[i0].x, (double)mx[i1].x);
        V2R8 dx = V2R8_abs(V2R8_sub(ix, jx));
        dx = V2R8_sub(dx, len);
        dx = V2R8_and(dx, V2R8_cmplt(ZERO, dx));

        const V2R8 jy = V2R8_set((double)mx[i0].y, (double)mx[i1].y);
        V2R8 dy = V2R8_abs(V2R8_sub(iy, jy));
        dy = V2R8_sub(dy, len);
        dy = V2R8_and(dy, V2R8_cmplt(ZERO, dy));

        const V2R8 jz = V2R8_set((double)mx[i0].z, (double)mx[i1].z);
        V2R8 dz = V2R8_abs(V2R8_sub(iz, jz));
        dz = V2R8_sub(dz, len);
        dz = V2R8_and(dz, V2R8_cmplt(ZERO, dz));

        V2R8 rsq = V2R8_madd(dz, dz, V2R8_madd(dy, dy, V2R8_mul(dx, dx)));

        V2R8_storel(r2+i0, rsq);
        V2R8_storeh(r2+i1, rsq);
    }

}
*/
/*
inline void vseparation_squared(const fvec3 cen,
                                const fvec3 len,
                                const v4r4 mx[],
                                double *r2){
    const V2R8 ZERO = V2R8_setzero();

    const V2R8 ix = V2R8_set((double)cen.x, (double)cen.x);
    const V2R8 iy = V2R8_set((double)cen.y, (double)cen.y);
    const V2R8 iz = V2R8_set((double)cen.z, (double)cen.z);
    const V2R8 lx = V2R8_set((double)len.x, (double)len.x);
    const V2R8 ly = V2R8_set((double)len.y, (double)len.y);
    const V2R8 lz = V2R8_set((double)len.z, (double)len.z);
