...
public:
  ...
  void input_params(char *filename) {
    ...
    if(ret == EOF) fprintf(stderr, 
			   "Input error of a model name in input_params()\n");
  ...
  ret = fscanf(param_fp,"%d",&noutput);
    if(ret == EOF) fprintf(stderr, 
			   "Input error of noutout in input_params()\n");
  ...
  void read_header(FILE *fp) {
    ...
    void writeBinary(FILE *fp) const {
        fwrite(&npart_total, sizeof(npart_total), 1, fp);
        fwrite(&npart_local, sizeof(npart_local), 1, fp);
    }
    
    PS::S64 readBinary(FILE *fp) {
        fread(&npart_total, sizeof(npart_total), 1, fp);
        fread(&npart_local, sizeof(npart_local), 1, fp);
        return npart_local;
    }
    
    void writeAscii(FILE* fp) const{
        fprintf(fp, "%lld\t%lld\t%e\t%e\t%e\n", npart_total, npart_local, tnow, anow, znow);
    }
