...
/*
static inline void cpe_bcast_f64(const int root_cpe_id,
                                 double * data) {
...
void RotateAndShiftZ(void * args){
    ...
    void *adr_z_shift = (void *)((unsigned long*)args)[4];
    ...
    for (i=0; i<n_loc; i+= CHUNK_SIZE) {
        ...
        //* Compute
        for(k=0; k<nn; k++){
            if( fabs(fp[k].pos.z) > fabs(z_max_loc) ){ z_max_loc = fp[k].pos.z; }
    ...
    if(my_id == 0){
        ...
        dma_set_mode(&dma_put, PE_MODE);
        dma_set_reply(&dma_put, &reply_put);
        dma_set_size(&dma_put, sizeof(F64_));
        dma(dma_put, (long)((F64_ *)adr_z_shift), (long)(&z_max));
        dma_wait(&reply_put, 1);
        while (reply_put != 1) {}
    }
    */
    ...
    dma_set_reply(&dma_put, &reply_put);
    dma_set_size(&dma_put, sizeof(F64_));
    dma(dma_put, (long)((F64_ *)adr_z_shift+my_id), (long)(&z_max_loc));
    dma_wait(&reply_put, 1);
    while (reply_put != 1) {}
