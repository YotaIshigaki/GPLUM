...
namespace ParticleSimulator{
    ...
    */
    ...
    static const S64 LARGE_INT = std::numeric_limits<S32>::max()/16;
    ...
    enum SEARCH_MODE{
        ...
        LONG_SYMMETRY_ONE_STAGE, // for ring or P^3T
        SHORT_GATHER,
        SHORT_SCATTER,
        SHORT_SYMMETRY,
    };
    ...
    struct TagSearchLongSymmetryOneStage{};
    ...
    struct SEARCH_MODE_LONG{
        ...
        typedef TagExLetOneStage ex_let_stage_type;
        enum{
    ...
    struct SEARCH_MODE_GATHER{
        ...
        typedef TagExLetTwoStage ex_let_stage_type;
        enum{
    ...
    struct SEARCH_MODE_LONG_SYMMETRY_ONE_STAGE{
        typedef TagForceLong force_type;
        typedef TagSearchLongSymmetryOneStage search_type;
        typedef TagExLetOneStage ex_let_stage_type;
        enum{
            search_type_id = LONG_SYMMETRY_ONE_STAGE,
        };
    };
...
#pragma omp critical
    ...
    */
	...
	template<class T> bool operator() (const T & left, const T & right) const {
#ifdef USE_96BIT_KEY
            return (left.key_hi_ != right.key_hi_) ? left.key_hi_ < right.key_hi_ : left.key_lo_ < right.key_lo_;
#else
	    return left.key_hi_ < right.key_hi_;
#endif
	}
    };
...
*/    
    ...
    public:
        ...
        F64 add_moment_as_sp_local_tree;
        F64 add_moment_as_sp_global_tree;
        F64 make_all_interaction_list_id;
        F64 calc_force_all;
        F64 set_LET_ep_to_global_tree;
        F64 set_LET_sp_to_global_tree;
        F64 set_adr_epj_spj_for_CPE;
        F64 copy_force_original_order;
        F64 morton_sort_FP;
        
        F64 copy_tc_to_etc_loc;
        F64 copy_tc_to_etc_glb;
        F64 balance_nwalk;
        ...
        TimeProfile () {
            ...
            add_moment_as_sp_local_tree = add_moment_as_sp_global_tree = make_all_interaction_list_id = calc_force_all = set_LET_ep_to_global_tree = set_LET_sp_to_global_tree = set_adr_epj_spj_for_CPE = copy_force_original_order = morton_sort_FP = 0.0;

            copy_tc_to_etc_loc = copy_tc_to_etc_glb = balance_nwalk = 0.0;
        ...
        TimeProfile operator + (const TimeProfile & rhs) const{
            ...
            ret.add_moment_as_sp_local_tree  = this->add_moment_as_sp_local_tree  + rhs.add_moment_as_sp_local_tree;
            ret.add_moment_as_sp_global_tree = this->add_moment_as_sp_global_tree + rhs.add_moment_as_sp_global_tree;
            ret.make_all_interaction_list_id = this->make_all_interaction_list_id + rhs.make_all_interaction_list_id;
            ret.calc_force_all   = this->calc_force_all + rhs.calc_force_all;
            ret.set_LET_ep_to_global_tree = this->set_LET_ep_to_global_tree + rhs.set_LET_ep_to_global_tree;
            ret.set_LET_sp_to_global_tree = this->set_LET_sp_to_global_tree + rhs.set_LET_sp_to_global_tree;
            ret.set_adr_epj_spj_for_CPE = this->set_adr_epj_spj_for_CPE + rhs.set_adr_epj_spj_for_CPE;

            ret.copy_force_original_order = this->copy_force_original_order + rhs.copy_force_original_order;
            ret.morton_sort_FP = this->morton_sort_FP + rhs.morton_sort_FP;

            ret.copy_tc_to_etc_loc = this->copy_tc_to_etc_loc + rhs.copy_tc_to_etc_loc;
            ret.copy_tc_to_etc_glb = this->copy_tc_to_etc_glb + rhs.copy_tc_to_etc_glb;

            ret.balance_nwalk = this->balance_nwalk + rhs.balance_nwalk;
        ...
        F64 getTotalTime() const {
            return collect_sample_particle + decompose_domain + exchange_particle + set_particle_local_tree + set_particle_global_tree + make_local_tree + make_global_tree + set_root_cell
                ...
                + add_moment_as_sp_local_tree + add_moment_as_sp_global_tree + make_all_interaction_list_id + calc_force_all
                + set_LET_ep_to_global_tree + set_LET_sp_to_global_tree + set_adr_epj_spj_for_CPE + copy_force_original_order + morton_sort_FP
...
namespace ParticleSimulator{
    ...
    F64 GetDistanceMinSqPeriodicX(const F64ort & pos0,
                                  const F64ort & pos1,
        ...
        dx = (dx < (peri_len_x-dx)) ? dx : (peri_len_x-dx);
        dx = (len.x < dx) ? dx-len.x : 0.0;
        const F64 dy = (len.y < fabs(cen0.y-cen1.y)) ? fabs(cen0.y-cen1.y)-len.y : 0.0;
        const F64 dz = (len.z < fabs(cen0.z-cen1.z)) ? fabs(cen0.z-cen1.z)-len.z : 0.0;
        return dx*dx + dy*dy + dz*dz;
    }
    ...
    F64 GetDistanceMinSqPeriodicX(const F64ort & pos0,
        ...
        dx = (dx < (peri_len_x-dx)) ? dx : (peri_len_x-dx);
        dx = (len.x < dx) ? dx-len.x : 0.0;
        const F64 dy = (len.y < fabs(cen0.y-pos1.y)) ? fabs(cen0.y-pos1.y)-len.y : 0.0;
        const F64 dz = (len.z < fabs(cen0.z-pos1.z)) ? fabs(cen0.z-pos1.z)-len.z : 0.0;
        return dx*dx + dy*dy + dz*dz;
    }

    template<class Tp>
    inline F64ort GetMinBoxSingleThreadForTree(const Tp ptcl[],
                                               const S32 n){
        static double MY_PI_2 = 2.0*4.0*atan(1.0);
        double x = ptcl[0].pos.x;
        double y = ptcl[0].pos.y;
        double z = ptcl[0].pos.z;
        double phi = atan2(y, x);
        if(phi < 0.0) phi += MY_PI_2;
        double r = sqrt(x*x + y*y);
        F64vec pos_cyl = F64vec(phi, r, z);
        F64ort pos_box(pos_cyl, pos_cyl);
        for(S32 i=1; i<n; i++){
            /*
            std::cerr<<"i= "<<i
                     <<" ptcl[i].pos.x= "<<ptcl[i].pos.x
            ...
            */
            ...
            std::cerr<<"phi= "<<phi
                     ...
                     <<std::endl;
            */
            pos_cyl = F64vec(phi, r, z);
            pos_box.merge(pos_cyl);
        }
        return pos_box;
    }
