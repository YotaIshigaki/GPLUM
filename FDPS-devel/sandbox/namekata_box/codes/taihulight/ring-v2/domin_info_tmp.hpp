...
#ifdef STATIC_DD
        void setDomain(const PS::F64ort _pos_domain){
            S32 np = Comm::getNumberOfProc();
            for(S32 i=0; i<np; i++){
                pos_domain_[i] = _pos_domain[i];
            }
        }
#endif
    };
