...
namespace ParticleSimulator{
    ...
    public:
        ...
        static KeyT getKey(const F64vec & pos){
            ...
            KeyT ret;
#ifdef USE_96BIT_KEY
            U64 nx_up = nx>>kLevMaxLo;
            U64 ny_up = ny>>kLevMaxLo;
            U64 nz_up = nz>>kLevMaxLo;
            ret.up_ = (getInstance().separateBit(nx_up)<<2
                       | getInstance().separateBit(ny_up)<<1
                       | getInstance().separateBit(nz_up));
            ...
            std::cerr<<std::oct<<"nx="<<nx<<" nx_up= "<<nx_up<<" nx_lo= "<<nx_lo<<std::endl;
            */
#else
            ret.up_ = (getInstance().separateBit(nx)<<2 | getInstance().separateBit(ny)<<1 | getInstance().separateBit(nz));
#endif
...
#ifdef USE_96BIT_KEY
            if(lev <= kLevMaxUp){
                s = mkey.up_ >> ( (kLevMaxUp - lev) * 3 );
            }
            else{
                s = mkey.lo_ >> ( (kLevMaxLo - (lev-kLevMaxUp)) * 3 );
            }
#else
            s = mkey.up_ >> ( (kLevMax - lev) * 3 );
#endif
            return (s & 0x7);
        }
    };
