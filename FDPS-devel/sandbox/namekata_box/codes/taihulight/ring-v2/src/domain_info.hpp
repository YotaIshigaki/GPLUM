	...
	// new version multi-dimensional gathering
        ...
        void initializeSuperDomain(){
            ...
            MPI_Allgather(pos_domain_+my_rank, 1, GetDataType<F64ort>(),
                          pos_sub_domain_.getPointer(), 1, GetDataType<F64ort>(), comm_sub_[0]); // in r-direction
            F64ort pos_super_domain_tmp;
            for(S32 i=0; i<n_proc_sub_[0]; i++){
                pos_super_domain_tmp.merge(pos_sub_domain_[i]);  // merge on all processes
            }
            if(my_rank_sub == 0){
            ...
            /*
            if(my_rank == 1){
                std::cerr<<"BEFORE Bcast"<<std::endl;
                std::cerr<<"pos_super_domain_[0]= "<<pos_super_domain_[0]<<std::endl;
                std::cerr<<"pos_super_domain_[1]= "<<pos_super_domain_[1]<<std::endl;
                std::cerr<<"pos_super_domain_[2]= "<<pos_super_domain_[2]<<std::endl;
            }
            */
            MPI_Bcast(pos_super_domain_.getPointer(), n_domain_[0], GetDataType<F64ort>(), 0, comm_sub_[0]);
            /*
            if(my_rank == 1){
                std::cerr<<"AFTER Bcast"<<std::endl;
                std::cerr<<"pos_super_domain_[0]= "<<pos_super_domain_[0]<<std::endl;
                std::cerr<<"pos_super_domain_[1]= "<<pos_super_domain_[1]<<std::endl;
                std::cerr<<"pos_super_domain_[2]= "<<pos_super_domain_[2]<<std::endl;
            }
        ...
        F64ort getPosSuperDomain(const S32 id=0) const {
            return pos_super_domain_[id];
        }
        void setPosDomainCyl(){
            F64 len_x = 2.0 * MY_PI;
            S32 nx = n_domain_[0];
            S32 ny = n_domain_[1];
            S32 n_proc = nx*ny;
            F64 dx = len_x / nx;
            F64 dy = delta_ax_ / ny;
            F64 dy_offset = 1.0-delta_ax_*0.5;
            for(S32 i=0; i<n_proc; i++){
                S32 rank_x = i / ny;
                S32 rank_y = i % ny;
                pos_domain_[i].low_.x = dx*rank_x;
                pos_domain_[i].low_.y = dy*rank_y + dy_offset;
                pos_domain_[i].low_.z = pos_root_domain_.low_.z;
                pos_domain_[i].high_.x = dx*(rank_x+1);
                pos_domain_[i].high_.y = dy*(rank_y+1) + dy_offset;
                pos_domain_[i].high_.z = pos_root_domain_.high_.z;
                if(rank_x == 0)    pos_domain_[i].low_.x  = pos_root_domain_.low_.x;
                if(rank_x == nx-1) pos_domain_[i].high_.x = pos_root_domain_.high_.x;
                if(rank_y == 0)    pos_domain_[i].low_.y  = pos_root_domain_.low_.y;
                if(rank_y == ny-1) pos_domain_[i].high_.y = pos_root_domain_.high_.y;
            }
        }
        void setPosSuperDomainCyl(const F64 delta_ax){
            F64 len_x = 2.0 * MY_PI;
            S32 nx = n_domain_[0];
            F64 dx = len_x / nx;
            pos_super_domain_.resizeNoInitialize(nx);
            for(S32 i=0; i<nx; i++){
                pos_super_domain_[i].low_.x  = dx*i;
                pos_super_domain_[i].high_.x = dx*(i+1);
                if(i == 0)    pos_super_domain_[i].low_.x  = pos_root_domain_.low_.x;
                if(i == nx-1) pos_super_domain_[i].high_.x = pos_root_domain_.high_.x;
                pos_super_domain_[i].low_.z  = pos_root_domain_.low_.z;
                pos_super_domain_[i].high_.z = pos_root_domain_.high_.z;
                pos_super_domain_[i].low_.y  = pos_root_domain_.low_.y;
                pos_super_domain_[i].high_.y = pos_root_domain_.high_.y;
            }
        }
    };
}
