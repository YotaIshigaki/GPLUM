...
#else
	for(int k=0; k<NC; k++) for(int j=0; j<NC; j++) for(int i=0; i<NC; i++) mmcell.set_mm(cell[k][j][i].mm,i,j,k);
	mmcell.convolution_PBC();
	for(int k=0; k<NC; k++) for(int j=0; j<NC; j++) for(int i=0; i<NC; i++) cell[k][j][i].le = mmcell.get_le(i,j,k);
#endif
