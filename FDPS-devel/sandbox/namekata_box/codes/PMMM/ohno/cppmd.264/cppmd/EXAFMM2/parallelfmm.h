...
public:
  ...
  void M2LSend() {
    ...
    int iforward = 0;
    int ix[3];
    float *commBytes = new float[maxLevel+1];
    for( int lev=0; lev<=maxLevel; lev++ ) commBytes[lev] = 0;
    for( ix[2]=-1; ix[2]<=1; ix[2]++ ) {
      for( ix[1]=-1; ix[1]<=1; ix[1]++ ) {
        for( ix[0]=-1; ix[0]<=1; ix[0]++ ) {
          if( ix[0] != 0 || ix[1] != 0 || ix[2] != 0 ) {
            assert( i == multipoleDispl[iforward] );
            ...
            for_3d ixp[d] = (ixc[d] + ix[d] + nunit[d]) % nunit[d];
            int recvRank = getGlobKey(ixp,maxGlobLevel);
            int sendDispl = multipoleDispl[iforward];
            int sendCount = multipoleCount[iforward];
            MPI_Isend(sendMultipole[sendDispl],sendCount*MTERM,MPI_FLOAT,
                      sendRank,iforward,MPI_COMM_WORLD,&requests[iforward]);
            int recvDispl = multipoleDispl[iforward];
            int recvCount = multipoleCount[iforward];
    ...
    for( int lev=maxLevel; lev>0; lev-- ) {
      ...
      fid << "level : " << maxGlobLevel+lev << " M2L comm (theoretical) : " << std::setw(8) << theoBytes << ",   (actual) : " << std::setw(8) << commBytes[lev] << " Bytes" << std::endl;
    }
#endif
    delete [] commBytes;
