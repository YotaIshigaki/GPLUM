...
namespace ParticleSimulator{
    ...
    getParticleMeshCellInfo(const S32vec idx, PMM::CellInfo<Tforce, Tepi, Tepj, Tspj> & info) const {
        auto itr = info_pm_cell_.find(idx);
        if ( itr != info_pm_cell_.end()) {
            info = info_pm_cell_.at(idx);
            return 0;
        } else {
            return -1;
        }
    ...
    decomposeParticleMeshCellImpl2(const DomainInfo & dinfo) {
        // Local classes
        class CellIdxPtclNumPair {
        public:
            S32vec idx;
            S32 n_ptcl;
        };
        class RankNumPtclNumPair {
        public:
            S32 rank_num;
            S32 n_ptcl;
        };
        ...
        if (first_call) {
            list_pm_cell_ = new std::vector<S32vec>[n_proc];
            first_call = false;
        } else {
            for (S32 i=0; i<n_proc; i++) list_pm_cell_[i].clear();
        }

        // Make a list of pairs of cell index and number of EPJ 
        ReallocatableArray<CellIdxPtclNumPair> * list_idx_nptcl;
        list_idx_nptcl = new ReallocatableArray<CellIdxPtclNumPair>[n_proc];
        for (const auto & e: info_pm_cell_) {
            CellIdxPtclNumPair tmp;
            tmp.idx = e.first;
            tmp.n_ptcl = e.second.n_epi;
            list_idx_nptcl[my_rank].push_back(tmp);
        }
        S32 n_cell_used[n_proc];
        S32 n_cell_used_loc = list_idx_nptcl[my_rank].size();
        Comm::allGather(&n_cell_used_loc, 1, n_cell_used);
        S32 n_cell_used_tot = 0;
        for (S32 i=0; i<n_proc; i++)
            n_cell_used_tot += n_cell_used[i];
        ReallocatableArray<CellIdxPtclNumPair> list_recv;
        list_recv.resizeNoInitialize(n_cell_used_tot);
        S32 n_list_disp[n_proc];
        n_list_disp[0] = 0;
        for (S32 i=0; i<n_proc-1; i++)
            n_list_disp[i+1] = n_list_disp[i] + n_cell_used[i];
        Comm::allGatherV(list_idx_nptcl[my_rank].getPointer(), n_cell_used_loc,
                         list_recv.getPointer(), n_cell_used, n_list_disp);
        S32 offset = 0;
        for (S32 i_rank=0; i_rank<n_proc; i_rank++) {
            if (i_rank != my_rank) {
                for (S32 i=0; i<n_cell_used[i_rank]; i++) 
                    list_idx_nptcl[i_rank].push_back(list_recv[offset+i]);
            }
            offset += n_cell_used[i_rank];
        }
        // Free memory of unused variables
        list_recv.freeMem(1);

        // Make 
        ReallocatableArray<RankNumPtclNumPair> list_rank_nptcl[n_cell_.z][n_cell_.y][n_cell_.x];
        for (S32 i_rank=0; i_rank<n_proc; i_rank++) {
            for (S32 i=0; i<n_cell_used[i_rank]; i++) {
                const S32vec idx = list_idx_nptcl[i_rank][i].idx;
                const S32 n_ptcl = list_idx_nptcl[i_rank][i].n_ptcl;
                RankNumPtclNumPair tmp;
                tmp.rank_num = i_rank;
                tmp.n_ptcl   = n_ptcl;
                list_rank_nptcl[idx.z][idx.y][idx.x].push_back(tmp);
            }
        }
        // Free memory of unused variables
        for (S32 i=0; i<n_proc; i++) list_idx_nptcl[i].freeMem(1);
        delete [] list_idx_nptcl;

        // Calculate a list of PM cell for which this process is responsible.
        ...
        list_pm_cell_shared_.clear();
        for (S32 k=0; k<n_cell_.z; k++) {
            for (S32 j=0; j<n_cell_.y; j++) {
                for (S32 i=0; i<n_cell_.x; i++) {
                    const S32 num_rank = list_rank_nptcl[k][j][i].size();
                    const S32vec idx = S32vec(i,j,k);
                    for (S32 n=0; n < num_rank; n++) {
                        const S32 rank_num = list_rank_nptcl[k][j][i][n].rank_num; 
                        const S32 n_ptcl   = list_rank_nptcl[k][j][i][n].n_ptcl;
                        list_pm_cell_[rank_num].push_back(idx);
                    }
                    if (num_rank > 1) {
                        list_pm_cell_shared_.push_back(idx);
                    }
                }
            }
        }
        ...
        for (S32 k=0; k<n_cell_.z; k++) {
            for (S32 j=0; j<n_cell_.y; j++) {
                for (S32 i=0; i<n_cell_.x; i++) {
                    S32 rank_max   = -1;
                    S32 n_ptcl_max = 0;
                    for (S32 n=0; n < list_rank_nptcl[k][j][i].size(); n++) {
                        const S32 rank_num = list_rank_nptcl[k][j][i][n].rank_num; 
                        const S32 n_ptcl   = list_rank_nptcl[k][j][i][n].n_ptcl;
                        if (n_ptcl > n_ptcl_max) {
                            n_ptcl_max = n_ptcl;
                            rank_max   = rank_num;
                        }
                    }
                    if (rank_max != -1) {
                        const S32vec idx = S32vec(i,j,k);
                        list_pm_cell_[rank_max].push_back(idx);
                    }
                }
            }
        }
        ...
        for (S32 i=0; i < list_pm_cell_[my_rank].size(); i++) {
            const S32vec idx = list_pm_cell_[my_rank][i];
            output_file << idx.x << " " << idx.y << " " << idx.z << std::endl;
        }
        output_file.close();
#if 0
...
#pragma omp parallel for
    ...
    setRootCell(const DomainInfo & dinfo){
        ...
        if(dinfo.getBoundaryCondition() == BOUNDARY_CONDITION_OPEN){
            ...
            calcRefPosForMortonKeyCalcOpen(typename TSM::search_type());
        }
        else{
            calcCenterAndLengthOfRootCellPeriodicImpl(typename TSM::search_type(), dinfo);
            calcRefPosForMortonKeyCalcPeriodic(typename TSM::search_type(), dinfo);
        }
        correctPosRootCellAndConfPMCell(typename TSM::search_type());
#ifdef PARTICLE_SIMULATOR_DEBUG_PRINT
            PARTICLE_SIMULATOR_PRINT_LINE_INFO();
...
#pragma omp parallel for
    ...
    makeInteractionListLongForZeroTheta(TagWithCutoffByCell, const S32 adr_ipg){
        const S32 ith = Comm::getThreadNum();
        const F64ort pos_target_box = PMM::GetMinBoxRoundedUpToParticleMeshCell((ipg_[adr_ipg]).vertex_in_,
                                                                                pos_ref_keycalc_.low_,
    ...
    void CalcNumShift(const F64ort root_domain,
...
#pragma omp critical
    ...
    template<class Tep2>
    void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcCenterAndLengthOfRootCellPeriodicImpl3(const Tep2 ep[],
                                               const DomainInfo & dinfo){
        //F64 rsearch_max_loc = -LARGE_FLOAT;
        F64ort box_loc;
        box_loc.initNegativeVolume();
...
#pragma omp critical
    ...
    calcRefPosForMortonKeyCalcOpenImpl(){
        pos_ref_keycalc_ = pos_root_cell_;
    }

    template<class TSM, class Tforce, class Tepi, class Tepj,
             class Tmomloc, class Tmomglb, class Tspj>
    void TreeForForce<TSM, Tforce, Tepi, Tepj, Tmomloc, Tmomglb, Tspj>::
    calcRefPosForMortonKeyCalcPeriodicImpl(const DomainInfo & dinfo){
        pos_ref_keycalc_ = dinfo.getPosRootDomain();
    }
    ...
    correctPosRootCellAndConfPMCellImpl(){
        // Compute cell widths
        width_pm_cell_ = PMM::GetWidthOfParticleMeshCell(pos_ref_keycalc_, n_cell_);

        // Compute tree level 
        S32vec n_cell_tot_reqmin, level;
        ...
        level_pm_cell_ = std::max(level.x, std::max(level.y, level.z));
        lev_leaf_limit_ = lev_group_limit_ = level_pm_cell_;

        S32vec n_cell_tot;
        n_cell_tot.x = n_cell_tot.y = n_cell_tot.z = std::pow(2,level_pm_cell_);

        // Compute idx_ref_keycalc_, pos_root_cell_
        idx_ref_keycalc_ = (n_cell_tot - n_cell_tot_reqmin)/2 + icut_;
        assert(idx_ref_keycalc_.x > (icut_ - 1));
        assert(idx_ref_keycalc_.y > (icut_ - 1));
        assert(idx_ref_keycalc_.z > (icut_ - 1));
        pos_root_cell_.low_.x = pos_ref_keycalc_.low_.x - width_pm_cell_.x * idx_ref_keycalc_.x;
        pos_root_cell_.low_.y = pos_ref_keycalc_.low_.y - width_pm_cell_.y * idx_ref_keycalc_.y;
        pos_root_cell_.low_.z = pos_ref_keycalc_.low_.z - width_pm_cell_.z * idx_ref_keycalc_.z;
        pos_root_cell_.high_.x = pos_ref_keycalc_.low_.x + width_pm_cell_.x * n_cell_tot.x;
        pos_root_cell_.high_.y = pos_ref_keycalc_.low_.y + width_pm_cell_.y * n_cell_tot.y;
        pos_root_cell_.high_.z = pos_ref_keycalc_.low_.z + width_pm_cell_.z * n_cell_tot.z;

#if 1
        std::cout << "pos_ref_keycalc_ = " << pos_ref_keycalc_ << std::endl;
        std::cout << "idx_ref_keycalc_ = " << idx_ref_keycalc_ << std::endl;
        ...
        if(!flag_reuse){
            FindScatterParticle<TSM, TreeCell<Tmomloc>, TreeParticle,
                ...
                (tc_loc_, tp_glb_,
                 ...
                 r_crit_sq,
                 pos_ref_keycalc_.low_, width_pm_cell_, icut_, level_pm_cell_);
...
#pragma omp for schedule(dynamic, 4)
            ...
            for(S32 i=0; i<n_ipg; i++){
                ...
                TargetBox<TSM> target_box;
                target_box.set(ipg_[i]);
                SetVertexOutOfTargetBox(target_box,
                                        pos_ref_keycalc_.low_,
                                        width_pm_cell_,
                                        icut_);
                if (typeid(TSM) != typeid(SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE)) {
