    ...
    */
    ...
    public:
        std::size_t operator() (const T & key) const {
            std::string bytes(reinterpret_cast<const char*>(&key), sizeof(T));
            return std::hash<std::string>()(bytes);
        }
    };
    template <>
    class HashFunctor<S32vec> {
    public:
        std::size_t operator() (const S32vec & key) const {
            return key.getHashCode();
        }
    };
    template <>
    class HashFunctor<F32vec> {
    public:
        std::size_t operator() (const F32vec & key) const {
            return key.getHashCode();
        }
    };
    template <>
    class HashFunctor<F64vec> {
    public:
        std::size_t operator() (const F64vec & key) const {
            return key.getHashCode();
        }
    };
