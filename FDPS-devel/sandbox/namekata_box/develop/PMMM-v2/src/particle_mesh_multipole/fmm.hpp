...
namespace ParticleSimulator {
    ...
    namespace ParticleMeshMultipole {
        ...
        public:
            ...
            void freeMem() {
                // This function explicitly free memory.
                std::vector<real_t>().swap(buf);
            }
        ...
        struct Rlm : public lmbuf<real_t, cplx_t>
        {
            ...
            void eval_opt(const real_t x, const real_t y, const real_t z){
                static TableManager<TableForRlm<real_t> > tbl_Rlm;
...
#pragma omp critical
                ...
                TableForRlm<real_t> tbl = tbl_Rlm.get(this->p);
                ...
                for(int m=0; m<=this->p; m++){
                    ...
                    for(int l=m; l<=this->p; ){
                        plm = p0;
                        const cplx_t val = (tbl.tbl_factinv[l+m] * plm) * um;
                        this->put_at(l, m, val);
                        break;
                    }
                    ...
                    for(int l=m+2; l<=this->p; l++ ){
                        plm = (tbl.tbl_inv[l-m]) * (c1 * zz * p1 - c0 * r2 * p0);
                        p0 = p1;
                        p1 = plm;
                        c0 += 1.0;
                        c1 += 2.0;
        ...
        struct Slm : public lmbuf<real_t, cplx_t>
        {
            ...
            __attribute__((noinline))
            void eval_opt(const real_t x, const real_t y, const real_t z){
                static TableManager<TableForSlm<real_t> > tbl_Slm;
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp critical
#endif
                {
                ...
                TableForSlm<real_t> tbl = tbl_Slm.get(this->p);
