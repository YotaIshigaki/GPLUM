namespace ParticleSimulator{
    ...
    inline void FindScatterParticle(const ReallocatableArray<Ttc> & tc_first,
        ...
        if (typeid(TSM) != typeid(SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE)) {
            ...
            F64ort vertex_in = GetOuterBoundaryOfMyTree(typename TSM::search_type(), tc_first);
            outer_boundary_of_my_tree = PMM::GetMinBoxRoundedUpToParticleMeshCell(vertex_in, pos_ref, cell_width, icut);
        }
        S32 adr_tc = 0;
...
#pragma omp for schedule(dynamic, 4)
            ...
            for(S32 i=0; i<n_proc; i++){
                ...
                if (typeid(TSM) != typeid(SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE)) {
                    ...
                    CalcNumberAndShiftOfImageDomain
                        (shift_per_image_tmp[ith],  dinfo.getPosRootDomain().getFullLength(),
                         dinfo.getPosDomain(my_rank), outer_boundary_of_tree[i], pa, false);
                }
                ...
                for(S32 j=n_image_tmp_prev; j<n_image_tmp; j++){
                    ...
                    SetVertexOutOfTargetBox(target_box, pos_ref, cell_width, icut);
