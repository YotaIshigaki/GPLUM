...
//
//  This program is a port of ewald.h in the original PMMM code,
//  where the computational box is assumed to be a cube whose
//  length of size is 1.
//
...
namespace ParticleSimulator {
    namespace Ewald {
        ...
        public:
            ...
            void initialize(const S32 MMAX, const F64vec & pos) {
                MMAX_ = MMAX;
                ...
                const cplx_t ex = expi(twopi * pos.x);
                const cplx_t ey = expi(twopi * pos.y);
                const cplx_t ez = expi(twopi * pos.z);
        ...
        public:
            ...
            void reduce() {
                ...
                real_t * sendbuf = new real_t[count];
                real_t * recvbuf = new real_t[count];
                // Reduce the real part
                for (S32 i=0; i<count; i++) sendbuf[i] = sum_[i].real();
                MPI_Allreduce(sendbuf, recvbuf, count,
                              GetDataType<real_t>(), MPI_SUM, MPI_COMM_WORLD);
                for (S32 i=0; i<count; i++) sum_[i].real(recvbuf[i]);
                // Reduce the imaginary part
                for (S32 i=0; i<count; i++) sendbuf[i] = sum_[i].imag();
                MPI_Allreduce(sendbuf, recvbuf, count,
                              GetDataType<real_t>(), MPI_SUM, MPI_COMM_WORLD);
                for (S32 i=0; i<count; i++) sum_[i].imag(recvbuf[i]);
                // Free
                delete [] sendbuf;
                delete [] recvbuf;
#endif
            ...
            void filter(const F64 alpha){
                ...
                const F64 coef = (pi/alpha) * (pi/alpha);
                for(S32 iz=0; iz<NZ_; iz++){
                    for(S32 iy=0; iy<NY_; iy++){
                        for(S32 ix=0; ix<NX_; ix++){
                            ...
                            const F64 m2 = F64(mx*mx + my*my + mz*mz);
                            const F64 factor = std::exp(-coef * m2) / m2;
                            const S32 idx = get_index(ix,iy,iz);
                            sum_[idx] *= factor;
                        }
                    }
                ...
                for (S32 iz=0; iz<NZ_; iz++){
                    for (S32 iy=0; iy<NY_; iy++){
                        for (S32 ix=0; ix<NX_; ix++){
                            ...
                            const S32 my = iy - MMAX_;
                            const S32 mz = iz - MMAX_;
                            const F64vec mvec = F64vec(F64(mx), F64(my), F64(mz));
