...
namespace ParticleSimulator {
    ...
    template <class T>
    static inline T tp_zero_val_org(){
        return T(0);
    }

    template <int p, class real_t=double, class cplx_t = std::complex<real_t> >
    class lmbuf0{
    public:
    	enum {
    		order  = p,
        ...
        lmbuf0(const lmbuf0 & src) {
            for (int n=0; n<length; n++) buf[n] = src.buf[n];
        }

    	void clear(){
    		for(int n=0; n<length; n++) buf[n] = tp_zero_val_org<real_t>();
    	}
    
    	// X_l^{-m} = (-1)^m conj(X_l^m)
    	cplx_t val_at(const int l, const int m) const {
#ifdef PARTICLE_SIMULATOR_RANGE_CHECK_LMBUF_ORG
    		assert(0 <= l && l <= p);
    		assert(abs(m) <= l);
#endif
    		...
    		if(m == 0) im = tp_zero_val_org<real_t>();
    		if(m < 0){
    			if(m&1) re = -re;
    			else    im = -im;
    		}
...
#ifdef PARTICLE_SIMULATOR_RANGE_CHECKR_LMBUF_ORG
    		assert(0 <= l && l <= p);
    		assert(0 <= m && m <= l);
#endif
    		/*     */ buf[l*(l+1) + m] += re;
    	...
    	void show_diff(const lmbuf0 &rhs, FILE *fp = stdout, const char *fmt = "%+e %+ei, ") const {
    		for(int l=0; l<=p; l++){
    			fprintf(fp, "l=%d: ", l);
    			for(int m=0; m<=l; m++){
#if 1
    ...
    static inline double factorial_org(const int i){
    	assert(i >= 0);
    	return i ? double(i) * factorial_org(i-1) 
    	         : 1.0;
    }
    static inline double factinv_org(const int i){
    	return 1.0 / factorial_org(i);
    }
    
    // Rlm = (r^l / (l+m)!) P_l^m (cos\theta) exp(im\phi), for m>=0
    template <int p, class real_t=double, class cplx_t = std::complex<real_t> >
    struct Rlm0 : public lmbuf0<p, real_t, cplx_t>
    {
    	static void dry_run(){
    		const real_t zero = tp_zero_val_org<real_t>();
    		Rlm0 tmp;
    	...
    	void eval_opt(const real_t x, const real_t y, const real_t z){
    		...
    		if(initcall){
    			...
    			for(int i=0; i<2*p+1; i++){
    				assert(factorial_org(i) > 0);
    				tbl_factinv[i] = 1.0 / factorial_org(i);
    			}
    		}
    ...
    template <int p, class real_t=double, class cplx_t = std::complex<real_t> >
    struct Slm0 : public lmbuf0<p, real_t, cplx_t>
    {
    	static void dry_run(){
    		const real_t zero = tp_zero_val_org<real_t>();
    		Slm0 tmp;
    	...
    	void transform_M2L(
    			const lmbuf0<psrc, real_t> &mm,
    			      lmbuf0<pdst, real_t> &le) const __restrict
    	{
    		assert(p >= psrc+pdst);
    ...
    template <int p, class real_t=double, class cplx_t = std::complex<real_t> >
    struct MultipoleMoment0 : public lmbuf0<p, real_t, cplx_t>
    {
    	MultipoleMoment0(){
    		this->clear();
    	}
    	...
    	template <int psrc>
    	void assign_from_MM(
    			const MultipoleMoment0<psrc, real_t, cplx_t> &MMsrc,
    			const real_t x,
    			const real_t y,
    			const real_t z)
    	{
    		Rlm0<psrc, real_t, cplx_t> rlm;
    ...
    template <int p, class real_t=double, class cplx_t = std::complex<real_t> >
    struct LocalExpansion0 : public lmbuf0<p, real_t, cplx_t>
    {
    	LocalExpansion0(){
    		this->clear();
    	}
    	...
    	template <int psrc>
    	void assign_from_LE(
    			const LocalExpansion0<psrc, real_t, cplx_t> &LEsrc,
    			const real_t x,
    			const real_t y,
    			const real_t z)
    	{
