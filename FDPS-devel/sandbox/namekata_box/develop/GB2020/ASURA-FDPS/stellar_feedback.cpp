...
bool StellarFeedback(PS::ParticleSystem<FP_star> & psys_star,
...
#ifdef ASURA_FDPS_ENABLE_FB_CHECK
    ...
    for (PS::S32 i = 0; i < ptcl_glb.size(); i++) {
        ...
        if (n_epj > 0) {
            ...
            for (PS::S32 j = 0; j < n_epj; j++) {
                if (epj[j].type == ParticleType::Gas && epj[j].rank == my_rank) {
    ...
    for (PS::S32 i = 0; i < psys_gas.getNumberOfParticleLocal(); i++) {
        psys_gas[i].applyTemperatureLimits();
    }
    ...
    // Update time_prof
    barrier();
    time_prof.feedback += PS::GetWtime() - t_start;
