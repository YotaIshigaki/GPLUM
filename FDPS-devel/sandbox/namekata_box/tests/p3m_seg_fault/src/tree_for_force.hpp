...
// new 
        ...
        */
        ...
        void calcForceMakingTree(Tfunc_ep_ep pfunc_ep_ep, 
            ...
            if(list_mode == MAKE_LIST || list_mode == MAKE_LIST_FOR_REUSE){
                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK1" << std::endl;
                setRootCell(dinfo);

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK2" << std::endl;
                mortonSortLocalTreeOnly();

                epi_org_.freeMem(1);

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK3" << std::endl;
                linkCellLocalTreeOnly();

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK4" << std::endl;
                calcMomentLocalTreeOnly();

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK5" << std::endl;
                exchangeLocalEssentialTreeReuseList(dinfo);

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK6" << std::endl;
                setLocalEssentialTreeToGlobalTree2(false);

                epj_send_.freeMem(1);
                spj_send_.freeMem(1);

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK7" << std::endl;
                mortonSortGlobalTreeOnly();

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK8" << std::endl;
                linkCellGlobalTreeOnly();

                epj_org_.freeMem(1);
                spj_org_.freeMem(1);

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK9" << std::endl;
                calcMomentGlobalTreeOnly();

                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK10" << std::endl;
                ...
                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK11" << std::endl;
                AddMomentAsSpImpl(typename TSM::force_type(), tc_glb_, spj_sorted_.size(), spj_sorted_);
                
                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK12" << std::endl;
                makeIPGroup();
                if (flag_debug_ == 1) {
                    Comm::barrier();
                    for (S32 irank=0; irank<Comm::getNumberOfProc(); irank++) {
                        if (irank == Comm::getRank()) {
                            std::cout << "n_ipg = " << ipg_.size()
                ...
                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK13" << std::endl;
                if(list_mode == MAKE_LIST){
                ...
                Comm::barrier(); if (flag_debug_ == 1) if (Comm::getRank() == 1) std::cout << "OK14" << std::endl;
            }
            else if(list_mode == REUSE_LIST){
                mortonSortLocalTreeOnly(true);
