...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp)
    ...
    calcForce(Tfunc_ep_ep pfunc_ep_ep,
        ...
        for(S32 i=0; i<Comm::getNumberOfThread(); i++) n_cell_open_[i] = 0;
        if(n_ipg > 0){
            //if (flag_debug_ == 0 || (flag_debug_ == 1 && Comm::getRank() == 1)) {//for debug
            //if (Comm::getRank() == 1) { //for debug
            //    if (flag_debug_ == 1) std::cout << "n_thread = "<<Comm::getNumberOfThread() << std::endl;
//#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
//#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
//#endif
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL
#pragma omp parallel for schedule(static, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
#endif
            for(S32 i=0; i<n_ipg; i++){
                const S32 ith = Comm::getThreadNum();
                //---- debug [start] ----
                flag_debug2_ = 0;
                if (flag_debug_ == 1) {
                    std::cout << "i = " << i << "/" << n_ipg << std::endl;
                    if (i == 0) flag_debug2_ = 1;
                    //if (i == 431) flag_debug2_ = 1;
                    //if (i == 503) flag_debug2_ = 1;
                }
                //---- debug [end] ----
                epj_for_force_[ith].clearSize();
    ...
    makeInteractionListImpl(TagForceLong,
        ...
        if (theta_ > 0.0){
            ...
            if (flag_debug2_ == 1) std::cout << "hi1" << std::endl;
            // theta_ = 0 case
