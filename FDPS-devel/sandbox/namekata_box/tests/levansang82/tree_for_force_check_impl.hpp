...
namespace ParticleSimulator{
    ...
    checkExchangeLocalEssentialTreeForLongImpl(TagSearchLongCutoff,
        ...
        F64 r_search_sq = epj_org_[0].getRSearch() * epj_org_[0].getRSearch(); //******// changed: F64vec r_search_sq = epj_org_[0].getRSearch();
                                                                      //******// added: F64 ellipsoid = 0.0;
        for(S32 i=0; i<n_loc; i++){
            ...
            for(S32 j=0; j<n_ep_recv; j++){
                F64vec dr = epj_org_[i].getPos() - epj_recv_[j].getPos();
                                                                          //******// added: const F64 ellipsoid = dr.x * dr.x / r_search_sq.x * r_search_sq.x +  dr.y * dr.y / r_search_sq.y * r_search_sq.y +  dr.z * dr.z / r_search_sq.z * r_search_sq.z;
                if( dr*dr < r_search_sq){                                 //******// changed: if(ellipsoid <= 1.0){
	...
	for(S32 i=0; i<n_i; i++){
		...
		const F64 r_crit_sq = epj_for_force_[ith][j].getRSearch() * epj_for_force_[ith][j].getRSearch();  //******// changed: const F64vec r_crit_sq = epj_for_force_[ith][i].getRSearch(); 	                                                                                                                                           //******// added: const F64 ellipsoid = dr.x * dr.x / r_crit_sq.x * r_crit_sq.x + dr.y * dr.y / r_crit_sq.y * r_crit_sq.y  + dr.z * dr.z / r_crit_sq.z * r_crit_sq.z;    
                if( dr*dr <= r_crit_sq) n_ngb[i]++;                 //******// changed: if( ellipsoid <= 1.0) n_ngb[i]++; 
	    }
	}
	Tepj * epj_tmp;
	S32 n_tot_tmp;
	...
	for(S32 i=0; i<n_i; i++){
		...
		const F64 r_crit_sq = epj_tmp[j].getRSearch() * epj_tmp[j].getRSearch();   //******// changed: const F64vec r_crit_sq = epj_tmp[i].getRSearch(); 
                                                                          //******// added: const F64 ellipsoid = dr.x * dr.x / r_crit_sq.x * r_crit_sq.x + dr.y * dr.y / r_crit_sq.y * r_crit_sq.y  + dr.z * dr.z / r_crit_sq.z * r_crit_sq.z; 
		if( dr*dr <= r_crit_sq) n_ngb_tmp[i]++;                   // changed: if( ellipsoid <= 1.0) n_ngb[i]++; 
		...
		for(S32 j=0; j<epj_for_force_[ith].size(); j++){
		    const F64vec dr = epi_sorted_[i+adr_ptcl].getPos() - epj_for_force_[ith][j].getPos();
		    const F64 r_crit_sq = epj_for_force_[ith][j].getRSearch() * epj_for_force_[ith][j].getRSearch();   //******// changed: const F64vec r_crit_sq = epj_for_force_[ith][j].getRSearch(); 
                                                                          //******// added: const F64 ellipsoid = dr.x * dr.x / r_crit_sq.x * r_crit_sq.x + dr.y * dr.y / r_crit_sq.y * r_crit_sq.y  + dr.z * dr.z / r_crit_sq.z * r_crit_sq.z;  
		    if( dr*dr <= r_crit_sq){                              //******// changed: if( ellipsoid <= 1.0){              
		...
		for(S32 j=0; j<n_tot_tmp; j++){
		    const F64vec dr = epi_sorted_[i+adr_ptcl].getPos() - epj_tmp[j].getPos();
		    const F64 r_crit_sq = epj_tmp[j].getRSearch() * epj_tmp[j].getRSearch();  //******// changed:  const F64vec r_crit_sq = epj_tmp[j].getRSearch(); 
                                                                          //******// added: const F64 ellipsoid = dr.x * dr.x / r_crit_sq.x * r_crit_sq.x + dr.y * dr.y / r_crit_sq.y * r_crit_sq.y  + dr.z * dr.z / r_crit_sq.z * r_crit_sq.z;  
		    if( dr*dr <= r_crit_sq){                              //******// changed: if( ellipsoid <= 1.0){  
	...
	for(S32 i=0; i<n_i; i++){
	    n_ngb[i] = 0;
	    const F64 r_crit_sq = epi_sorted_[i+adr_ptcl].getRSearch() * epi_sorted_[i+adr_ptcl].getRSearch(); //******// changed: const F64vec r_crit_sq = epi_sorted_[i+adr_ptcl].getRSearch(); 
	    for(S32 j=0; j<n_ep; j++){
		const F64vec dr = epi_sorted_[i+adr_ptcl].getPos() - epj_for_force_[ith][j].getPos();
                                                         //******// added: const F64 ellipsoid = dr.x * dr.x / r_crit_sq.x * r_crit_sq.x + dr.y * dr.y / r_crit_sq.y * r_crit_sq.y  + dr.z * dr.z / r_crit_sq.z * r_crit_sq.z;  
		if( dr*dr <= r_crit_sq) n_ngb[i]++;      //******// changed: if( ellipsoid <= 1.0) n_ngb[i]++;   
	...
	for(S32 i=0; i<n_i; i++){
		...
		const F64vec dr = epi_sorted_[i+adr_ptcl].getPos() - epj_tmp[j].getPos();
                                                           //******// added: const F64 ellipsoid = dr.x * dr.x / r_crit_sq.x * r_crit_sq.x + dr.y * dr.y / r_crit_sq.y * r_crit_sq.y  + dr.z * dr.z / r_crit_sq.z * r_crit_sq.z;                          
		if( dr*dr <= r_crit_sq) n_ngb_tmp[i]++;    //******// changed: if( ellipsoid <= 1.0) n_ngb_tmp[i]++;
	    }
	    if(n_ngb_tmp[i] != n_ngb[i]){
