...
void setParticlesColdUniformSphere(Tpsys & psys,
    ...
    for(PS::S32 i = 0; i < n_loc; i++){
        ...
        psys[i].vel  = vel[i];
        psys[i].id   = i;
#ifdef TEST_GET_EPJ_FROM_ID
        //* Check for API getEpjFromId
        if (i == 511) {
            std::cout << "id = " << psys[i].id     << std::endl;
            std::cout << "x  = " << psys[i].pos.x  << std::endl;
            std::cout << "y  = " << psys[i].pos.y  << std::endl;
            std::cout << "z  = " << psys[i].pos.z  << std::endl;
            std::cout << "-----" << std::endl;
        }
#endif
    }
...
#ifdef TEST_SORT_PTCL
bool comp(const FPGrav & left, const FPGrav & right) {
...
int main(int argc, char *argv[]) {
...
#ifdef TEST_SORT_PTCL
    //* Check for API sortParticle
    system_grav.sortParticle(comp);
    std::string filename;
    std::ofstream output_file;
    filename = "check_sort.txt";
    output_file.open(filename.c_str(),std::ios::trunc);
    for (PS::S32 i=0; i<system_grav.getNumberOfParticleLocal(); i++) {
       output_file << system_grav[i].id << " " 
                   << system_grav[i].pos.x << " "
                   << std::endl;
    }
    output_file.close();
    ...
    PS::S64 id = 511;
    FPGrav *epj;
    epj = tree_grav.getEpjFromId(id);
    std::cout << "id = " << epj->id     << std::endl;
    std::cout << "x  = " << epj->pos.x  << std::endl;
    std::cout << "y  = " << epj->pos.y  << std::endl;
    std::cout << "z  = " << epj->pos.z  << std::endl;
    PS::Finalize();
    std::exit(0);
#endif
...
#ifdef TEST_REUSE_LIST
        PS::INTERACTION_LIST_MODE list_mode;
        if (n_loop % 4 == 0) {
            dinfo.decomposeDomainAll(system_grav);
            system_grav.exchangeParticle(dinfo);
