    ...
    for(PS::S32 i=0; i<n_EPI; i++){
        ...
        //    PS::F64 g_p3m = cutfunc_p3m(r*r_scale, comp_flag );
            ...
            PS::F64 qj_r3    = g_p3m*r*r2_inv*r2_inv*EPJ[j].getCharge(); // qj/r^3
                    i_field += qj_r3*r_ij;
            //--- Error check (S) ---
            if (std::isnan(r2) || std::isnan(qj_r3) ||
                std::isnan(i_field.x) || std::isnan(i_field.y) || std::isnan(i_field.z)) {
                std::cout << "----- NaN detected @ CalcForceCoulomb_perm() -----" << std::endl;
                std::cout << " myrank    = " << PS::Comm::getRank() << std::endl;
                std::cout << " i         = " << i << std::endl;
                std::cout << " r2        = " << r2 << std::endl;
                std::cout << " qj_r3     = " << qj_r3 << std::endl;
                std::cout << " i_field.x = " << i_field.x << std::endl;
                std::cout << " i_field.y = " << i_field.y << std::endl;
                std::cout << " i_field.z = " << i_field.z << std::endl;
