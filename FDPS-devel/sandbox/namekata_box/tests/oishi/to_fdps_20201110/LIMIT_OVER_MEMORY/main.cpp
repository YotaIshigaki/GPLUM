...
void SetupIC(PS::ParticleSystem<FP>& sph_system, PS::F64 *end_time, boundary *box){
   // Place SPH particles
   std::vector<FP> ptcl;
   // const PS::F64 dx = 1.0 / 128.0; original
   const PS::F64 dx = 1.0 / 1280.0;
   box->x = 1.0;
   box->y = box->z = box->x / 8.0;
   PS::S32 i = 0;
   for(PS::F64 x = 0 ; x < box->x * 0.5 ; x += dx){ // 0 ~ 63
      for(PS::F64 y = 0 ; y < box->y ; y += dx){    // 0 ~ 15
         for(PS::F64 z = 0 ; z < box->z ; z += dx){ // 0 ~ 15
            FP ith;
            ith.pos.x = x;
            ith.pos.y = y;
            ith.pos.z = z;
   ...
   for(PS::F64 x = box->x * 0.5 ; x < box->x * 1.0 ; x += dx * 2.0){ // 0 ~ 31
