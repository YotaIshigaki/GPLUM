...
PS::F64 W(const PS::F64vec dr, const PS::F64 h){
   ...
   PS::F64 s2 = 0.5 - s;
   if (s2 < 0.0) s2 = 0.0;
   PS::F64 r_value = (s1*s1*s1) - 4.0*(s2*s2*s2);
   r_value *= 16.0/(pi*H*H*H);
   return r_value;
#endif
}
...
#else
void CalcDensity(const EP * ep_i,
                 const PS::S32 Nip,
                 const EP * ep_j,
                 const PS::S32 Njp,
                 Dens* dens){
...
void CalcHydroForce(const EP * ep_i,
                    const PS::S32 Nip,
                    const EP * ep_j,
                    const PS::S32 Njp,
                    Hydro * hydro){
...
int main(int argc, char* argv[]){
   ...
   PS::S32 step = 0;
   PS::F64 wt_sta = get_time_of_day();
   for(PS::F64 time = 0 ; time < end_time ; time += dt){
   ...
   PS::F64 wt_end = get_time_of_day();

   if (PS::Comm::getRank() == 0) {
       PS::F64 wt = wt_end - wt_sta;
       PS::F64 wt_per_step = wt/step;
       std::cout << "nstep = " << step << std::endl;
       std::cout << "wtime = " << wt << " [s]" << std::endl;
       std::cout << "wtime = " << wt_per_step << " [s/step]" << std::endl;
   }
