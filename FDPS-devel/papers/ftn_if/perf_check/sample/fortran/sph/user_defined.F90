...
module user_defined_types
   ...
   !-------------------------------------------------------------------
   pure function W_mod(dr,h)
      implicit none
      real(kind=c_double) :: W_mod
      real(kind=c_double), dimension(3), intent(in) :: dr
      real(kind=c_double), intent(in) :: h
      !* Local variables
      real(kind=c_double) :: s,s1,s2
      ...
      s2 = 0.5d0 - s
      if (s2 < 0.0d0) s2 = 0.0d0
      W_mod = (s1*s1*s1) - 4.0d0*(s2*s2*s2)
      W_mod = W_mod * 16.0d0/(pi*h*h*h)
   ...
   !-------------------------------------------------------------------
   pure function gradW_mod(dr,h)
      implicit none
      real(kind=c_double), dimension(3) :: gradW_mod
      real(kind=c_double), dimension(3), intent(in) :: dr
      real(kind=c_double), intent(in) :: h
      !* Local variables
      real(kind=c_double) :: dr_abs,s,s1,s2,coef

      dr_abs = dsqrt(dr(1)*dr(1) &
                    +dr(2)*dr(2) &
                    +dr(3)*dr(3))
      s = dr_abs/h
      s1 = 1.0d0 - s
      ...
      coef = coef * 16.0d0/(pi*h*h*h)
      coef = coef / (dr_abs*h + 1.0d-6*h)
      gradW_mod(1) = dr(1) * coef
      gradW_mod(2) = dr(2) * coef
      gradW_mod(3) = dr(3) * coef

   end function gradW_mod
   ...
   subroutine calc_density(ep_i,n_ip,ep_j,n_jp,f) bind(c)
      ...
      !* [New version]
      real(kind=c_double) :: hi
      real(kind=c_double), dimension(3) :: xi,dr
      real(kind=c_double), dimension(n_jp) :: mj
      real(kind=c_double), dimension(3,n_jp) :: xj
#else
      !* [Original version]
      type(fdps_f64vec) :: dr
#endif
   ...
   subroutine calc_hydro_force(ep_i,n_ip,ep_j,n_jp,f) bind(c)
     ...
     !* [New version]
      ...
      real(kind=c_double) :: mi,smth_i,dens_i,pres_i,snds_i
      real(kind=c_double), dimension(3) :: xi,vi,dr,dv,gradW_ij
      real(kind=c_double), dimension(n_jp) :: mj,dens_j,pres_j,smth_j,snds_j
      real(kind=c_double), dimension(3,n_jp) :: xj,vj
#else
     !* [Original version]
      ...
      do j=1,n_jp
         ...
         vj(2,j) = ep_j(j)%vel%y
         vj(3,j) = ep_j(j)%vel%z
         dens_j(j) = ep_j(j)%dens
         pres_j(j) = ep_j(j)%pres
         smth_j(j) = ep_j(j)%smth
         snds_j(j) = ep_j(j)%snds
      end do
      do i=1,n_ip
         !* Zero-clear
         v_sig_max = 0.0d0
         ...
         do j=1,n_jp
            ...
            povrho2_j = pres_j(j)/(dens_j(j)*dens_j(j))
            !* Compute dr & dv
            dr(1) = xi(1) - xj(1,j)
            dr(2) = xi(2) - xj(2,j)
            dr(3) = xi(3) - xj(3,j)
            ...
            dr_dv = dr(1) * dv(1) + dr(2) * dv(2) + dr(3) * dv(3)
            if (dr_dv < 0.0d0) then
               w_ij = dr_dv / sqrt(dr(1) * dr(1) + dr(2) * dr(2) + dr(3) * dr(3))
            else
               w_ij = 0.0d0
            end if
            v_sig = snds_i + snds_j(j) - 3.0d0 * w_ij
            v_sig_max = max(v_sig_max, v_sig)
            !* Compute the artificial viscosity
            AV = - 0.5d0*v_sig*w_ij / (0.5d0*(dens_i+dens_j(j)))
            !* Compute the average of the gradients of kernel
            gradW_ij = 0.5d0 * (gradW_mod(dr,smth_i) + gradW_mod(dr,smth_j(j)))
            !* Compute the acceleration and the heating rate
            f(i)%acc%x = f(i)%acc%x - mj(j)*(povrho2_i+povrho2_j+AV)*gradW_ij(1)
            f(i)%acc%y = f(i)%acc%y - mj(j)*(povrho2_i+povrho2_j+AV)*gradW_ij(2)
            f(i)%acc%z = f(i)%acc%z - mj(j)*(povrho2_i+povrho2_j+AV)*gradW_ij(3)
            f(i)%eng_dot = f(i)%eng_dot &
                         + mj(j) * (povrho2_i + 0.5d0*AV) &
                          *(dv(1) * gradW_ij(1) &
                           +dv(2) * gradW_ij(2) &
                           +dv(3) * gradW_ij(3))
         end do
