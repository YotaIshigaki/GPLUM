...
#PJM --stgin "./src.tar.gz ./"
#PJM --stgin "./scripts/* ./scripts/"
#PJM --stgin "./user_defined.F90 ./"
#PJM --stgin "./f_main.F90 ./"
#PJM --stgin "./Makefile.K.native.computing ./Makefile"
#PJM --stgout "./*.s ./"
...
export LD_LIBRARY_PATH=/opt/klocal/Python-2.7/lib:${LD_LIBRARY_PATH}
# Uncompress src.tar.gz
tar -xvzf src.tar.gz
# Compile
make
...
for ((i=0; i < ${NMEAS}; i++)); do
   ...
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_10-${id}th.log 2>&1
   echo 2048 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_11-${id}th.log 2>&1
   echo 4096 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_12-${id}th.log 2>&1
   echo 8192 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_13-${id}th.log 2>&1
   echo 16384 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_14-${id}th.log 2>&1
   echo 32768 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_15-${id}th.log 2>&1
   echo 65536 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_16-${id}th.log 2>&1
   echo 131072 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_17-${id}th.log 2>&1
   echo 262144 > nptcl.input
   ./nbody.out -Wl,-T > ${DIR}/00stdout-2_18-${id}th.log 2>&1
done
# Compress
