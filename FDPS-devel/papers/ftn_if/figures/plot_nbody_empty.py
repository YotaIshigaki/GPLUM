...
basename  = 'nbody_comp_empty'
ps_file   = basename + '.ps'
eps_file  = basename + '.eps'
pdf_file  = basename + '.pdf'

# [2] Read files
Kcomp.read_file("nbody_data_empty_K.txt")
ofp.read_file("nbody_data_empty_ofp.txt")
#knl.read_file("nbody_data_empty_knl.txt")
ajisai.read_file("nbody_data_empty_ajisai.txt")
xc30.read_file("nbody_data_empty_xc30.txt")
...
mpyplot.rcParams['text.latex.preamble']=[r"\usepackage{amsmath}"]
...
panel_name = r'\textbf{(a) K Computer}' \
           + "\n" \
           + r'\hspace{1.5pc}\textbf{(empty case)}'
Kcomp.plot_panel(ax,panel_name=panel_name,\
                 with_xlabel=False,with_y2label=False)
# Plot Oakforest-PACS
ax = fig.add_subplot(gs[0,1])
panel_name = r'\textbf{(b) Oakforest-PACS}' \
           + "\n" \
           + r'\hspace{1.5pc}\textbf{(empty case)}'
...
panel_name = r'\textbf{(c) Cray XC30}' \
           + "\n" \
           + r'\hspace{1.5pc}\textbf{(empty case)}'
