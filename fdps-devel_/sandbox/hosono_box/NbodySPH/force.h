	...
	void operator () (const EPI::Drvt* ep_i, const PS::S32 Nip, const EPJ::Drvt* ep_j, const PS::S32 Njp, RESULT::Drvt* const drvt){
		for(PS::S32 i = 0; i < Nip ; ++ i){
			...
			for(PS::S32 j = 0; j < Njp ; ++ j){
				...
				drvt[i].grad_smth -= jth.mass / ith.smth * (3.0 * kernel.W(dr, ith.smth) - sqrt(dr * dr) * sqrt(kernel.gradW(dr, ith.smth) * kernel.gradW(dr, ith.smth)));
			}
			drvt[i].div_v /= ith.dens;
			drvt[i].rot_v /= ith.dens;
			drvt[i].grad_smth = 1.0 / (1.0 + ith.smth * drvt[i].grad_smth / (3.0 * ith.dens));
		}
	}
};
...
template <class TPtclJ> class CalcGravityForce{
	public:
	void operator () (const EPI::Grav* const ep_i, const PS::S32 Nip, const TPtclJ* const ep_j, const PS::S32 Njp, RESULT::Grav* const grav){
		for(PS::S32 i = 0; i < Nip ; ++ i){
			const EPI::Grav& ith = ep_i[i];
			for(PS::S32 j = 0; j < Njp ; ++ j){
				const TPtclJ& jth = ep_j[j];
				const PS::F64vec dr = ith.pos - jth.pos;
				//if(ith.id == 0) std::cout << dr << std::endl;
