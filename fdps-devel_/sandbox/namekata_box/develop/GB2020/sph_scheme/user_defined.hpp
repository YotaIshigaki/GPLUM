...
const PS::F64 specific_heat_ratio = 5.0/3.0;
...
PS::F64 dWdr(const PS::F64 r, const PS::F64 h){
    // This function gives dW(r,h)/dr, i.e.,
    // \dfrac{\partial W(r,h)}{\partial r}.
    const PS::F64 u = r/h;
    const PS::F64 cc=-48.0/(math_const::pi*h*h*h*h);
    if (u <= 0.5) {
        return cc*u*(2.0 - 3.0*u);
    }
    else if ((0.5 < u) && (u <= 1.0)) {
        const PS::F64 s = 1.0-u;
        return cc*s*s;
...
PS::F64 d2Wdrdh(const PS::F64 r, const PS::F64 h) {
    // This function gives d^{2}W(r,h)/(dr dh), i.e.,
    // \dfrac{\partial^{2} W(r,h)}{\partial r \partial h}.
    const PS::F64 u = r/h;
    const PS::F64 cc=96.0/(math_const::pi*h*h*h*h*h);
    if (u <= 0.5) {
        return cc*u*(9.0*u-5.0);
    }
    else if ((0.5 < u) && (u <= 1.0)) {
        return cc*u*(1.0-u);
    }
...
PS::F64 d2Wdh2(const PS::F64 r, const PS::F64 h) {
    // This function gives d^{2}W(r,h)/(dh dh), i.e.,
    // \dfrac{\partial^{2} W(r,h)}{\partial h^{2}}.
    const PS::F64 u = r/h;
    const PS::F64 cc=96.0/(math_const::pi*h*h*h*h*h);
    if (u <= 0.5) {
        return cc*(1.0+u*u*(21.0*u-15.0));
    }
    else if ((0.5 < u) && (u <= 1.0)) {
        return cc*(1.0-u)*(2.0+u*(7.0*u-8.0));
    }
    else {
        return 0.0;
    }
...
public:
    ...
    PS::F64    dens_dot; // time rate of change of `dens`
    PS::F64    eng; // specific internal energy
    PS::F64    ent; // entropy
    PS::F64    pres; // pressure
    PS::F64    smth; // smoothing length
    PS::F64    drhodh; // \dfrac{\partial \rho_{i}}{\partial h_{i}}
    PS::F64    drhodh_dot; // time rate of change of `drhodh`
    PS::F64    gradh; // grad-h term
    PS::F64    divv; // divergence of velocity
    PS::F64    divv_dot; // time rate of change of `divv`
    PS::F64vec rotv; // rotation of velocity
    PS::F64vec rotv_dot; // time rate of change of `rotv`
    ...
    PS::F64    eng_half;
    PS::F64    ent_half;
    // for two-staged time step model
    // (for simplified version)
    PS::F64vec pos_save;
    PS::F64vec vel_save;
    PS::F64vec acc_grav_save;
    PS::F64vec acc_hydro_save;
    PS::F64 eng_save;
    PS::F64 ent_save;
    PS::F64 eng_dot_save;
    PS::F64 ent_dot_save;
    // (for full version)
    PS::F64 dens_save;
    PS::F64 dens_dot_save;
    PS::F64 drhodh_save;
    PS::F64 drhodh_dot_save;
    PS::F64 divv_save;
    PS::F64vec rotv_save;
    PS::F64 divv_dot_save;
    PS::F64vec rotv_dot_save;
    ...
    void copyFromForce(const Force_rate& f) {
        this->dens_dot   = f.dens_dot;
        this->drhodh_dot = f.drhodh_dot;
        this->divv       = f.divv;
        this->divv_dot   = f.divv_dot;
        this->rotv       = f.rotv;
        this->rotv_dot   = f.rotv_dot;
    }
    ...
    void save() {
        pos_save = pos;
        vel_save = vel;
        acc_grav_save = acc_grav;
        acc_hydro_save = acc_hydro;
        eng_save = eng;
        ent_save = ent;
        eng_dot_save = eng_dot;
        ent_dot_save = ent_dot;
        
        dens_save = dens;
        dens_dot_save = dens_dot;
        drhodh_save = drhodh;
        drhodh_dot_save = drhodh_dot;
        divv_save = divv;
        divv_dot_save = divv_dot;
        rotv_save = rotv;
        rotv_dot_save = rotv_dot;
    }
};
    ...
    void operator () (const EP_hydro * ep_i,
        ...
        for (PS::S32 i = 0; i < n_ip; i++) {
            ...
            for (;;) {
                ...
                const PS::F64 M = 4.0 * math_const::pi * h * h * h * dens / 3.0;
                ...
                h = std::pow((3.0 * M_trgt)/(4.0 * math_const::pi * dens), 1.0/3.0);
                if ((h <= h_L) || (h == h_U)) {
            ...
            if (force[i].flag == 1) {
                ...
                force[i].drhodh = drho_dh;
                force[i].gradh = 1.0 / (1.0 + (h * drho_dh) / (3.0 * dens));
            } 
            else {
                force[i].drhodh = 1.0; // dummy value
...
public:
    void operator () (const EP_hydro * ep_i,
        ...
        for (PS::S32 i = 0; i < n_ip; i++){
            ...
            const PS::F64 smth_i = ep_i[i].smth;
            PS::F64 sum_dWdr = 0.0;
            PS::F64 sum_d2Wdrdh = 0.0;
            PS::F64 sum_dWdh = 0.0;
            PS::F64 sum_d2Wdh2 = 0.0;
            for (PS::S32 j = 0; j < n_jp; j++){
                const PS::F64 mj = ep_j[j].mass;
                const PS::F64vec dr = ep_i[i].pos - ep_j[j].pos;
                const PS::F64vec dv = ep_i[i].vel - ep_j[j].vel;
                const PS::F64vec da = ep_i[i].acc - ep_j[j].acc;
                const PS::F64 r = std::sqrt(dr * dr);
                // Terms proportional to some derivative of SPH kernel
                if (r > 0.0) {
                    const PS::F64 rinv = 1.0/r;
                    sum_dWdr    += mj * dWdr(r,smth_i) * dr * dv * rinv;
                    sum_d2Wdrdh += mj * d2Wdrdh(r,smth_i) * dr * dv * rinv;
                }
                sum_dWdh   += mj * dWdh(r,smth_i);
                sum_d2Wdh2 += mj * d2Wdh2(r,smth_i);
                // divv, rotv, diva, rota
                force[i].divv     -= mj * dv * gradW(dr, smth_i);
                force[i].rotv     -= mj * dv ^ gradW(dr, smth_i);
                force[i].divv_dot -= mj * da * gradW(dr, smth_i);
                force[i].rotv_dot -= mj * da ^ gradW(dr, smth_i);
            }
            force[i].divv /= dens_i;
            force[i].rotv /= dens_i;
            force[i].divv_dot /= dens_i;
            force[i].rotv_dot /= dens_i;
            // \dfrac{D \rho_{i}}{D t}
            force[i].dens_dot = sum_dWdr/(1.0 + smth_i * sum_dWdh/(3.0 * dens_i));
            // \dfrac{D}{Dt}\dfrac{\partial \rho_{i}}{\partial h_{i}}
            force[i].drhodh_dot = sum_d2Wdrdh - smth_i * force[i].dens_dot * sum_d2Wdh2 / (3.0 * dens_i);
        }
    }
};
