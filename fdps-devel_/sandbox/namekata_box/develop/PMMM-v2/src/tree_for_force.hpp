...
#include<particle_mesh_multipole/particle_mesh_multipole_utils.hpp>
#include<sort.hpp>
#include<tree.hpp>
...
namespace ParticleSimulator{
    ...
    public:
        ...
        S32 n_leaf_limit_;
        S32 lev_leaf_limit_; // determined later
        S32 n_group_limit_;
        S32 lev_group_limit_; // determined later
        F64 theta_;
        S32 icut_; // the minimum cell separation for multipole expansion
        S32vec n_cell_; // the number of cells used for particle mesh calculation

        //F64 length_; // length of a side of the root cell
        //F64vec center_; // new member (not used)
        F64ort pos_root_cell_;
        F64ort pos_ref_keycalc_; // the reference position used for Morton key calculation
        S32vec idx_ref_keycalc_; // the index of the PM cell whose position is pos_ref_keycalc_
        S32 level_pm_cell_; // the tree level at which a PM cell matches with a tree cell
        F64vec width_pm_cell_; // the width of a PM cell
        std::vector<S32vec> * list_pm_cell_;
        std::vector<S32vec> list_pm_cell_shared_;
        std::unordered_map< S32vec, PMM::CellInfo<Tforce, Tepi, Tepj, Tspj>, HashFunctor<S32vec> > info_pm_cell_; // information on particle buffers related to PM cell
        // [TODO] Check if std::map is better or not.
        ...
        void calcMomentLocalTreeOnlyImpl(TagSearchLongParticleMeshMultipole);
...
// probably, calcMomentGlobalTreeOnlyImpl is classified depending on force_type.
        ...
        void calcMomentGlobalTreeOnlyImpl(TagSearchLongParticleMeshMultipole);
        ...
        void makeInteractionListLongForZeroTheta(TagWithCutoffByCell, const S32 adr_ipg);
        ...
        void checkCalcMomentLocalTreeImpl(TagSearchLongParticleMeshMultipole, const F64 tolerance, std::ostream & fout);
        ...
        void checkCalcMomentGlobalTreeImpl(TagSearchLongParticleMeshMultipole, const F64 tolerance, std::ostream & fout);
        ...
        void calcCenterAndLengthOfRootCellOpenImpl(TagSearchLongParticleMeshMultipole){
            calcCenterAndLengthOfRootCellOpenNoMargenImpl(epj_org_.getPointer());
        }
        ...
        void calcCenterAndLengthOfRootCellPeriodicImpl(TagSearchLongParticleMeshMultipole,
                                                       const DomainInfo & dinfo){
            calcCenterAndLengthOfRootCellPeriodicImpl3(epj_org_.getPointer(), dinfo);
        }
        ///////////////
        // for compile 
        ...
        template<class Tep2>
        void calcCenterAndLengthOfRootCellPeriodicImpl3(const Tep2 ep[], const DomainInfo & dinfo);


        /////////////
        // for PM^3
        // --- for open boundary
        void calcRefPosForMortonKeyCalcOpenImpl();

        void calcRefPosForMortonKeyCalcOpen(TagSearchLong) {}
        void calcRefPosForMortonKeyCalcOpen(TagSearchLongCutoff) {}
        void calcRefPosForMortonKeyCalcOpen(TagSearchLongScatter) {}
        void calcRefPosForMortonKeyCalcOpen(TagSearchLongSymmetry) {}
        void calcRefPosForMortonKeyCalcOpen(TagSearchLongCutoffScatter) {}
        void calcRefPosForMortonKeyCalcOpen(TagSearchLongParticleMeshMultipole) {
            calcRefPosForMortonKeyCalcOpenImpl();
        }
        void calcRefPosForMortonKeyCalcOpen(TagSearchShortScatter) {}
        void calcRefPosForMortonKeyCalcOpen(TagSearchShortGather) {}
        void calcRefPosForMortonKeyCalcOpen(TagSearchShortSymmetry) {}
        // --- for periodic boundary
        void calcRefPosForMortonKeyCalcPeriodicImpl(const DomainInfo & dinfo);

        void calcRefPosForMortonKeyCalcPeriodic(TagSearchLong, const DomainInfo & dinfo) {}
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchLongCutoff, const DomainInfo & dinfo) {}
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchLongScatter, const DomainInfo & dinfo) {}
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchLongSymmetry, const DomainInfo & dinfo) {}
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchLongCutoffScatter, const DomainInfo & dinfo) {}
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchLongParticleMeshMultipole,
                                                const DomainInfo & dinfo) {
            calcRefPosForMortonKeyCalcPeriodicImpl(dinfo);
        }
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchShortScatter, const DomainInfo & dinfo) {}
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchShortGather, const DomainInfo & dinfo) {}
        void calcRefPosForMortonKeyCalcPeriodic(TagSearchShortSymmetry, const DomainInfo & dinfo) {}
        //
        void correctPosRootCellAndConfPMCellImpl();
        
        void correctPosRootCellAndConfPMCell(TagSearchLong) {}
        void correctPosRootCellAndConfPMCell(TagSearchLongCutoff) {}
        void correctPosRootCellAndConfPMCell(TagSearchLongScatter) {}
        void correctPosRootCellAndConfPMCell(TagSearchLongSymmetry) {}
        void correctPosRootCellAndConfPMCell(TagSearchLongCutoffScatter) {}
        void correctPosRootCellAndConfPMCell(TagSearchLongParticleMeshMultipole) {
            correctPosRootCellAndConfPMCellImpl();
        }
        void correctPosRootCellAndConfPMCell(TagSearchShortScatter) {}
        void correctPosRootCellAndConfPMCell(TagSearchShortGather) {}
        void correctPosRootCellAndConfPMCell(TagSearchShortSymmetry) {}
        ...
        void checkMakeInteractionListImpl(TagSearchLongCutoff,
                                          ...
                                          const F64 tolerance,
                                          std::ostream & fout);
        void checkMakeInteractionListImpl(TagSearchLongParticleMeshMultipole,
                                          const DomainInfo & dinfo,
                                          const S32 adr_ipg,
                                          const S32 ith,
                                          const F64 tolerance,
...
// new 
        ...
        void initialize(const U64 n_glb_tot,
                        const S32vec n_cell,
                        const S32 icut=FDPS_DFLT_VAL_PMMM_ICUT,
                        const F64 theta=FDPS_DFLT_VAL_THETA,
                        const U32 n_leaf_limit=FDPS_DFLT_VAL_N_LEAF_LIMIT,
                        const U32 n_group_limit=FDPS_DFLT_VAL_N_GROUP_LIMIT);
        ...
        */
        ...
        /////////////////////////////
        // for APIs for PMM module
        void calcParticleMeshCellInfoOnEpi() {
            calcParticleMeshCellInfoOnEpiImpl(typename TSM::search_type());
        }
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchShortGather) {}
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchShortScatter) {}
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchShortSymmetry) {}
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchLong) {}
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchLongCutoff) {}
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchLongParticleMeshMultipole) {
            const S32 adr_sp_tree_first = 0; // 0 means non-use
            const F64 r_crit_sq = -1.0;
            // Negative value means non-use of opening angle criterion.
            const std::vector<S32vec> dummy_list;
            CalcParticleMeshCellInfo
                <TSM, TreeCell<Tmomloc>, TreeParticle, Tepi, Tspj,
                 PMM::CellInfo<Tforce, Tepi, Tepj, Tspj>,
                 TagChopCellFalse, TagCopyIPInfo>
                (adr_tc_level_partition_loc_,
                 tc_loc_, tp_glb_, epi_sorted_, spj_sorted_,
                 n_leaf_limit_, lev_leaf_limit_, adr_sp_tree_first, r_crit_sq,
                 pos_ref_keycalc_.low_, icut_,
                 level_pm_cell_, width_pm_cell_, dummy_list, info_pm_cell_);
        }
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchLongScatter) {}
        void calcParticleMeshCellInfoOnEpiImpl(TagSearchLongSymmetry) {}

        void calcParticleMeshCellInfoOnEpjAndSpj() {
            calcParticleMeshCellInfoOnEpjAndSpjImpl(typename TSM::search_type());
        }
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchShortGather) {}
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchShortScatter) {}
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchShortSymmetry) {}
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchLong) {}
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchLongCutoff) {}
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchLongParticleMeshMultipole) {
            const S32 adr_tree_sp_first = spj_sorted_.size() - tc_glb_.size(); 
            const F64 r_crit_sq = (length_ * length_) / (theta_ * theta_);
            const std::vector<S32vec> list_my_pm_cell = list_pm_cell_[Comm::getRank()];
            CalcParticleMeshCellInfo
                <TSM, TreeCell<Tmomglb>, TreeParticle, Tepi, Tspj,
                 PMM::CellInfo<Tforce, Tepi, Tepj, Tspj>,
                 TagChopCellTrue, TagCopyJPInfo>
                (adr_tc_level_partition_glb_,
                 tc_glb_, tp_glb_, epj_sorted_, spj_sorted_,
                 n_leaf_limit_, lev_leaf_limit_, adr_tree_sp_first, r_crit_sq,
                 pos_ref_keycalc_.low_, icut_,
                 level_pm_cell_, width_pm_cell_, list_my_pm_cell, info_pm_cell_);
                
#if !defined(PARTICLE_SIMULATOR_USE_SPJ_FOR_MM_CALC_IN_PMM)
            // Set epj_is_shared
            for (auto itr = info_pm_cell_.begin(); itr != info_pm_cell_.end(); ++itr) {
                const S32vec idx = itr->first;
                bool has_ptcl = false;
                for (S32 n=0; n < itr->second.n_epj.size(); n++)
                    if (itr->second.n_epj[n] > 0) has_ptcl=true;
                if (has_ptcl) {
                    auto ret = std::find(list_pm_cell_shared_.begin(),
                                         list_pm_cell_shared_.end(),
                                         idx);
                    if (ret != list_pm_cell_shared_.end()) {
                        itr->second.epj_is_shared = true;
                    }
                }
            }
#endif
        }
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchLongScatter) {}
        void calcParticleMeshCellInfoOnEpjAndSpjImpl(TagSearchLongSymmetry) {}

        void calcParticleMeshCellInfoOnForce() {
            calcParticleMeshCellInfoOnForceImpl(typename TSM::search_type());
        }
        void calcParticleMeshCellInfoOnForceImpl(TagSearchShortGather) {}
        void calcParticleMeshCellInfoOnForceImpl(TagSearchShortScatter) {}
        void calcParticleMeshCellInfoOnForceImpl(TagSearchShortSymmetry) {}
        void calcParticleMeshCellInfoOnForceImpl(TagSearchLong) {}
        void calcParticleMeshCellInfoOnForceImpl(TagSearchLongCutoff) {}
        void calcParticleMeshCellInfoOnForceImpl(TagSearchLongParticleMeshMultipole) {
            for (auto itr = info_pm_cell_.begin(); itr != info_pm_cell_.end(); ++itr) {
                if (itr->second.n_epi > 0) {
                    const S32 adr = itr->second.adr_epi_first;
                    itr->second.force_first = force_sorted_.getPointer(adr);
                }
            }
        }
        void calcParticleMeshCellInfoOnForceImpl(TagSearchLongScatter) {}
        void calcParticleMeshCellInfoOnForceImpl(TagSearchLongSymmetry) {}


        void decomposeParticleMeshCellImpl2(const DomainInfo & dinfo); // tree_for_force_impl.hpp
        void decomposeParticleMeshCell(const DomainInfo & dinfo) {
            decomposeParticleMeshCellImpl(typename TSM::search_type(), dinfo);
        }
        void decomposeParticleMeshCellImpl(TagSearchShortGather, const DomainInfo & dinfo) {}
        void decomposeParticleMeshCellImpl(TagSearchShortScatter, const DomainInfo & dinfo) {}
        void decomposeParticleMeshCellImpl(TagSearchShortSymmetry, const DomainInfo & dinfo) {}
        void decomposeParticleMeshCellImpl(TagSearchLong, const DomainInfo & dinfo) {}
        void decomposeParticleMeshCellImpl(TagSearchLongCutoff, const DomainInfo & dinfo) {}
        void decomposeParticleMeshCellImpl(TagSearchLongParticleMeshMultipole, const DomainInfo & dinfo) {
            decomposeParticleMeshCellImpl2(dinfo);
        }
        void decomposeParticleMeshCellImpl(TagSearchLongScatter, const DomainInfo & dinfo) {}
        void decomposeParticleMeshCellImpl(TagSearchLongSymmetry, const DomainInfo & dinfo) {}

        S32 getICut() const {return icut_;}
        S32vec getNCell() const {return n_cell_;}
        F64ort getPosUnitCell() const {return pos_ref_keycalc_;}
        S32 getNumberOfEpiSorted() const {return epi_sorted_.size();}
        S32 getNumberOfEpjSorted() const {return epj_sorted_.size();}
        S32 getParticleMeshCellInfo(const S32vec idx, PMM::CellInfo<Tforce, Tepi, Tepj, Tspj> & info) const;
        ...
        void exchangeLocalEssentialTreeReuseListImpl(TagSearchLongCutoff,
                                                     const DomainInfo & dinfo,
                                                     const bool flag_reuse=false);
        void exchangeLocalEssentialTreeReuseListImpl(TagSearchLongParticleMeshMultipole,
                                                     const DomainInfo & dinfo,
                                                     const bool flag_reuse=false);
        ...
        void calcForceMakingTree(Tfunc_ep_ep pfunc_ep_ep, 
            ...
            if(list_mode == MAKE_LIST || list_mode == MAKE_LIST_FOR_REUSE){
                ...
                calcParticleMeshCellInfoOnEpi(); // epi 

                decomposeParticleMeshCell(dinfo);
                ...
                calcParticleMeshCellInfoOnEpjAndSpj(); // epj and spj
                ...
                calcParticleMeshCellInfoOnForce(); // force
        ...
        void setLocalEssentialTreeToGlobalTree2(const bool flag_reuse = false){
            ...
            setLocalEssentialTreeToGlobalTreeImpl2(typename TSM::force_type(),
                                                   typename TSM::key_type(),
                                                   flag_reuse);
        ...
        void setLocalEssentialTreeToGlobalTreeImpl2(TagForceLong,
                                                    TagKeyNormal,
                                                    const bool flag_reuse = false){
            SetLocalEssentialTreeToGlobalTreeWithKeyNormalImpl
                (epj_org_, spj_org_, n_loc_tot_, tp_glb_, flag_reuse);
        }
        void setLocalEssentialTreeToGlobalTreeImpl2(TagForceLong,
                                                    TagKeyMeshBased,
                                                    const bool flag_reuse = false){
            SetLocalEssentialTreeToGlobalTreeWithKeyMeshBasedImpl
                (epj_org_, spj_org_, n_loc_tot_, tp_glb_, flag_reuse);
        }
        void setLocalEssentialTreeToGlobalTreeImpl2(TagForceShort,
                                                    ...
                                                    const bool flag_reuse = false){
            SetLocalEssentialTreeToGlobalTreeWithKeyNormalImpl
                (epj_org_, n_loc_tot_, tp_glb_, flag_reuse);
        }
        void setLocalEssentialTreeToGlobalTreeImpl2(TagForceShort,
                                                    TagKeyMeshBased,
                                                    const bool flag_reuse = false){
...
#pragma omp parallel for
    ...
    public:
        ...
        typedef TreeForForce
        <SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE,
         Tforce, Tepi, Tepj,
         Tmom, Tmom, Tsp>  WithParticleMeshMultipole; // for PM^3
    ...
    public:
        ...
        <SEARCH_MODE_LONG_PARTICLE_MESH_MULTIPOLE,
         Tforce, Tepi, Tepj,
         MomentMonopoleGeometricCenterPMMM,
         MomentMonopoleGeometricCenterPMMM,
         SPJMonopoleGeometricCenterPMMM> MonopoleGeometricCenterParticleMeshMultipole;
