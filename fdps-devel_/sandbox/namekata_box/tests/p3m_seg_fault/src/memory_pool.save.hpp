    ...
    public:
        ...
        static void unifyMem() {
            ...
            if (!inParallelRegion() && n_emerg_bufs > 0) {
                ...
                //std::cout << "emergency buffers released! (size = "
                //          << getInstance().emerg_bufs_.size() << ")" << std::endl;
            } 
        }
        ...
        static void alloc(const size_t _size, int & _id_mpool, void * ptr_data, void *& ret){
            ...
            if(!flag_break){
                ...
                if (!inParallelRegion()) {
                    ...
                    if(getInstance().cap_ < getInstance().size_ + size_align){
                        reInitialize(size_align);
                    }
                    void * top_prev = getInstance().top_;
                    ...
                    getInstance().emerg_bufs_[idx].data = malloc(size_align);
        ...
        static void allocWithDebug(const size_t _size, int & _id_mpool, void * ptr_data, void *& ret, const bool debug_flag){
            unifyMem();
            const size_t size_align = getAlignSize(_size);
            size_t cap_cum = 0;
            bool flag_break = false;
            //std::cout<<"&_id_mpool = "<<&_id_mpool<<std::endl;
            if (debug_flag) {
                std::cout<<"size_align = "<<size_align<<std::endl;
                std::cout<<"A) _id_mpool= "<<_id_mpool
                         <<" getInstance().n_segment_= "<<getInstance().n_segment_
                         <<" getInstance().size_= "<<getInstance().size_
                         <<" getInstance().top_= "<<getInstance().top_
                         <<std::endl;
            ...
            if(!flag_break){
                ...
                //    std::cout<<"_id_mpool (old value)    = "<<_id_mpool<<std::endl;
                //    std::cout<<"getInstance().n_segment_ = "<<getInstance().n_segment_<<std::endl;
                //    std::cout<<"getInstance().cap_       = "<<getInstance().cap_<<std::endl;
                //    std::cout<<"getInstance().size_      = "<<getInstance().size_<<std::endl;
                //    std::cout<<"getInstance().bottom_    = "<<getInstance().bottom_<<std::endl;
                //    std::cout<<"getInstance().top_       = "<<getInstance().top_<<std::endl;
                //    std::cout<<"getInstance().used_per_seg_[getInstance().n_segment_-1]= "<<getInstance().used_per_seg_[getInstance().n_segment_-1]<<std::endl;
                //    std::cout<<"getInstance().cap_per_seg_[getInstance().n_segment_-1]= "<<getInstance().cap_per_seg_[getInstance().n_segment_-1]<<std::endl;
                //}
                if (!inParallelRegion()) {
                    if((size_t)_id_mpool == getInstance().n_segment_-1 && getInstance().n_segment_ > 0){
                        getInstance().n_segment_--;
                        getInstance().top_  = ((char*)getInstance().top_) - getInstance().cap_per_seg_[getInstance().n_segment_];
                        getInstance().size_ = getInstance().size_ - getInstance().cap_per_seg_[getInstance().n_segment_];
                        assert(getInstance().size_ <= getInstance().cap_);
                    }

                    if(getInstance().cap_ < getInstance().size_ + size_align){
                        //if (rank_tmp == 1) {
                        //    std::cout<<"C-2) reInitialize() is called!" << std::endl;
                        //    std::cout<<"[bef.] getInstance().bottom_ = "<<getInstance().bottom_<<std::endl;
                        //}
                        reInitialize(size_align);
                        //if (rank_tmp == 1) {
                        //    std::cout<<"[aft.] getInstance().bottom_ = "<<getInstance().bottom_<<std::endl;
                        //}
                    }
                    void * top_prev = getInstance().top_;
                    getInstance().top_   = ((char*)getInstance().top_) + size_align;
                    ...
                    if (debug_flag) {
                        ...
                        std::cout<<"getInstance().bottom_    = "<<getInstance().bottom_<<std::endl;
                        std::cout<<"getInstance().top_       = "<<getInstance().top_<<std::endl;
                        std::cout<<"getInstance().bottom_    = "<<(size_t)getInstance().bottom_<<" [bytes]"<<std::endl;
                        std::cout<<"getInstance().top_       = "<<(size_t)getInstance().top_<<" [bytes]"<<std::endl;
                        std::cout<<"size_align               = "<<(size_t)size_align<<" [bytes]"<<std::endl;
                        std::cout<<"top_ - top_prev          = "<<(size_t)getInstance().top_-(size_t)top_prev<<" [bytes]"<<std::endl;
                        std::cout<<"top_ - bottom_           = "<<(size_t)getInstance().top_-(size_t)getInstance().bottom_<<" [bytes]"<<std::endl;
                        std::cout<<"getInstance().cap_per_seg_[getInstance().n_segment_-1]= "<<getInstance().cap_per_seg_[getInstance().n_segment_-1]<<std::endl;
                        std::cout<<"top_prev = "<<top_prev<<std::endl;
                    }
                } else {
                    int n_emerg_bufs = getInstance().emerg_bufs_.size();
                    // Newly create an emergency buffer
