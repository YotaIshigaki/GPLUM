...
//#ifdef __HPC_ACE__
    ...
    void MakeInteractionListLongCutoffEPForZeroTheta(const ReallocatableArray<Ttc> & tc_first,
                                                     ...
                                                     const F64 r_cut_sq,
                                                     const S32 n_leaf_limit,
                                                     const S32 flag_debug){
        if (flag_debug == 1) std::cout << "adr_tc = " << adr_tc << std::endl;
        if (flag_debug == 1 && adr_tc == 4784) std::cout << "ok1" << std::endl;
        U32 open_bits = 0;
        for(S32 i=0; i<N_CHILDREN; i++){
#if 0
            const F64ort child_cell_box = makeChildCellBox(i, cell_box);
        ...
        if (flag_debug == 1 && adr_tc == 4784) std::cout << "ok2" << std::endl;
        for(S32 i=0; i<N_CHILDREN; i++){
            if (flag_debug == 1 && adr_tc == 4784) std::cout << "i = " << i << std::endl;
            const S32 adr_tc_child = adr_tc + i;
            Ttc * tc_child = tc_first.getPointer(adr_tc_child);
            const S32 n_child = tc_child->n_ptcl_;
            if(n_child == 0) continue;
            else if( (open_bits>>i) & 0x1 ){
                if (flag_debug == 1 && adr_tc == 4784) std::cout << "open" << std::endl;
                if( !(tc_child->isLeaf(n_leaf_limit)) ){
                    if (flag_debug == 1 && adr_tc == 4784) std::cout << "further descend" << std::endl;
                    const F64ort child_cell_box = makeChildCellBox(i, cell_box);
                    MakeInteractionListLongCutoffEPForZeroTheta<Ttc, Ttp, Tep>
                        (tc_first, tc_first[adr_tc_child].adr_tc_, tp_first,
                         ep_first, ep_list, child_cell_box,
                         pos_target_box, r_cut_sq, n_leaf_limit, flag_debug);
                }
                else{
                    if (flag_debug == 1 && adr_tc == 4784) std::cout << "register" << std::endl;
                    S32 adr_ptcl_tmp = tc_child->adr_ptcl_;
                    if (flag_debug == 1 && adr_tc == 4784) std::cout << "-> cp1" << std::endl;
                    if (flag_debug == 1 && adr_tc == 4784) {
                        std::cout << "   n_child = " << n_child << std::endl;
                        std::cout << "   size    = " << ep_list.size() << std::endl;  
                        std::cout << "   cap     = " << ep_list.capacity() << std::endl;  
                        std::cout << "   mode    = " << ep_list.alloc_mode() << std::endl;  
                    }
                    //ep_list.reserveEmptyAreaAtLeast( n_child );
                    bool debug_flag = false;
                    if (flag_debug == 1 && adr_tc == 4784) debug_flag=true;
                    ep_list.reserveEmptyAreaAtLeastWithDebug( n_child, debug_flag );
                    if (flag_debug == 1 && adr_tc == 4784) std::cout << "-> cp2" << std::endl;
                    for (S32 ip=0; ip<n_child; ip++){
                        if (flag_debug == 1 && adr_tc == 4784) std::cout << "-> ip = " << ip << std::endl;
                        if( GetMSB(tp_first[adr_ptcl_tmp].adr_ptcl_) == 0){
                            ep_list.pushBackNoCheck(ep_first[adr_ptcl_tmp++]);
                        }
                    }
                    if (flag_debug == 1 && adr_tc == 4784) std::cout << "end of register" << std::endl;
                }
            }
        }
    }
