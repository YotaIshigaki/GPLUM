...
#ifdef DEBUG_PRINT_MAIN
    PS::Comm::barrier();if(PS::Comm::getRank()==0)std::cerr<<"OK6-0 @main"<<std::endl;
#endif
    TreeType tree_grav;
    TREE_POINTER = & tree_grav;
#ifdef DEBUG_PRINT_MAIN
    PS::Comm::barrier();if(PS::Comm::getRank()==0)std::cerr<<"OK6-1 @main"<<std::endl;
#endif
    tree_grav.epi_sorted_.copyPointer(&system_grav[0]);
#ifdef DEBUG_PRINT_MAIN
    PS::Comm::barrier();if(PS::Comm::getRank()==0)std::cerr<<"OK6-2 @main"<<std::endl;
#endif
    tree_grav.epi_sorted_.setSize(system_grav.getNumberOfParticleLocal());
#ifdef DEBUG_PRINT_MAIN
    PS::Comm::barrier();if(PS::Comm::getRank()==0)std::cerr<<"OK6-3 @main"<<std::endl;
#endif
...
#ifdef DEBUG_PRINT_MAIN
    PS::Comm::barrier();if(PS::Comm::getRank()==0)std::cerr<<"OK8-2 @main"<<std::endl;
#endif
        /*
    CalcForce(tree_grav,
