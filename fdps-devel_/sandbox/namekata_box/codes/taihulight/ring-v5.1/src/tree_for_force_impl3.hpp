...
#if 0
    Comm::barrier();
    const S32 target_rank=0;
    if (Comm::getRank() == target_rank) {
        std::cout << "outputing tp_*_[] in mortonSortLocalTreeOnlyImpl()." << std::endl;
        std::ofstream ofs;
        std::stringstream filenum;
        std::string filename;
        filenum << std::setfill('0') << std::setw(5) << Comm::getRank();
        filename = "./tp_org" + filenum.str() + ".dat";
        ofs.open(filename.c_str(), std::ios::binary|std::ios::trunc);
#ifdef REMOVE_TP_LOC
        S32 tp_num = tp_glb_.size();
        std::cout << "n_loc_tot_         = " << n_loc_tot_ << std::endl;
        std::cout << "tp_glb_.size()     = " << tp_num << std::endl;
        std::cout << "sizeof(tp_glb_[0]) = " << sizeof(tp_glb_[0]) << std::endl;
        ofs.write((char *)&tp_num, sizeof(S32));
        ofs.write((char *)&tp_glb_[0], tp_num * sizeof(tp_glb_[0]));
#else
...
#endif
        ofs.close();
        std::cout << "output of tp_*_[] is completed!" << std::endl;
    }
    athread_halt();
    Finalize();
    std::exit(0);
        ...
        // [Check]
        if (Comm::getRank() == 1) {
            std::cout << "outputing epj_org...." << std::endl;
            std::ofstream ofs;
            std::stringstream filenum;
            std::string filename;
            filenum << std::setfill('0') << std::setw(5) << Comm::getRank();
            filename = "./epj_org" + filenum.str() + ".txt";
            ofs.open(filename.c_str(), std::ios::trunc);
            for (S32 i=0; i<n_loc_tot_; i++) {
            ...
            ofs.close();
            std::cout << "output of epj_org_ is completed!" << std::endl;
        }
        Comm::barrier();
...
#pragma omp parallel for
        ...
        // [Check]
        if (Comm::getRank() == 1) {
            std::cout << "outputing _epj_sorted...." << std::endl;
            std::ofstream ofs;
            std::stringstream filenum;
            std::string filename;
            filenum << std::setfill('0') << std::setw(5) << Comm::getRank();
            filename = "./epj_sorted" + filenum.str() + ".txt";
            ofs.open(filename.c_str(), std::ios::trunc);
            for (S32 i=0; i<n_loc_tot_; i++) {
                //ofs << std::setprecision(15)
                //    << _epj_sorted[i].mass << " "
            ...
            ofs.close();
            std::cout << "output of _epj_sorted is completed!" << std::endl;
        }
        Comm::barrier();
        if (Comm::getRank() == 0) std::cout << "OK bokujo!" << std::endl;
        athread_halt();
...
#pragma omp single
    ...
    exchangeLocalEssentialTreeSuperDomain2(const DomainInfo & dinfo,
        ...
        Comm::barrier();
        if (my_rank == 0)
            std::cerr<<"OK1 @exchangeLocalEssentialTreeSuperDomain2()"<<std::endl;
        Comm::barrier();
#endif
        //////////////////////
        // initialize
        if(!reuse_list){
            ...
            Comm::barrier();
            if (my_rank == 0)
                std::cerr<<"OK1-1 @exchangeLocalEssentialTreeSuperDomain2()"<<std::endl;
            Comm::barrier();
#endif
            ...
            Comm::barrier();
            if (my_rank == 0)
                std::cerr<<"OK1-2 @exchangeLocalEssentialTreeSuperDomain2()"<<std::endl;
            Comm::barrier();
#endif
            ...
            Comm::barrier();
            if (my_rank == 0)
                std::cerr<<"OK1-3 @exchangeLocalEssentialTreeSuperDomain2()"<<std::endl;
            Comm::barrier();
            //athread_halt();
            //Finalize();
            //std::exit(0);
            ...
            Comm::barrier();
            if (my_rank == 0)
                std::cerr<<"OK1-4 @exchangeLocalEssentialTreeSuperDomain2()"<<std::endl;
            Comm::barrier();
#endif
...
//#endif
            ...
            */
...
#ifdef CHECK_NAN
            {
                S32 num_nan_loc = 0;
                for (S32 i=0; i<n_loc_tot_; i++) {
                    if ((std::isfinite(epj_sorted_loc_[i].pos.x) != true) ||
                        (std::isfinite(epj_sorted_loc_[i].pos.y) != true) ||
                        (std::isfinite(epj_sorted_loc_[i].pos.z) != true) ||
                        (std::isfinite(epj_sorted_loc_[i].vel.x) != true) ||
                        (std::isfinite(epj_sorted_loc_[i].vel.y) != true) ||
                        (std::isfinite(epj_sorted_loc_[i].vel.z) != true) ||
                        (std::isfinite(epj_sorted_loc_[i].mass) != true)) {
                        num_nan_loc++;
                    }
                }
                S32 num_nan = 0;
                ...
                if (Comm::getRank() == 0)
                    std::cout << "LM-A) [nan-epj] " << num_nan << std::endl;                              
                if (num_nan_loc > 0) std::cout << "#a my_rank = " << Comm::getRank() << std::endl;
            }
#endif
            ...
            */
            ...
            {
                S32 num_nan_loc = 0;
                for (S32 i=0; i<n_loc_tot_+n_epj_add; i++) {
                    if ((std::isfinite(epj_sorted_[i].pos.x) != true) ||
                        (std::isfinite(epj_sorted_[i].pos.y) != true) ||
                        (std::isfinite(epj_sorted_[i].pos.z) != true) ||
                        (std::isfinite(epj_sorted_[i].vel.x) != true) ||
                        (std::isfinite(epj_sorted_[i].vel.y) != true) ||
                        (std::isfinite(epj_sorted_[i].vel.z) != true) ||
                        (std::isfinite(epj_sorted_[i].mass) != true)) {
                        num_nan_loc++;
                    }
                }
                S32 num_nan = 0;
                MPI::COMM_WORLD.Allreduce(&num_nan_loc,&num_nan,1,MPI::INT,MPI::SUM);
                if (Comm::getRank() == 0)
                    std::cout << "LM-B) [nan-epj] " << num_nan << std::endl;                              
                if (num_nan_loc > 0) std::cout << "#b my_rank = " << Comm::getRank() << std::endl;
            }
#endif
