...
__thread_local volatile int my_id, my_row_id, my_col_id;
...
static int searchWhichDomainParticleGoToPeriodicX(F64vec_ *pos,
    ...
    dma_set_size(&dma_get,  sizeof(F64ort_));
    //** x direction
    volatile int shift = n_domain[1] * n_domain[2];
...
void FindDestinationRank(void *args) {
    ...
    sync_array_();
    if (my_id == ID_CHECK) {
        printf("dma_descriptor_init completed.\n");
    }
#endif
    ...
    sync_array_();
    if ((myrank == RANK_CHECK) && (my_id == ID_CHECK)) {
        printf("Setup of atan coefs. completed.\n");
    }
    sync_array_();
#endif
    ...
    sync_array_();
    if ((myrank == RANK_CHECK) && (my_id == ID_CHECK)) {
        printf("Mem. alloc. of local bufs. completed.\n");
    }
    sync_array_();
#endif
...
#endif
        volatile int nrem = nptcl_loc - i;
        volatile int nn   = nrem < CYCLE_SIZE ? nrem : CYCLE_SIZE;
#ifdef CHECK_FIND_DEST_RANK
        // Output nrem,nn
        if ((myrank == RANK_CHECK) && (my_id == ID_CHECK)) {
            printf("nrem      = %d\n",nrem);
    ...
    assert(n_dest_rank <= TBL_SIZE);
    //if (my_id == 0) printf("[3]n_dest_rank = %d\n",n_dest_rank);
    //** n_dest_rank 
    reply_put = 0;
    dma_set_op(&dma_put,    DMA_PUT);
    dma_set_mode(&dma_put,  PE_MODE);
    ...
    dma_set_size(&dma_put,  sizeof(int));
    dma(dma_put,
        (long*)((int *)adr_n_dest_rank_cpe + my_id),
        (long*)(&n_dest_rank));
    dma_wait(&reply_put, 1);
    while (reply_put != 1) {}
    //if (my_id == 0) printf("[4]n_dest_rank = %d\n",n_dest_rank);
    //** dest_rank_list
    if (n_dest_rank > 0) {
        reply_put = 0;
        dma_set_op(&dma_put,    DMA_PUT);
        ...
        dma(dma_put,
            (long*)((int *)adr_dest_rank_list_cpe + my_id * nprocs),
            (long*)(dest_rank_list));
        dma_wait(&reply_put, 1);
        while (reply_put != 1) {}
    }
