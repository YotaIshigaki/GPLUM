...
PS::S32 DispatchKernelWithSP(
    ...
    for (PS::S32 ig=0; ig<n_walk; ig++) {
        for (PS::S32 ip=n_disp_epi[ig]; ip<n_disp_epi[ig+1]; ip++) {
            if(epi[ip].id==122731769486){
                std::cerr<<"epi[ip].id= "<<epi[ip].id
                         <<" ig= "<<ig
                         <<" ip= "<<ip
                         <<" n_epi[ig]= "<<n_epi[ig]
                         <<" n_disp_epi[ig]= "<<n_disp_epi[ig]
                         <<std::endl;
            }
        }
    }
