...
void DebugPrintMsgMain(const char str[]){
#ifdef DEBUG_PRINT_MAIN
    PS::Comm::barrier();
    if (PS::Comm::getRank() == 0){
        std::cout << str  << std::endl;
        fout_debug << str  << std::endl;
        fout_debug.flush();
    }
#endif
}
...
int main(int argc, char *argv[]) {
...
#if defined(POLLING_FOR_CPE_ERROR) || defined(PRINT_HOST_NAME)
    if (my_rank == 0) {
    ...
    if(PS::Comm::getRank()==0){
        std::cerr<<"PS::TREE_Z_COORD_OFFSET= "<<PS::TREE_Z_COORD_OFFSET
                 <<" r_phy= "<<r_phy
                 <<std::endl;;
        fout_debug<<"PS::TREE_Z_COORD_OFFSET= "<<PS::TREE_Z_COORD_OFFSET
                  <<" r_phy= "<<r_phy
                  <<std::endl;;
        fout_debug.flush();
    }
    ...
    if(PS::Comm::getRank() == 0){
        ...
        fout_debug<<"after make satellite"<<std::endl;
        fout_debug.flush();
    }
    NAN_TEST(system_grav,"[nan-0]");
    
    DebugPrintMsgMain("OK1 @main");
    ...
    PS::Comm::barrier();  wtime_main_0 = wtime();
    
    n_loc = system_grav.getNumberOfParticleLocal();

    std::cerr<<"a) n_loc= "<<n_loc<<std::endl;
    ...
    */
    ...
    for(auto i=0; i<n_loc; i++){
        assert(dinfo.getPosDomain(MY_RANK_MPI).contained(system_grav[i].pos));
    }
    ConvertCyl2Car(system_grav);
    
    PS::Comm::barrier();
    if(MY_RANK_MPI==0){
        std::cerr<<"finish exchange particles"<<std::endl;
    }
    std::cerr<<"b) n_loc= "<<n_loc<<std::endl;
    //PS::Finalize();
    //return 0;
    ...
    PS::Comm::barrier();  wtime_main_0 = wtime();    
    checkParticleIsInMyDomain(system_grav, dinfo);
    PS::Comm::barrier(); if (PS::Comm::getRank() == 0) std::cout << "etime(check_is_is_domain ) = " << wtime() - wtime_main_0 << std::endl;
    ...
    */
    ...
    std::cerr<<"&system_grav[0]= "<<&system_grav[0]<<std::endl;
    std::cerr<<"&tree_grav.epi_sorted_[0]= "<<&tree_grav.epi_sorted_[0]<<std::endl;
    ...
    CalcForce(tree_grav,
        ...
        */
        ...
        //std::cerr<<"system_grav.getNumberOfParticleLocal()= "<<system_grav.getNumberOfParticleLocal()<<std::endl;
