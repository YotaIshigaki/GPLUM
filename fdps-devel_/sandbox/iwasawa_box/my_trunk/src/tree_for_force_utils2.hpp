namespace ParticleSimulator{
    ...
    ////////////////////
    // for exchange LET
    // TYPE: LONG, copy send buffer from tc_loc
    template<class Tptcl, class Ttc>
    inline void CopyMomentToSendBuf(TagSearchLongCutoff,
        ...
        for(S32 j=0; j<n_ptcl; j++){
            const S32 adr_src = adr_ptcl_send[n_ptcl_offset+j];
            const S32 adr_dst = n_ptcl_offset+j;
            ptcl_send[adr_dst].copyFromMoment(tc_first[adr_src].mom_);
            ptcl_send[adr_dst].setPos(ptcl_send[adr_dst].getPos() - shift);
        }
    }
    template<class Tptcl, class Ttc>
    inline void CopyMomentToSendBuf(TagSearchLong,
                                    ReallocatableArray<Tptcl> & ptcl_send,
                                    const ReallocatableArray<Ttc> & tc_first,
                                    const ReallocatableArray<S32> & adr_ptcl_send,
                                    const S32 n_ptcl,
    ...
    template<class Tptcl, class Ttc>
    inline void CopyMomentToSendBuf(TagSearchLongScatter,
                                    ReallocatableArray<Tptcl> & ptcl_send,
                                    const ReallocatableArray<Ttc> & tc_first,
                                    const ReallocatableArray<S32> & adr_ptcl_send,
                                    const S32 n_ptcl,
    ...
    template<class Tptcl, class Ttc>
    inline void CopyMomentToSendBuf(TagSearchLongSymmetry,
                                    ReallocatableArray<Tptcl> & ptcl_send,
                                    const ReallocatableArray<Ttc> & tc_first,
                                    const ReallocatableArray<S32> & adr_ptcl_send,
                                    const S32 n_ptcl,
    ...
    inline void FindScatterParticle(const ReallocatableArray<Ttc> & tc_first,
        ...
        ReallocatableArray<Tsp> sp_first; // dummy
...
#pragma omp parallel for
    ...
    inline void ExchangeLet(const ReallocatableArray<Tep> & ep_first,
...
#pragma omp parallel for schedule(dynamic, 4)
        ...
        for(S32 i=0; i<n_image_tot; i++){
            ...
            CopyPtclToSendBuf(typename TSM::search_type(), ep_send, ep_first, adr_ep_send, n_ep,
                              n_ep_offset, shift);
            S32 n_sp = n_sp_per_image[i];
            S32 n_sp_offset = n_disp_sp_per_image[i];
            CopyMomentToSendBuf(typename TSM::search_type(), sp_send, tc_first, adr_sp_send, n_sp,
                              n_sp_offset, shift);
        }
...
#pragma omp parallel for schedule(dynamic, 4)
        ...
        for(S32 i=0; i<n_image_tot; i++){
            ...
            /*
            CopyPtclToSendBuf
                (typename TSM::search_type(), ep_send, ep, adr_ep_send, n_ep,
                 n_ep_offset, shift);
            */
...
#pragma omp parallel for
    ...
    (const ReallocatableArray<Tepj0> & epj_recv,
     ...
     const bool flag_reuse = false){
        ...
        spj_org.resizeNoInitialize( n_sp_add );
#else
...
#ifdef SHRINK_SIZE_OF_SPJ_SORTED
                    spj_org[i] = spj_recv[i];
                    tp_glb[offset_sp+i].setFromSP(spj_recv[i], i);
#else
                    spj_org[offset_sp+i] = spj_recv[i];
