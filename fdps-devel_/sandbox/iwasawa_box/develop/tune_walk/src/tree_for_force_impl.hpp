...
#pragma omp parallel for
    ...
    linkCellLocalTreeOnly(){
        ...
        */
        ...
        LinkCell(tc_loc_,  adr_tc_level_partition_, tp_loc_.getPointer(), 
		 lev_max_, length_, n_loc_tot_, n_leaf_limit_);
    ...
    exchangeLocalEssentialTreeImpl(TagSearchLong, const DomainInfo & dinfo){
        ...
        //std::cerr<<"inv_theta_sq="<<inv_theta_sq<<std::endl;
#ifdef PARTICLE_SIMULATOR_THREAD_PARALLEL	
#pragma omp parallel
#endif	
        {
...
#pragma omp for schedule(dynamic, 4)
            ...
            for(S32 ib=0; ib<n_proc; ib++){
                ...
                const F64ort pos_domain = dinfo.getPosDomain(ib);
                const S32 n_ep_cum = id_ep_send_buf_[ith].size();
                const S32 n_sp_cum = id_sp_send_buf_[ith].size();
                std::cerr<<"pos_domain= "<<pos_domain<<std::endl;
                SearchSendParticleLong<TreeCell<Tmomloc>, Tepj>
                    (tc_loc_,    id_ep_send_buf_[ith], id_sp_send_buf_[ith],
                     pos_domain, inv_theta_sq,         n_leaf_limit_);
                //std::cerr<<"n_ep_cum= "<<n_ep_cum<<" n_sp_cum= "<<n_sp_cum<<std::endl;
                std::cerr<<"id_ep_send_buf_[ith].size()= "<<id_ep_send_buf_[ith].size()
                         <<" id_sp_send_buf_[ith].size()= "<<id_sp_send_buf_[ith].size()<<std::endl;
...
#pragma omp single
        ...
        //std::cerr<<"befor alltoall"<<std::endl;
...
#endif
                    /*
                    std::cerr<<"spj_org_[ClearMSB(adr)].mass= "
                             <<spj_org_[ClearMSB(adr)].mass
                             <<std::endl;
                    */
...
#pragma omp parallel for
    ...
    linkCellGlobalTreeOnly(){
        ...
        */
        LinkCell(tc_glb_,  adr_tc_level_partition_, tp_glb_.getPointer(), 
		 lev_max_, length_, n_glb_tot_, n_leaf_limit_);
#ifdef PARTICLE_SIMULATOR_DEBUG_PRINT
        PARTICLE_SIMULATOR_PRINT_LINE_INFO();
	...
	*/
    ...
    makeIPGroupImpl(TagForceLong){
        ...
        MakeIPGroupLong(ipg_, tc_loc_, n_group_limit_);
#endif
...
#pragma omp parallel for
	...
	if(clear){
        ...
        MakeInteractionListLongEPSP(tc_glb_,
                                    tp_glb_,
                                    epj_sorted_,
                                    epj_for_force_[ith],
                                    spj_sorted_,
...
#pragma omp parallel for schedule(dynamic, 4) reduction(+ : ni_tmp, nj_tmp, n_interaction_ep_ep_tmp, n_interaction_ep_sp_tmp)
    ...
    calcForceMultiWalkImpl(TagForceLong,
        ...
        static const S32 size_vec = 8;

        static ReallocatableArray<Tepj> ** epj_for_force_tmp;
        static ReallocatableArray<Tspj> ** spj_for_force_tmp;
        /*
        static ReallocatableArray<S32> ** adr_cell_recorder;
        static ReallocatableArray<S32> ** record_type; // 0:do nothing, 1:leaf, 2:cell
        */

        //static __m256i * n_cell_recorder_pi32;
        //static __m256i * n_ptcl_recorder_pi32;
        static ReallocatableArray<S32> ** cell_recorder;
        static ReallocatableArray<S32> ** ptcl_recorder;

        if(first){

            //n_cell_recorder_pi32 = new __m256i[n_thread];
            //n_ptcl_recorder_pi32 = new __m256i[n_thread];
            ...
            epj_for_force_tmp = new ReallocatableArray<Tepj>*[n_thread];
            spj_for_force_tmp = new ReallocatableArray<Tspj>*[n_thread];
            /*
            adr_cell_recorder = new ReallocatableArray<S32>*[n_thread];
            record_type = new ReallocatableArray<S32>*[n_thread];
            */

            cell_recorder = new ReallocatableArray<S32>*[n_thread];
            ptcl_recorder = new ReallocatableArray<S32>*[n_thread];

            for(S32 i=0; i<n_thread; i++){
                cell_recorder[i] = new ReallocatableArray<S32>[size_vec];
                ptcl_recorder[i] = new ReallocatableArray<S32>[size_vec];

                epj_for_force_tmp[i] = new ReallocatableArray<Tepj>[size_vec];
                spj_for_force_tmp[i] = new ReallocatableArray<Tspj>[size_vec];
                /*
                adr_cell_recorder[i] = new ReallocatableArray<S32>[size_vec];
                record_type[i] = new ReallocatableArray<S32>[size_vec];
                */
                for(S32 k=0; k<size_vec; k++){
                    cell_recorder[i][k].reserve(10000);
                    ptcl_recorder[i][k].reserve(10000);
                    epj_for_force_tmp[i][k].reserve(100000);
                    spj_for_force_tmp[i][k].reserve(100000);
                    /*
                    adr_cell_recorder[i][k].reserve(5000);
                    record_type[i][k].reserve(5000);
                    */
                }
            }
            /*
            for(S32 i=0; i<n_thread; i++){
                epj_for_force_[i].reserve(1000000);
                spj_for_force_[i].reserve(1000000);
            }
            */
            first = false;
        }
        ...
        if(n_ipg > 0){
            for(int wg=0; wg<n_loop_max; wg++){
                //std::cerr<<"wg="<<wg<<" n_loop_max="<<n_loop_max<<std::endl;
...
#pragma omp parallel
#endif
                {
                    ...
                    //const __m256i two_pi32 = _mm256_set1_epi32(2);
                    ...
                    S32 iw_head = (n_walk/n_thread)*ith + std::min(ith, n_walk%n_thread);
                    ...
                    for(int iw=iw_head; iw<iw_tail; iw += size_vec){
                        ...
                        __m256 ipg_low_x = {(float)ipg_[id_ipg_base+0].vertex_.low_.x, (float)ipg_[id_ipg_base+1].vertex_.low_.x, 
                                            (float)ipg_[id_ipg_base+2].vertex_.low_.x, (float)ipg_[id_ipg_base+3].vertex_.low_.x,
                                            (float)ipg_[id_ipg_base+4].vertex_.low_.x, (float)ipg_[id_ipg_base+5].vertex_.low_.x, 
                                            (float)ipg_[id_ipg_base+6].vertex_.low_.x, (float)ipg_[id_ipg_base+7].vertex_.low_.x};
                        __m256 ipg_low_y = {(float)ipg_[id_ipg_base+0].vertex_.low_.y, (float)ipg_[id_ipg_base+1].vertex_.low_.y, 
                                            (float)ipg_[id_ipg_base+2].vertex_.low_.y, (float)ipg_[id_ipg_base+3].vertex_.low_.y,
                                            (float)ipg_[id_ipg_base+4].vertex_.low_.y, (float)ipg_[id_ipg_base+5].vertex_.low_.y, 
                                            (float)ipg_[id_ipg_base+6].vertex_.low_.y, (float)ipg_[id_ipg_base+7].vertex_.low_.y};
                        __m256 ipg_low_z = {(float)ipg_[id_ipg_base+0].vertex_.low_.z, (float)ipg_[id_ipg_base+1].vertex_.low_.z, 
                                            (float)ipg_[id_ipg_base+2].vertex_.low_.z, (float)ipg_[id_ipg_base+3].vertex_.low_.z,
                                            (float)ipg_[id_ipg_base+4].vertex_.low_.z, (float)ipg_[id_ipg_base+5].vertex_.low_.z, 
                                            (float)ipg_[id_ipg_base+6].vertex_.low_.z, (float)ipg_[id_ipg_base+7].vertex_.low_.z};

                        __m256 ipg_high_x = {(float)ipg_[id_ipg_base+0].vertex_.high_.x, (float)ipg_[id_ipg_base+1].vertex_.high_.x, 
                                             (float)ipg_[id_ipg_base+2].vertex_.high_.x, (float)ipg_[id_ipg_base+3].vertex_.high_.x,
                                             (float)ipg_[id_ipg_base+4].vertex_.high_.x, (float)ipg_[id_ipg_base+5].vertex_.high_.x, 
                                             (float)ipg_[id_ipg_base+6].vertex_.high_.x, (float)ipg_[id_ipg_base+7].vertex_.high_.x};
                        __m256 ipg_high_y = {(float)ipg_[id_ipg_base+0].vertex_.high_.y, (float)ipg_[id_ipg_base+1].vertex_.high_.y, 
                                             (float)ipg_[id_ipg_base+2].vertex_.high_.y, (float)ipg_[id_ipg_base+3].vertex_.high_.y,
                                             (float)ipg_[id_ipg_base+4].vertex_.high_.y, (float)ipg_[id_ipg_base+5].vertex_.high_.y, 
                                             (float)ipg_[id_ipg_base+6].vertex_.high_.y, (float)ipg_[id_ipg_base+7].vertex_.high_.y};
                        __m256 ipg_high_z = {(float)ipg_[id_ipg_base+0].vertex_.high_.z, (float)ipg_[id_ipg_base+1].vertex_.high_.z, 
                                             (float)ipg_[id_ipg_base+2].vertex_.high_.z, (float)ipg_[id_ipg_base+3].vertex_.high_.z,
                                             (float)ipg_[id_ipg_base+4].vertex_.high_.z, (float)ipg_[id_ipg_base+5].vertex_.high_.z, 
                                             (float)ipg_[id_ipg_base+6].vertex_.high_.z, (float)ipg_[id_ipg_base+7].vertex_.high_.z};
                        ...
                        //__m256i adr_ptcl_pi32 = zero_pi32;
                        ...
                        for(S32 k=0; k<8; k++){
                            n_epj_tmp[k] = n_spj_tmp[k] = 0;
                            cell_recorder[ith][k].clearSize();
                            ptcl_recorder[ith][k].clearSize();
                        }

                        __m256i cell_recorder_base_pi32 = _mm256_set_epi32( (int)(cell_recorder[ith][7].getPointer()-cell_recorder[ith][0].getPointer()),
                                                                            (int)(cell_recorder[ith][6].getPointer()-cell_recorder[ith][0].getPointer()),
                                                                            (int)(cell_recorder[ith][5].getPointer()-cell_recorder[ith][0].getPointer()),
                                                                            (int)(cell_recorder[ith][4].getPointer()-cell_recorder[ith][0].getPointer()),
                                                                            (int)(cell_recorder[ith][3].getPointer()-cell_recorder[ith][0].getPointer()),
                                                                            (int)(cell_recorder[ith][2].getPointer()-cell_recorder[ith][0].getPointer()),
                                                                            (int)(cell_recorder[ith][1].getPointer()-cell_recorder[ith][0].getPointer()),
                                                                            (int)(cell_recorder[ith][0].getPointer()-cell_recorder[ith][0].getPointer()) );

                        __m256i ptcl_recorder_base_pi32 = _mm256_set_epi32( (int)(ptcl_recorder[ith][7].getPointer()-ptcl_recorder[ith][0].getPointer()),
                                                                            (int)(ptcl_recorder[ith][6].getPointer()-ptcl_recorder[ith][0].getPointer()),
                                                                            (int)(ptcl_recorder[ith][5].getPointer()-ptcl_recorder[ith][0].getPointer()),
                                                                            (int)(ptcl_recorder[ith][4].getPointer()-ptcl_recorder[ith][0].getPointer()),
                                                                            (int)(ptcl_recorder[ith][3].getPointer()-ptcl_recorder[ith][0].getPointer()),
                                                                            (int)(ptcl_recorder[ith][2].getPointer()-ptcl_recorder[ith][0].getPointer()),
                                                                            (int)(ptcl_recorder[ith][1].getPointer()-ptcl_recorder[ith][0].getPointer()),
                                                                            (int)(ptcl_recorder[ith][0].getPointer()-ptcl_recorder[ith][0].getPointer()) );

                        __m256i n_cell_recorder_pi32 = zero_pi32;
                        __m256i n_ptcl_recorder_pi32 = zero_pi32;


                        do{
                            *(v8si*) adr_cell = (v8si) adr_cell_pi32;
                            __m256 fulle_len_sq_ps = {tc_glb_[adr_cell[0]].full_len_sq_, tc_glb_[adr_cell[1]].full_len_sq_, 
                            ...
                            __m256i idx_pi32 = _mm256_add_epi32(cell_recorder_base_pi32, n_cell_recorder_pi32); // idx for cell
                            __m256i set_mask_pi32 = _mm256_andnot_si256(is_finished, (__m256i)is_far);
                            v8si_mask_scatter((v8si)adr_cell_pi32, (v8si)set_mask_pi32, cell_recorder[ith][0].getPointer(), (v8si)idx_pi32);
                            /*
                            for(S32 k=0; k<size_vec; k++){
                                int tmp[8] __attribute__((aligned(32)));
                                *(v8si*) tmp = (v8si)set_mask_pi32;
                                if(tmp[k] == -1){
                                    cell_recorder[ith][k].push_back(adr_cell[k]);
                                    Tspj sp_tmp;
                                    sp_tmp.copyFromMoment(tc_glb_[adr_cell[k]].mom_);
                                }
                            }
                            */
                            n_cell_recorder_pi32 = _mm256_add_epi32(n_cell_recorder_pi32, _mm256_and_si256(set_mask_pi32, one_pi32));

                            idx_pi32 = _mm256_add_epi32(ptcl_recorder_base_pi32, n_ptcl_recorder_pi32);
                            set_mask_pi32 = _mm256_andnot_si256(is_finished, (__m256i)_mm256_and_ps(is_close, is_leaf));
                            v8si_mask_scatter((v8si)adr_cell_pi32, (v8si)set_mask_pi32, ptcl_recorder[ith][0].getPointer(), (v8si)idx_pi32);
                            /*
                            for(S32 k=0; k<size_vec; k++){
                                int tmp[8] __attribute__((aligned(32)));
                                *(v8si*) tmp = (v8si)set_mask_pi32;
                                if(tmp[k] != 0){
                                    ptcl_recorder[ith][k].push_back(adr_cell[k]);
                                }
                            }
                            */
                            n_ptcl_recorder_pi32 = _mm256_add_epi32(n_ptcl_recorder_pi32, _mm256_and_si256(set_mask_pi32, one_pi32));
                        ...
                        S32 n_cell[size_vec] __attribute__((aligned(32)));
                        S32 n_ptcl[size_vec] __attribute__((aligned(32)));
                        *(v8si*) n_cell = (v8si)n_cell_recorder_pi32;
                        *(v8si*) n_ptcl = (v8si)n_ptcl_recorder_pi32;
                        for(S32 k=0; k<size_vec_new; k++){
                            S32 n_sp = 0;
                            S32 n_ep = 0;
                            for(S32 ic=0; ic<n_cell[k]; ic++){
                                spj_for_force_tmp[ith][k][n_sp++].copyFromMoment(tc_glb_[cell_recorder[ith][k][ic]].mom_);
                                //Tspj sp_tmp;
                                //sp_tmp.copyFromMoment(tc_glb_[cell_recorder[ith][k][ic]].mom_);
                                //spj_for_force_tmp[ith][k].push_back(sp_tmp);
                                //n_sp++;
                            }
                            for(S32 ic=0; ic<n_ptcl[k]; ic++){
                                S32 adr_ptcl = tc_glb_[ptcl_recorder[ith][k][ic]].adr_ptcl_;
                                S32 n_ptcl   = tc_glb_[ptcl_recorder[ith][k][ic]].n_ptcl_;
                                for(S32 ip=0; ip<n_ptcl; ip++, adr_ptcl++){
                                    if( GetMSB( tp_glb_[adr_ptcl].adr_ptcl_ ) ){
                                        spj_for_force_tmp[ith][k][n_sp++] = spj_sorted_[adr_ptcl];
                                        //spj_for_force_tmp[ith][k].push_back(spj_sorted_[adr_ptcl]);
                                        //n_sp++;
                                    }
                                    else{
                                        epj_for_force_tmp[ith][k][n_ep++] = epj_sorted_[adr_ptcl];
                ...
                //std::cerr<<"n_walk= "<<n_walk<<std::endl;
                ...
                /*
                for(S32 i=0; i<n_walk; i++){
                    std::cerr<<"i= "<<i<<std::endl;
                    for(S32 j=0; j<n_epj_array[i]; j++){
                        std::cerr<<"j= "<<j<<" epj_array[i][j].pos= "<<epj_array[i][j].pos<<std::endl;
                    }
                    for(S32 j=0; j<n_spj_array[i]; j++){
                        std::cerr<<"j= "<<j<<" spj_array[i][j].pos= "<<spj_array[i][j].pos<<std::endl;
                    }
                }
                */
...
//#pragma simd
                for(int iw=0; iw<n_walk; iw++){
                    ...
                    /*
                    std::cerr<<"iw= "<<iw
                             <<" n_epj_array[iw]= "<<n_epj_array[iw]
                             <<" n_spj_array[iw]= "<<n_spj_array[iw]
                             <<std::endl;
                    */
                ...
                //std::cerr<<"wg= "<<wg<<" n_epj_array[0]= "<<n_epj_array[0]<<" n_spj_array[0]= "<<n_spj_array[0]<<std::endl;
                /*
                for(S32 i=0; i<n_epj_array[0]; i++){
                    std::cerr<<"i= "<<i<<" epj_array[0][i].pos= "<<epj_array[0][i].pos<<std::endl;
                }
                for(S32 i=0; i<n_spj_array[0]; i++){
                    std::cerr<<"i= "<<i<<" spj_array[0][i].pos= "<<spj_array[0][i].pos<<std::endl;
                }
                */
