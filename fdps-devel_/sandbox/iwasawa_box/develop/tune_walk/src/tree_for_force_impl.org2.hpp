...
#pragma omp parallel 
		...
		//for(S32 ig=ig_head; ig<ig_tail; ig += n_walk_limit_per_thread){
		for(S32 loop=0; loop<n_loop_limit; loop++, ig += n_walk_limit_per_thread){
		    /*
		    for(int it=0; it<n_thread; it++){
			if(ith == it){
			    std::cerr<<"ith="<<ith<<" loop="<<loop<<std::endl;
			}
#pragma omp barrier
		    }
		    */
...
#pragma omp barrier			
			...
			n_epj_disp[adr+1] = epj_for_force_[ith].size();
			n_spj_disp[adr+1] = spj_for_force_[ith].size();
			n_epj_array[adr] = n_epj_disp[adr+1] - n_epj_disp[adr];
			n_spj_array[adr] = n_spj_disp[adr+1] - n_spj_disp[adr];
		    }
		    /*
		    for(int it=0; it<n_thread; it++){
#pragma omp barrier
			if(ith == it){
			    std::cerr<<"ith="<<ith<<" check 1"<<std::endl;
			}
...
#pragma omp barrier
		    ...
		    for(S32 iw=0; iw<n_walk; iw++){
			epj_array[n_walk_offset+iw] = epj_for_force_[ith].getPointer(n_epj_disp[iw]);
			spj_array[n_walk_offset+iw] = spj_for_force_[ith].getPointer(n_spj_disp[iw]);			    
		    }
