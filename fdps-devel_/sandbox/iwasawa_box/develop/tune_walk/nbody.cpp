...
#endif
    /*
    for(PS::S32 i=0; i<tree_grav.ipg_.size(); i++){
        std::cerr<<"i="<<i<<std::endl;
        tree_grav.ipg_[i].dump(std::cerr);
    }
    */
    ...
    while(n_loop <= 16){
        ...
        if(n_loop % 8 == 0 && n_loop > 0){
            ...
            //PS::S32 rank_dinfo_max[n_profile], rank_system_max[n_profile], rank_dens_max[n_profile], rank_hydr_max[n_profile], rank_grav_max[n_profile];
            PS::S32 rank_grav_max[n_profile];
