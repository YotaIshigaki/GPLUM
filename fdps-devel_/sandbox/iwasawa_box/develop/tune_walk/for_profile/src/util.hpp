...
namespace  ParticleSimulator{
    ...
    inline F64 GetDistanceMinSQF64 (const F64vec & pos, const F64ort & box){
        const F64vec b_high = box.high_;
        const F64vec b_low = box.low_;
        const F64 dx = (pos.x > b_high.x) ? (pos.x - b_high.x) : ( (pos.x < b_low.x) ? (b_low.x - pos.x) : F64(0.0) );
        const F64 dy = (pos.y > b_high.y) ? (pos.y - b_high.y) : ( (pos.y < b_low.y) ? (b_low.y - pos.y) : F64(0.0) );
        const F64 dz = (pos.z > b_high.z) ? (pos.z - b_high.z) : ( (pos.z < b_low.z) ? (b_low.z - pos.z) : F64(0.0) );
    ...
    inline F64 GetDistanceMinSQF64 (const F64ort & box0, const F64ort & box1){
            const F64vec a_high = box0.high_;
            const F64vec a_low  = box0.low_;
            const F64vec b_high = box1.high_;
            const F64vec b_low  = box1.low_;
            ...
            F64 dy = (y0 > y1) ? y0 : y1;
            F64 dz = (z0 > z1) ? z0 : z1;
            dx = (x0*x1 < 0) ? dx : F64(0.0);
            dy = (y0*y1 < 0) ? dy : F64(0.0);
            dz = (z0*z1 < 0) ? dz : F64(0.0);
            return dx*dx + dy*dy + dz*dz;	
    }
    */
#ifdef ENABLE_AVX2
    ...
    /*
    inline __m256 operator- (__m256 lhs, __m256 rhs){
    ...
    */
    ...
    static inline __m256 GetDistanceMinSQV8R4(const F32ort & box, const F32vec pos[8]){
        ...
        return GetDistanceMinSQV8R4(ix, iy, iz, jx, jy, jz, lx, ly, lz);
    }
#endif
