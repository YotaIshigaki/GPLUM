namespace ParticleSimulator{
    ...
    ////////////////
    // for long mode
    template<class TSM, class Ttc, class Ttp, class Tep, class Tsp, class Twalkmode, typename Tmoment>
...
PS_OMP_PARALLEL_FOR
        ...
        /*
        if(Comm::getRank()==0){
            std::cerr<<"r_crit_send= "<<sqrt(r_crit_send)<<std::endl;
            std::cerr<<"top_moment[my_rank].vertex_in_= "<<top_moment[my_rank].vertex_in_<<std::endl;
        }
        */
PS_OMP_PARALLEL
        {
            ...
            for(S32 ib=head; ib<end; ib++){
                ...
                if( ((typeid(TSM) == typeid(SEARCH_MODE_LONG_SCATTER) || typeid(TSM) == typeid(SEARCH_MODE_LONG_SYMMETRY))
                    ...
                    if( (GetDistanceMinSq(top_moment[ib].vertex_in_, top_moment[my_rank].spj_.getPos(), len_peri) <= r_crit_send)
                        || ( (GetDistanceMinSq(top_moment[ib].vertex_in_, minimum_tree_boundary_of_domain[my_rank], len_peri) <= 0.0)
                    ...
                    if( (GetDistanceMinSq(top_moment[my_rank].vertex_in_, top_moment[ib].spj_.getPos(), len_peri) <= r_crit_recv)
                       || ( (GetDistanceMinSq(top_moment[my_rank].vertex_in_, minimum_tree_boundary_of_domain[ib], len_peri) <= 0.0)
                            && (exchange_let_mode_ == EXCHANGE_LET_P2P_EXACT) ) ){
                        rank_recv_tmp[ith].push_back(ib);
                        flag_recv_allgather = false;
                    }
                ...
                for(S32 j=n_image_tmp_prev; j<n_image_tmp; j++){
                    ...
                    MakeListUsingTreeRecursiveTop
                        ...
                        (tc_first, adr_tc, tp_first,
                         ...
                         target_box,
                         n_leaf_limit,
                         adr_tree_sp_first, F64vec(0.0), theta);
                    n_ep_per_image_tmp[ith].push_back(adr_ep_send_tmp[ith].size() - n_ep_prev_2);
                    n_sp_per_image_tmp[ith].push_back(adr_sp_send_tmp[ith].size() - n_sp_prev_2);
                }
...
PS_OMP_PARALLEL_FOR
    ...
    template<class TSM, class Ttc, class Ttp, class Tep, class Tsp, class Twalkmode, typename Tgeometry>
                ...
                for(S32 j=n_image_tmp_prev; j<n_image_tmp; j++){
                    ...
                    MakeListUsingTreeRecursiveTop
                        ...
                        (tc_first, adr_tc, tp_first,
                         ...
                         target_box,
                         n_leaf_limit,
                         adr_tree_sp_first, F64vec(0.0), theta_tmp);
                    n_ep_per_image_tmp[ith].push_back(adr_ep_send_tmp[ith].size() - n_ep_prev_2);
                }
                n_ep_send[rank] = adr_ep_send_tmp[ith].size() - n_ep_prev;
            }
