...
int main(int argc, char *argv[]) {
    ...
    /*
    if(PS::Comm::getRank()==0){
        PS::F64ort box(PS::F64vec(-10.0), PS::F64vec(10.0));
        for(PS::S32 i=0; i<PS::Comm::getNumberOfProc(); i++){
            std::cerr<<"i= "<<i<<std::endl;
            std::cerr<<"dinfo.getPosDomain(i)= "<<dinfo.getPosDomain(i)<<std::endl;
            std::cerr<<"dinfo.getPosDomain(box, i)= "<<dinfo.getPosDomain(box,i)<<std::endl;
        }
    }
    exit(1);
    */
    ...
    PS::TimeProfile tp_ptcl, tp_domain, tp_tree;
    PS::F64 wtime_tot = 0.0;
    PS::S32 n_loop_diag = 16;
    
    while(time_sys < time_end){

        if(n_loop%n_loop_diag == 0){
            //std::cerr<<"wtime_tot= "<<wtime_tot / n_loop_diag<<std::endl;
            //std::cerr<<"tp_ptcl.getTotalTime()= "<<tp_ptcl.getTotalTime() / n_loop_diag
            //         <<" tp_domain.getTotalTime()= "<<tp_domain.getTotalTime() / n_loop_diag
            //         <<" tp_tree.getTotalTime()= "<<tp_tree.getTotalTime() / n_loop_diag
            //         <<" tp_tree.make_LET_1st= "<<tp_tree.make_LET_1st / n_loop_diag
            //         <<" tp_tree.exchange_LET_1st= "<<tp_tree.exchange_LET_1st / n_loop_diag
            //         <<std::endl;
            wtime_tot = 0.0;
            tp_ptcl.clear();
            tp_domain.clear();
        ...
        const auto wtime0 = PS::GetWtime();
        
        dinfo.clearTimeProfile();
        if(n_loop % 4 == 0){
            dinfo.decomposeDomainAll(system_grav);
        }
        tp_domain = tp_domain + dinfo.getTimeProfile();
        
        system_grav.clearTimeProfile();
        system_grav.exchangeParticle(dinfo);
        tp_ptcl = tp_ptcl + system_grav.getTimeProfile();
        ...
        tree_grav.calcForceAllAndWriteBack(CalcGravity<FPGrav>,
                                           ...
                                           dinfo);
#endif
        tp_tree = tp_tree + tree_grav.getTimeProfile();

        wtime_tot += PS::GetWtime() - wtime0;
