...
def calcEccentricityDistribution(pp, w_b=2., m_sun=1.0) :
    ...
    Returns
    ----------
    dist :    list of [float, float, int, float, float, float, float, float, float]. 
              minimun mass of bin, maximum mass of bin, 
              the number of particle, total mass, mean mass, 
              mean eccentricity, standard error of eccentricity,
              mean inclination, standard error of inclination.
    """
    ...
    while m0 < m_max :
        ...
        while i < len(Ptcl) and Ptcl[i][0] < m1 :
            ...
            sigma_ecc = sigma_ecc + Ptcl[i][1]*Ptcl[i][1]
            sigma_inc = sigma_inc + Ptcl[i][2]*Ptcl[i][2]
            n_p  = n_p + 1
            i = i + 1
        ...
        if n_p > 0 :
            ...
            mu_inc    = None
        if n_p > 1 :
            sigma_ecc = m.sqrt((sigma_ecc/n_p - mu_ecc*mu_ecc)/(n_p-1))
            sigma_inc = m.sqrt((sigma_inc/n_p - mu_inc*mu_inc)/(n_p-1))
        else :
            sigma_ecc = None
            sigma_inc = None

            
        dist.append([m0, m1, n_p, mass_tot, mass_mean, mu_ecc, sigma_ecc, mu_inc, sigma_inc])
