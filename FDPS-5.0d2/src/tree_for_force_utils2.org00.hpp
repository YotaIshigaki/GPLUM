...
PS_OMP_PARALLEL_FOR
        ...
        //F64 wtime_add_0 = GetWtime() - wtime_offset;

        F64 r_crit_send = outer_boundary_of_tree[my_rank].getFullLength().getMax();
        if(theta > 0.0){
...
PS_OMP_PARALLEL
        ...
        //F64 wtime_add_1 = GetWtime() - wtime_offset;
        //if(my_rank==0){
        //    std::cout<<"wtime_add_0= "<<wtime_add_0
        //            <<" wtime_add_1= "<<wtime_add_1<<std::endl;
        //}
PS_OMP_PARALLEL_FOR
        for(S32 i=0; i<n_proc; i++){
...
PS_OMP_PARALLEL_FOR
        for(S32 i=0; i<n_proc; i++){
            n_ep_send[i] = n_image_per_proc[i] = 0;
        }
        bool pa[DIMENSION];
        dinfo.getPeriodicAxis(pa);
        F64ort pos_root_domain = dinfo.getPosRootDomain();
...
PS_OMP_PARALLEL_FOR
        ...
        //F64 wtime_ex_number = GetWtime() - wtime_offset;
        //if(Comm::getRank()==0){
        //    std::cout<<"wtime_ex_number= "<<wtime_ex_number<<std::endl;
        //}
        const S32 n_proc = Comm::getNumberOfProc();
...
PS_OMP_PARALLEL_FOR
    ...
    inline void ExchangeNumber(const ReallocatableArray<S32> & n_ep_send,
                               ...
                               const ReallocatableArray<S32> & rank_send,
                               const ReallocatableArray<S32> & rank_recv){
        n_ep_recv.resizeNoInitialize(rank_recv.size());
        Comm::isendrecv(n_ep_send.getPointer(), rank_send.getPointer(), 1, rank_send.size(),
                        n_ep_recv.getPointer(), rank_recv.getPointer(), 1, rank_recv.size());
    }
#else
    inline void ExchangeNumber(const ReallocatableArray<S32> & n_ep_send,
                               ReallocatableArray<S32> & n_ep_recv){
...
/*
        for(int i=0; i<rank_send.size(); i++){
            F64 mass_send = 0.0;
            for(int j=n_disp_ep_send[i]; j<n_disp_ep_send[i+1]; j++){
                mass_send += ep_send[j].mass;
            }
            for(int j=n_disp_sp_send[i]; j<n_disp_sp_send[i+1]; j++){
                mass_send += sp_send[j].mass;
            }
            //if(Comm::getRank()==3){
            //    std::cerr<<"A) rank_send[i]= "<<rank_send[i]
            //             <<" mass_send= "<<mass_send<<std::endl;
            //}
        }
*/
        /*
        for(int i=0; i<rank_recv.size(); i++){
            F64 mass_recv = 0.0;
            for(int j=n_disp_ep_recv[i]; j<n_disp_ep_recv[i+1]; j++){
                mass_recv += ep_org[n_ep_offset+j].mass;
            }
            for(int j=n_disp_sp_recv[i]; j<n_disp_sp_recv[i+1]; j++){
                mass_recv += sp_org[j].mass;
            }
            if(Comm::getRank()==0){
                std::cerr<<"B) i= "<<i
                         <<" rank_recv[i]= "<<rank_recv[i]
                         <<" n_disp_ep_recv[i]= "<<n_disp_ep_recv[i]
                         <<" n_disp_sp_recv[i]= "<<n_disp_sp_recv[i]
                         <<" mass_recv= "<<mass_recv
                         <<std::endl;
            }            
        }
        */
        //exit(1);
        //std::cerr<<"B) my_rank= "<<Comm::getRank()
        //         <<" mass_send= "<<mass_send
        //    //<<" mass_recv= "<<mass_recv
        //         <<std::endl;
        //exit(1);
        ...
        //if(Comm::getRank()==0){
        //    std::cerr<<"ep_org.size()= "<<ep_org.size()
        //             <<" n_ep_offset= "<<n_ep_offset
        //             <<" sp_org.size()= "<<sp_org.size()
        //             <<std::endl;
        //    for(int i=n_ep_offset; i<ep_org.size(); i++){
        //        std::cerr<<" ep_org[i].getCharge()= "<<ep_org[i].getCharge()
        //                 <<" ep_org[i].getPos()= "<<ep_org[i].getPos()
        //                 <<std::endl;
        //    }
        //    for(int i=0; i<sp_org.size(); i++){
        //        std::cerr<<" sp_org[i].getCharge()= "<<sp_org[i].getCharge()
        //                 <<" sp_org[i].getPos()= "<<sp_org[i].getPos()
        //                 <<std::endl;
        //    }
        //}
        //Comm::barrier();
...
#pragma omp parallel for schedule(dynamic, 4)
    ...
    inline void FindExchangeParticleDoubleWalk
        (const ReallocatableArray<Tepj> & epj_A, // received particles
