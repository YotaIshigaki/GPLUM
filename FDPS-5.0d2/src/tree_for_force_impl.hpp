...
#pragma omp parallel for
    ...
    exchangeLocalEssentialTreeReuseListLong(const DomainInfo & dinfo,
        ...
        ReallocatableArray<Tspj> top_moment(n_proc, n_proc, 1);
        Tspj my_moment;
        my_moment.copyFromMoment(tc_loc_[0].mom_);
        Comm::allGather(&my_moment, 1, top_moment.getPointer());
        if(!flag_reuse){
            FindScatterParticle<TSM, TreeCell<Tmomloc>, TreeParticle,
                ...
                (tc_loc_, tp_glb_,
                 ...
                 top_moment,
                 inner_boundary_of_local_tree_,
                 theta_,
                 r_crit_sq,
                 comm_table_.rank_recv_a2a_);
            /*
            if(Comm::getRank()==23){
                for(S32 i=0; i<n_proc; i++){
                    std::cout<<"i= "<<i
                             <<" comm_table_.n_ep_send_= "
                             <<comm_table_.n_ep_send_[i]
                             <<" comm_table_.n_sp_send_= "
                             <<comm_table_.n_sp_send_[i]
                             <<std::endl;
                }
            }
            Comm::barrier();
...
PS_OMP(omp parallel for reduction(+:n_ep_recv_tot_tmp), reduction(+:n_sp_recv_tot_tmp))
        ...
        const auto n_write = comm_table_.rank_recv_a2a_.size();


        //const auto n_write_glb = Comm::getSum(n_write);
        //if(my_rank == 0){
        //    std::cout<<"n_write_glb= "<<n_write_glb<<std::endl;
        //}
        ...
        n_sp_send_loc--; // remove sp from my domain.
        //std::cerr<<"my_rank= "<<my_rank
        //         <<" n_sp_send_loc= "<<n_sp_send_loc
        //         <<std::endl;
...
PS_OMP_PARALLEL
        {
            ...
            for(S32 i=head; i<end; i++){
                spj_org_[i+spj_offset] = top_moment[comm_table_.rank_recv_a2a_[i]];
            }
        }
        ...
        for(int i=0; i<epj_org_.size(); i++){
            ...
            //std::cerr<<"A) mass= "<<mass<<std::endl;
        }
        for(int i=0; i<spj_org_.size(); i++){
            mass += spj_org_[i].getCharge();
            //std::cerr<<"B) mass= "<<mass<<std::endl;
        }
        Comm::barrier();
        if(mass != 1.0){
...
PS_OMP(omp parallel for reduction(+:n_ep_recv_tot_tmp))
        ...
        time_offset = GetWtime();
        ExchangeLet<TSM, Tepj>(epj_sorted_,
                               comm_table_.n_ep_send_,
                               comm_table_.n_ep_recv_,
                               comm_table_.n_ep_per_image_,
                               comm_table_.adr_ep_send_,
